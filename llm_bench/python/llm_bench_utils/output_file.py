...
def output_input_text(
    input_text, args, model_precision, prompt_idx, batchsize_idx, proc_id
):
    if args["batch_size"] > 1:
        ...
        text_file_name = (
            args["model_name"] + "_" + model_precision + "_p" + str(prompt_idx)
        )
...
def output_image_input_text(input_text, args, prompt_idx, batchsize_idx, proc_id):
    if args["batch_size"] > 1 and batchsize_idx is not None:
        text_file_name = (
            args["model_name"] + "_p" + str(prompt_idx) + "_bs" + str(batchsize_idx)
        )
    else:
...
def output_gen_text(
    generated_text, args, model_precision, prompt_idx, iteration, batchsize_idx, proc_id
):
    if args["batch_size"] > 1:
        text_file_name = (
            args["model_name"]
