...
def set_bf16(model, device, **kwargs):
    try:
        if (
            len(kwargs["config"]) > 0
            and kwargs["config"].get("PREC_BF16")
            and kwargs["config"]["PREC_BF16"] is True
        ):
...
def torch_compile_child_module(
    model, child_modules, backend="openvino", dynamic=None, options=None
):
...
def run_torch_compile(
    model, backend="openvino", dynamic=None, options=None, child_modules=None
):
    if backend == "pytorch":
        ...
        if child_modules and len(child_modules) > 0:
            compiled_model = torch_compile_child_module(
                model, child_modules, backend, dynamic, options
            )
        else:
            compiled_model = torch.compile(
                model, backend=backend, dynamic=dynamic, options=options
            )
        end = time.perf_counter()
        compile_time = end - start
        log.info(f"Compiling model via torch.compile() took: {compile_time}")
...
def create_text_gen_model(model_path, device, **kwargs):
    ...
    if model_path.exists():
        if model_path.is_dir() and len(os.listdir(model_path)) != 0:
            ...
            model_class = PT_MODEL_CLASSES_MAPPING.get(
                model_type, PT_MODEL_CLASSES_MAPPING[default_model_type]
            )
            ...
            raise RuntimeError(
                f"==Failure ==: model path:{model_path} is not directory or directory is empty"
            )
    else:
        raise RuntimeError(f"==Failure ==: model path:{model_path} is not exist")

    log.info(
        f"model path:{model_path}, from pretrained time: {from_pretrain_time:.2f}s"
    )
    ...
    if device is not None:
        ...
        if device.upper() == "GPU":
            device = (
                ...
                else log.info("CUDA device is unavailable")
            )
        else:
        ...
        if any(
            x in real_base_model_name
            for x in [gptjfclm, lfclm, bfclm, gpt2lmhm, gptneoxclm, chatglmfcg]
        ):
    ...
    if kwargs["torch_compile_backend"]:
        ...
        compiled_model = run_torch_compile(
            model, backend, dynamic, options, child_modules
        )
