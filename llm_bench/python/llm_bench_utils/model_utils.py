                            ...
                            for line in f:
                                ...
                                if "prompt" in data:
                                    if data["prompt"] != "":
                                        ...
                                        raise RuntimeError(
                                            f"== prompt in prompt file:{input_prompt} should not be empty string =="
                                        )
                                else:
                                    raise RuntimeError(
                                        f'== key word "prompt" does not exist in prompt file:{input_prompt} =='
                                    )
                    else:
                        raise RuntimeError(
                            f"== The prompt file:{input_prompt} does not exist =="
                        )
                else:
                    raise RuntimeError(
                        f"== The prompt file:{input_prompt} should be ended with .jsonl =="
                    )
    return prompts_list
                            ...
                            for line in f:
                                ...
                                if "prompt" in data:
                                    if data["prompt"] != "":
                                        ...
                                        raise RuntimeError(
                                            "== prompt in prompt file:{input_prompt} should not be empty string =="
                                        )
                                else:
                                    raise RuntimeError(
                                        f'== key word "prompt" does not exist in prompt file:{input_prompt} =='
                                ...
                                if "guidance_scale" in data:
                                    image_param["guidance_scale"] = float(
                                        data["guidance_scale"]
                                    )
...
def analyze_args(args):
    ...
    if model_args['prompt_index'] is not None:
        # Deduplication
        [
            model_args["prompt_index"].append(i)
            for i in args.prompt_index
            if i not in model_args["prompt_index"]
        ]
    ...
    if (
        use_case == "code_gen"
        and not model_args["prompt"]
        and not model_args["prompt_file"]
    ):
        ...
        for case, model_ids in USE_CASES.items():
            for model_id in model_ids:
                if model_name.lower().startswith(model_id):
                    log.info(
                        f"==SUCCESS FOUND==: use_case: {case}, model_type: {model_name}"
                    )
        ...
        for case, model_ids in USE_CASES.items():
            for idx, model_id in enumerate(normalize_model_ids(model_ids)):
                if (
                    ...
                    log.info(
                        f"==SUCCESS FOUND==: use_case: {case}, model_type: {model_id}"
                    )
...
def get_config(config):
    with open(config, "r") as f:
        try:
            ...
            raise RuntimeError(
                f"==Parse file:{config} failiure, json format is incorrect =="
            )
    return ov_config
