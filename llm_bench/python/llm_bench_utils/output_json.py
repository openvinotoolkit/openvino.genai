...
def write_result(
    ...
    for i in range(total_iters):
        ...
        res_data = {
            ...
            "generation_time": (
                round(generation_time, 5) if generation_time != "" else generation_time
            ),
            "output_size": iter_data["output_size"],
            "latency": round(latency, 5) if latency != "" else latency,
            "result_md5": result_md5,
            "first_latency": (
                round(first_latency, 5) if first_latency != "" else first_latency
            ),
            "second_avg_latency": (
                round(other_latency, 5) if other_latency != "" else other_latency
            ),
            "first_infer_latency": (
                round(first_token_infer_latency, 5)
                if first_token_infer_latency != ""
                else first_token_infer_latency
            ),
            "second_infer_avg_latency": (
                round(other_token_infer_latency, 5)
            ...
            "tokenization_time": (
                round(tokenization_time, 5)
                if tokenization_time != ""
                else tokenization_time
            ),
            "detokenization_time": (
                round(detokenization_time, 5)
                if detokenization_time != ""
                else detokenization_time
            ),
        }
    ...
    output_result = {
        "metadata": metadata,
        "perfdata": {"compile_time": pretrain_time, "results": result},
    }
