...
def print_generated(iter_num, warm_up=False, generated=None):
    ...
    if generated is not None:
        try:
            ...
            try:
                utf8_generated = generated.encode(
...
def print_stable_diffusion_infer_latency(iter_str, iter_data, stable_diffusion):
    ...
    log.info(
        f"[{iter_str}] First step of unet latency: {iter_data['first_token_latency']:.2f} ms/step, "
        f"other steps of unet latency: {iter_data['other_tokens_avg_latency']:.2f} ms/step",
    )
    log.info(
        f"[{iter_str}] Text encoder latency: {stable_diffusion.get_text_encoder_latency():.2f} ms/step, "
...
def print_ldm_unet_vqvae_infer_latency(iter_num, iter_data, tms=None, warm_up=False):
    ...
    iter_data["other_tokens_avg_latency"] = (
        sum(tms[1 : (len_tms - 1)]) / (len_tms - 2) * 1000 if len_tms > 2 else 0
    )
    iter_data["first_token_infer_latency"] = iter_data["first_token_latency"]
    iter_data["other_tokens_infer_avg_latency"] = iter_data["other_tokens_avg_latency"]
    ...
    if len_tms > 1:
        log.info(
            f"[{iter_str}] Unet latency: {(sum(tms[0:(len_tms - 1)]) / (len_tms - 1)) * 1000:.2f} ms/step, "
...
def output_avg_statis_tokens(
    prompt_dict, prompt_idx_list, iter_data_list, batch_size, is_text_gen
):
    for p_idx in prompt_idx_list:
        ...
        for iter_data in iter_data_list:
            ...
            if iter_data["prompt_idx"] == p_idx:
                ...
                avg_input_size += (
                    iter_data["input_size"] if iter_data["input_size"] != "" else 0
                )
