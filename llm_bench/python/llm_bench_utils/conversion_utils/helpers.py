...
def is_ov_model_provided(
    model_id, model_dir, precision, model_name="openvino_model.xml"
):
    model_dirs = []
...
def get_fp_path(args, model_subpath):
    ...
    if Path(args.model_id).is_dir():
        ...
        model_dirs.extend(
            [
                base_model_dir,
                base_model_dir / args.precision,
                base_model_dir / OV_DIR / args.precision,
                base_model_dir / PYTORCH_DIR / OV_DIR / args.precision,
            ]
        )
...
def get_nncf_dataset(ov_model, tokenizer, config, dataset_name, subset_size):
    ...
    nncf_dataset = Dataset(
        dataset, lambda x: transform_fn(config=config, input_shapes=input_shapes, **x)
    )
    return nncf_dataset


def compress_ov_model_weights_helper(
    ov_model,
    tok,
    config,
    out_path,
    compress_weights_format="INT8",
    fp16=False,
    args={},
    ...
    if "INT8" in compress_weights_format and "INT8_ASYM" in COMPRESSION_OPTIONS:
        warnings.warn(
            "Usage INT8 mode is deprecated and will be removed soon. Please use INT8_ASYM instead",
            DeprecationWarning,
        )
    ...
    if dataset_name is not None and tok is not None:
        nncf_dataset = get_nncf_dataset(
            ov_model,
            tok,
            config,
            dataset_name,
            compression_args.get("subset_size", None),
        )
        compression_args["dataset"] = nncf_dataset

    compressed_ov_model = compress_weights(ov_model, **compression_args)
    save_ov_model_helper(
        compressed_ov_model, out_path, model_name, fp16=fp16, tok=tok, config=config
    )


def save_ov_model_helper(
    ov_model, out_path, model_name="openvino_model", fp16=False, tok=None, config=None
):
