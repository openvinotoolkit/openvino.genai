...
def run_text_generation(input_text, num, model, tokenizer, args, iter_data_list, warmup_md5, prompt_index, model_precision, start_of_log):
    set_seed(args['seed'])
    input_text_list = [input_text] * args['batch_size']
    if args["output_dir"] is not None and num == 0:
        for bs_index, in_text in enumerate(input_text_list):
            utils.output_file.output_input_text(in_text, args, model_precision, prompt_index, bs_index, start_of_log)
    ...
    for bs_idx in range(args['batch_size']):
        ...
        if args["output_dir"] is not None:
            utils.output_file.output_gen_text(result_text, args, model_precision, prompt_index, num, bs_idx, start_of_log)
    ...
    utils.metrics_print.print_metrics(
        ...
        start_of_log=start_of_log,
    )
    if num > 0:
        warmup_md5_list = warmup_md5[prompt_index]
        if result_md5_list != warmup_md5_list:
            log.warning(f"{start_of_log}[{num}] Prompt[{prompt_index}]'s md5 {result_md5_list} is different from warm-up's md5 {warmup_md5_list}")
            utils.metrics_print.print_generated(num, warm_up=(num == 0), generated=generated_text[0], start_of_log=start_of_log)
    else:
        utils.metrics_print.print_generated(num, warm_up=(num == 0), generated=generated_text[0], start_of_log=start_of_log)
...
def run_text_generation_benchmark(model_path, framework, device, args, num_iters, start_of_log):
    model, tokenizer, pretrain_time = FW_UTILS[framework].create_text_gen_model(model_path, device, start_of_log, **args)
    ...
    log.info(f"{start_of_log}Numbeams: {args['num_beams']}, benchmarking iter nums(exclude warm-up): {num_iters}, "
    ...
    if args['subsequent'] is False:
        for num in range(num_iters + 1):
            for prompt_idx, input_text in enumerate(input_text_list):
                if num == 0:
                    log.info(f'{start_of_log}[warm-up] Input text: {input_text}')
                run_text_generation(input_text, num, model, tokenizer, args, iter_data_list, warmup_md5, prompt_idx, model_precision, start_of_log)
    else:
        for prompt_idx, input_text in enumerate(input_text_list):
            for num in range(num_iters + 1):
                if num == 0:
    ...
    utils.metrics_print.print_average(iter_data_list, prompt_idx_list, args['batch_size'], True, start_of_log)
    return iter_data_list, pretrain_time


def run_image_generation(image_param, num, image_id, pipe, args, iter_data_list, stable_diffusion_hook, start_of_log):
    set_seed(args['seed'])
    ...
    log.info(
        f"{start_of_log}[{'warm-up' if num == 0 else num}] Input params: Batch_size={args['batch_size']}, "
    ...
    if num == 0 and args["output_dir"] is not None:
        for bs_idx, in_text in enumerate(input_text_list):
            utils.output_file.output_image_input_text(in_text, args, image_id, bs_idx, start_of_log)
    start = time.perf_counter()
    ...
    if (args['mem_consumption'] == 1 and num == 0) or args['mem_consumption'] == 2:
        ...
        mem_consumption.clear_max_memory_consumption()
    for bs_idx in range(args['batch_size']):
        rslt_img_fn = utils.output_file.output_gen_image(res[bs_idx], args, image_id, num, bs_idx, '.png', start_of_log)
    ...
    utils.metrics_print.print_generated(num, warm_up=(num == 0), generated=rslt_img_fn, start_of_log=start_of_log)
    if stable_diffusion_hook is not None:
        stable_diffusion_hook.clear_statistics()


def run_image_generation_benchmark(model_path, framework, device, args, num_iters, start_of_log):
    pipe, pretrain_time = FW_UTILS[framework].create_image_gen_model(model_path, device, start_of_log, **args)
    iter_data_list = []
    input_image_list = utils.model_utils.get_image_param_from_prompt_file(args)
    if len(input_image_list) == 0:
        raise RuntimeError('==Failure prompts is empty ==')
    stable_diffusion_hook = None
    if framework == "ov":
        stable_diffusion_hook = StableDiffusionHook()
    ...
    log.info(f'{start_of_log}Benchmarking iter nums(exclude warm-up): {num_iters}, prompt nums: {len(input_image_list)}')
    ...
    if args['subsequent'] is False:
        for num in range(num_iters + 1):
            for image_id, image_param in enumerate(input_image_list):
                run_image_generation(image_param, num, image_id, pipe, args, iter_data_list, stable_diffusion_hook, start_of_log)
    else:
        for image_id, image_param in enumerate(input_image_list):
            for num in range(num_iters + 1):
                run_image_generation(image_param, num, image_id, pipe, args, iter_data_list, stable_diffusion_hook, start_of_log)

    utils.metrics_print.print_average(iter_data_list, prompt_idx_list, args['batch_size'], False, start_of_log)
    return iter_data_list, pretrain_time
...
def run_ldm_super_resolution(img, num, pipe, args, framework, iter_data_list, image_id, tm_list, start_of_log):
    set_seed(args['seed'])
    ...
    log.info(
        f"{start_of_log}[{'warm-up' if num == 0 else num}] Input params: steps={nsteps}, "
    ...
    result_md5_list = []
    if framework == 'ov':
        rslt_img_fn = utils.output_file.output_gen_image(res[0], args, image_id, num, None, '.png', start_of_log)
        result_md5_list.append(hashlib.md5(Image.open(rslt_img_fn).tobytes()).hexdigest())
    ...
    utils.metrics_print.print_generated(num, warm_up=(num == 0), generated=rslt_img_fn, start_of_log=start_of_log)
    utils.metrics_print.print_ldm_unet_vqvae_infer_latency(num, iter_data, tm_list, warm_up=(num == 0), start_of_log=start_of_log)


def run_ldm_super_resolution_benchmark(model_path, framework, device, args, num_iters, start_of_log):
    ...
    if len(input_image_list) > 0:
        ...
        if args['images'] is not None:
            ...
            raise RuntimeError('==Failure image is empty ==')
    log.info(f'{start_of_log}Benchmarking iter nums(exclude warm-up): {num_iters}, prompt nums: {len(images)}')
    ...
    for num in range(num_iters + 1):
        ...
        for img in images:
            if num == 0:
                if args["output_dir"] is not None:
                    utils.output_file.output_image_input_text(str(img['prompt']), args, image_id, None, start_of_log)
                log.info(f"{start_of_log}[{'warm-up' if num == 0 else num}] Input image={img['prompt']}")
            run_ldm_super_resolution(img, num, pipe, args, framework, iter_data_list, image_id, tm_list, start_of_log)
            tm_list.clear()
            image_id = image_id + 1
    utils.metrics_print.print_average(iter_data_list, [], 0, False, start_of_log)
...
def case_to_bench_thread(model_path, model_name, framework, model_args, args):
    try:
        thread_id = threading.get_native_id()
        proc_id = os.getpid()
        ...
        if args.report is not None or args.report_json is not None:
            ...
            if args.report is not None:
                index = args.report.find('.csv')
                output_csv_file = args.report[:index] + '_tid' + str(thread_id) + args.report[index:]
                utils.global_var.get_value('thread_lock').acquire()
                utils.global_var.get_value('csv_file_list').append(output_csv_file)
            ...
            if args.report_json is not None:
                index = args.report.find('.json')
                output_json_file = args.report[:index] + '_tid' + str(thread_id) + args.report[index:]
...
def main():
    ...
    log.info(out_str)
    if args.report is not None:
        utils.global_var.set_value('csv_file_list', [])
    if args.memory_consumption:
        mem_consumption.start_collect_mem_consumption_thread()
    lock = threading.Lock()
    utils.global_var.set_value('thread_lock', lock)
    threads = []
    for i in range(args.thread_nums):
        t = threading.Thread(target=case_to_bench_thread, args=[model_path, model_name, framework, model_args, args])
        threads.append(t)
        t.start()
    for t in threads:
        t.join()
    ...
    if args.report is not None:
        combined_csv = pd.concat([pd.read_csv(f) for f in utils.global_var.get_value('csv_file_list')])
        combined_csv.to_csv(args.report, index=False, encoding='utf-8-sig')
