...
def gen_iterate_data(
    ...
    iter_data["tokenization_time"] = (
        tokenization_time[0] if len(tokenization_time) > 0 else ""
    )
    iter_data["detokenization_time"] = (
        tokenization_time[1] if len(tokenization_time) > 1 else ""
    )
    return iter_data
...
def run_text_generation(
    ...
    if args["batch_size"] > 1:
        ...
        out_str += "all input token size after padding: {} * {}, ".format(
            input_token_size, args["batch_size"]
        )
        if args["infer_count"] is not None:
            out_str += "all max_output_token_size: {} * {}".format(
                args["infer_count"], args["batch_size"]
            )
        log.info(out_str)
    ...
    max_gen_tokens = (
        DEFAULT_OUTPUT_TOKEN_SIZE
        if args["infer_count"] is None
        else args["infer_count"]
    )
    ...
    if (args["mem_consumption"] == 1 and num == 0) or args["mem_consumption"] == 2:
        ...
        max_rss_mem_consumption, max_shared_mem_consumption, max_uss_mem_consumption = (
    ...
    for bs_idx in range(args["batch_size"]):
        if "sum" not in args["model_name"] and result[bs_idx][:input_token_size].equal(
            input_tokens[bs_idx]
        ):
        ...
        if args["output_dir"] is not None:
            llm_bench_utils.output_file.output_gen_text(
                result_text, args, model_precision, prompt_index, num, bs_idx, proc_id
            )
        result_md5_list.append(
            hashlib.new("md5", result_text.encode(), usedforsecurity=False).hexdigest()
        )
    ...
    if bench_hook is not None:
        ...
        [
            log.debug("[{}]{:.4f}".format(idx, tm))
            for idx, tm in enumerate(tm_infer_list)
        ]
    ...
    if num > 0:
        ...
        if result_md5_list != prev_md5:
            ...
            llm_bench_utils.metrics_print.print_generated(
                num, warm_up=(num == 0), generated=generated_text[0]
            )
            if num == 1:
...
def run_text_generation_genai(
    ...
    generation_result = model.generate(
        input_text_list, max_new_tokens=max_gen_tokens, num_beams=args["num_beams"]
    )
    end = time.perf_counter()
...
def run_text_generation_benchmark(model_path, framework, device, args, num_iters):
    model, tokenizer, pretrain_time, bench_hook, use_genai = FW_UTILS[
        framework
    ].create_text_gen_model(model_path, device, **args)
    ...
    if args["prompt_index"] is None:
        prompt_idx_list = [
            prompt_idx for prompt_idx, input_text in enumerate(input_text_list)
        ]
    ...
    llm_bench_utils.metrics_print.print_average(
        iter_data_list, prompt_idx_list, args["batch_size"], True
    )
    return iter_data_list, pretrain_time


def run_image_generation(
    image_param, num, image_id, pipe, args, iter_data_list, proc_id
):
    set_seed(args["seed"])
    ...
    nsteps = image_param.get(
        "steps",
        (
            DEFAULT_INFERENCE_STEPS
            if "lcm" not in args["model_name"]
            else LCM_DEFAULT_INFERENCE_STEPS
        ),
    )
    ...
    if num == 0 and args["output_dir"] is not None:
        for bs_idx, in_text in enumerate(input_text_list):
            llm_bench_utils.output_file.output_image_input_text(
                in_text, args, image_id, bs_idx, proc_id
            )
    start = time.perf_counter()
    res = pipe(
        input_text_list,
    ...
    if (args["mem_consumption"] == 1 and num == 0) or args["mem_consumption"] == 2:
        ...
        mem_consumption.clear_max_memory_consumption()
    for bs_idx in range(args["batch_size"]):
        rslt_img_fn = llm_bench_utils.output_file.output_gen_image(
            res[bs_idx], args, image_id, num, bs_idx, proc_id, ".png"
        )
        result_md5_list.append(
            hashlib.md5(
                Image.open(rslt_img_fn).tobytes(), usedforsecurity=False
            ).hexdigest()
        )
    generation_time = end - start
    iter_data = gen_iterate_data(
    ...
    llm_bench_utils.metrics_print.print_generated(
        num, warm_up=(num == 0), generated=rslt_img_fn
    )
...
def run_image_generation_benchmark(model_path, framework, device, args, num_iters):
    if args["genai"]:
        log.warning(
            "GenAI pipeline is not supported for this task. Switched on default benchmarking"
        )
    pipe, pretrain_time = FW_UTILS[framework].create_image_gen_model(
        model_path, device, **args
    )
    iter_data_list = []
    input_image_list = llm_bench_utils.model_utils.get_image_param_from_prompt_file(
        args
    )
    if framework == "ov":
    ...
    if args["prompt_index"] is None:
        prompt_idx_list = [
            image_id for image_id, input_text in enumerate(input_image_list)
        ]
    ...
    if len(image_list) == 0:
        raise RuntimeError("==Failure prompts is empty ==")
    log.info(
        f"Benchmarking iter nums(exclude warm-up): {num_iters}, prompt nums: {len(image_list)}, prompt idx: {prompt_idx_list}"
    )
    ...
    llm_bench_utils.metrics_print.print_average(
        iter_data_list, prompt_idx_list, args["batch_size"], False
    )
    return iter_data_list, pretrain_time
...
def run_ldm_super_resolution(
    img, num, pipe, args, framework, iter_data_list, image_id, tm_list, proc_id
):
    set_seed(args["seed"])
    ...
    if framework == "ov":
        rslt_img_fn = llm_bench_utils.output_file.output_gen_image(
            res[0], args, image_id, num, None, proc_id, ".png"
        )
        result_md5_list.append(
            hashlib.md5(
                Image.open(rslt_img_fn).tobytes(), usedforsecurity=False
    ...
    llm_bench_utils.metrics_print.print_ldm_unet_vqvae_infer_latency(
        num, iter_data, tm_list, warm_up=(num == 0)
    )
...
def run_ldm_super_resolution_benchmark(model_path, framework, device, args, num_iters):
    ...
    pipe, pretrain_time = FW_UTILS[framework].create_ldm_super_resolution_model(
        model_path, device, **args
    )
    iter_data_list = []
    tm_list = []
    ...
    if len(input_image_list) > 0:
        ...
        for image in input_image_list:
            if args["prompt"] is None and args["prompt_file"] is None:
                ...
                image["prompt"] = os.path.join(
                    os.path.dirname(args["prompt_file"][0]),
    ...
    for num in range(num_iters + 1):
        for image_id, img in enumerate(image_list):
            ...
            tm_list.clear()
    llm_bench_utils.metrics_print.print_average(
        iter_data_list, prompt_idx_list, 1, False
    )
...
def get_argprser():
    ...
    parser.add_argument(
        "-m",
        "--model",
        help="model folder including IR files or Pytorch files",
        required=TabError,
    )
    ...
    parser.add_argument(
        ...
        "--images",
        default=None,
        help="test images for vision tasks. Can be directory or path to single image",
    )
    parser.add_argument(
        "-s",
        "--seed",
    ...
    parser.add_argument(
        "--convert_tokenizer",
        action="store_true",
        help="Convert tokenizer to OpenVINO format",
    )
    parser.add_argument(
        "--subsequent",
        action="store_true",
    ...
    parser.add_argument(
        "-od",
        "--output_dir",
        help="Save the input text and generated text, images to files",
    )
    llm_bench_utils.model_utils.add_stateful_model_arguments(parser)
    parser.add_argument(
        "--genai",
        action="store_true",
        help="Use OpenVINO GenAI optimized pipelines for benchmarking",
    )
    parser.add_argument(
        "--use_cb", action="store_true", help="Use Continuous Batching inference mode"
    )
    parser.add_argument(
        "--cb_config",
        required=False,
        default=None,
        help="Path to file with Continuous Batching Scheduler settings",
    )
    parser.add_argument(
        "--end_token_stopping",
        action="store_true",
...
def main():
    ...
    if model_args["torch_compile_backend"]:
        ...
        os.system(
            "echo [ INFO ] OPENVINO_TORCH_BACKEND_DEVICE=$OPENVINO_TORCH_BACKEND_DEVICE"
        )
    ...
    if framework == "ov":
        ...
        if (
            model_args["config"].get("PREC_BF16")
            and model_args["config"]["PREC_BF16"] is True
        ):
            log.warning(
                "[Warning] Param bf16/prec_bf16 only work for framework pt. It will be disabled."
            )
    log.info(out_str)
        ...
        if args.report is not None or args.report_json is not None:
            ...
            if framework == "ov":
                ir_conversion_frontend = (
                    llm_bench_utils.model_utils.get_ir_conversion_frontend(
                        model_name, model_path.parts
                    )
                )
                if ir_conversion_frontend != "":
                    framework = framework + "(" + ir_conversion_frontend + ")"
                model_precision = llm_bench_utils.model_utils.get_model_precision(
                    model_path.parts
                )
