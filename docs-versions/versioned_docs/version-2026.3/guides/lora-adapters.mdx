...
:::info
For more details about LoRA, see [Low-Rank Adaptation (LoRA)](../concepts/lora.md).
:::

OpenVINO GenAI provides built-in support for LoRA adapters in [text generation](../use-cases/text-generation/index.mdx), [image generation](../use-cases/image-generation/index.mdx), [image processing (VLM)](../use-cases/image-processing/index.mdx), and [video generation](../use-cases/video-generation/index.mdx) pipelines.
This capability allows you to dynamically switch between or combine multiple adapters without recompiling the model.

:::info
See [Supported Models](../supported-models/index.mdx) for the list of models that support LoRA adapters.
:::
