...
int32_t main(int32_t argc, char* argv[]) {
    ...
    const uint32_t steps = result["step"].as<size_t>();
    const uint32_t seed = result["seed"].as<size_t>();
    ...
    for (uint32_t n = 0; n < num_images; n++) {
        seed_vec.push_back(num_images == 1 ? seed: n);
        std::string output_bmp_path = std::string("./images/seed_") + std::to_string(seed_vec.back()) + ".bmp";
