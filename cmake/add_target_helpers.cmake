...
#[[
function to create CMake target and setup its options in a declarative style. The target is built into samples directory.
Example:
genai_add_sample_target(
   NAME core_lib
   ADD_CLANG_FORMAT
   TYPE <SHARED / STATIC / EXECUTABLE>
   ROOT ${CMAKE_CURRENT_SOURCE_DIR}
...
#]]
function(genai_add_sample_target)
    set(options)
    set(oneValueRequiredArgs
        NAME # name of target
        )
    ...
    set_target_properties(${ARG_NAME} PROPERTIES
        RUNTIME_OUTPUT_DIRECTORY "$<1:${CMAKE_BINARY_DIR}/samples/${RELATIVE_SAMPLE_PATH}>"
        LIBRARY_OUTPUT_DIRECTORY "$<1:${CMAKE_BINARY_DIR}/samples/${RELATIVE_SAMPLE_PATH}>"
        ARCHIVE_OUTPUT_DIRECTORY "$<1:${CMAKE_BINARY_DIR}/samples/${RELATIVE_SAMPLE_PATH}>"
    )
...
#]]
function(genai_add_c_sample_target)
    set(options)
    set(oneValueRequiredArgs
        NAME # name of target
        )
    ...
    target_link_libraries(${ARG_NAME} PRIVATE openvino::genai::c)

    get_target_property(srcs ${ARG_NAME} SOURCES)
...
function(genai_add_tool_target)
    ...
    set_target_properties(${target} PROPERTIES
        RUNTIME_OUTPUT_DIRECTORY "$<1:${CMAKE_BINARY_DIR}/tools/${RELATIVE_TOOL_PATH}>"
        LIBRARY_OUTPUT_DIRECTORY "$<1:${CMAKE_BINARY_DIR}/tools/${RELATIVE_TOOL_PATH}>"
        ARCHIVE_OUTPUT_DIRECTORY "$<1:${CMAKE_BINARY_DIR}/tools/${RELATIVE_TOOL_PATH}>"
    )
endfunction()
