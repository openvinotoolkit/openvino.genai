...
int main(int argc, char* argv[]) try {
    ...
    // draft_model per iteration. NOTE: ContinuousBatching backend uses `num_assistant_tokens` as is. Stateful backend
    // uses `num_assistant_tokens`'s copy as initial value and adjusts it based on recent number of accepted tokens. If
    // `num_assistant_tokens` is not set, it defaults to `5` for both backends.
    ...
    // higher than `assistant_confidence_threshold`. NOTE: `assistant_confidence_threshold` is supported only by
    // ContinuousBatching backend. config.assistant_confidence_threshold = 0.4;
