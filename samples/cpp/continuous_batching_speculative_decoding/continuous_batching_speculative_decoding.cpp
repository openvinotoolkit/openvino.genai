...
int main(int argc, char* argv[]) try {
    ...
    SpeculativeDecodingPipeline pipe(models_path, assisting_models_path, k, ov::genai::Tokenizer{models_path}, scheduler_config, "CPU");
    ...
    std::cout << "Infer number: " << pipe.matches_info.size() << std::endl;
    std::cout << "MAX matches number: " << pipe.max_matches << std::endl;
    auto a = std::accumulate(pipe.matches_info.begin(), pipe.matches_info.end(), 0);
    std::cout << "AVG matches number: " << (float(a) / pipe.matches_info.size()) << std::endl;
    double c = double(pipe.assisting_model_duration) * 100 / std::chrono::duration_cast<std::chrono::nanoseconds>(duration).count();
    std::cout << "Speculative model time duration: " << pipe.assisting_model_duration * 1e-9 << " in %: " << c << std::endl;
