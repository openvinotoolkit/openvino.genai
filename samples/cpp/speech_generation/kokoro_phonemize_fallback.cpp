...
int main(int argc, char* argv[]) try {
    OPENVINO_ASSERT(
        ...
        "Usage: ",
        argv[0],
        " <MODEL_DIR> \"<PROMPT>\" --speaker_embedding_file_path <FILE.bin> [--language <en-us|en-gb>] "
    ...
    while (idx < argc) {
        ...
        if (option == "--speaker_embedding_file_path") {
    ...
    OPENVINO_ASSERT(speaker_embedding_path.has_value(),
                    "kokoro_phonemize_fallback sample requires --speaker_embedding_file_path");
    ...
    auto result = pipe.generate(prompt, speaker_embedding);
