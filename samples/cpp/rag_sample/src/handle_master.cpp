...
std::function<void(const httplib::Request&, httplib::Response&)> HandleMaster::get_handle(std::string handle_name,
    ...
    } else if (auto llm_pointer = std::get_if<std::shared_ptr<util::LLMPipelineUtil>>(&handle_type)) {
        if (handle_name == "llm_init") {
            return get_handle_llm_init(*llm_pointer, args);
        } else {
            return get_handle_llm(*llm_pointer);
        }
    }
    return {};
}
...
std::function<void(const httplib::Request&, httplib::Response&)> HandleMaster::get_handle_llm_init(
    ...
    util::Args args) {
    const auto handle_llm_init = [&llm_pointer_ref, args](const httplib::Request& req,
                                                          httplib::Response& res) {
        llm_pointer_ref->pipe_pointer = std::make_shared<ov::genai::LLMPipeline>(args.llm_model_path, args.llm_device);
        llm_pointer_ref->config = llm_pointer_ref->pipe_pointer->get_generation_config();
        llm_pointer_ref->config.max_new_tokens = args.max_new_tokens;
    };
...
std::function<void(const httplib::Request&, httplib::Response&)> HandleMaster::get_handle_llm(
    std::shared_ptr<util::LLMPipelineUtil>& llm_pointer_ref) {
    const auto handle_llm = [&llm_pointer_ref](const httplib::Request& req_llm,
                                               httplib::Response& res_llm) {
        ...
        llm_pointer_ref->pipe_pointer->start_chat();
        std::string response = llm_pointer_ref->pipe_pointer->generate(prompt, llm_pointer_ref->config);
        llm_pointer_ref->pipe_pointer->finish_chat();
...
std::function<void(const httplib::Request&, httplib::Response&)> HandleMaster::get_handle_embeddings(
    ...
    const auto handle_embeddings = [&embedding_ref](const httplib::Request& req_embedding,
        ...
        std::vector<std::vector<std::vector<float>>> res = embedding_ref->run(inputs);
