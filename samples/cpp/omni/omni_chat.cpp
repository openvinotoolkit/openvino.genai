...
int main(int argc, char* argv[]) try {
    ...
    ov::genai::RawSpeechInput pcm = utils::audio::read_wav(argv[3]);
    ov::Tensor audio_tensor(ov::element::f32, {pcm.size()});
    std::memcpy(audio_tensor.data<float>(), pcm.data(), pcm.size() * sizeof(float));
    std::vector<ov::Tensor> audios = {std::move(audio_tensor)};
    std::cout << "[Loaded audio: " << audios[0].get_size() << " samples at 16kHz]\n";
    ...
    std::cout << "\n=== Building OmniPipeline from Components ===\n";
    std::cout << "Creating VLM base and Talker separately...\n";

    auto vlm_base = ov::genai::VLMPipeline::create_base(models_path, "CPU", {});
    auto talker = std::shared_ptr<ov::genai::Talker>(new ov::genai::Talker(models_path, "CPU"));
    ov::genai::OmniPipeline pipe(vlm_base, talker);
    ...
    if (!decoded_results.speech_result.speech_outputs.empty()) {
        std::cout << "\n[Speech output: " << decoded_results.speech_result.speech_outputs[0].get_size() << " samples at 24kHz]"
                  << std::endl;
    }
