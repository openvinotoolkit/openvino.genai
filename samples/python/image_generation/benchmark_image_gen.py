...
def print_one_generate(metrics, prefix, idx):
    ...
    if len(metrics.raw_metrics.transformer_inference_durations) > 0:
        ...
        print(
            f"{prefix_idx} transformer iteration num: {len(metrics.raw_metrics.iteration_durations)}, first iteration time: {first_iter_time:.2f} ms, other iteration avg time: {other_iter_avg_time:.2f} ms"
        )
        print(
            f"{prefix_idx} transformer inference num: {len(metrics.raw_metrics.transformer_inference_durations)}, first inference time: {first_infer_time:.2f} ms, other inference avg time: {other_infer_avg_time:.2f} ms"
        )
    else:
        first_infer_time, other_infer_avg_time = metrics.get_first_and_other_unet_infer_duration()
        print(
            f"{prefix_idx} unet iteration num: {len(metrics.raw_metrics.iteration_durations)}, first iteration time: {first_iter_time:.2f} ms, other iteration avg time: {other_iter_avg_time:.2f} ms"
        )
        print(
            f"{prefix_idx} unet inference num: {len(metrics.raw_metrics.unet_inference_durations)}, first inference time: {first_infer_time:.2f} ms, other inference avg time: {other_infer_avg_time:.2f} ms"
        )
    print(
        f"{prefix_idx} vae encoder infer time: {metrics.get_vae_encoder_infer_duration():.2f} ms, vae decoder infer time: {metrics.get_vae_decoder_infer_duration():.2f} ms"
    )
...
def print_statistic(warmup_metrics, iter_metrics):
    ...
    print(
        f"Warmup number: {warmup_num}, first generate warmup time: {generate_warmup:.2f} ms, infer warmup time: {inference_warmup:.2f} ms"
    )
    print(
        f"Generate iteration number: {iter_num}, for one iteration, generate avg time: {generate_mean:.2f} ms, "
...
def main():
    ...
    parser.add_argument(
        "-t", "--pipeline_type", type=str, default="text2image", help="pipeline type: text2image/image2image/inpainting"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="The number of inference steps used to denoise initial noised latent to final image",
    )
    parser.add_argument(
        "-ni",
        ...
        type=int,
        default=1,
        help="The number of images to generate per generate() call",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.8,
        help="Indicates extent to transform the reference `image`. Must be between 0 and 1",
    )
