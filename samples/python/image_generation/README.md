...
There are several sample files:
 ...
 - [`decode_intermediate_result_concurrency.py`](./decode_intermediate_result_concurrency.py) demonstrates how to run with callback function to decode intermediate latent result in parallel based on heterogeneous pipeline. Recommend to run Unet and VAE in different execution device.
