...
- Image generated with HuggingFace / Optimum Intel is not the same generated by this sample, as `openvino_genai.CppStdGenerator` used in this sample wraps C++ MT19937 random generator.

C++ random generation with MT19937 results differ from `numpy.random.randn()` and `diffusers.utils.randn_tensor`. So, it's expected that image generated using this generator is different from images generated with Python random generator. Users can implement their own random generator derived from `openvino_genai.Generator` and pass it to `pipe.generate()` method.
