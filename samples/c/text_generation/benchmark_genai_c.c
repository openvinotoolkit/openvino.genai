...
void print_usage() {
    ...
    printf("  -N, --num_warmup      Number of warmup iterations (default: %d)\n", DEFAULT_NUM_WARMUP);
    printf("  -n, --num_iter         Number of iterations (default: %d)\n", DEFAULT_NUM_ITER);
    printf("  -M, --max_new_tokens  Maximal number of new tokens (default: %d)\n", DEFAULT_MAX_NEW_TOKENS);
...
int parse_arguments(int argc, char* argv[], Options* options) {
    for (int i = 1; i < argc; i++) {
        ...
        } else if (strcmp(argv[i], "-N") == 0 || strcmp(argv[i], "--num_warmup") == 0) {
            if (i + 1 < argc) {
        ...
        } else if (strcmp(argv[i], "-M") == 0 || strcmp(argv[i], "--max_new_tokens") == 0) {
            if (i + 1 < argc) {
