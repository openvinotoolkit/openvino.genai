...
int main(int argc, char* argv[]) {
    ...
    streamer_callback streamer = {.callback_func = print_callback, .args = NULL};
    ...
    while (fgets(prompt, MAX_PROMPT_LENGTH, stdin)) {
        ...
        int message_json_len =
            snprintf(message_json, sizeof(message_json), "{\"role\": \"user\", \"content\": \"%s\"}", escaped_prompt);
        ...
        CHECK_JSON_CONTAINER_STATUS(ov_genai_json_container_create_from_json_string(&message_container, message_json));

        // Push message using JsonContainer
        CHECK_CHAT_HISTORY_STATUS(ov_genai_chat_history_push_back(chat_history, message_container));

        results = NULL;
        CHECK_STATUS(ov_genai_llm_pipeline_generate_with_history(pipeline, chat_history, config, &streamer, &results));
        ...
        if (results) {
            ...
            int assistant_message_json_len = snprintf(assistant_message_json,
                                                      sizeof(assistant_message_json),
                                                      "{\"role\": \"assistant\", \"content\": \"%s\"}",
                                                      escaped_output);
            if (assistant_message_json_len < 0 ||
                (size_t)assistant_message_json_len >= sizeof(assistant_message_json)) {
                fprintf(stderr,
                        "[ERROR] Assistant message JSON truncated: buffer too small (needed %d bytes)\n",
