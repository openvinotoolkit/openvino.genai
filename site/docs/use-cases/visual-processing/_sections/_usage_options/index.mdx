import BasicGenerationConfiguration from '@site/docs/use-cases/_shared/_basic_generation_configuration.mdx';
import ChatScenario from '@site/docs/use-cases/_shared/_chat_scenario.mdx';
import GenerationConfigurationWorkflow from '@site/docs/use-cases/_shared/_generation_configuration_workflow.mdx';
import Streaming from '@site/docs/use-cases/_shared/_streaming.mdx';

## Additional Usage Options

:::tip
Check out [Python](https://github.com/openvinotoolkit/openvino.genai/tree/master/samples/python/visual_language_chat), [C++](https://github.com/openvinotoolkit/openvino.genai/tree/master/samples/cpp/visual_language_chat) and [JavaScript](https://github.com/openvinotoolkit/openvino.genai/tree/master/samples/js/visual_language_chat) visual language chat samples.
:::

### Use Media Tags in Prompt

The prompt can contain `<ov_genai_image_i>`, `<ov_genai_video_i>` or `<ov_genai_audio_i>`, with `i` replaced with an actual zero based index, to refer to an image, a video or an audio. Outside a chat, `i` indexes the list passed to the call. In a chat, indices are conversation-absolute: each modality counts on from the media of earlier turns. If turn 1 passed one audio, a new audio on turn 2 is `<ov_genai_audio_1>`, not `<ov_genai_audio_0>`. Reference to media used in previous prompts isn't implemented, so an index from an earlier turn is rejected. A model's native tag can be used instead of the universal one; the native tags are listed per modality below.

If the prompt doesn't contain image, video or audio tags, but images, videos or audios are provided, the tags are prepended to the prompt.

A tag expands to as many placeholder tokens as its media requires, so you do not write the placeholder count yourself.

Tags let you place media anywhere, including between spans of text:

<LanguageTabs>
    <TabItemPython>
        ```python
        # Audio between two pieces of text, and two audios referenced by index.
        pipe.generate("Compare <ov_genai_audio_0> against <ov_genai_audio_1> and explain", audios=[first, second])
        ```
    </TabItemPython>
    <TabItemCpp>
        ```cpp
        // Audio between two pieces of text, and two audios referenced by index.
        pipe.generate("Compare <ov_genai_audio_0> against <ov_genai_audio_1> and explain",
                      ov::genai::audios(std::vector<ov::Tensor>{first, second}));
        ```
    </TabItemCpp>
</LanguageTabs>

#### Image Tags

1. InternVL2: `<image>\n`
2. llava-1.5-7b-hf: `<image>`
3. LLaVA-NeXT: `<image>`
4. LLaVA-NeXT-Video: `<image>`
5. nanoLLaVA: `<image>\n`
6. nanoLLaVA-1.5: `<image>\n`
7. MiniCPM-o-2_6: `<image>./</image>\n`
8. MiniCPM-V-2_6: `<image>./</image>\n`
9. Phi-3-vision: `<|image_i|>\n` - the index starts with one
10. Phi-4-multimodal-instruct: `<|image_i|>\n` - the index starts with one
11. Qwen2-VL: `<|vision_start|><|image_pad|><|vision_end|>`
12. Qwen2.5-VL: `<|vision_start|><|image_pad|><|vision_end|>`
13. Qwen3-VL: `<|vision_start|><|image_pad|><|vision_end|>`
14. gemma-3-4b-it: `<start_of_image>`
15. gemma3n: `<image_soft_token>`
16. gemma4: `<|image|>`
17. Muse Glimmer: `<|image|>`
18. DeepSeek-OCR-2: `<image>`

#### Video Tags

1. LLaVA-NeXT-Video: `<video>`
2. Qwen2-VL: `<|vision_start|><|video_pad|><|vision_end|>`
3. Qwen2.5-VL: `<|vision_start|><|video_pad|><|vision_end|>`
4. Qwen3-VL: `<|vision_start|><|video_pad|><|vision_end|>`
5. VideoChat_Flash_Qwen: `<|image_i|>\n` - the index starts with one
6. gemma4: `<|video|>`
7. Muse Glimmer: `<|video|>`

#### Audio Tags

1. Qwen3-Omni: `<|audio_start|><|audio_pad|><|audio_end|>`

#### Qwen3-Omni Notes

Qwen3-Omni is trained with media placed before the text of a turn, so prepending — the default when
you supply no tags — is the recommended form. Interleaving is supported but off-distribution.

:::warning Audio and video in the same prompt

Audio placeholders are positioned as text tokens, while image and video placeholders feed the 4D
mRoPE position ids. Moving audio away from the front of the prompt therefore shifts where those
text-positioned tokens sit relative to the vision tokens.

Prompts with audio alone are unaffected, and so are prompts that leave the audio prepended. A
prompt that interleaves audio with video has not been validated for output quality: the token
accounting is correct and nothing raises, but generation quality may differ from the reference
implementation. If you need that combination, compare against `transformers` on your own data
before relying on it.
:::

### Use Different Generation Parameters

Similar to [text generation](/docs/use-cases/text-generation/#use-different-generation-parameters), VLM pipelines support various generation parameters to control the text output.

<GenerationConfigurationWorkflow />

<BasicGenerationConfiguration>
  <LanguageTabs>
      <TabItemPython>
          ```python
          import openvino_genai as ov_genai

          pipe = ov_genai.VLMPipeline(model_path, "CPU")

          # Get default configuration
          config = pipe.get_generation_config()

          # Modify parameters
          config.max_new_tokens = 100
          config.temperature = 0.7
          config.top_k = 50
          config.top_p = 0.9
          config.repetition_penalty = 1.2

          # Generate text with custom configuration
          output = pipe.generate(prompt, images, config)
          ```
      </TabItemPython>
      <TabItemCpp>
          ```cpp
          int main() {
              ov::genai::VLMPipeline pipe(model_path, "CPU");

              // Get default configuration
              auto config = pipe.get_generation_config();

              // Modify parameters
              config.max_new_tokens = 100;
              config.temperature = 0.7f;
              config.top_k = 50;
              config.top_p = 0.9f;
              config.repetition_penalty = 1.2f;

              // Generate text with custom configuration
              auto output = pipe.generate(prompt, images, config);
          }
          ```
      </TabItemCpp>
      <TabItemJS>
          ```javascript
          import { VLMPipeline }  from 'openvino-genai-node';

          const pipe = await VLMPipeline(modelPath, "CPU", {});

          // Create custom generation configuration
          const config = {
              max_new_tokens: 100,
              temperature: 0.7,
              top_k: 50,
              top_p: 0.9,
              repetition_penalty: 1.2
          };

          // Generate text with custom configuration
          const output = await pipe.generate(prompt, {
              images: images,
              generationConfig: config
          });
          ```
      </TabItemJS>
  </LanguageTabs>
</BasicGenerationConfiguration>

### Video Metadata and Sampling

When a video is provided as an input, video metadata can be passed along with the video to control the sampling of video frames.

Passing `VideoMetadata` with `frames_indices` defined enables sampling based on provided indices within the pipeline, and any model-specific sampling logic will be skipped (if defined).
Leave `frames_indices` empty to apply model-specific sampling (e.g. for Qwen3-VL).

Refer to the video-to-text [Python](https://github.com/openvinotoolkit/openvino.genai/blob/master/samples/python/visual_language_chat/video_to_text_chat.py) and [C++](https://github.com/openvinotoolkit/openvino.genai/blob/master/samples/cpp/visual_language_chat/video_to_text_chat.cpp) samples for more details.

### Working with LoRA Adapters

For Vision Language Models (VLMs), LoRA adapters can customize the generated text by applying adapters to the language-model (LLM) part.
LoRA adapters that target the vision encoder or other multimodal components are not supported.

Refer to the [LoRA Adapters](/docs/guides/lora-adapters) guide for more details on working with LoRA adapters.

### Performance Metrics

`VLMPipeline` returns `VLMDecodedResults` with a `perf_metrics` field of type `VLMPerfMetrics`, which extends the standard [`PerfMetrics`](/docs/guides/performance-metrics).

In addition to common metrics (TTFT, TPOT, throughput, etc.), `VLMPerfMetrics` provides:

| Metric | Type | Description |
|--------|------|-------------|
| `get_prepare_embeddings_duration()` | `MeanStdPair`, ms | Total time for all embedding preparation steps before LM inference (includes pre-processing, vision/audio encoding, text embedding, merging etc.) |
| `get_vision_encoding_duration()` | `MeanStdPair`, ms | Time spent encoding images and video frames through the vision encoder |
| `get_audio_encoding_duration()` | `MeanStdPair`, ms | Time spent encoding audio inputs (applicable to audio-capable models like Qwen3-Omni) |
| `get_text_embedding_duration()` | `MeanStdPair`, ms | Time spent computing token embeddings from the text embedding model |
| `get_total_image_slice_count()` | integer | Total number of image slices processed across all input images |

Raw values are available in `perf_metrics.vlm_raw_metrics`.

:::tip
For more details, refer to the [Python](https://github.com/openvinotoolkit/openvino.genai/blob/master/samples/python/visual_language_chat/benchmark_vlm.py) and [C++](https://github.com/openvinotoolkit/openvino.genai/blob/master/samples/cpp/visual_language_chat/benchmark_vlm.cpp) VLM benchmark samples.
:::

<ChatScenario />

<Streaming />
