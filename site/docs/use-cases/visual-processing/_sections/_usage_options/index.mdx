...
The prompt can contain `<ov_genai_image_i>`, `<ov_genai_video_i>` or `<ov_genai_audio_i>`, with `i` replaced with an actual zero based index, to refer to an image, a video or an audio. Reference to media used in previous prompts isn't implemented. A model's native tag can be used instead of the universal one; the native tags are listed per modality below.
