...
:::tip
Check out [Python](https://github.com/openvinotoolkit/openvino.genai/tree/master/samples/python/automatic_speech_recognition), [C++](https://github.com/openvinotoolkit/openvino.genai/tree/master/samples/cpp/automatic_speech_recognition), and [JavaScript](https://github.com/openvinotoolkit/openvino.genai/tree/master/samples/js/automatic_speech_recognition) speech recognition samples.
:::
...
<BasicGenerationConfiguration>
  <LanguageTabs>
      ...
      <TabItemJS>
          ...
          const pipeline = await ASRPipeline(modelPath, "CPU");
...
Language handling depends on the model architecture. Models with language identification support can automatically detect the language of the input audio. You can also specify the language to improve accuracy. The detected or specified language is available in the result via the `languages` field.
...
<LanguageTabs>
    <TabItemPython>
        ...
        # Automatic language detection for models with language identification support
...
<LanguageTabs>
    ...
    <TabItemCpp>
        ...
        int main() {
            // Word timestamps require decomposition of cross-attention decoder SDPA layers,
            // so word_timestamps must be passed to the pipeline constructor (not just in generation config)
            ov::genai::ASRPipeline pipe(model_path, "CPU", ov::genai::word_timestamps(true));
    ...
    <TabItemJS>
        ...
        // Word timestamps require decomposition of cross-attention decoder SDPA layers,
        // so word_timestamps must be passed to the pipeline constructor (not just in generation config)
        const pipeline = await ASRPipeline(modelPath, "CPU", { word_timestamps: true });
        ...
        // Print word-level timestamps
        for (const w of result.words?.[0] ?? []) {
          console.log(`[${w.startTs.toFixed(2)}, ${w.endTs.toFixed(2)}]: ${w.text}`);
        }
        ```
    </TabItemJS>
</LanguageTabs>

For Qwen3-ASR word timestamps, OpenVINO GenAI uses the companion Qwen3 Forced Aligner model. Qwen3-ASR and the forced aligner are separate upstream models exported to two independent directories. The forced aligner is passed explicitly to `ASRPipeline` through the `forced_aligner` constructor property.

When the property is supplied, `ASRPipeline` loads and compiles the forced aligner at construction using the same device and compile properties as the Qwen3-ASR pipeline. Word-level timestamps are then selected with the existing `word_timestamps` generation option. For long audio processed in multiple chunks, word timestamps are returned in full-audio (global) time.

The forced aligner is optional. Qwen3-ASR transcription works normally without it; requesting `word_timestamps=true` without a configured forced aligner raises an error asking you to construct the pipeline with `ov::genai::forced_aligner(path)`.

OpenVINO GenAI currently supports forced-alignment timestamps for Chinese, Cantonese, English, German, Spanish, French, Italian, Portuguese, and Russian. The Qwen3 Forced Aligner model also supports Japanese and Korean, but Japanese and Korean segmentation are not currently implemented in OpenVINO GenAI. The Unicode helpers used for preprocessing target reference-compatible segmentation of the supported languages. Forced-aligner word chunks currently have empty `token_ids`.

Because Qwen3-ASR and its forced aligner are distributed separately, export them to two independent directories. Install `qwen-asr` before exporting. Both models must be exported as stateful `automatic-speech-recognition-with-past` models: the GenAI Qwen3-ASR and forced-aligner decoders require the stateful decoder with the `beam_idx` interface, which the default stateless export does not provide.
...
# Export its companion aligner to a separate directory.
optimum-cli export openvino \
    --model Qwen/Qwen3-ForcedAligner-0.6B \
    --task automatic-speech-recognition-with-past \
    --trust-remote-code \
    qwen3-forced-aligner-ov
```

Pass the forced-aligner directory to `ASRPipeline` through the `forced_aligner` property:

<LanguageTabs>
    <TabItemPython>
        ```python
        pipe = openvino_genai.ASRPipeline(asr_model_path, "CPU", forced_aligner=forced_aligner_model_path)
    ...
    <TabItemCpp>
        ```cpp
        int main() {
            ov::genai::ASRPipeline pipe(asr_model_path, "CPU", ov::genai::forced_aligner(forced_aligner_model_path));
...
The forced aligner is also available as a standalone `ASRForcedAligner` class, usable independently of `ASRPipeline`. It aligns a transcript you already have to its audio segment, so it can add word-level timestamps to transcripts produced elsewhere (for example by a separate transcription run). The transcript must correspond to the supplied audio segment, and returned timestamps are relative to the start of that audio.

The API is currently scalar (one audio/transcript per `align` call). The same language support applies: English, Chinese, Cantonese, German, Spanish, French, Italian, Portuguese, and Russian; Japanese and Korean are present in the model configuration but are not currently implemented by OpenVINO GenAI.

<LanguageTabs>
    <TabItemPython>
        ```python
        aligner = openvino_genai.ASRForcedAligner(forced_aligner_model_path, "CPU")

        words = aligner.align(raw_speech, transcript, language="english")
    ...
    <TabItemCpp>
        ```cpp
        int main() {
            ov::genai::ASRForcedAligner aligner(forced_aligner_model_path, "CPU");

            auto words = aligner.align(raw_speech, transcript, "english");
...
<LanguageTabs>
    ...
    <TabItemCpp>
        ...
        int main() {
            ...
            result = pipe.generate(raw_speech, ov::genai::initial_prompt("Polychrome"));
            // He has gone and gone for good answered Polychrome who...
        }
        ```
    </TabItemCpp>
...
`ASRPipeline` returns `ASRDecodedResults` with a `perf_metrics` field of type `ASRPerfMetrics`, which extends [`PerfMetrics`](/docs/guides/performance-metrics).

In addition to common metrics (TTFT, TPOT, throughput, etc.), `ASRPerfMetrics` provides:

| Metric | Type | Description |
|--------|------|-------------|
| `get_features_extraction_duration()` | `MeanStdPair`, ms | Time spent extracting audio features (mel spectrogram) |
| `get_encode_inference_duration()` | `MeanStdPair`, ms | Encoder model inference duration |
| `get_decode_inference_duration()` | `MeanStdPair`, ms | Decoder model inference duration (all token generation steps) |
| `get_word_level_timestamps_processing_duration()` | `MeanStdPair`, ms | Time spent processing word-level timestamps |

Raw values are available in the `asr_raw_metrics` field of `ASRPerfMetrics`.
