...
Whisper can transcribe multiple independent audio inputs in one call. Native multi-audio batching is currently supported by `WhisperPipeline`; the generic `ASRPipeline` does not provide this batch overload.

`WhisperPipeline.generate()` provides a batched overload:

```python
pipe = ov_genai.WhisperPipeline(model_path, "CPU")

audio_0 = read_wav("speech_sample.wav")
audio_1 = read_wav("french_sample.wav")

results = pipe.generate([audio_0, audio_1], max_new_tokens=100)

for result in results:
    print(result.language, result.texts[0])
```

The batched overload returns one `WhisperDecodedResults` per input, in input order.

#### Limitations for Batch Sizes Greater Than One

For batch sizes greater than one, Whisper supports short-form and long-form audio, greedy decoding,
`return_timestamps`, and per-input language detection. The following restrictions apply:

- **Devices**: only `CPU` and `GPU` are supported.
- **Greedy decoding only**: `num_beams` must be `1` and `do_sample` must be `false`.
- **No streaming**.
- **No word timestamps**: `word_timestamps` is not supported.

:::note
Unsupported configurations raise an error instead of falling back to sequential generation.

Timestamp decoding mode is selected for the whole batch. If any input is long-form, short inputs in the same batch also use timestamp-mode decoding internally and may produce different text than when decoded alone with `return_timestamps=false`. Timestamp chunks are still not returned when `return_timestamps=false`.
:::

#### Single-Audio Batches

A single-element batch delegates to scalar generation and therefore retains the normal single-audio behavior.

The batched overload still returns a one-element list: `WhisperPipeline.generate([audio])` returns a list
containing one `WhisperDecodedResults`, not a single `WhisperDecodedResults`.

#### Performance Metrics

Performance metrics describe the entire batched call, not individual inputs.

For `WhisperPipeline`, every returned result contains a copy of the same aggregate metrics.
