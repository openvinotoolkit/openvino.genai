...
For Qwen3-ASR word timestamps, OpenVINO GenAI uses the companion Qwen3 Forced Aligner model.

OpenVINO GenAI currently supports forced-alignment timestamps for Chinese, Cantonese, English, German, Spanish, French, Italian, Portuguese, and Russian. The Qwen3 Forced Aligner model also supports Japanese and Korean, but Japanese and Korean segmentation are not currently implemented in OpenVINO GenAI.

Qwen3-ASR and its forced aligner are distributed separately and should be exported to separate directories. Install `qwen-asr` before export.

```sh
pip install qwen-asr

# Required for ASRPipeline transcription with Qwen3-ASR.
...
# Required for forced alignment (standalone or ASRPipeline word timestamps).
optimum-cli export openvino \
...
Construct the `ASRForcedAligner` and pass the instance into `ASRPipeline`. The ASR pipeline and forced aligner may use different devices.

<LanguageTabs>
    <TabItemPython>
        ```python
        aligner = openvino_genai.ASRForcedAligner(forced_aligner_model_path, "CPU")
        pipe = openvino_genai.ASRPipeline(asr_model_path, "GPU", forced_aligner=aligner)

        result = pipe.generate(raw_speech, word_timestamps=True, language="English")
    ...
    <TabItemCpp>
        ```cpp
        int main() {
            auto aligner = std::make_shared<ov::genai::ASRForcedAligner>(forced_aligner_model_path, "CPU");
            ov::genai::ASRPipeline pipe(asr_model_path, "GPU", ov::genai::forced_aligner(aligner));

            auto result = pipe.generate(raw_speech, ov::genai::word_timestamps(true), ov::genai::language("English"));
...
When word timestamps are enabled for Qwen3-ASR, audio is processed in chunks of approximately 180 seconds. This can change chunk boundaries and therefore the transcription compared with plain transcription.

The forced aligner is also available as a standalone `ASRForcedAligner` class, usable independently of `ASRPipeline`.
...
<LanguageTabs>
    ...
    <TabItemCpp>
        ...
        int main() {
            ...
            auto words = aligner.align(raw_speech, transcript, ov::genai::language("english"));
