...
Whisper can transcribe multiple independent audio inputs in one call.

#### Using `ASRPipeline`

`ASRPipeline.generate()` accepts either a single audio input or a batch of independent inputs:
...
result = pipe.generate([audio_0, audio_1], max_new_tokens=100)

for text, language in zip(result.texts, result.languages):
    print(language, text)
```

A batched call returns one `ASRDecodedResults` object. Its vector fields preserve input order:

- `texts[i]`
- `scores[i]`
- `languages[i]`
- `chunks[i]` and `words[i]`, when present
...
- **Model support**: Qwen3-ASR and FunASR do not support batch sizes greater than one.
...
For `ASRPipeline`, the single `perf_metrics` object contains the aggregate metrics.
