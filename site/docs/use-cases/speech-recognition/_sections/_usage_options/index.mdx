...
Batches of more than one audio support short-form and long-form audio, greedy decoding, `return_timestamps`, and independent per-row language detection. The following restrictions apply:

- **Devices**: supported on `CPU` and `GPU` only.
