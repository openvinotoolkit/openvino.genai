...
For an LTX model with a timestep-conditioned VAE decoder, configure decode-time conditioning as follows:
...
<LanguageTabs>
    ...
    <TabItemCpp>
        ...
        ov::Tensor video = pipe.generate(
            prompt,
            ov::genai::decode_timestep(0.05f),
            ov::genai::decode_noise_scale(0.025f)
        ).video;
        ```
    </TabItemCpp>
</LanguageTabs>
...
- `decode_timestep`: Timestep conditioning value passed to a timestep-conditioned LTX VAE decoder. The default is `0.0`.
- `decode_noise_scale`: Interpolation factor between denoised latents and random noise before VAE decoding. When omitted, `decode_timestep` is used. This parameter requires a timestep-conditioned LTX VAE decoder.
...
Refer to the [Image Generation Performance Metrics](/docs/use-cases/image-generation/#performance-metrics) for details.
