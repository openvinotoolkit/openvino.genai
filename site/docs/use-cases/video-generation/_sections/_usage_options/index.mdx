...
`Text2VideoPipeline` returns `VideoGenerationResult` with a `performance_stat` field of type `VideoGenerationPerfMetrics`, which inherits from `ImageGenerationPerfMetrics`.
