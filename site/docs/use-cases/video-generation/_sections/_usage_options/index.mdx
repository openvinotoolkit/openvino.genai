    ...
    <TabItemCpp>
        ...
        int main(int argc, char* argv[]) {
            ...
            ov::Tensor video = pipe.generate(
                ...
                ov::genai::decode_timestep(0.0f),
                ov::genai::decode_noise_scale(0.0f)
                // highlight-end
            ).video;
...
- `decode_timestep`: Decode-time timestep for timestep-conditioned VAE decoders. If omitted (`std::nullopt` in C++ config), pipeline default is used.
- `decode_noise_scale`: Decode-time conditioning noise scale for timestep-conditioned VAE decoders. If omitted (`std::nullopt` in C++ config), default decode_timestep value is used.

These decode-time parameters are forwarded to VAE only when the VAE config enables `timestep_conditioning`.
