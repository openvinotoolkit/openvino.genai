...
:::tip
Check out [Python](https://github.com/openvinotoolkit/openvino.genai/tree/master/samples/python/rag) and [C++](https://github.com/openvinotoolkit/openvino.genai/tree/master/samples/cpp/rag) multimodal embedding samples (`image_video_embedding`).
:::
...
The text-embedding behavior of the pipeline can be tuned by passing configuration parameters at construction time. They can be passed directly as properties or bundled into a `TextEmbeddingPipeline.Config` and supplied via `text_embedding_config`.
...
<LanguageTabs>
    <TabItemPython>
        ...
        query_vec = np.asarray(query.data, dtype=np.float32).reshape(-1)

        # Embed an image and compare it with the query
        image = ov.Tensor(np.array(Image.open("cat.jpg").convert("RGB")))
        image_vec = np.asarray(pipeline.embed(images=[image]).embeddings.data, dtype=np.float32).reshape(-1)

        similarity = np.dot(query_vec, image_vec) / (np.linalg.norm(query_vec) * np.linalg.norm(image_vec))
    ...
    <TabItemCpp>
        ...
        // Compare query.embeddings with image_embedding.embeddings (e.g. cosine similarity)
        ```
    </TabItemCpp>
</LanguageTabs>
...
`EmbeddingPipeline` can also be used with text-only embedding models. Simply pass text to `embed()` and omit the `images` and `videos` arguments:
...
<LanguageTabs>
    <TabItemPython>
        ...
        embeddings = pipeline.embed(["What is the capital of France?", "The capital of France is Paris."]).embeddings
        ```
    </TabItemPython>
    <TabItemCpp>
        ```cpp
        ...
        // Embed a single text or a batch of texts
        ov::genai::EmbedResult embeddings = pipeline.embed(
            std::vector<std::string>{"What is the capital of France?", "The capital of France is Paris."}
        );
        ```
    </TabItemCpp>
</LanguageTabs>
...
:::tip
For dedicated text embedding (with `embed_documents`/`embed_query` helpers and async APIs), see the [Semantic Search using Text Embedding](/docs/use-cases/text-embedding) guide.
:::

:::info
For the full list of configuration options, see the [TextEmbeddingPipeline API Reference](https://docs.openvino.ai/2026/api/genai_api/_autosummary/openvino_genai.TextEmbeddingPipeline.html).
:::
