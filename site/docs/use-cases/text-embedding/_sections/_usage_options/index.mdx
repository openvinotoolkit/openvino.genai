...
- `max_length`: Maximum length of tokens passed to the embedding model. Texts longer than this will be truncated.
...
Setting `batch_size`, `max_length`, and `pad_to_max_length=true` together will fix the model shape for optimal inference performance. Fixed shapes are also required for NPU device inference.
