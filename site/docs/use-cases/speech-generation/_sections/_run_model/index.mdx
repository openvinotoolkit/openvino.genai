...
:::tip
Check out [Python](https://github.com/openvinotoolkit/openvino.genai/blob/master/samples/python/speech_generation/text2speech.py), [C++](https://github.com/openvinotoolkit/openvino.genai/blob/master/samples/cpp/speech_generation/text2speech.cpp), and [Node.js](https://github.com/openvinotoolkit/openvino.genai/blob/master/samples/js/speech_generation/test.js) speech generation samples.
:::
...
<LanguageTabs>
    ...
    <TabItemJS>
        ...
        const embeddingWav = await readFile(embeddingPath);
        const decodedEmbedding = decode(embeddingWav);
        const maxEmbeddingLength = 512;
        const speakerEmbedding = new ov.Tensor(
            'f32',
            [1, maxEmbeddingLength],
            decodedEmbedding.channelData[0].slice(0, maxEmbeddingLength)
        );
