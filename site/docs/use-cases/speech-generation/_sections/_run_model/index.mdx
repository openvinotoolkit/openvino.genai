...
The [`Text2SpeechPipeline`](https://docs.openvino.ai/2026/api/genai_api/_autosummary/openvino_genai.Text2SpeechPipeline.html) is the main object for generating speech from text.
It automatically loads the TTS model and vocoder from the converted model directory.
The output sample rate is model-dependent and is returned in the generation result.
...
:::tip
Check out [Python](https://github.com/openvinotoolkit/openvino.genai/blob/master/samples/python/speech_generation/text2speech.py), [C++](https://github.com/openvinotoolkit/openvino.genai/blob/master/samples/cpp/speech_generation/text2speech.cpp), and [Node.js](https://github.com/openvinotoolkit/openvino.genai/blob/master/samples/js/speech_generation/text2speech.js) speech generation samples.
:::

### Use Speaker Embedding File

To generate speech using the SpeechT5 model, you can specify a target voice by providing a speaker embedding file.
...
<LanguageTabs>
    <TabItemPython>
        ...
        sf.write("output_audio.wav", speech.data[0], samplerate=result.output_sample_rate)
        ```
    </TabItemPython>
    <TabItemCpp>
        ```cpp
    ...
    <TabItemJS>
        ...
        const pipeline = await Text2SpeechPipeline(modelPath, "CPU");

        const speakerEmbedding = await readSpeakerEmbedding(embeddingPath);
...
`Text2SpeechPipeline` returns `Text2SpeechDecodedResults` with a `perf_metrics` field of type `SpeechGenerationPerfMetrics`, which extends [`PerfMetrics`](/docs/guides/performance-metrics).

In addition to common metrics, `SpeechGenerationPerfMetrics` provides:

| Metric | Type | Description |
|--------|------|-------------|
| `num_generated_samples` | integer | Number of audio samples generated |
