...
All three are usable with the `LLMPipeline` Python and C++ APIs. Fast Draft and EAGLE3 are supported on both the Continuous Batching and Stateful backends; Prompt Lookup is supported on the Continuous Batching backend only.
