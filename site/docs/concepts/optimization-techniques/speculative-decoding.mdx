...
All three are usable with the `LLMPipeline` Python and C++ APIs. Fast Draft and EAGLE3 can be supported on both the Continuous Batching(CPU/GPU/NPU) and Stateful backends(NPU only); Prompt Lookup is supported on the Continuous Batching backend only.
