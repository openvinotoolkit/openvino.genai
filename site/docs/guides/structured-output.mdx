...
Structured output generation contains several steps:
1. Compile the structured-output configuration (JSON Schema, regex, or EBNF grammar) into an internal grammar representation.
2. After the model computes the probability distribution for the next token, apply a mask that prevents tokens incompatible with the current grammar state from being sampled.
