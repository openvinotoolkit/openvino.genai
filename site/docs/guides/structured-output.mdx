...
- Built-in model tool-call formats: construct a `StructuredOutputConfig` for model-specific function calling syntax from OpenAI-compatible `tools` and `tool_choice` JSON.
...
Use `StructuredOutputConfig::from_model_format` when a model expects tool calls in one of the built-in model-specific formats. The helper converts an OpenAI-compatible tools list and tool choice into the corresponding `structural_tags_config`.

Supported `model_format` keys are:
...
Unknown keys raise an exception that lists the supported formats.
...
so_config = ov_genai.StructuredOutputConfig.from_model_format(
    "llama",
    tools,
...
gen_config = ov_genai.GenerationConfig(structured_output_config=so_config)
```
    </TabItemPython>
    <TabItemCpp>
```cpp
...
JsonContainer tools = JsonContainer::from_json_string(R"([
    {
        "type": "function",
        "function": {
            "name": "get_weather",
...
JsonContainer tool_choice = JsonContainer::from_json_string(R"({
    "type": "function",
...
options.any_order = true;

StructuredOutputConfig so_config =
    StructuredOutputConfig::from_model_format("llama", tools, tool_choice, options);
...
The `tools` argument must be a JSON array. Each item must be an object with a string `type` field.

Function tools use the OpenAI-compatible function tool shape:
...
For function tools, `function.name` is required. `function.parameters` is optional and must be an object or `null` when present. `function.strict` is optional and must be a boolean or `null` when present. If `parameters` is missing or `null`, or if `strict` is `false`, the arguments schema is treated as unconstrained.

Built-in tools use a non-`function` `type` and can include an optional `name` and optional `parameters` object:
...
If a built-in tool omits `name`, the `type` is used as the name. Built-in tools are only useful with model formats that define syntax for them, such as `harmony`.

### Tool Choice JSON

`tool_choice` is optional. In C++, the default is `JsonContainer("auto")`; in Python, the default is `"auto"`. Passing `null` or `None` is also treated as `auto`.

Accepted string values are:

- `"auto"` - allow regular text or one of the provided tools.
- `"none"` - disable tool calls for this generation by clearing the tool set.
- `"required"` - require a tool call. The effective tools list must not be empty.

Force one function tool by name:
...
Restrict the available tools and choose whether a tool is optional or required:
...
`allowed_tools.mode` must be `"auto"` or `"required"`. Every referenced tool must exist in the `tools` list. Function references match by `function.name`; built-in references match by `type`.

Force one built-in tool by type:
...
A forced tool choice must resolve to exactly one available tool.

### Options

`ModelStructuralTagOptions` controls how the model-format configuration is built:

- `reasoning` - include reasoning tags for formats that support them. Defaults to `true`.
- `any_order` - allow JSON object properties to be generated in any order inside generated argument objects. Defaults to `false`.
- `exclude_special_tokens` - exclude format-specific special tokens from unconstrained text spans. Defaults to `true`.

Python exposes the same options as keyword arguments on `StructuredOutputConfig.from_model_format`.
