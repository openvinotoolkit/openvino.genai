...
For example, when the pipeline is configured with:
...
```
Note: 
`save_ov_model_quantize_mode` maps to `OVModelQuantizeMode` in
[`llm_pipeline.hpp`](../../../src/cpp/include/openvino/genai/llm_pipeline.hpp).

Since `save_ov_model_quantize_mode` supports saving multiple model variants,
mode-specific directories are used to avoid overwriting files (for example, `ov_model_gpu_optimized`).