...
When setting the environment variable `OPENVINO_LOG_LEVEL` > `ov::log::Level::INFO`, the following information will be displayed if applicable: the backend type used(PA/SDPA), CDPruner operation details, speech generation information.
...
<Tabs groupId="os">
    ...
    <TabItem label="Windows" value="windows">
        ```sh
        set OPENVINO_LOG_LEVEL=2
        ```
    </TabItem>
</Tabs>

For example, if `LLMPipeline` or `VLMPipeline` is created, after pipeline initialization and reading the model, the next message will be printed to the console.
```sh title="Output:"
