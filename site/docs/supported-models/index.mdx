...
3. If the number of input frames is not divisible by `mm_local_num_frames` (as defined in `config.json`), pad additional frames by duplicating the last frame. For example, if there are 10 frames and `mm_local_num_frames = 4`, pad to 12 frames.
