...
3. If the number of input frames is not divisible by `mm_local_num_frames` (as defined in `config.json`), the remaining frames are discarded. Example: 10 frames with mm_local_num_frames = 4, the last 2 frames are dropped.
