...
import EmbeddingsModelsTable from './_components/embeddings-models-table';
...
Embeddings models are used with the `EmbeddingPipeline`, which supports both text-only and multimodal (text, image, and video) embedding models.

:::info LoRA Support
Embeddings pipeline does **not** support LoRA adapters.
:::

<EmbeddingsModelsTable />
