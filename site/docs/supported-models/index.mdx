...
1. Current implementation is video-only. Image inference is not supported for this model type.
2. Visual history across rounds is not supported, so multi-round visual context is limited.
3. If input frame count is not divisible by mm_local_num_frames in config.json, it will pad with last frame (e.g., 10 frames with mm_local_num_frames=4, pad it to 12 frames and the last 3 frames will be same).
