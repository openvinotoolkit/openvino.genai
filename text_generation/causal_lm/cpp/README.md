...
`--no-build-isolation` solves https://github.com/AutoGPTQ/AutoGPTQ/issues/535, if there's no prebuilt `auto-gptq` for your environment, ARM64 for example.
...
```sh
source <INSTALL_DIR>/setupvars.sh
python3 -m pip install --no-build-isolation --upgrade-strategy eager "transformers<4.38" -r ../../../llm_bench/python/requirements.txt ../../../thirdparty/openvino_tokenizers/[transformers] --extra-index-url https://download.pytorch.org/whl/cpu
...
```bat
<INSTALL_DIR>\setupvars.bat
python -m pip install --no-build-isolation --upgrade-strategy eager "transformers<4.38" -r ..\..\..\llm_bench\python\requirements.txt ..\..\..\thirdparty\openvino_tokenizers\[transformers] --extra-index-url https://download.pytorch.org/whl/cpu
