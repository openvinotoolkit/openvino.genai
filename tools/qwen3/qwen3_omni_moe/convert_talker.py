...
def convert_talker_embedding(model: Qwen3OmniMoeForConditionalGeneration, output_dir: Path) -> None:
    ...
    embed_tokens = model.talker.model.get_input_embeddings()
...
def convert_talker_language(
    ...
    output_path = output_dir / TALKER_LANGUAGE_NAME
    if output_path.exists():
        return
    ...
    lang_model.forward = types.MethodType(_forward_wrap_talker, lang_model)
    ...
    print("Talker language model saved")
