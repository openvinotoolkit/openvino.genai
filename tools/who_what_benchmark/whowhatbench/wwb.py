...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Text-to-video only. Optional decode-time timestep for timestep-conditioned VAE decoders.",
    )
    parser.add_argument(
        "--decode-noise-scale",
        type=float,
        default=None,
        help="Text-to-video only. Optional decode-time conditioning noise scale.",
    )
    parser.add_argument(
        "--pruning_ratio",
...
def check_args(args):
    ...
    has_video_decode_conditioning = args.decode_timestep is not None or args.decode_noise_scale is not None
    if has_video_decode_conditioning and args.model_type != "text-to-video":
        raise ValueError(
...
def main():
    ...
    def apply_text2video_decode_conditioning_config(model):
        if model is None or args.model_type != "text-to-video":
            return

        if args.decode_timestep is None and args.decode_noise_scale is None:
            return

        if args.genai:
            generation_config = model.get_generation_config()
            if args.decode_timestep is not None:
                generation_config.decode_timestep = args.decode_timestep
            if args.decode_noise_scale is not None:
                generation_config.decode_noise_scale = args.decode_noise_scale
            model.set_generation_config(generation_config)

        logger.info(
            "Text-to-video decode conditioning: decode_timestep=%s, decode_noise_scale=%s",
    ...
    if args.base_model is not None:
        ...
        apply_text2video_decode_conditioning_config(base_model)
    ...
    if args.target_data or args.target_model:
        if args.target_data and os.path.exists(args.target_data):
            ...
            apply_text2video_decode_conditioning_config(target_model)
