...
class TextAgentEvaluator(BaseEvaluator):
    ...
    def score(self, model_or_data, gen_answer_fn=None, **kwargs):
        ...
        result_folder = (
            ...
            if output_dir
            else (os.path.join(self.gt_dir, "target") if self.gt_dir else "target")
        )
    ...
    def _get_records(self) -> List[Dict[str, Any]]:
        if self.test_data is None:
            raise ValueError("text-agent evaluator requires a JSON dataset (--dataset) with messages records")
    ...
    def _message_has_content(msg: Dict[str, Any]) -> bool:
        ...
        if isinstance(content, list):
            return any(
                (isinstance(item, dict) and item.get("text", "").strip()) or (isinstance(item, str) and item.strip())
                for item in content
            )
    ...
    def _get_chat_template_for_model(self, model, tokenizer=None):
        if self.chat_template_source:
            logger.info("Using chat template source from --chat-template-source: %s", self.chat_template_source)
            return self.chat_template_source
    ...
    def _load_chat_template(self, model, tokenizer):
        ...
        if chat_template_source:
            if os.path.isfile(chat_template_source):
                logger.info("Loading chat template from file: %s", chat_template_source)
                with open(chat_template_source, "r", encoding="utf-8") as f:
                    return f.read()

            if chat_template_source.startswith(("http://", "https://")):
                parsed_url = urllib.parse.urlparse(chat_template_source)
                if parsed_url.scheme not in {"http", "https"}:
                    raise ValueError(
                        f"Unsupported URL scheme for chat template source: {parsed_url.scheme}. "
                        "Only http and https are allowed."
                    )
                logger.info("Loading chat template from URL: %s", chat_template_source)
                request = urllib.request.Request(chat_template_source, method="GET")
    ...
    def _render_messages_to_prompt(self, messages, tools, chat_template):
        ...
        try:
            ...
            logger.error(
                "Template render failed. Messages: %s",
                json.dumps(messages, ensure_ascii=False, default=str),
            )
            logger.error(
                "Template render failed. Tools: %s",
                json.dumps(tools, ensure_ascii=False, default=str) if tools else None,
            )
            raise
    ...
    def _apply_tokenizer_chat_template_to_prompt(tokenizer, messages, tools):
        ...
        if not isinstance(prompt, str) or not prompt.strip():
            raise ValueError(
                "Rendered prompt is empty after chat template application; check/fix the template or messages"
            )
        return prompt
    ...
    def _generate_data(self, model, gen_answer_fn=None, result_dir=None):
        ...
        for idx, record in enumerate(tqdm(records, desc="Evaluate pipeline")):
            ...
            if not isinstance(prompt_preview, str) or not prompt_preview.strip():
                raise ValueError(
                    f"Prompt preview is empty for record index {idx}; check the messages content in your dataset"
                )
