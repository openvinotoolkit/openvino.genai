...
class YoutuVLInputsPreprocessor(VLMInputsPreprocessor):
    """Inputs preprocessor for Tencent Youtu-VL (model_type ``youtu_vl``).

    The remote-code ``YoutuVLProcessor`` exposes the standard
    ``apply_chat_template`` + ``__call__(text=..., images=...)`` interface and
    uses ``<|image_pad|>`` / ``<|video_pad|>`` media tokens, so the message
    construction mirrors the Qwen2-VL family. The processor's ``__call__``
    accepts ``text`` and ``images`` directly (videos are forwarded through
    ``**kwargs``), and it performs the media-token expansion internally.
    """
    ...
    def preprocess_inputs(
        ...
        # YoutuVLProcessor.__call__ forwards unknown kwargs, but only pass
        # videos when present to avoid an empty/None videos branch.
