...
def run_test_optimized(model_id, model_type, tmp_path, pruning_ratio, relevance_weight):
    """Optimized version: uses convert_model() from conftest (cached) instead of inline conversion."""
    if sys.platform == "darwin":
        pytest.xfail("Ticket 173169")
    if sys.platform == "win32":
        pytest.xfail("Ticket 178790")

    _log(f"run_test_optimized: model_id={model_id} model_type={model_type} pruning_ratio={pruning_ratio} relevance_weight={relevance_weight}")
    GT_FILE = tmp_path / "gt.csv"

    with _stage("convert_model_cached"):
        MODEL_PATH = convert_model(model_id)
    _log(f"Model converted (cached) to: {MODEL_PATH}")
...
def test_pruner_basic_optimized(model_id, model_type, tmp_path, pruning_ratio, relevance_weight):
    _log(f"test_pruner_basic_optimized: model_id={model_id} model_type={model_type} pruning_ratio={pruning_ratio} relevance_weight={relevance_weight}")
    run_test_optimized(model_id, model_type, tmp_path, pruning_ratio, relevance_weight)
