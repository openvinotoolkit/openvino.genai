...
def get_argparser():
    ...
    parser.add_argument(
        "-f", "--framework", default="ov", choices={"ov", "pt"}, help="inference framework, ov: OpenVINO, pt: PyTorch"
    )
    ...
    parser.add_argument(
        ...
        required=False,
        default=None,
        help="Path to file with Continuous Batching Scheduler settings or dict",
    )
    parser.add_argument(
        "--draft_model",
        required=False,
    ...
    parser.add_argument(
        ...
        required=False,
        default=None,
        help="Config option max_ngram_size for Prompt Lookup decoding",
        type=int,
    )
    parser.add_argument(
        "--end_token_stopping",
    ...
    parser.add_argument(
        ...
        type=int,
        required=False,
        help="The length of tokens print each time in streaming mode, chunk streaming.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--static_reshape",
        action="store_true",
        help="Reshape image generation pipeline to specific width & height at pipeline creation time. Applicable for Image and Video Generation.",
    )
    parser.add_argument(
        "--frame_rate",
        type=float,
        required=False,
        help="Frame rate for video generation and saving. Applicable only for Video Generation.",
    )
    parser.add_argument(
        "--num_frames",
        type=int,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Disable modification prompt from run to run to allow prefix caching",
    )
    parser.add_argument(
        "--embedding_pooling",
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Speech language. For text-to-speech Kokoro models this can be one of en-us, en-gb, es, fr-fr, hi, it, "
            "pt-br, ja, zh. For speech-to-text models it selects the transcription language (e.g. en for FunASR, "
            "English for Qwen3-ASR, <|en|> for Whisper); a per-entry JSON 'language' takes precedence over this."
        ),
    )
    parser.add_argument(
        "--speech_voice",
...
def main():
    ...
    if args.streaming and args.tokens_len is None:
        ...
        exit(1)
    model_path, framework, model_args = analyze_args(args)
    ...
    if framework == "ov":
        ...
        if "cpu" in args.device.lower():
            ...
            if env_omp is None or env_omp != "PASSIVE":
                log.warning(
                    "It is recommended to set the environment variable OMP_WAIT_POLICY to PASSIVE, "
