...
def run_video_generation_benchmark(model_path, framework, device, args, num_iters, mem_consumption):
    # Build the prompt schedule via BenchPrompter, which reads and parses the
    # video-generation prompt file, honours args['prompt_index'], and drives
    # both --subsequent modes from one iter_schedule() loop.
    #
    # NOTE: this task's --subsequent handling changed. See "Iteration order"
    # in the README — video generation used to interpret the flag inverted.
