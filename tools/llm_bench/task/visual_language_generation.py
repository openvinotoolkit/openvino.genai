...
def run_visual_language_generation_optimum(
    ...
    input_data = model.preprocess_inputs(
        image=images[0] if images else None, video=videos[0] if videos else None, text=prompts[0], **processor
    )
    ...
    generated_text = processor["tokenizer"].batch_decode(
        result[:, input_data["input_ids"].shape[1] :], skip_special_tokens=True
    )
