...
def run_ldm_super_resolution(
    img, num, pipe, args, framework, iter_data_list, image_id, tm_list, proc_id, mem_consumption
):
    set_seed(args["seed"])
    ...
    metrics_print.print_metrics(num, iter_data, warm_up=(num == 0), prompt_idx=image_id)
...
def run_ldm_super_resolution_benchmark(model_path, framework, device, args, num_iters, mem_consumption):
    ...
    log.info(
        f"Benchmarking iter nums(exclude warm-up): {num_iters}, image nums: {len(image_list)}, prompt idx: {prompt_idx_list}"
    )
            ...
            for image in image_param_list:
                if args["prompt_file"] is not None and len(args["prompt_file"]) > 0:
                    image["prompt"] = os.path.join(
                        os.path.dirname(args["prompt_file"][0]), image["prompt"].replace("./", "")
                    )
                images.append(image)
    else:
