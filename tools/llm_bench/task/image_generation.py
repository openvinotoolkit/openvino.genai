...
def collects_input_args(
    image_param, model_name, infer_count=None, height=None, width=None, callback=None, image_as_ov_tensor=True
):
    input_args = {}
    ...
    if infer_count is None:
        input_args["num_inference_steps"] = image_param.get(
            "steps", DEFAULT_INFERENCE_STEPS if "lcm" not in model_name else LCM_DEFAULT_INFERENCE_STEPS
        )
    else:
...
def run_image_generation(
    image_param, num, image_id, pipe, args, iter_data_list, proc_id, mem_consumption, callback=None
):
    set_seed(args["seed"])
    input_text = image_param["prompt"]
    input_args = collects_input_args(
        image_param,
        args["model_name"],
        args["num_steps"],
        args.get("height"),
        args.get("width"),
        image_as_ov_tensor=False,
    )
    out_str = (
        f"Input params: Batch_size={args['batch_size']}, "
    ...
    if (args["mem_consumption"] == 1 and num == 0) or args["mem_consumption"] == 2:
        ...
        max_rss_mem_consumption, max_rss_mem_increase, max_sys_mem_consumption, max_sys_mem_increase = (
            mem_consumption.get_data()
        )
    ...
    metrics_print.print_metrics(
        num, iter_data, warm_up=(num == 0), stable_diffusion=stable_diffusion_hook, prompt_idx=image_id
    )
...
def run_image_generation_genai(
    image_param, num, image_id, pipe, args, iter_data_list, proc_id, mem_consumption, callback=None
):
    set_seed(args["seed"])
    input_text = image_param["prompt"]
    ...
    input_args = collects_input_args(
        image_param, args["model_name"], args["num_steps"], args.get("height"), args.get("width"), callback
    )
    out_str = (
        f"Input params: Batch_size={args['batch_size']}, "
        f"steps={input_args['num_inference_steps']}, width={input_args['width']}, height={input_args['height']}"
    ...
    if args.get("static_reshape", False) and "guidance_scale" in input_args:
        ...
        if new_gs != reshaped_gs:
            log.warning(
                f"image generation pipeline was reshaped with guidance_scale={reshaped_gs}, but is being passed into generate() as {new_gs}"
            )
    ...
    iter_data_list.append(iter_data)
    metrics_print.print_metrics(
        num, iter_data, warm_up=(num == 0), stable_diffusion=performance_metrics, prompt_idx=image_id
    )
    metrics_print.print_generated(num, warm_up=(num == 0), generated=rslt_img_fn, prompt_idx=image_id)
    stable_diffusion_hook.clear_statistics()
...
def run_image_generation_benchmark(model_path, framework, device, args, num_iters, mem_consumption):
    ...
    if args.get("static_reshape", False):
        static_input_args = collects_input_args(
            image_list[0],
            args["model_name"],
            args["num_steps"],
            args.get("height"),
    ...
    pipe, pretrain_time, use_genai, callback = FW_UTILS[framework].create_image_gen_model(
