...
def run_speech_2_txt_generation(input_param, args, md5_list, iter_data_list):
    ...
    if num > 0:
        ...
        if result_md5_list != prev_md5:
            log.warning(
                f"[{num}] Prompt[{speech_id}]'s md5 {result_md5_list} "
...
def run_speech_2_txt_benchmark(model_path, framework, device, args, num_iters, mem_consumption):
    ...
    log.info(
        f"Benchmarking iter nums(exclude warm-up): {num_iters}, speech file nums: {len(speech_file_list)}, speech idx: {speech_idx_list}"
    )
    ...
    input_param = {"pipe": pipe, "mem_consumption": mem_consumption, "processor": processor, "use_genai": use_genai}
        ...
        if len(speech_param_list) > 0:
            for speech_file in speech_param_list:
                if args["prompt_file"] is not None and len(args["prompt_file"]) > 0:
                    speech_file["media"] = os.path.join(
                        os.path.dirname(args["prompt_file"][0]), speech_file["media"].replace("./", "")
                    )
