...
def torch_compile_child_module(model, child_modules, backend="openvino", dynamic=None, options=None):
    if len(child_modules) == 1:
        setattr(
            ...
            child_modules[0],
            torch.compile(
                getattr(model, child_modules[0]), backend=backend, dynamic=dynamic, fullgraph=True, options=options
            ),
        )
        return model
    setattr(
        model,
        child_modules[0],
        torch_compile_child_module(getattr(model, child_modules[0]), child_modules[1:], backend, dynamic, options),
    )
    return model


def run_torch_compile(
    model, backend="openvino", dynamic=None, options=None, child_modules=None, memory_data_collector=None
):
...
def create_text_gen_model(model_path, device, memory_data_collector, **kwargs):
    ...
    if kwargs["torch_compile_backend"]:
        ...
        compiled_model = run_torch_compile(
            ...
            options,
            child_modules,
            memory_data_collector if kwargs.get("mem_consumption") else None,
        )
