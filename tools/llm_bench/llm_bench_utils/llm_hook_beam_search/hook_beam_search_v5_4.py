...
# Transformers version: v5.4.0 .. v5.13.0
...
def new_beam_search(
    ...
    **model_kwargs,
    ...
    while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
        ...
        # Compat: v5.13 upstream selects by key presence; v5.4-v5.12 upstream guards on `is not None`. Callers on
        # v5.4-v5.12 may store the cache key with a None value, so we additionally require `is not None` here to
        # avoid regressing those versions while preserving the ALL_CACHE_NAMES generalization from v5.13.
        active_cache_key = next((k for k in ALL_CACHE_NAMES if model_kwargs.get(k) is not None), None)
        if active_cache_key is not None:
            beam_idx = self._flatten_beam_dim(running_beam_indices[..., cur_len - decoder_prompt_len])
            if hasattr(self, "_reorder_cache"):
                model_kwargs[active_cache_key] = self._reorder_cache(model_kwargs[active_cache_key], beam_idx)
            elif hasattr(model_kwargs[active_cache_key], "reorder_cache"):
                model_kwargs[active_cache_key].reorder_cache(beam_idx)
            else:
                raise ValueError(
