...
KNOWN_PRECISIONS = [
    ...
    "OV_FP32-4BIT_MAXIMUM",
    "OV_FP16-4BIT_MAXIMUM",
]
    ...
    if args["prompt_file"] is None:
        if not isinstance(input_key, (list, tuple)):
            ...
            if args["use_case"].task not in ["visual_text_gen", "image_gen", "video_gen"]:
                raise RuntimeError(
                    "Multiple sources for benchmarking supported for Visual Language Models / Image To Image Models / Inpainting Models"
                )
            data_dict = {}
            if "media" in input_key:
                if args["media"] is None and args["images"] is None:
                    if args["use_case"].task == "visual_text_gen":
                        ...
                        raise RuntimeError(
                            "No input image. ImageToImage/Inpainting Models cannot start generation without one. Please, provide an image."
                        )
                else:
            ...
            if "mask_image" in input_key:
                if args.get("mask_image"):
                    ...
                    raise RuntimeError(
                        "Mask image is not provided. Inpainting Models cannot start of generation without it. Please, provide a mask image."
                    )
...
def analyze_args(args):
    ...
    if args.from_onnx and not optimum:
        log.warning(
            "ONNX model initialization supported only using Optimum. Benchmarking will be switched to this backend"
        )
        optimum = True
    ...
    has_torch_compile_options = any(
        [args.torch_compile_options is not None, args.torch_compile_options is not None, args.torch_compile_dynamic]
    )
    if model_args["torch_compile_backend"] is None and has_torch_compile_options:
        log.warning(
            "torch.compile configuration options provided, but backend is not selected, openvino backend will be used"
        )
    ...
    if args.draft_model:
        if (
            args.draft_device != "NPU"
            and args.device != "NPU"
            and model_args["config"]["ATTENTION_BACKEND"] != PA_ATTENTION_BACKEND
        ):
    ...
    if Path(speaker_embeddings_file).is_file():
        if ".pt" in speaker_embeddings_file:
            try:
                ...
                raise RuntimeError(
                    f"==Parse file with torch: {speaker_embeddings_file} failure, format is incorrect =="
                )
        else:
            try:
        ...
        if speaker_embeddings.numel() != np.prod(expected_shape):
            raise RuntimeError(
                f"==Expected {np.prod(expected_shape)} elements, but got {speaker_embeddings.numel()} in {speaker_embeddings_file}=="
            )
