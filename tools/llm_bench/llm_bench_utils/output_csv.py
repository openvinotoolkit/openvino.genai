...
def gen_data_to_csv(
    ...
    pretrain_time: int,
    iter_timestamp: dict,
    memory_data_collector: MemoryDataSummarizer | None,
    mem_unit: MemoryUnit,
):
    ...
    result["start"], result["end"] = output_json.get_timestamp(
        iter_data["iteration"], iter_data["prompt_idx"], iter_timestamp
    )
...
def write_result(
    ...
    mem_unit = (
        memory_data_collector.memory_monitor.memory_unit if memory_data_collector else MemoryDataSummarizer.DEF_MEM_UNIT
    )
    header = [
        "iteration",
        "model",
