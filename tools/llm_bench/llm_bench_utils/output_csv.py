...
from llm_bench_utils.memory_monitor import MemMonitorWrapper, MemoryUnit
...
def gen_data_to_csv(result: dict, iter_data: dict, pretrain_time: int, iter_timestamp: dict,
                    memory_data_collector: MemMonitorWrapper | None, mem_unit: MemoryUnit):
...
def write_result(report_file, model, framework, device, model_args, iter_data_list, pretrain_time, model_precision, iter_timestamp, memory_data_collector):
    mem_unit = memory_data_collector.mmw.memory_unit if memory_data_collector.mmw else MemMonitorWrapper.DEF_MEM_UNIT
    header = [
        'iteration',
        'model',
        'framework',
