...
def print_metrics(
    ...
    if iter_data.get("mm_embeddings_preparation_time", "") != "":
        output_str += " Multimodal Embeddings Preparation Time: {:.2f}ms, ".format(
            iter_data["mm_embeddings_preparation_time"]
        )
    ...
    if tms is not None:
        ...
        first_token_latency = (
            "NA" if iter_data["first_token_latency"] == -1 else f"{iter_data['first_token_latency']:.2f} ms"
        )
        other_token_latency = (
            "NA"
            if iter_data["other_tokens_avg_latency"] == -1
            else f"{iter_data['other_tokens_avg_latency']:.2f} ms/{latency_unit}"
        )
    ...
    if tms_infer is not None:
        ...
        first_infer_latency = (
            "NA" if iter_data["first_token_infer_latency"] == -1 else f"{iter_data['first_token_infer_latency']:.2f} ms"
        )
        other_infer_latency = (
            "NA"
            if iter_data["other_tokens_infer_avg_latency"] == -1
            else f"{iter_data['other_tokens_infer_avg_latency']:.2f} ms/infer"
        )
        log.info(
...
def print_stable_diffusion_infer_latency(iter_str, iter_data, stable_diffusion=None, prompt_idx=-1):
    ...
    prefix = f"[{iter_str}][P{prompt_idx}]"
    log.info(
        f"{prefix} First step of {main_model_name} latency: {iter_data['first_token_latency']:.2f} ms, "
...
def print_ldm_unet_vqvae_infer_latency(iter_num, iter_data, tms=None, warm_up=False, prompt_idx=-1):
    ...
    other_token_latency = (
        "NA" if iter_data["other_tokens_avg_latency"] == -1 else f"{iter_data['other_tokens_avg_latency']:.2f} ms/step"
    )
    prefix = f"[{iter_str}][P{prompt_idx}]"
    log.info(
        f"{prefix} First step of unet latency: {first_token_latency}, "
        f"other steps of unet latency: {other_token_latency}",
    )
    if len_tms > 1:
        log.info(
            f"{prefix} Unet latency: {(sum(tms[0 : (len_tms - 1)]) / (len_tms - 1)) * 1000:.2f} ms/step, "
...
def output_avg_statis_tokens(
    prompt_dict, prompt_idx_list, iter_data_list, batch_size, is_text_gen, is_embed, loop_idx, latency_unit=None
):
    for p_idx in prompt_idx_list:
        ...
        if index_num > 0:
            ...
            prefix = (
                f"[ INFO ] [Average] P[{p_idx}]L[{loop_idx}]" if loop_idx != -1 else f"[ INFO ] [Average] P[{p_idx}]"
            )
            if is_text_gen:
                output_info = ""
                if avg_input_size > 0:
...
def print_average(
    iter_data_list, prompt_idx_list, batch_size, is_text_gen=False, is_embed=False, loop_idx=-1, latency_unit=None
):
    ...
    if total_iters > 0:
        ...
        output_avg_statis_tokens(
            prompt_dict, prompt_idx_list, iter_data_list, batch_size, is_text_gen, is_embed, loop_idx, latency_unit
        )
