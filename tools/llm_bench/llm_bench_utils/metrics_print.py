...
def print_metrics(
    ...
    if tms_infer is not None:
        ...
        log.info(
            f"{prefix} First infer latency: {first_infer_latency}, " f"other infers latency: {other_infer_latency}, inference count: {len(tms_infer)}",
        )
        if len(tms_infer) == 0:
    ...
    if cb_metric and "avg_cache_usage" in cb_metric and "max_cache_usage" in cb_metric:
        log.info(
            f'Running average of the KV cache usage {cb_metric["avg_cache_usage"]:.2f}%, ' f'max KV cache usage: {cb_metric["max_cache_usage"]:.2f}%',
        )
...
def print_ldm_unet_vqvae_infer_latency(iter_num, iter_data, tms=None, warm_up=False, prompt_idx=-1):
    ...
    prefix = f"[{iter_str}][P{prompt_idx}]"
    log.info(
        f"{prefix} First step of unet latency: {first_token_latency}, " f"other steps of unet latency: {other_token_latency}",
    )
    if len_tms > 1:
        log.info(
