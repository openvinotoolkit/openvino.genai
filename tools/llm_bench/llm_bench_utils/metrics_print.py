...
def print_metrics(iter_num, iter_data, tms=None, tms_infer=None, warm_up=False,
                  ...
                  tts=None, cb_metric=None, text_rerank=None, chat_idx=None):
    ...
    if tms is not None:
        ...
        first_token_latency = 'NA' if iter_data['first_token_latency'] == -1 else f"{iter_data['first_token_latency']:.2f} "
...
def print_generated(iter_num, warm_up=False, generated=None, prompt_idx=-1, chat_idx=-1):
    iter_str = str(iter_num)
    if warm_up:
        iter_str = 'warm-up'
    prefix = f"[{iter_str}]"
    prefix += f"[P{prompt_idx}]" if prompt_idx != -1 else ""
    prefix += f"[C{chat_idx}]" if chat_idx != -1 else ""
...
def output_avg_statis_tokens(inputs_dict, inputs_idx_list, iter_data_list, batch_size, is_text_gen, is_embed, loop_idx, latency_unit=None, chat_mode=False):
    ...
    for iter_idx in inputs_idx_list:
        ...
        for iter_data in iter_data_list:
            ...
            if iter_data['iteration'] == 0:
                continue
            if (chat_mode and iter_data['chat_idx'] == iter_idx) or (not chat_mode and iter_data['prompt_idx'] == iter_idx):
        ...
        if index_num > 0:
            ...
            if avg_1st_token_latency >= 0:
                avg_1st_token_latency = f"{avg_1st_token_latency:.2f} ms/{latency_unit}" if chat_mode else f"{avg_1st_token_latency:.2f} ms"
            else:
            ...
            prefix = f'[ INFO ] [Average] {iter_alias}[{iter_idx}]L[{loop_idx}]' if loop_idx != -1 else f'[ INFO ] [Average] {iter_alias}[{iter_idx}]'
            if is_text_gen:
                output_info = ''
                if avg_input_size > 0:
...
def print_average(iter_data_list, inputs_idx_list, batch_size, is_text_gen=False, is_embed=False, loop_idx=-1, latency_unit=None, chat_mode=False):
    ...
    if total_iters > 0:
        inputs_dict = {}
        output_avg_statis_tokens(inputs_dict, inputs_idx_list, iter_data_list, batch_size, is_text_gen, is_embed, loop_idx, latency_unit, chat_mode)
