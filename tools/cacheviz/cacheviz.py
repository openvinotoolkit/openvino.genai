...
def is_evictable(logical_block_idx: int, total_occupied_logical_blocks: int):
    ...
    if (
        ...
        <= (EVICTION_START_SIZE + EVICTION_EVICTABLE_SIZE + EVICTION_RECENT_SIZE)
        / BLOCK_SIZE
    ):
        return False
...
class StepDumpData:
    ...
    num_blocks: int = None
    occupied_blocks: Dict[int, List[Tuple[int, int]]] = field(
        default_factory=lambda: defaultdict(list)
    )
    occupied_blocks_per_sequence: Dict[int, List[int]] = field(
        default_factory=lambda: defaultdict(list)
    )
    ...
    for dump_file_name in tqdm.tqdm(step_file_names_in_order):
        ...
        with open(dump_file_name, "r") as f:
            ...
            for i in range(num_sequence_groups):
                ...
                collected_data.sequence_groups[sequence_group_id] = (
                    sequence_group_seq_ids
                )

            for i, line in enumerate(f):
                tokens = line.split()
                seq_id, block_idx, ref_count = (
                    int(tokens[0]),
                    int(tokens[1]),
                    int(tokens[2]),
                )
                if block_idx not in collected_data.occupied_blocks:
                    ...
                    collected_data.occupied_blocks[block_idx].append(
                        (seq_id, ref_count)
                    )
...
def draw_from_step_data(plot_axes: plt.Axes, step_data: StepDumpData) -> plt.Axes:
    ...
    seq_id_to_sequence_group_id: Dict[int, int] = {
        seq_id: seq_group_id
        for seq_group_id, seq_id_list in sequence_groups.items()
    ...
    for block_idx, patch_xpos in enumerate(patch_x_positions):
        ...
        plot_axes.annotate(
            text, center, ha="center", va="center", color=base_text_color
        )
        ...
        if block_idx in occupied_blocks:
            for seq_idx, ref_count in occupied_blocks[block_idx]:
                ...
                seq_group_color = get_hashed_rgb_color(
                    -seq_id_to_sequence_group_id[seq_idx] - 1
                )
                linestyle = "solid"
                logical_idx_in_seq = occupied_blocks_per_sequence[seq_idx].index(
                    block_idx
                )
                if is_evictable(
                    logical_idx_in_seq, len(occupied_blocks_per_sequence[seq_idx])
                ):
                    linestyle = "dotted"
    ...
    plot_axes.legend(
        handles=[
            patches.Patch(
                facecolor=get_hashed_rgb_color(-seq_group_idx - 1),
                label=f"Sequence group {seq_group_idx}",
            )
...
def load_and_draw_usage(
    ...
    with open(usage_dump_file, "r") as f:
        while True:
            ...
            usage_values[step_num] = (
                float(before_cache_usage),
                float(after_cache_usage),
            )
    ...
    leaked_before_series = [
        r - a if (r - a) > 0 else 0
        for r, a in zip(before_series, allocated_usage_before_series)
    ]
    leaked_after_series = [
        r - a if (r - a) > 0 else 0
        for r, a in zip(after_series, allocated_usage_after_series)
    ]
    ...
    plot_axes.legend(
        [
            "after eviction",
            "before eviction",
            "leaked (after eviction)",
            "leaked (before eviction)",
        ]
    )
...
def main():
    ...
    parser.add_argument(
        "--step", help="Step ID to show at startup", required=False, default=0, type=int
    )
    ...
    current_file_idx_displayed: int = (
        args.step * 2
    )  # 2 files per step - before and after eviction
    ...
    def on_press(event):
        ...
        mode = get_eviction_relation(
            step_data[current_file_idx_displayed].dump_file_name
        )
