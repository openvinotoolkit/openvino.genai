...
def test_text_prompts(tmp_path, prompt, model_id):
    ...
    hf_tokenizer = retry_request(lambda: AutoTokenizer.from_pretrained(snapshot_download(model_id), **hf_tok_load_params, trust_remote_code=True))  # required to avoid HF rate limits
