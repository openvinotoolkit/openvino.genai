...
>>>>>>> Stashed changes
    ...
    for prompt in quenstions:
        ...
        answer = model_opt.generate(
            **tokenized, **generation_config, do_sample=False, repetition_penalty=None
        )
        answer_str = tokenizer.decode(
            answer[0, tokenized["input_ids"].numel() :], skip_special_tokens=True
        )
...
def test_chat_compare_text_history_with_HF(model_descr, generation_config: Dict):
    ...
    for prompt in quenstions:
        ...
        chat_prompt = pipe.get_tokenizer().apply_chat_template(
            chat_history_ov, add_generation_prompt=True
        )
...
def test_chat_compare_statefull_vs_text_history(model_descr, generation_config: Dict):
    ...
    pipe_with_kv_cache = ov_genai.LLMPipeline(
        str(path), device, config={"ENABLE_MMAP": False}
    )
...
def test_apply_chat_template(model_tmp_path, chat_config: Tuple[str, Dict]):
    ...
    full_history_str_hf = tokenizer.apply_chat_template(
    ...
    full_history_str = tok.apply_chat_template(
...
def test_chat_continuous_batching_vs_stateful(model_descr, generation_config: Dict):
    model_id, path, tokenizer, model, stateful = read_model(
        (model_descr[0], model_descr[1] / "_test_chat")
    )
