...
COMMON_QUESTIONS = ["1+1=", "What is the previous answer?", "Why is the Sun yellow?", "What was my first question?"]
...
@pytest.mark.skip(
    reason="CVS-162891: Fix test_continuous_batching_vs_stateful tests after we started to compare cb vs sdpa"
)
...
@pytest.mark.parametrize(
    "prompt", ["The Sun is yellow because", "Difference between Jupiter and Mars is that", "table is made of"]
)
...
def test_continuous_batching_add_request_health_check(
    llm_model: OVConvertedModelSchema, generation_config_kwargs: dict, pipeline_type: PipelineType
):
    models_path = llm_model.models_path
...
def test_dynamic_split_fuse_doesnt_affect_generated_text():
    ...
    cb_pipe_target = create_ov_pipeline(
        models_path, scheduler_config=scheduler_config_target, pipeline_type=pipeline_type
    )

    generation_config = GenerationConfig(
        do_sample=False, max_new_tokens=20, eos_token_id=cb_pipe_ref.get_tokenizer().get_eos_token_id()
    )
...
def test_speculative_decoding_extended_perf_metrics(pipeline_type: PipelineType, main_model_id, draft_model_id, prompt):
    ...
    if pipeline_type == PipelineType.SPECULATIVE_DECODING:
        ...
        assert num_generated_tokens_draft > 0 and num_generated_tokens_draft < (
            (generation_config.max_new_tokens - 1) * generation_config.num_assistant_tokens + 1
        )
