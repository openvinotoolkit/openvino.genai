...
def test_speculative_decoding_extended_perf_metrics(pipeline_type):
    ...
    if (pipeline_type == PipelineType.SPECULATIVE_DECODING):
        ...
        # max num_generated_tokens for draft model will be reached if each step it will generate num_assistant_tokens
        ...
        assert num_generated_tokens_main > 0 and num_generated_tokens_draft < ((generation_config.max_new_tokens - 1) * generation_config.num_assistant_tokens + 1)
