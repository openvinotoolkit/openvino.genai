...
def test_vlm_continuous_batching_generate_vs_add_request(
    ...
    for idx, images in enumerate(image_links_list):
        handle = ov_continious_batching_pipe.add_request(idx, PROMPTS[0], images, generation_config)
...
@pytest.mark.precommit
@parametrize_all_models
def test_vlm_pipeline_chat_streamer_cancel_second_generate(ov_pipe_model: VlmModelInfo, image_sequence):
    ov_pipe = ov_pipe_model.pipeline
    callback_questions = [
...
def test_model_tags_representation(ov_pipe_model: VlmModelInfo, cat_tensor: openvino.Tensor):
    ...
    def workaround_inconsistent_inference():
        automatic_tags = ov_pipe.generate(prompt, images=[cat_tensor])
...
def test_model_tags_prepend_native(
    ...
    def workaround_inconsistent_inference():
        ...
        native_tag0 = ov_pipe.generate(
            image_tag(0) + conversation_requests[0][0], images=conversation_requests[0][1]
        )
        assert native_tag0.texts == answers[0].texts
        assert native_tag0.scores == answers[0].scores
        native_tags1 = ov_pipe.generate(
            image_tag(1) + image_tag(2) + conversation_requests[1][0], images=conversation_requests[1][1]
        )
...
def test_model_tags_append(
    ...
    def workaround_inconsistent_inference():
        ov_pipe.start_chat()
        native_tag0 = ov_pipe.generate(
            conversation_requests[0][0] + image_tag(0), images=conversation_requests[0][1]
        )
        native_tags1 = ov_pipe.generate(
            conversation_requests[1][0] + image_tag(1) + image_tag(2), images=conversation_requests[1][1]
        )
        ov_pipe.finish_chat()
...
def test_model_tags_same_reference(ov_pipe_model: VlmModelInfo, cat_tensor: openvino.Tensor):
    ...
    def workaround_inconsistent_inference():
        one_image = ov_pipe.generate("<ov_genai_image_0>" * 2, images=[cat_tensor])
...
def test_vlm_pipeline_match_optimum_preresized(request, ov_pipe_model: VlmModelInfo):
    ...
    # Gemma3 input_ids has two bos tokens when running with optimum: 
    # one in chat template + "add_bos_token" is set to True in tokenizer_config.json
    ...
    genai_output = ov_pipe.generate(prompt, images=[openvino.Tensor(resized_image)], max_new_tokens=max_new_tokens)
