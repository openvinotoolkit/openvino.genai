...
@pytest.fixture(scope="module")
def ov_pipe_empty_scheduler(vlm_pipeline_provider: Callable[[str, str], VLMPipeline]) -> VLMPipeline:
    model_path = _get_ov_model(MODEL_IDS[0])
    return VLMPipeline(model_path, "CPU", ATTENTION_BACKEND=ATTENTION_BACKEND[0], scheduler_config=SchedulerConfig())
...
def test_vlm_continuous_batching_generate_vs_add_request(
    ...
    ov_pipe_empty_scheduler: VLMPipeline
):
    ...
    for images in image_links_list:
        res_generate.append(
            ov_pipe_empty_scheduler.generate(
...
def test_vlm_continuous_batching_vs_stateful(
    ...
    for idx, images in enumerate(image_links_list):
        stateful_pipe = vlm_pipeline_provider(MODEL_IDS[0], "SDPA")
...
def test_vlm_pipeline_chat_streamer_cancel_first_generate(
    ...
    ov_pipe = vlm_pipeline_provider(model_id, backend)
