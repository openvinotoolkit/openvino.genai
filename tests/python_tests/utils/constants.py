...
class OvTestCacheManager:
    def __init__(self, pytestconfig: pytest.Config):
        ...
        self._cache_lock_file = self.base_cache_dir / ".cache_manager.lock"
        self._cache_lock = FileLock(str(self._cache_lock_file), timeout=300)
        
    @contextmanager
    def get_model_lock(self, model_identifier: str):
        lock_file = self.base_cache_dir / f".{model_identifier}.lock"
