...
def convert_to_hf(
    default_generation_config: HFGenerationConfig, generation_config: GenerationConfig
) -> HFGenerationConfig:
    kwargs = {}
...
def run_hugging_face(
    ...
    for prompt, generation_config in zip(prompts, generation_configs):
        ...
        generate_outputs = model.generate(
            ...
            generation_config=convert_to_hf(model.generation_config, generation_config),
        ...
        all_text_batch = hf_tokenizer.batch_decode(
            [
                ...
                for generated_ids in generate_outputs.sequences
            ],
        ...
        if generation_config.is_beam_search():
            generation_result.m_scores = [
                score for score in generate_outputs.sequences_scores
            ]
...
def run_continuous_batching(
    ...
    pipe = ContinuousBatchingPipeline(
        model_path.absolute().as_posix(), scheduler_config, "CPU", {}, {}
    )
...
def save_ov_model_from_optimum(model, hf_tokenizer, model_path: Path):
    ...
    tokenizer, detokenizer = convert_tokenizer(
        hf_tokenizer, with_detokenizer=True, skip_special_tokens=True
    )
...
def get_model_and_tokenizer(model_id: str, use_optimum=True):
    ...
    model = (
        ...
        if use_optimum
        else AutoModelForCausalLM.from_pretrained(model_id)
    )
    return model, hf_tokenizer


def generate_and_compare_with_hf(
    model_id: str,
    prompts: List[str],
    generation_configs: List[GenerationConfig],
    scheduler_config: SchedulerConfig,
...
def _generate_and_compare_with_reference_results(
    model_path: Path,
    prompts: List[str],
    reference_results: List[GenerationResult],
    generation_configs: List[GenerationConfig],
    scheduler_config: SchedulerConfig,
):
    ov_results: List[GenerationResult] = run_continuous_batching(
        model_path, scheduler_config, prompts, generation_configs
    )

    assert len(prompts) == len(reference_results)
    assert len(prompts) == len(ov_results)

    for prompt, ref_result, ov_result, generation_config in zip(
        prompts, reference_results, ov_results, generation_configs
    ):
        print(
...
def generate_and_compare_with_reference_text(
    model_path: Path,
    prompts: List[str],
    ...
    for prompt, ref_texts_for_this_prompt, ov_result, generation_config in zip(
        prompts, reference_texts_per_prompt, ov_results, generation_configs
    ):
        print(
        ...
        for ref_text, ov_text in zip(
            ref_texts_for_this_prompt, ov_result.m_generation_ids
        ):
            assert ref_text == ov_text


def run_test_pipeline(
    tmp_path: str, model_id: str, scheduler_params: dict = None, generation_config=None
):
    ...
    generate_and_compare_with_hf(
        model_id, prompts, generation_configs, scheduler_config, tmp_path
    )


DEFAULT_SCHEDULER_CONFIG = get_scheduler_config(
    {
        "num_kv_blocks": 300,
