...
from common import (
    ...
    get_beam_search,
    get_beam_search_min_and_max_tokens,
    get_beam_search_with_single_stop_string,
    get_beam_search_with_multiple_stop_strings,
...
@pytest.mark.parametrize(
    "model_id",
    get_models_list(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "models", "precommit")
    ),
)
@pytest.mark.xfail(
    raises=RuntimeError,
...
@pytest.mark.parametrize(
    "model_id",
    get_models_list(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "models", "nightly")
    ),
)
...
@pytest.mark.parametrize(
    ...
    get_models_list(
        os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "models", "real_models"
        )
    ),
)
...
def test_individual_generation_configs_deterministic(tmp_path, generation_config):
    ...
    generate_and_compare_with_hf(
        model_id, prompts, generation_configs, DEFAULT_SCHEDULER_CONFIG, tmp_path
    )
...
def test_individual_generation_configs_random(
    ...
    _ = run_continuous_batching(
        model_path, DEFAULT_SCHEDULER_CONFIG, prompts, generation_configs
    )
...
def test_post_oom_health(tmp_path, sampling_config):
    ...
    pipe = ContinuousBatchingPipeline(
        ...
        Tokenizer(model_path.absolute().as_posix(), {}),
        scheduler_config,
        "CPU",
        {},
    )
