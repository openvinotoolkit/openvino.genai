...
def read_model(params, **tokenizer_kwargs):
    ...
    return (
        ...
        tokenizer,
        opt_model,
        ov_genai.LLMPipeline(str(path), 'CPU', **{"ENABLE_MMAP": False}),
    )
...
def get_continuous_batching(path):
    ...
    return ov_genai.LLMPipeline(str(path), ov_genai.Tokenizer(str(path)), 'CPU', **{"scheduler_config": scheduler_config})
