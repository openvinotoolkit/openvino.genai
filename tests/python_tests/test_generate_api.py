...
def run_hf_ov_genai_comparison_batched(
    model_descr, generation_config: Dict, prompts: Union[str, List[str]]
):
    ...
    if generation_config_hf.get("stop_criteria"):
        generation_config_hf["early_stopping"] = STOP_CRITERIA_MAP[
            generation_config_hf.pop("stop_criteria")
        ]
    ...
    hf_encoded_outputs = model.generate(
        prompt_ids, attention_mask=attention_mask, **generation_config_hf
    )
    ...
    for idx, hf_encoded_out in enumerate(hf_encoded_outputs):
        ...
        hf_outputs.append(
            tokenizer.decode(
                hf_encoded_out[prompt_ids[prompt_count].shape[0] :],
...
def run_hf_ov_genai_comparison(model_descr, generation_config: Dict, prompt: str):
    ...
    hf_output = tokenizer.decode(
        hf_encoded_output[0, encoded_prompt.shape[1] :], skip_special_tokens=True
    )
...
def hf_ov_genai_tensors_comparison(
    ...
    if attention_mask is not None:
        inputs_ov = ov_genai.TokenizedInputs(
            ov.Tensor(input_ids), ov.Tensor(attention_mask)
        )
        inputs_hf = dict(
            inputs=torch.tensor(input_ids), attention_mask=torch.tensor(attention_mask)
        )
    else:
...
def test_ov_tensors(model_descr, inputs):
    hf_ov_genai_tensors_comparison(
        read_model(model_descr), dict(max_new_tokens=20), *inputs
    )
...
def test_multibatch(model_descr, generation_config, prompts):
    run_hf_ov_genai_comparison_batched(
        read_model(model_descr), generation_config, prompts
    )
...
def test_beam_search_decoding(
    model_descr, num_beam_groups, group_size, max_new_tokens, diversity_penalty, prompt
):
    generation_config = dict(
...
@pytest.mark.parametrize(
    "stop_criteria", [StopCriteria.NEVER, StopCriteria.EARLY, StopCriteria.HEURISTIC]
)
@pytest.mark.parametrize("prompt", prompts)
...
@pytest.mark.parametrize("model_descr", get_models_list())
@pytest.mark.nightly
def test_beam_search_long_sentences(
    model_descr, num_beam_groups, group_size, max_new_tokens, prompt
):
    generation_config = dict(
        num_beam_groups=num_beam_groups,
        num_beams=num_beam_groups * group_size,
...
@pytest.mark.parametrize(
    "callback", [print, user_defined_callback, lambda subword: print(subword)]
)
@pytest.mark.precommit
@pytest.mark.nightly
...
class Printer(ov_genai.StreamerBase):
    ...
    def put(self, token_id):
        ...
        print(
            token_id
        )  # print only token because self.tokenizer.decode([token_id]) are not implemented yet
...
def test_streamer_kwargs_one_string():
    ...
    printer = Printer(pipe.get_tokenizer())
    pipe.generate(
        "table is made of", max_new_tokens=10, do_sample=False, streamer=printer
    )
...
invalid_configs = [
    ...
    dict(
        eos_token_id=42, ignore_eos=True
    ),  # no max_new_tokens, no max_len with ignore_eos
    dict(
        repetition_penalty=-1.0, eos_token_id=42, max_new_tokens=20
    ),  # invalid penalty
    dict(
        temperature=-1.0, do_sample=True, eos_token_id=42, max_new_tokens=20
    ),  # invalid temp
    dict(
        top_p=-1.0, do_sample=True, eos_token_id=42, max_new_tokens=20
    ),  # invalid top_p
...
def test_python_generation_config_validation(model_tmp_path, generation_config):
    ...
    return_exception_type = (
        ValueError if "unexisting_key_name" in generation_config else RuntimeError
    )
...
@pytest.mark.nightly
@pytest.mark.skipif(
    sys.platform.startswith("win"), reason="not enough space for this model on Win"
)
def test_left_pad():
...
def test_continuous_batching_vs_stateful(prompt, generation_config):
    model_id, path, tokenizer, model, stateful = read_model(
        ("facebook/opt-125m", Path("opt-125m"))
    )
...
def test_cb_streamer_vs_return_vs_stateful(prompt):
    ...
    cb = get_continuous_batching(path)
    streamed = []
    generated = cb.generate(
        prompt, max_new_tokens=20, streamer=lambda subword: streamed.append(subword)
    )
...
def run_perf_metrics_collection(
    model_descr, generation_config: Dict, prompt: str
...
def test_perf_metrics(model_descr, generation_config, prompt):
    ...
    perf_metrics = run_perf_metrics_collection(
        read_model(model_descr), generation_config, prompt
    )
