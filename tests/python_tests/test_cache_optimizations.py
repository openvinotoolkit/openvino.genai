...
class CacheOptTestStruct:
    ...
    avg_cache_usage_optimization_ratio: (
        float  # expecting no less than these optimization ratios
    )
...
)  # prefix caching shouldn't impact similarity
def test_cache_optimized_generation_is_similar_to_unoptimized(
    converted_model, test_struct, enable_prefix_caching
):
    seqs_per_request = 5
