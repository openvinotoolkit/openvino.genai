...
from openvino_genai import ContinuousBatchingPipeline, SchedulerConfig, GenerationConfig, CacheEvictionConfig, AggregationMode, AnchorPoints
...
SHORT_CACHE_EVICTION_CONFIG = CacheEvictionConfig(start_size=32, recent_size=32, max_cache_size=96, cl_size=8, anchor_point=AnchorPoints.RANDOM, aggregation_mode=AggregationMode.NORM_SUM)
LONGBENCH_CACHE_EVICTION_CONFIG = CacheEvictionConfig(start_size=32, recent_size=128, max_cache_size=672, cl_size=0, anchor_point=AnchorPoints.RANDOM, aggregation_mode=AggregationMode.NORM_SUM)
...
def test_optimized_generation_longbench(device, test_struct):
    ...
    print("device", device, "num_kv_blocks", num_kv_blocks)
