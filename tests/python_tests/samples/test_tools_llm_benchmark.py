...
class TestBenchmarkLLM:
    ...
    @pytest.mark.parametrize("convert_model, sample_args", [
        pytest.param("tiny-random-qwen2vl", ["-d", "cpu", "-n", "1", "--genai", "-vf", "5"]),
        pytest.param("tiny-random-qwen2vl", ["-d", "cpu", "-n", "1", "--genai", "-vf", "-150"]),
        pytest.param("tiny-random-qwen2vl", ["-d", "cpu", "-n", "1", "--optimum", "-vf", "5"]),
        # pytest.param("Qwen2-VL-2B-Instruct", ["-d", "cpu", "-n", "1", "--genai", "-vf", "5"]),
        # pytest.param("Qwen2-VL-2B-Instruct", ["-d", "cpu", "-n", "1", "--genai", "-vf", "-150"]),
        # pytest.param("Qwen2-VL-2B-Instruct", ["-d", "cpu", "-n", "1", "--optimum", "-vf", "5"]),
    ], indirect=["convert_model"])
