        ...
        [
            ...
            pytest.param(
                ...
                marks=pytest.mark.skip(
                    reason="Linux and mac failed with chinese input due to CVS-173471, Windows due to CVS-173467"
                ),
            ),
        ],
        indirect=True,
        ...
        for prompt in sample_args:
            if tokenizer.chat_template:
                prompt = tokenizer.apply_chat_template(
                    [{"role": "user", "content": f'"{prompt}"'}], tokenize=False, add_generation_prompt=True
                )
