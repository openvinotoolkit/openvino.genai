...
class TestGreedyCausalLM:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                "SmolLM2-135M-GGUF", "return 0", marks=pytest.mark.skipif(sys.platform == "win32", reason="CVS-173467")
            ),
            pytest.param("Qwen2-0.5B-Instruct", "69"),
            pytest.param(
                "Qwen2-0.5B-Instruct-GGUF", "69", marks=pytest.mark.skipif(sys.platform == "win32", reason="CVS-173467")
            ),
    ...
    def test_sample_greedy_causal_lm(self, request, convert_model, sample_args):
        ...
        assert cpp_result.stdout == c_result.stdout, "Results should match"
        assert c_result.stdout == js_result.stdout, "Results should match"
        ...
        for output in transformers.AutoModelForCausalLM.from_pretrained(model["name"], local_files_only=True).generate(
            **tokenized, max_length=100, do_sample=False
        ):
