...
class TestTextEmbeddingPipeline:
    ...
    def test_sample_text_embedding_pipeline(self, convert_model):
        # Run C++ sample
        cpp_sample = (SAMPLES_CPP_DIR / "text_embeddings").as_posix()
        cpp_command = [cpp_sample, convert_model, "Document 1", "Document 2"]
        cpp_result = run_sample(cpp_command)

        # Run Python sample
        py_script = (SAMPLES_PY_DIR / "rag/text_embeddings.py").as_posix()
        py_command = [sys.executable, py_script, convert_model, "Document 1", "Document 2"]
        py_result = run_sample(py_command)

        # Run JS sample
        js_sample = (SAMPLES_JS_DIR / "rag/text_embeddings.js").as_posix()
...
class TestTextRerankPipeline:
    ...
    def test_sample_text_rerank_pipeline(self, convert_model):
        # Run Python sample
        py_script = (SAMPLES_PY_DIR / "rag/text_rerank.py").as_posix()
...
range of computing tasks."
        ...
        # Run C++ sample
        cpp_sample = (SAMPLES_CPP_DIR / "text_rerank").as_posix()
