...
class TestWhisperSpeechRecognition:
    ...
    def test_sample_whisper_speech_recognition(self, convert_model, download_test_content):
        # Run C++ sample
        cpp_sample = (SAMPLES_CPP_DIR / 'whisper_speech_recognition').as_posix()
        cpp_command = [cpp_sample, convert_model, download_test_content]
        cpp_result = run_sample(cpp_command)

        # Run Python sample
        py_script = (SAMPLES_PY_DIR / "whisper_speech_recognition/whisper_speech_recognition.py").as_posix()
        py_command = [sys.executable, py_script, convert_model, download_test_content]
        py_result = run_sample(py_command)

        # Run C sample
        c_sample = (SAMPLES_C_DIR / 'whisper_speech_recognition_c').as_posix()
