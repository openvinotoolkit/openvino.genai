...
class TestImageGenerationCallback:
    def test_text2image_with_simple_callback(self, image_generation_model):
        ...
        image = pipe.generate("test prompt", width=64, height=64, num_inference_steps=2, callback=callback)
    ...
    def test_text2image_multiple_generates_with_callback(self, image_generation_model):
        ...
        for i in range(3):
            ...
            image = pipe.generate(f"test prompt {i}", width=64, height=64, num_inference_steps=2, callback=callback)
    ...
    def test_image2image_with_callback(self, image_generation_model):
        ...
        image = pipe.generate("test prompt", input_image, strength=0.8, callback=callback)
    ...
    def test_inpainting_with_callback(self, image_generation_model):
        ...
        image = pipe.generate("test prompt", input_image, mask_image, callback=callback)
