...
def _local_llama_dflash_models():
    if os.environ.get(_LLAMA_DFLASH_E2E_ENV) != "1":
        pytest.skip(f"Set {_LLAMA_DFLASH_E2E_ENV}=1 to run local Llama DFlash artifact tests")
...
def _generate_target_then_dflash(device, prompts, generation_config, properties=None, model_paths=None):
    if device == "GPU":
        _require_gpu_device()
    target_model_path, draft_model_path = model_paths if model_paths is not None else _local_dflash_models()
...
def test_dflash_local_cpu_fp32_prompt2_exercises_target_kv_rollback():
    ov_genai = pytest.importorskip("openvino_genai")
    target_model_path, _ = _local_dflash_models()
    ...
    target_snapshots, dflash_results = _generate_target_then_dflash(
        "CPU",
...
def test_llama_dflash_local_cpu_exercises_target_kv_rollback():
    ...
    target_results, dflash_results = _generate_target_then_dflash(
        "CPU",
        prompts,
        generation_config,
        _TARGET_PIPELINE_PROPERTIES.copy(),
