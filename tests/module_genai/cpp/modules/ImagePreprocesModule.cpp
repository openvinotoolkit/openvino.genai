...
using test_params = std::tuple<std::vector<std::string>>;
...
protected:
    std::string get_yaml_content() override {
        ...
        model_path["model_path"] = get_model_path() + "/Qwen2.5-VL-3B-Instruct/INT4/";
    ...
    ov::AnyMap prepare_inputs() override {
        ...
        if (image_paths.size() == 1u) {
            auto img1 = utils::load_image(get_data_path() + "/" + image_paths[0]);
            EXPECT_TRUE(img1) << "Failed to load test image: " + image_paths[0];
            if (!img1) return inputs;
            inputs["image"] = img1;
        } else if (image_paths.size() > 1u) {
            std::vector<ov::Tensor> batch_images;
            for (const auto& image_path : image_paths) {
                auto img = utils::load_image(get_data_path() + "/" + image_path);
    ...
    void check_output_input_1(ov::genai::module::ModulePipeline& pipe) {
        ...
        EXPECT_TRUE(compare_big_tensor(raw_data, expected_input_ids_for_input_1, threshold))
    ...
    void check_output_input_2(ov::genai::module::ModulePipeline& pipe) {
        ...
        for (size_t i = 0; i < raw_datas.size(); ++i) {
            EXPECT_TRUE(compare_big_tensor(raw_datas[i], expected_input_ids[i], threshold))
                << "raw_data do not match expected values";
...
auto test_image_1 = std::vector<std::string>{"cat_120_100.png"};
auto test_image_2 = std::vector<std::string>{"cat_120_100.png", "dog_120_120.png"};

INSTANTIATE_TEST_SUITE_P(ModuleTestSuite,
                         ImagePreprocesModuleTest,
                         ::testing::Values(test_params{test_image_1},
                                           test_params{test_image_2}),
