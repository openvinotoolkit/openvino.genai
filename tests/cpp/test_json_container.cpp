...
TEST(JsonContainerTest, copy) {
    ...
    JsonContainer root(
        {{"section1", JsonContainer({{"value", "old"}})}, {"section2", JsonContainer({{"value", "other"}})}});
...
TEST(JsonContainerTest, json_string) {
    JsonContainer any_map_json({{"string", "test"},
                                {"number", 42},
                                {"boolean", true},
    ...
    EXPECT_EQ(any_map_json_string,
              R"({"array":[1,2],"boolean":true,"number":42,"object":{"nested":"value"},"string":"test"})");
