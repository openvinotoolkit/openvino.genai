...
std::shared_ptr<ov::Model> make_annotated_hidden_state_model() {
    ...
    auto result = std::make_shared<ov::op::v0::Result>(layer2);
    result->output(0).set_names({"logits"});
    auto model = std::make_shared<ov::Model>(ov::ResultVector{result}, ov::ParameterVector{input});
    model->set_rt_info(
        std::string(R"({"version":1,"layers":{"0":"ov.hidden_states.decoder_layer_0","1":"ov.hidden_states.decoder_layer_1","2":"ov.hidden_states.decoder_layer_2"}})"),
...
TEST(Eagle3ModelTransforms, ThrowsForMissingAnnotatedHiddenStateLayer) {
    ...
    EXPECT_THROW(ov::genai::utils::eagle3::transform_hidden_state(model, {0, 1, 3}), ov::Exception);
}
