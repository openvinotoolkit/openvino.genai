...
TEST(Text2VideoPipelineTest, TemporalSlicerShapeVerification) {
    // 1. Initialize pipeline with a dummy model directory 
    ...
    try {
        // CRITICAL FIX: We instantiate the public pipeline, NOT the hidden LTXPipeline
        Text2VideoPipeline pipeline(models_dir);
        
        // 2. Create an oversized dummy latent tensor to trigger the internal slicer logic.
        ...
        // Fill with zeros to avoid NaN math during the linear cross-fade interpolation
        std::memset(dummy_latent.data<float>(), 0, dummy_latent.get_byte_size());
        
        // 3. Execute the public decode method (which secretly routes through your new decode_temporal_slices)
        VideoGenerationResult result = pipeline.decode(dummy_latent);
        ov::Tensor out_video = result.video;
        
        // 4. Verify the anchored stride mathematics.
        ...
        EXPECT_EQ(out_shape[2], expected_frames) 
            << "Temporal slicer failed to align frame count. Expected " << expected_frames << " but got " << out_shape[2];
