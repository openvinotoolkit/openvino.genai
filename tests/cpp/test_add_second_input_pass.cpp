...
TEST(AddSecondInputTest, add_second_input_test_1) {
    ...
    ASSERT_EQ(
        combine_segments_node->get_input_size(),
        combine_inputs.size() + 6
    );  // 6 extra inputs for the second input and eos token
}
...
TEST(AddSecondInputTest, error_post_processor_pair_not_widening) {
    ...
    ASSERT_FALSE(ok);
    ASSERT_NE(
        pass_errors.str().find("Paired inputs are allowed only when it's widening the single input"),
        std::string::npos
    );
}
...
TEST(AddSecondInputTest, error_post_processor_single_not_one_sequence) {
    ...
    ASSERT_FALSE(ok);
    ASSERT_NE(
        pass_errors.str().find("Could not add second input. Input signature from rt_info does not "),
        std::string::npos
    );
}
