...
TEST(TestScheduler, general_test) {
    ...
    for (auto scheduler_config : configs) {
        ...
        SequenceGroup::Ptr sequence_group1 =
            std::make_shared<SequenceGroup>(0,
        ...
        SequenceGroup::Ptr sequence_group2 =
            std::make_shared<SequenceGroup>(1,
                                            ov::Tensor(ov::element::i64, {tokens.size()}, tokens.data()),
                                            utils::get_greedy_config(),
                                            4);
        auto idx1 = (*sequence_group2)[0]->get_id();
        SequenceGroup::Ptr sequence_group3 =
            std::make_shared<SequenceGroup>(1,
                                            ov::Tensor(ov::element::i64, {tokens.size()}, tokens.data()),
                                            utils::get_greedy_config(),
                                            4);
...
INSTANTIATE_TEST_SUITE_P(VariousSchedulerConfigs,
...
TEST(TestScheduler, test_partial_preemption_beam_search) {
    ...
    for (auto scheduler_config : configs) {
        ...
        SequenceGroup::Ptr sequence_group_greedy =
            std::make_shared<SequenceGroup>(0,
                                            ov::Tensor(ov::element::i64, {tokens.size()}, tokens.data()),
                                            utils::get_greedy_config(),
                                            4);
...
TEST(TestScheduler, FullyPreemptsCacheEvictedSequences) {
    ...
    std::vector<uint64_t> tokens2 = {
        ...
        7,
        8,
        9};  // 5 full blocks, larger than eviction arena size (3 blocks) - will start evicting already at prompt stage
    ...
    const std::vector<size_t> ref_block_table1_after_preemption{0,
                                                                1,
                                                                2,
                                                                3};  // 3 was the first to be freed after preemption
    ...
    const std::vector<size_t> ref_block_table2_after_recompute{
        ...
        0,
        1,
        2};  // should restore the old state before first eviction in terms of block count
...
TEST(TestScheduler, prefix_caching_embeddings_test) {
    ...
    for (auto scheduler_config : configs) {
        ...
        for (size_t chat_iteration = 0; chat_iteration < chat_iterations; chat_iteration++) {
            ...
            histrory_embeddings.insert(histrory_embeddings.end(),
                                       generated_embeddings.begin(),
                                       generated_embeddings.end());
