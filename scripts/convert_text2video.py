...
text_encoder.eval()
ov_text = ov.convert_model(text_encoder, example_input=torch.ones((1, 77), dtype=torch.long))
ov.save_model(ov_text, output_dir / "openvino_text_encoder_model.xml")
del ov_text, text_encoder
gc.collect()

# 2. U-Net
unet = UNet3DConditionModel.from_pretrained(model_id, subfolder="unet", torch_dtype=torch.float16, variant="fp16")
unet.eval()
dummy_inputs = (
...
# 3. VAE & Configs
vae = AutoencoderKL.from_pretrained(model_id, subfolder="vae", torch_dtype=torch.float16, variant="fp16")
vae.eval()
ov_vae = ov.convert_model(vae.decoder, example_input=torch.randn(1, 4, 32, 32, dtype=torch.float16))
