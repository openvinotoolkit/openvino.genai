...
DecodedResults StatefulLLMPipeline::generate(
    ...
    if (is_chat_conversation)
        OPENVINO_ASSERT(
            m_chat_input_type == ov::genai::utils::GenerationChatInputsType::STRING,
            "Chat doesn't support switching between input types. Please, continue using StringInputs or restart the "
            "chat."
        );
...
DecodedResults StatefulLLMPipeline::generate(
    ...
    if (is_chat_conversation)
        OPENVINO_ASSERT(
            m_chat_input_type == ov::genai::utils::GenerationChatInputsType::CHAT_HISTORY,
            "Chat doesn't support switching between input types. Please, continue using ChatHistory or restart the "
            "chat."
        );
...
EncodedResults StatefulLLMPipeline::generate(
    ...
    if (auto data = std::get_if<ov::Tensor>(&inputs)) {
        if (m_is_npu) {
            ...
            OPENVINO_ASSERT(
                data->get_size() <= m_max_prompt_len,
        ...
        if (m_is_npu) {
            // Prefill model in NPU is reshaped to NPUW_LLM_MAX_PROMPT_LEN x NPUW_LLM_MAX_PROMPT_LEN
            OPENVINO_ASSERT(
                data->input_ids.get_size() <= m_max_prompt_len,
                "Stateful LLM pipeline on NPU may only process prompts or hold chat history up to ",
                m_max_prompt_len,
                " tokens. ",
    ...
    if (is_chat_conversation && !m_cache_state.get_state().empty() && !m_use_full_chat_history) {
        ...
        std::copy(start_atten_hst, start_atten_hst + cache_len, new_atten_mask.data<int64_t>());
        std::copy(
    ...
    if (is_chat_conversation) {
        ...
        if (config.is_beam_search()) {
            m_cache_state.num_tokens_to_trim =
