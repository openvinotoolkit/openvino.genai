...
public:
    ContinuousBatchingAdapter(const ov::InferRequest& request,
                              const Tokenizer& tokenizer,
                              OptionalGenerationConfig generation_config)
        : LLMPipelineImplBase{dont_construct(), GenerationConfig{}},
          m_impl{
              std::make_unique<ContinuousBatchingPipeline>(std::filesystem::path{}, SchedulerConfig{}, std::string{})} {
    }

    ContinuousBatchingAdapter(const std::filesystem::path& models_path,
                              const Tokenizer& tokenizer,
        ...
        m_impl = std::make_unique<ContinuousBatchingPipeline>(models_path,
                                                              tokenizer,
                                                              scheduler_config,
                                                              device,
                                                              mutable_plugin_config);
        m_generation_config = m_impl->get_config();
    }

    ContinuousBatchingAdapter(const std::string& model_str,
        ...
        auto mutable_plugin_config = plugin_config;
        mutable_plugin_config["sampler_num_threads"] = 1;
        m_impl = std::make_unique<ContinuousBatchingPipeline>(model_str,
                                                              weights_tensor,
                                                              tokenizer,
                                                              scheduler_config,
                                                              device,
    ...
    DecodedResults generate(StringInputs inputs,
        ...
        std::vector<std::string> prompts = std::visit(overloaded{[](const std::string& prompt) {
        ...
        std::vector<GenerationResult> generated =
            m_impl->generate(prompts, std::vector<GenerationConfig>{prompts.size(), config}, streamer);
        std::vector<std::string> plain_replies;
        std::vector<float> plain_scores;
        for (GenerationResult& res : generated) {
            OPENVINO_ASSERT(res.m_status == GenerationStatus::FINISHED || res.m_status == GenerationStatus::STOP ||
                                res.m_status == GenerationStatus::CANCEL,
        ...
        raw_metrics.generate_durations.emplace_back(
            PerfMetrics::get_microsec(std::chrono::steady_clock::now() - start_time));
        // Need to reevaluate statistics with the updated start_time which includes tokenization/detokenization
        // durations.
        perf_metrics.m_evaluated = false;
        perf_metrics.evaluate_statistics(start_time);

        return {std::move(plain_replies),
                std::move(plain_scores),
                std::move(perf_metrics),
                generated[0].extended_perf_metrics};
    }
    ...
    DecodedResults generate(const ChatHistory& history,
        ...
        std::vector<GenerationResult> generated =
            m_impl->generate(histories, std::vector<GenerationConfig>{histories.size(), config}, streamer);
    ...
    EncodedResults generate(const EncodedInputs& inputs,
        ...
        std::vector<ov::Tensor> input_ids =
            std::visit(overloaded{[](const ov::Tensor& inp) {
        ...
        const GenerationConfig& config = generation_config.value_or(m_generation_config);
        // -1 == config.eos_token_id and config.validate() are handled in m_impl.
        std::vector<EncodedGenerationResult> generated =
            m_impl->generate(input_ids, std::vector<GenerationConfig>{input_ids.size(), config}, streamer);
