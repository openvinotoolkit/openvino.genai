...
Tokenizer dont_construct() {
    OPENVINO_THROW(
        "Continuous Batching backend can't be constructed"
...
public:
    ...
    ContinuousBatchingAdapter(
        ...
        m_impl = std::make_unique<ContinuousBatchingPipeline>(
            models_path,
            tokenizer,
            scheduler_config,
            device,
    ...
    generate(StringInputs inputs, OptionalGenerationConfig generation_config, StreamerVariant streamer) override {
        ...
        std::vector<std::string> prompts = std::visit(
            overloaded{
        ...
        for (GenerationResult& res : generated) {
            OPENVINO_ASSERT(
                res.m_status == GenerationStatus::FINISHED || res.m_status == GenerationStatus::STOP ||
    ...
    EncodedResults generate(
        ...
        std::vector<ov::Tensor> input_ids = std::visit(
            overloaded{
                [](const ov::Tensor& inp) {
