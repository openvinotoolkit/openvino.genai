...
namespace {
ov::genai::WhisperGenerationConfig to_whisper_config(const ov::genai::ASRGenerationConfig& asr_config) {
...
ov::genai::ASRDecodedResults to_asr_results(ov::genai::WhisperDecodedResults&& whisper_result) {
    ov::genai::PerfMetrics base_metrics = static_cast<ov::genai::PerfMetrics&>(whisper_result.perf_metrics);
    ...
    result.perf_metrics = ov::genai::ASRPerfMetrics{base_metrics};
    ...
    if (whisper_result.words.has_value()) {
        std::vector<ov::genai::ASRDecodedResultChunk> words;
