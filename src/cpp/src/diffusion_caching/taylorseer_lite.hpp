...
public:
    ...
    void update(std::size_t current_step, const ov::Tensor& output) {
        ...
        OPENVINO_ASSERT(
            is_first_update || current_step > *m_last_update_step,
            "Current step (",
            current_step,
        ...
        if (!is_first_update) {
            ...
            OPENVINO_ASSERT(
                output.get_shape() == prev_factor.get_shape(),
                "Output tensor shape ",
                output.get_shape(),
    ...
    ov::Tensor predict(std::size_t current_step) const {
        OPENVINO_ASSERT(
            m_taylor_factors.size() >= m_max_order,
        ...
        OPENVINO_ASSERT(
            current_step > *m_last_update_step,
...
private:
    ...
    bool should_compute_at_step(
        std::size_t current_step,
        const TaylorSeerCacheConfig& config,
        std::size_t num_inference_steps
    ) const {
