...
ChatHistory ChatHistoryInternalState::build_normalized_history(const ChatHistory& history) const {
    OPENVINO_ASSERT(
        m_messages_metadata.size() == history.size(),
        "Internal state size (",
...
void ChatHistoryInternalState::detect_chat_history_format(const ChatHistory& history) {
    ...
    OPENVINO_ASSERT(
        last_user_message.contains("content"),
    ...
    OPENVINO_ASSERT(
        detected_format != ChatHistoryFormat::UNKNOWN,
    ...
    OPENVINO_ASSERT(
        m_chat_history_format == ChatHistoryFormat::UNKNOWN || m_chat_history_format == detected_format,
