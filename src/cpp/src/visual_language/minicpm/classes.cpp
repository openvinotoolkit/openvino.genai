...
ov::Tensor prepare_vis_position_ids(const ov::Tensor& pixel_values,
    ...
    for (size_t batch_idx = 0; batch_idx < batch_size; ++batch_idx) {
        ...
        std::transform(fractional_coords_h.begin(),
        ...
        std::transform(fractional_coords_w.begin(),
...
std::pair<EncodedImage, ImageSliceResult> llava_image_embed_make_with_bytes_slice(clip_ctx& ctx_clip,
                                                                                  const ov::Tensor& img,
    ...
    if (1 < preprocessed.size()) {
        for (size_t row = 1; row < preprocessed.size(); ++row) {
            ...
            for (size_t col = 0; col < n_slices; ++col) {
                ...
                std::fill_n(attention_data + ((row - 1) * n_slices + col + 1) * max_size / patch_size / patch_size,
    ...
    for (size_t col = 0; col < preprocessed.size() - 1; ++col) {
        for (size_t row = 0; row < preprocessed.at(1).size(); ++row) {
            std::copy_n(out + (col * preprocessed.at(1).size() + row + 1) * n_patches * old_hidden_size,
...
EncodedImage VisionEncoderMiniCPM::encode(const ov::Tensor& image, const ov::AnyMap& config_map) {
    ...
    auto [encoded_image, image_slice_result] = llava_image_embed_make_with_bytes_slice(ctx_clip,
                                                                                       image,
                                                                                       encoder,
...
ov::Tensor get_2d_sincos_pos_embed_from_grid(size_t embed_dim, const ov::Tensor& grid) {
    ...
    ov::Tensor emb_w = get_1d_sincos_pos_embed_from_grid_new(
        embed_dim / 2,
        ov::Tensor{ov::element::f32, plane_shape, grid_data + plane_shape.at(0) * plane_shape.at(1)});  // (H, W, D/2)
...
ov::Tensor InputsEmbedderMiniCPM::get_inputs_embeds(const std::string& unified_prompt,
...
ov::Tensor VisionEncoderMiniCPM::resample(const ov::Tensor& encoded_image,
                                          const ImageSize& target_size,
                                          size_t pad_to_max) {
...
VisionEncoderMiniCPM::VisionEncoderMiniCPM(const std::filesystem::path& model_dir,
...
VisionEncoderMiniCPM::VisionEncoderMiniCPM(const ModelsMap& models_map,
...
InputsEmbedderMiniCPM::InputsEmbedderMiniCPM(const VLMConfig& vlm_config,
