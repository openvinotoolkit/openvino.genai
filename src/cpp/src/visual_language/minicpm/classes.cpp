...
std::pair<int, int>
find_best_resize(std::pair<int, int> original_size, int scale_resolution, int patch_size, bool allow_upscale = false) {
...
std::pair<int, int> get_refine_size(
...
std::vector<std::vector<clip_image_u8>> slice_image(
...
ov::Tensor unfold(const ov::Tensor& images_tensor, size_t kernel) {
    ...
    OPENVINO_ASSERT(
        images_h >= kernel && images_w >= kernel,
...
std::vector<int64_t>
bucket_size_right(const std::vector<float>& fractional_coords, const std::vector<float>& boundaries) {
    std::vector<int64_t> bucket_coords(fractional_coords.size());
    std::transform(
        fractional_coords.begin(),
...
ov::Tensor prepare_vis_position_ids(
    ...
    for (size_t batch_idx = 0; batch_idx < batch_size; ++batch_idx) {
        ...
        std::transform(
            fractional_coords_h.begin(),
            fractional_coords_h.end(),
            fractional_coords_h.begin(),
            [nb_patches_h](float val) {
        ...
        std::transform(
            fractional_coords_w.begin(),
            fractional_coords_w.end(),
            fractional_coords_w.begin(),
            [nb_patches_w](float val) {
...
std::pair<EncodedImage, ImageSliceResult> llava_image_embed_make_with_bytes_slice(
    clip_ctx& ctx_clip,
    const ov::Tensor& img,
    ...
    std::transform(
        ...
        imgs.end(),
        preprocessed.begin(),
        [&ctx_clip, &max_size, &n_images](const std::vector<clip_image_u8>& row) {
            std::vector<clip_image_f32> processed_row{row.size()};
            std::transform(
                row.begin(),
                row.end(),
                processed_row.begin(),
                [&ctx_clip, &max_size, &n_images](const clip_image_u8& raw) {
    ...
    for (size_t c_idx = 0; c_idx < channels; ++c_idx) {
        for (size_t k_idx = 0; k_idx < patch_size; k_idx++) {
            ...
            // pixel_values and patch_attention_mask are multiplied instead of ignoring the corresponding pixel_values
            // resulting in NaN instead of 0.0f if pixel_values has NaN.
        ...
        for (size_t row = 1; row < preprocessed.size(); ++row) {
            ...
            for (size_t col = 0; col < n_slices; ++col) {
                ...
                for (size_t c_idx = 0; c_idx < channels; ++c_idx) {
                    for (size_t k_idx = 0; k_idx < patch_size; k_idx++) {
                        std::copy(clip_value_data, clip_value_data + d3_clip_pixel, pixel_value_data);
                        // pixel_values and patch_attention_mask are multiplied instead of ignoring the corresponding
                        // pixel_values resulting in NaN instead of 0.0f if pixel_values has NaN.
                        memset(pixel_value_data + d3_clip_pixel, 0, (d3_all_pixel - d3_clip_pixel) * sizeof(float));
                        clip_value_data += d3_clip_pixel;
                        pixel_value_data += d3_all_pixel;
                    }
    ...
    ov::Tensor patch_attention_mask{
        ov::element::f32,
        {pixel_values.get_shape().at(0), 1, max_size / patch_size / patch_size}
    };
    ...
    if (1 < preprocessed.size()) {
        for (size_t row = 1; row < preprocessed.size(); ++row) {
            ...
            for (size_t col = 0; col < n_slices; ++col) {
                ...
                std::fill_n(
                    attention_data + ((row - 1) * n_slices + col + 1) * max_size / patch_size / patch_size,
    ...
    ov::Tensor position_ids = prepare_vis_position_ids(
        pixel_values,
    ...
    image_slice_result.slices =
        ov::Tensor{ov::element::f32, {preprocessed.size() - 1, preprocessed.at(1).size(), n_patches, old_hidden_size}};
    for (size_t col = 0; col < preprocessed.size() - 1; ++col) {
        for (size_t row = 0; row < preprocessed.at(1).size(); ++row) {
            std::copy_n(
                ...
                image_slice_result.slices.data<float>() +
                    (col * preprocessed.at(1).size() + row) * n_patches * old_hidden_size
            );
        }
    }
...
EncodedImage VisionEncoderMiniCPM::encode(const ov::Tensor& image, const ov::AnyMap& config_map) {
    ...
    encoded_image.resampled_image =
        resample_encoded_image(encoded_image, image_slice_result.slices, image_slice_result.target_size);
...
ResampledImage VisionEncoderMiniCPM::resample_encoded_image(
    ...
    const ov::Tensor& resampled_source =
        resample(encoded_image.resized_source, encoded_image.resized_source_size, pad_to_max);
...
ov::Tensor get_2d_sincos_pos_embed_from_grid(size_t embed_dim, const ov::Tensor& grid) {
    ...
    ov::Tensor emb_h = get_1d_sincos_pos_embed_from_grid_new(
        embed_dim / 2,
        ov::Tensor{ov::element::f32, plane_shape, grid_data}
    );  // (H, W, D/2)
    ov::Tensor emb_w = get_1d_sincos_pos_embed_from_grid_new(
        embed_dim / 2,
        ov::Tensor{ov::element::f32, plane_shape, grid_data + plane_shape.at(0) * plane_shape.at(1)}
    );  // (H, W, D/2)
...
void adjust_pos_cache(const std::vector<ImageSize>& target_sizes, size_t hidden_size, ov::Tensor& pos_embed_cache) {
    size_t max_h =
        std::max_element(target_sizes.begin(), target_sizes.end(), [](const ImageSize& left, const ImageSize& right) {
...
NormalizedPrompt InputsEmbedderMiniCPM::normalize_prompt(
    const std::string& prompt,
    size_t base_id,
    const std::vector<EncodedImage>& images
) const {
    auto [unified_prompt, image_sequence] = normalize(prompt, NATIVE_TAG, NATIVE_TAG + '\n', base_id, images.size());
    std::string unk64;
...
ov::Tensor InputsEmbedderMiniCPM::get_inputs_embeds(
    ...
    for (size_t image_id : images_sequence) {
        ...
        if (slices_shape.size()) {
            ...
            for (size_t i = 0; i < slices_shape.at(0); ++i) {
                for (size_t ja = 0; ja < slices_shape.at(1); ++ja) {
                    const ov::Tensor& vision_embed_tensor_i_j =
                        encoded_image.resampled_image.vision_embed_tensors[i][ja];
                    ids = std::find(ids, end, slice_start_id);
                    OPENVINO_ASSERT(end != ids);
                    ++ids;
                    std::copy_n(
                        vision_embed_tensor_i_j.data<float>(),
...
ov::Tensor
VisionEncoderMiniCPM::resample(const ov::Tensor& encoded_image, const ImageSize& target_size, size_t pad_to_max) {
