...
VisionEncoderLLaVA::VisionEncoderLLaVA(const std::filesystem::path& model_dir,
...
VisionEncoderLLaVA::VisionEncoderLLaVA(const ModelsMap& models_map,
...
void VisionEncoderLLaVA::initialize_text_components(const std::filesystem::path& model_dir,
                                                    const std::string& device,
                                                    const ov::AnyMap& properties) {
    try {
...
void VisionEncoderLLaVA::initialize_text_components(const ModelsMap& models_map,
                                                    const std::filesystem::path& config_dir_path,
                                                    const std::string& device,
                                                    const ov::AnyMap& properties) {
    try {
        ...
        if (models_map.find("text_embeddings") != models_map.end()) {
            try {
                ...
                m_text_embedding_model = EmbeddingsModel::create(text_model_pair.first,
        ...
        if (!embedding_model_initialized && models_map.find("language") != models_map.end()) {
            try {
                ...
                m_text_embedding_model = EmbeddingsModel::create(lang_model_pair.first,
...
EncodedImage VisionEncoderLLaVA::encode_with_pruning(const ov::Tensor& image,
    ...
    try {
        ...
        size_t original_tokens =
            full_encoded_image.resized_source_size.height * full_encoded_image.resized_source_size.width;
    ...
    for (const ov::Tensor& image : single_images) {
        if (use_pruning) {
            ...
            try {
                ...
                if (llava_encoder) {
                    EncodedImage pruned_image =
                        llava_encoder->encode_with_pruning(image, text_prompt, pruning_ratio, vision_config);
...
std::pair<std::string, std::vector<size_t>> InputsEmbedderLLaVA::normalize_prompt(
...
bool InputsEmbedderLLaVA::is_image_features_pruned(const ov::Tensor& image_features,
...
ov::Tensor InputsEmbedderLLaVA::get_inputs_embeds(const std::string& unified_prompt,
    ...
    if (has_pruned_features) {
        ...
        auto [corrected_prompt, corrected_sequence] = normalize_prompt_with_pruning_support(unified_prompt, 0, images);
