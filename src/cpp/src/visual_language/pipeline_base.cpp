...
std::vector<ov::Tensor> VLMPipeline::VLMBackend::extract_audios_from_config_map(
...
VLMPipeline::VLMBackend::HiddenStatesCollectionScope::HiddenStatesCollectionScope(VLMBackend& pipeline)
    : m_pipeline(&pipeline) {
    m_pipeline->enable_hidden_states_collection(true);
}

VLMPipeline::VLMBackend::HiddenStatesCollectionScope::~HiddenStatesCollectionScope() {
    if (m_pipeline) {
