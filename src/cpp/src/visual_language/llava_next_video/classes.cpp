...
void set_preprocess_parameters(ov::InferRequest& encoder,
...
std::pair<size_t, size_t> get_unpadded_features(size_t height,
                                                size_t width,
...
clip_image_f32 preprocess_clip_image_llava_next_video(const clip_image_u8& image, ProcessorConfig& config) {
    ...
    auto resized_size = calculate_resize_dimensions({static_cast<size_t>(image.ny), static_cast<size_t>(image.nx)},
...
VisionEncoderLLaVANextVideo::VisionEncoderLLaVANextVideo(const std::filesystem::path& model_dir,
...
VisionEncoderLLaVANextVideo::VisionEncoderLLaVANextVideo(const ModelsMap& models_map,
...
EncodedImage VisionEncoderLLaVANextVideo::encode(const ov::Tensor& image, const ov::AnyMap& config_map) {
    ...
    if (use_ov_vision_preprocess) {
        ...
        pixel_values_shape = {num_patches,
                              3,
                              static_cast<size_t>(config.crop_size_height),
                              static_cast<size_t>(config.crop_size_width)};
    } else {
    ...
    std::tie(unpadded_features, newline_features) = get_unpadded_features(original_image_size.height,
...
InputsEmbedderLLaVANextVideo::InputsEmbedderLLaVANextVideo(const VLMConfig& vlm_config,
