    ...
    auto find_closest_aspect_ratio = [&](float ar, const std::vector<std::pair<int, int>>& ratios) {
        ...
        for (const auto& ratio : ratios) {
            ...
            if (ratio_diff < best_ratio_diff) {
                ...
                ratio_diff == best_ratio_diff && area > 0.5 * image_size * image_size * ratio.first * ratio.second
            ) {
                best_ratio = ratio;
            }
        }
    ...
    for (size_t i = 0; i < batch_size; ++i) {
        for (size_t j = 0; j < seq_len; ++j) {
            ...
            if (image_context_tokens_mask[flat_idx]) {
                ...
                std::copy_n(
                    image_embeds_data + image_context_token_idx * embed_dim,
                    embed_dim,
...
NormalizedPrompt InputsEmbedderInternVLChat::normalize_prompt(
...
ov::Tensor InputsEmbedderInternVLChat::get_inputs_embeds(
