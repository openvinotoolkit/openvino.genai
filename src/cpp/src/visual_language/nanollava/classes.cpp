...
void merge_text_and_image_embeddings_nanollava(const ov::Tensor& input_ids,
...
InputsEmbedderNanoLLaVA::InputsEmbedderNanoLLaVA(const VLMConfig& vlm_config,
...
NormalizedPrompt InputsEmbedderNanoLLaVA::normalize_prompt(const std::string& prompt,
                                                           size_t base_id,
...
ov::Tensor InputsEmbedderNanoLLaVA::get_inputs_embeds(const std::string& unified_prompt,
    ...
    if (image_embeds.size() > 0) {
        if (m_image_features_size > 0) {
            OPENVINO_ASSERT(m_image_features_size == image_embeds[0].get_shape()[1],
                            "Got unexpected shape of image embeddings.");
        } else {
...
ov::Tensor InputsEmbedderNanoLLaVA::tokenize_without_image_tag(const std::string& prompt, bool add_special_tokens) {
    ...
    while (image_pos != std::string::npos) {
        ...
        encoded_substrings.emplace_back(
            m_tokenizer.encode(substr, ov::genai::add_special_tokens(add_special_tokens)).input_ids);
    ...
    encoded_substrings.emplace_back(
        m_tokenizer.encode(last_substr, ov::genai::add_special_tokens(add_special_tokens)).input_ids);
    encoded_size += encoded_substrings[encoded_substrings.size() - 1].get_shape()[1];
    ...
    ov::Tensor new_chat_tokens =
        ov::Tensor(encoded_substrings[encoded_substrings.size() - 1].get_element_type(), {1, encoded_size + n_images});
...
ov::Tensor InputsEmbedderNanoLLaVA::apply_chat_template_tokenize(const std::string& prompt,
                                                                 ov::genai::VLMPerfMetrics& metrics) {
