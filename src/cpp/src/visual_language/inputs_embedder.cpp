...
InputsEmbedder::InputsEmbedder(const std::filesystem::path& model_dir,
    ...
    } else if (vlm_config.model_type == VLMModelType::DEEPSEEK_VL_V2) {
        m_impl = std::make_shared<InputsEmbedderDeepseekVLV2>(vlm_config, model_dir, tokenizer, device, device_config);
...
InputsEmbedder::InputsEmbedder(const ModelsMap& models_map,
    ...
    if (vlm_config.model_type == VLMModelType::MINICPM) {
        ...
        m_impl = std::make_shared<InputsEmbedderDeepseekVLV2>(vlm_config, models_map, tokenizer, config_dir_path, device, device_config);
    } else if (vlm_config.model_type == VLMModelType::MUSE_GLIMMER) {
