...
public:
    ...
    VLMChatContext(const ChatHistory& history,
...
private:
    ...
    void fill_messages_metadata(size_t start_index,
                                const std::vector<size_t>& new_image_indices,
                                const std::vector<size_t>& new_video_indices);

    std::string multipart_message_to_string(const JsonContainer& message,
