...
public:
    ...
    ProcessedChatData
    process(const std::vector<ov::Tensor>& new_images, const std::vector<ov::Tensor>& new_videos = {});
...
private:
    ...
    void encode_visions_if_needed(const std::vector<size_t>& image_indices, const std::vector<size_t>& video_indices);
    ...
    std::string
    multipart_message_to_string(const JsonContainer& message, size_t base_image_index, size_t base_video_index) const;
};
