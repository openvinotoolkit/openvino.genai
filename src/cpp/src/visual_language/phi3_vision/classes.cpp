...
ov::Tensor reshape_hd_patches_2x2merge(
    ...
    for (size_t batch_id = 0; batch_id < nhwc.at(0); ++batch_id) {
        for (size_t row_id = 0; row_id < nhwc.at(1); ++row_id) {
            for (size_t col_id = 0; col_id < nhwc.at(2); ++col_id) {
                std::copy_n(
                    in + batch_id * nhwc.at(1) * nhwc.at(2) * nhwc.at(3) + row_id * nhwc.at(2) * nhwc.at(3) +
                        col_id * nhwc.at(3),
                    nhwc.at(3),
            ...
            std::copy(
                sub_GN.begin(),
                sub_GN.end(),
                out + batch_id * nhwc.at(1) * (nhwc.at(2) + 1) * nhwc.at(3) + row_id * (nhwc.at(2) + 1) * nhwc.at(3) +
                    nhwc.at(2) * nhwc.at(3)
            );
        }
    }
...
ov::Tensor
concatenate_2d(const ov::Tensor& first_1lf, const std::vector<float>& second_f, const ov::Tensor& third_1lf) {
...
ov::Tensor hd_feature_transform(
    const EncodedImage& image_features,
    InferRequest& hd_feature_transformer,
...
std::shared_ptr<ov::Node>
create_concatenate_batch(std::shared_ptr<ov::Node> global_processed, std::shared_ptr<ov::Node> hd_sliced) {
    using namespace ov::op;
...
std::shared_ptr<ov::Node>
create_pad_to_max_crops(std::shared_ptr<ov::Node> input_nchw, std::shared_ptr<ov::Node> max_crops_param) {
    using namespace ov::op;
    ...
    auto zero_3 = v0::Constant::create(
        ov::element::i64,
        ov::Shape{3},
        std::vector<int64_t>{0, 0, 0}
    );  // Zeros for C, H, W dimensions
...
std::vector<std::variant<ov::Tensor, size_t>>
split_tokenize(const std::string& text, ov::genai::Tokenizer& tokenizer, const std::regex& native_pattern) {
...
EncodedImage VisionEncoderPhi3V::encode(const ov::Tensor& image, const ov::AnyMap& config_map) {
    ...
    encoded_image.images_features_projection = hd_feature_transform(
        encoded_image,
...
NormalizedPrompt InputsEmbedderPhi3V::normalize_prompt(
    const std::string& prompt,
    size_t base_id,
...
ov::Tensor InputsEmbedderPhi3V::get_inputs_embeds(
...
void InputsEmbedderPhi3V::update_chat_history(
