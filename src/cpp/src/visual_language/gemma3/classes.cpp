...
NormalizedPrompt InputsEmbedderGemma3::normalize_prompt(
...
ov::Tensor InputsEmbedderGemma3::get_inputs_embeds(
    ...
    OPENVINO_THROW(
        "[InputsEmbedderGemma3] The method get_inputs_embeds is not supported for Gemma3 models because token type IDs "
        "are required to distinguish between text and image tokens in the input sequence. "
        "Please use get_inputs_embeds_with_token_type_ids instead, which returns both the input embeddings and the "
        "necessary token type IDs. "
...
InputsEmbedderGemma3::get_position_ids(const size_t inputs_embeds_size, const size_t history_size) {
