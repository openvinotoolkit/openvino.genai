...
void fill_video_metadata(
    ...
    OPENVINO_ASSERT(
        !(video_config.fps != 0.0f && video_config.num_frames != 0),
    ...
    if (num_frames == 0 && video_config.fps != 0.0f) {
        OPENVINO_ASSERT(
            encoded_video.metadata.fps != 0.0f,
    ...
    OPENVINO_ASSERT(
        num_frames > 1 && num_frames <= total_num_frames,
...
std::pair<ov::Tensor, ov::Tensor> get_position_interpolation_indices_and_weights(
    ...
    for (size_t corner = 0; corner < 4; ++corner) {
        std::memcpy(
            indices_data + corner * total_positions,
            indices_list[corner].data(),
            total_positions * sizeof(int64_t)
        );
        std::memcpy(
            weights_data + corner * total_positions,
 ...
 */
ov::Tensor
create_visual_pos_masks(const ov::Tensor& input_ids, int64_t image_pad_token_id, int64_t video_pad_token_id) {
...
std::pair<ov::Tensor, ov::Tensor> InputsEmbedderQwen3VL::run_video_image_embeddings_merger(
    ...
    combined_grid_thw
        .insert(combined_grid_thw.end(), reordered_videos_grid_thw.begin(), reordered_videos_grid_thw.end());
    combined_grid_thw
        .insert(combined_grid_thw.end(), reordered_images_grid_thw.begin(), reordered_images_grid_thw.end());
...
ov::Tensor InputsEmbedderQwen3VL::get_inputs_embeds(
    ...
    for (const auto& encoded_video : videos) {
        videos_grid_thw.push_back(
            {encoded_video.frame_num, encoded_video.resized_source_size.height, encoded_video.resized_source_size.width}
        );
    }
