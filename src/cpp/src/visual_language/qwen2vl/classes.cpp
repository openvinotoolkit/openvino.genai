...
ov::Tensor InputsEmbedderQwen2VL::get_inputs_embeds(const std::string& unified_prompt,
    ...
    auto apply_pruning = [&](size_t vision_count,
        ...
        if (auto pruning_result = execute_pruning_pipeline(pruning_context)) {
            ...
            // Log pruned input_ids shape and content
            auto pruned_shape = input_ids.get_shape();
            GENAI_INFO("[Qwen2VL/Qwen2.5-VL] [CDPruner] Pruned Input IDs Shape: [%zu, %zu]", 
                       pruned_shape[0], pruned_shape[1]);
            GENAI_INFO("[Qwen2VL/Qwen2.5-VL] [CDPruner] Updated RoPE Delta: %ld", m_rope_delta);
            ...
            // Diagnostic: check grid_thw vs actual vision tokens in pruned input_ids
            {
                ...
                GENAI_INFO("[Qwen2VL/Qwen2.5-VL] [CDPruner] [DIAG] Before create_position_ids: "
                ...
                if (grid_expected_tokens != actual_vision_tokens) {
                    GENAI_INFO("[Qwen2VL/Qwen2.5-VL] [CDPruner] [DIAG] WARNING: grid_thw expects %zu vision tokens "
            ...
            GENAI_INFO("[Qwen2VL/Qwen2.5-VL] [CDPruner] [DIAG] create_position_ids returned (may have overflowed)");
            ...
            std::ostringstream pruned_pos_shape_str;
            pruned_pos_shape_str << "[";
            for (size_t i = 0; i < pruned_pos_shape.size(); ++i) {
                pruned_pos_shape_str << pruned_pos_shape[i];
                if (i < pruned_pos_shape.size() - 1) pruned_pos_shape_str << ", ";
            }
            pruned_pos_shape_str << "]";
            
            int64_t pruned_pos_max = *std::max_element(pruned_pos_data, pruned_pos_data + pruned_total_size);
            GENAI_INFO("[Qwen2VL/Qwen2.5-VL] [CDPruner] Pruned Position IDs Shape: %s (total_size=%zu)", 
                       pruned_pos_shape_str.str().c_str(), pruned_total_size);
            GENAI_INFO("[Qwen2VL/Qwen2.5-VL] [CDPruner] Pruned Position IDs Max: %ld", pruned_pos_max);
            
            // Show pruned sequence with vision token counts
            size_t batch_size = pruned_shape[0];
            size_t seq_len = pruned_shape[1];
            const int64_t* pruned_ids_data = input_ids.data<const int64_t>();
            
            for (size_t b = 0; b < batch_size; ++b) {
                // Count vision tokens in pruned sequence
                size_t vision_count = 0;
                size_t vision_start_idx = 0;
                size_t vision_end_idx = 0;
                bool found_vision_start = false;
                
                for (size_t i = 0; i < seq_len; ++i) {
                    int64_t token_id = pruned_ids_data[b * seq_len + i];
                    if (token_id == vision_start_token_id && !found_vision_start) {
                        vision_start_idx = i;
                        found_vision_start = true;
                    }
                    if (token_id == vision_pad_token_id) {
                        vision_count++;
                    }
                    if (token_id == vision_end_token_id && found_vision_start) {
                        vision_end_idx = i;
                    }
                }
                
                GENAI_INFO("[Qwen2VL/Qwen2.5-VL] [CDPruner] Batch %zu: %zu tokens total, %zu vision_pad tokens (index %zu-%zu)", 
                           b, seq_len, vision_count, vision_start_idx, vision_end_idx);
                
                // Show a sample of the pruned tokens (first/last 10 vision tokens)
                if (vision_count > 0 && vision_count <= 20) {
                    std::ostringstream vision_tokens;
                    vision_tokens << "[CDPruner] Vision token indices: [";
                    size_t count = 0;
                    for (size_t i = 0; i < seq_len && count < vision_count; ++i) {
                        if (pruned_ids_data[b * seq_len + i] == vision_pad_token_id) {
                            vision_tokens << i;
                            count++;
                            if (count < vision_count) vision_tokens << ", ";
                        }
                    }
                    vision_tokens << "]";
                    GENAI_INFO("[Qwen2VL/Qwen2.5-VL] %s", vision_tokens.str().c_str());
                }
            }
        }
    };
