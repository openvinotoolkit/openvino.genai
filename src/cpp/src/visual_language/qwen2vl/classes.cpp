...
ov::Tensor reshape_image_patches(const ov::Tensor& patches,
                                 const size_t grid_t,
                                 const size_t grid_h,
                                 const size_t grid_w,
...
ov::Tensor transpose_image_patches(const ov::Tensor& reshaped_patches) {
    ...
    for (size_t idx = 0; idx < total_elements; idx++) {
        ...
        std::vector<size_t> output_indices = {input_indices.at(0),
                                              input_indices.at(3),
                                              input_indices.at(6),
                                              input_indices.at(4),
                                              input_indices.at(7),
...
ov::Tensor concatenate_video_image_embeds(const std::vector<ov::Tensor>& reordered_video_embeds,
    ...
    if (reordered_video_embeds.size() > 0u && reordered_image_embeds.size() > 0u) {
        OPENVINO_ASSERT(reordered_video_embeds.at(0).get_element_type() ==
                        reordered_image_embeds.at(0).get_element_type());
        OPENVINO_ASSERT(reordered_video_embeds.at(0).get_shape().at(1) ==
                        reordered_image_embeds.at(0).get_shape().at(1));
    }
...
void VisionEncoderQwen2VL::encode_with_imagepreprocess_cpp(const std::vector<ov::Tensor>& images,
    ...
    ov::Tensor reshaped_patches = qwen2_vl_utils::reshape_image_patches(patches,
                                                                        grid_t,
                                                                        grid_h,
                                                                        grid_w,
                                                                        channel,
    ...
    ov::Shape flattened_patches_shape{grid_t * grid_h * grid_w,
...
VisionEncoderQwen2VL::EncodeFunc VisionEncoderQwen2VL::get_encode_func() {
    if (use_ov_vision_preprocess) {
        return [this](const std::vector<ov::Tensor>& images,
                      const ProcessorConfig& config,
                      ov::Tensor& out_tensor,
                      ImageSize& out_rsz_size,
                      size_t frame_num,
...
void VisionEncoderQwen2VL::encode_frames_with_config(EncodedVideo& encoded_video,
...
InputsEmbedderQwen2VL::InputsEmbedderQwen2VL(const VLMConfig& vlm_config,
...
void InputsEmbedderQwen2VL::expand_video_tags_in_prompt(std::string& unified_prompt,
...
std::pair<ov::Tensor, std::optional<int64_t>> InputsEmbedderQwen2VL::get_position_ids(const size_t inputs_embeds_size,
