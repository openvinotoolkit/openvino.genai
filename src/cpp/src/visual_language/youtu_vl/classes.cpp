...
InputsEmbedderYoutuVL::InputsEmbedderYoutuVL(const VLMConfig& vlm_config,
    ...
    load_special_tokens(config_dir_path);
}
...
void InputsEmbedderYoutuVL::load_special_tokens(const std::filesystem::path& config_dir) {
    std::ifstream stream(config_dir / "tokenizer.json");
    OPENVINO_ASSERT(stream.is_open(),
                    "Failed to open '", (config_dir / "tokenizer.json").string(),
                    "' required for Youtu-VL special-token handling.");
    nlohmann::json parsed = nlohmann::json::parse(stream);
    if (parsed.contains("added_tokens") && parsed.at("added_tokens").is_array()) {
        for (const auto& tok : parsed.at("added_tokens")) {
            if (tok.value("special", false) && tok.contains("content") && tok.contains("id")) {
                m_special_tokens[tok.at("content").get<std::string>()] = tok.at("id").get<int64_t>();
            }
        }
    }
    auto it = m_special_tokens.find(m_vlm_config.image_pad_token);
    OPENVINO_ASSERT(it != m_special_tokens.end(),
                    "Youtu-VL image pad token '", m_vlm_config.image_pad_token,
                    "' not found among tokenizer special tokens.");
    m_image_pad_token_id = it->second;

    std::ifstream cfg_stream(config_dir / "config.json");
    if (cfg_stream.is_open()) {
        nlohmann::json cfg = nlohmann::json::parse(cfg_stream);
        if (cfg.contains("bos_token_id") && cfg.at("bos_token_id").is_number_integer()) {
            m_bos_token_id = cfg.at("bos_token_id").get<int64_t>();
        }
    }
}

// The exported OpenVINO tokenizer splits Youtu-VL added/special tokens into character-level
// subtokens instead of emitting their atomic ids (unlike the HuggingFace tokenizer used by
// optimum-intel). We therefore split the text on the known special-token strings, tokenize the
// ordinary text spans with the OpenVINO tokenizer, and splice the correct special-token ids back.
ov::Tensor InputsEmbedderYoutuVL::encode_with_special_tokens(const std::string& text, bool add_special_tokens) {
    // Collect all non-overlapping special-token matches (earliest position first, longest match wins).
    struct Match { size_t pos; size_t len; int64_t id; };
    std::vector<Match> matches;
    for (const auto& [str, id] : m_special_tokens) {
        if (str.empty()) {
            continue;
        }
        size_t start = 0;
        while (true) {
            size_t p = text.find(str, start);
            if (p == std::string::npos) {
                break;
            }
            matches.push_back({p, str.length(), id});
            start = p + 1;
        }
    }
    ...
    auto append_text_span = [&](const std::string& span) {
        if (span.empty()) {
            return;
        }
        ov::Tensor span_ids = m_tokenizer.encode(span, ov::genai::add_special_tokens(false)).input_ids;
        const int64_t* data = span_ids.data<const int64_t>();
        ids.insert(ids.end(), data, data + span_ids.get_size());
    };
    ...
    for (const Match& m : matches) {
        if (m.pos < guard_end) {
            continue;  // overlaps a previously selected (longer/earlier) match
        }
        if (m.pos > cursor) {
            append_text_span(text.substr(cursor, m.pos - cursor));
        }
        ids.push_back(m.id);
        cursor = m.pos + m.len;
        guard_end = cursor;
    ...
    if (cursor < text.length()) {
        append_text_span(text.substr(cursor));
    }

    if (add_special_tokens) {
        // Mirror the HF tokenizer adding a single BOS at the very beginning when requested
        // and not already present (the chat template normally injects it explicitly).
        if (m_bos_token_id >= 0 && (ids.empty() || ids.front() != m_bos_token_id)) {
            ids.insert(ids.begin(), m_bos_token_id);
        }
    }

    ov::Tensor result(ov::element::i64, ov::Shape{1, ids.size()});
    std::copy(ids.begin(), ids.end(), result.data<int64_t>());
    return result;
}

ov::Tensor InputsEmbedderYoutuVL::apply_chat_template_tokenize(const std::string& prompt,
    ...
    if (m_is_chat_conversation) {
        ov::Tensor new_chat_tokens = encode_with_special_tokens(prompt, add_special_tokens_flag);
        encode_timer.end();
        metrics.raw_metrics.tokenization_durations.emplace_back(encode_timer.get_duration_microsec());
        return new_chat_tokens;
    }

    std::string text_to_encode = prompt;
    bool applied_template = false;
    TimePoint template_end_time;
    if (m_apply_chat_template) {
        ChatHistory history({{{"role", "user"}, {"content", prompt}}});
        constexpr bool add_generation_prompt = true;
        text_to_encode = m_tokenizer.apply_chat_template(history, add_generation_prompt);
        template_end_time = std::chrono::steady_clock::now();
        applied_template = true;
    }
    ov::Tensor encoded_input_ids = encode_with_special_tokens(text_to_encode, add_special_tokens_flag);
    encode_timer.end();
    if (applied_template) {
