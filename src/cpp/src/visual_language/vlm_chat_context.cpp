...
VLMChatContext::VLMChatContext(const ChatHistory& history,
...
VLMChatContext::ProcessedChatData VLMChatContext::process(const std::vector<ov::Tensor>& new_images,
    ...
    const auto& last_user_message_metadata =
        m_history_state->get_messages_metadata().at(m_history_state->get_last_user_message_index());

    auto resolved_new_visions =
        m_history_state->resolve_visions_with_sequence(last_user_message_metadata.image_sequence,
...
void VLMChatContext::encode_visions_if_needed(const std::vector<size_t>& image_indices,
...
void VLMChatContext::fill_messages_metadata(size_t start_index,
 ...
 */
std::string VLMChatContext::multipart_message_to_string(const JsonContainer& message,
                                                        size_t base_image_index,
                                                        size_t base_video_index) const {
    std::string result = "";
 ...
 * Updates normalized_content in MessageMetadata directly with the pruned result.
 */
void VLMChatContext::apply_pruning_to_last_message() {
    ...
    // Update normalized_content directly with the pruned prompt
    last_metadata.normalized_content = m_inputs_embedder.get_last_pruned_prompt(last_metadata.normalized_content);
}
