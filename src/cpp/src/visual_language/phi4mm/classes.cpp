...
std::unique_ptr<ov::genai::CircularBufferQueue<ov::InferRequest>> create_image_preprocessors_v2() {
    using namespace ov;
...
std::ostream& operator<<(std::ostream& os, const TargetSizes& sizes) {
    os << "TargetSizes(width: " << sizes.new_sizes[0]
       << ", height: " << sizes.new_sizes[1]
       << ", padding_width: " << sizes.padding_width
       << ", padding_height: " << sizes.padding_height
       << ", attention_mask: " << sizes.attention_mask.get_shape() << ")";
    return os;
}
...
TargetSizes get_target_sizes(const ov::Tensor& image, const size_t dynamic_hd = 36) {
    ...
    if (w_crop_num * h_crop_num > dynamic_hd) {
        ...
        for (size_t n = min_num; n <= max_num; ++n) {
        ...
        for (const auto& ratio : target_ratios_vec) {
            float target_aspect_ratio_value = static_cast<float>(ratio.first) / ratio.second;
            float ratio_diff = std::abs(aspect_ratio - target_aspect_ratio_value);
    ...
    if (padding_width >= 14) {
        size_t padding_width_blocks = static_cast<size_t>(std::floor(static_cast<float>(padding_width) / 14));
    ...
    if (padding_height >= 14) {
        size_t row_width = MASK_RESOLUTION * target_aspect_ratio.first;
        size_t column_height = MASK_RESOLUTION * target_aspect_ratio.second;
        size_t padding_height_blocks = static_cast<size_t>(std::floor(static_cast<float>(padding_height) / 14));
...
m_image_preprocessors{create_image_preprocessors_v2()},
...
EncodedImage VisionEncoderPhi4MM::encode(const ov::Tensor& image, const ov::AnyMap& config_map) {
    ...
    std::cout << "Target sizes: " << target_sizes << std::endl;
    {
        ...
        image_preprocessor.set_tensor("new_size", ov::Tensor{ov::element::i64, {2}, target_sizes.new_sizes.data()});
        std::vector<size_t> padding_width{target_sizes.padding_width};
        image_preprocessor.set_tensor("padding_width", ov::Tensor{ov::element::i64, {}, padding_width.data()});
        std::vector<size_t> padding_height{target_sizes.padding_height};
        image_preprocessor.set_tensor("padding_height", ov::Tensor{ov::element::i64, {}, padding_height.data()});
