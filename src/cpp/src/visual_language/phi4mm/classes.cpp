...
std::unique_ptr<ov::genai::CircularBufferQueue<ov::InferRequest>> create_image_preprocessors(
    ...
    auto t23 =
        make_shared<Concat>(NodeVector{t5, t14, t15, t21, t22}, 0);  // i64[1], i64[1], i64[1], i64[1], i64[1] -> i64[5]
    ...
    auto t30 = make_shared<Constant>(i64,
                                     Shape{2},
                                     9223372036854775807);  //  -> i64[2]([9223372036854775807, 9223372036854775807])
    ...
    auto t33 =
        make_shared<Slice>(t28, t29, t30, t31, t32);     // f32[?,32,32], i64[2], i64[2], i64[2], i64[2] -> f32[?,16,16]
    ...
    auto t37 = make_shared<Concat>(NodeVector{t34, t14, t21, t35, t36},
                                   0);                 // i64[1], i64[1], i64[1], i64[1], i64[1] -> i64[5]
    ...
    Interpolate::InterpolateAttrs t94_attrs{Interpolate::InterpolateMode::BILINEAR_PILLOW,
                                            Interpolate::ShapeCalcMode::SIZES,
                                            vector<size_t>{0, 0, 0, 0},
                                            vector<size_t>{0, 0, 0, 0},
                                            Interpolate::CoordinateTransformMode::PYTORCH_HALF_PIXEL,
    ...
    Interpolate::InterpolateAttrs t139_attrs{Interpolate::InterpolateMode::CUBIC,
                                             Interpolate::ShapeCalcMode::SIZES,
                                             vector<size_t>{0, 0, 0, 0},
                                             vector<size_t>{0, 0, 0, 0},
                                             Interpolate::CoordinateTransformMode::PYTORCH_HALF_PIXEL,
                                             ...
                                             -0.75};
    auto t139 =
        make_shared<Interpolate>(t136, t137, t138, t139_attrs);  // f32[1,3,?,?], i32[2], i32[2] -> f32[1,3,448,448]
    ...
    auto t152 = make_shared<Concat>(NodeVector{t140, t141, t146, t147, t150, t151},
                                    0);                   // i64[1], i64[1], i64[1], i64[1], i64[1], i64[1] -> i64[6]
    auto t153 = make_shared<Reshape>(t127, t152, false);  // f32[3,?,?], i64[6] -> f32[?,?,?,?,?,?]
    auto t154 =
        make_shared<Constant>(i64, Shape{6}, vector<int64_t>{0, 2, 4, 1, 3, 5});  //  -> i64[6]([0, 2, 4, 1, 3, 5])
    auto t155 = make_shared<Transpose>(t153, t154);  // f32[?,?,?,?,?,?], i64[6] -> f32[?,?,?,?,?,?]
    auto t156 =
        make_shared<Constant>(i64, Shape{4}, vector<int64_t>{-1, 3, 448, 448});  //  -> i64[4]([-1, 3, 448, 448])
    auto t157 = make_shared<Reshape>(t155, t156, false);  // f32[?,?,?,?,?,?], i64[4] -> f32[?,3,448,448]
    auto t158 =
        make_shared<Concat>(NodeVector{t139, t157}, 0);  // f32[1,3,448,448], f32[?,3,448,448] -> f32[1..,3,448,448]
    ...
    return make_unique<CircularBufferQueue<InferRequest>>(compiled.get_property(ov::optimal_number_of_infer_requests),
                                                          [&compiled]() -> ov::InferRequest {
                                                              return compiled.create_infer_request();
                                                          });
}

ov::Tensor calculate_patch_position_ids(const ov::Tensor& input_image_embeds,
...
std::unique_ptr<ov::genai::CircularBufferQueue<ov::InferRequest>> create_separator_inserters(
    ...
    auto t44 = make_shared<Concat>(NodeVector{t30, t31, t40, t40, t43},
                                   0);                 // i64[1], i64[1], i64[1], i64[1], i64[1] -> i64[5]
    ...
    auto t103 =
        make_shared<Slice>(t55, t56, t100, t101, t102);  // f32[?,?,?,?], i64[2], i64[2], i64[2], i64[2] -> f32[?,?,?,?]
    ...
    auto t117 = make_shared<Constant>(i64, Shape{1}, 0);              //  -> i64[1]([0])
    auto t118 =
        make_shared<Slice>(t8, t114, t115, t116, t117);   // f32[?,?,?], i64[1], i64[1], i64[1], i64[1] -> f32[..1,?,?]
    ...
    SinkVector sinks{};
    ParameterVector parameters{std::move(t0),
                               std::move(t1),
                               std::move(t2),
                               std::move(t3),
                               std::move(t4),
...
TargetSizes get_target_sizes(const ov::Tensor& image, const size_t dynamic_hd = 36) {
    ...
    if (w_crop_num * h_crop_num > dynamic_hd) {
        ...
        for (const auto& ratio : target_ratios_vec) {
            ...
            } else if (ratio_diff == best_ratio_diff && orig_width * orig_height > 0.5f * DYHD_BASE_RESOLUTION *
    ...
    if (padding_width >= 14) {
        ...
        for (size_t i = 0; i < column_height; ++i) {
            std::fill_n(attention_mask.data<float>() + (i * row_width) + (row_width - padding_width_blocks),
                        padding_width_blocks,
                        0.0f);
        }
    }
    if (padding_height >= 14) {
        ...
        std::fill(attention_mask.data<float>() + (column_height - padding_height_blocks) * row_width,
                  attention_mask.data<float>() + (row_width * column_height),
                  0.0f);
    }
...
VisionEncoderPhi4MM::VisionEncoderPhi4MM(const std::filesystem::path& model_dir,
                                         const std::string& device,
...
VisionEncoderPhi4MM::VisionEncoderPhi4MM(const ModelsMap& models_map,
    ...
    auto compiled_model =
        utils::singleton_core().compile_model(vision_projection_model, vision_projection_weights, device, properties);
    m_ireq_queue_vision_projection = std::make_unique<CircularBufferQueue<ov::InferRequest>>(
        compiled_model.get_property(ov::optimal_number_of_infer_requests),
        [&compiled_model]() -> ov::InferRequest {
            return compiled_model.create_infer_request();
...
EncodedImage VisionEncoderPhi4MM::encode(const ov::Tensor& image, const ov::AnyMap& config_map) {
    ...
    ov::Tensor img_features{ov::element::f32, {}};
    {
        ...
        std::transform(image_attention_mask.data<float>(),
                       image_attention_mask.data<float>() + image_attention_mask.get_size(),
                       bools.data<bool>(),
                       [](float v) {
                           return v > 0.5f;
                       });
    ...
    ov::Tensor img_features_with_separators{ov::element::f32, {}};
    {
        ...
        ov::Tensor sub_GN{ov::element::f32,
                          {1, 1, 1, m_vlm_config.sub_GN.size()},
                          const_cast<float*>(m_vlm_config.sub_GN.data())};
        ov::Tensor glb_GN{ov::element::f32,
                          {1, 1, m_vlm_config.glb_GN.size()},
                          const_cast<float*>(m_vlm_config.glb_GN.data())};
...
InputsEmbedderPhi4MM::InputsEmbedderPhi4MM(const VLMConfig& vlm_config,
...
NormalizedPrompt InputsEmbedderPhi4MM::normalize_prompt(const std::string& prompt,
                                                        size_t base_id,
...
ov::Tensor InputsEmbedderPhi4MM::get_inputs_embeds(const std::string& image_prompt,
...
void InputsEmbedderPhi4MM::update_chat_history(const std::string& decoded_results,
