...
std::unique_ptr<ov::genai::CircularBufferQueue<ov::InferRequest>> create_image_preprocessors(
    ...
    auto t30 = make_shared<Constant>(
        i64,
        Shape{2},
        9223372036854775807
    );                                                   //  -> i64[2]([9223372036854775807, 9223372036854775807])
    ...
    auto t37 = make_shared<Concat>(
        NodeVector{t34, t14, t21, t35, t36},
        0
    );                                                 // i64[1], i64[1], i64[1], i64[1], i64[1] -> i64[5]
    ...
    Interpolate::InterpolateAttrs t94_attrs{
        Interpolate::InterpolateMode::BILINEAR_PILLOW,
    ...
    Interpolate::InterpolateAttrs t139_attrs{
    ...
    auto t152 = make_shared<Concat>(
        NodeVector{t140, t141, t146, t147, t150, t151},
        0
    );                                                    // i64[1], i64[1], i64[1], i64[1], i64[1], i64[1] -> i64[6]
...
std::unique_ptr<ov::genai::CircularBufferQueue<ov::InferRequest>> create_separator_inserters(
    ...
    auto t44 = make_shared<Concat>(
        NodeVector{t30, t31, t40, t40, t43},
        0
    );                                                 // i64[1], i64[1], i64[1], i64[1], i64[1] -> i64[5]
    ...
    SinkVector sinks{};
    ParameterVector
        parameters{std::move(t0), std::move(t1), std::move(t2), std::move(t3), std::move(t4), std::move(t5)};
    auto model = make_shared<Model>(std::move(results), std::move(sinks), std::move(parameters));
    using namespace ov::genai;
    CompiledModel compiled = utils::singleton_core().compile_model(model, device);
    return make_unique<CircularBufferQueue<InferRequest>>(
...
TargetSizes get_target_sizes(const ov::Tensor& image, const size_t dynamic_hd = 36) {
    ...
    if (w_crop_num * h_crop_num > dynamic_hd) {
        ...
        for (const auto& ratio : target_ratios_vec) {
            ...
            if (ratio_diff < best_ratio_diff) {
                ...
                ratio_diff == best_ratio_diff && orig_width * orig_height > 0.5f * DYHD_BASE_RESOLUTION *
                                                                                DYHD_BASE_RESOLUTION * ratio.first *
                                                                                ratio.second
            ) {
                target_aspect_ratio = ratio;
            }
    ...
    if (padding_width >= 14) {
        ...
        for (size_t i = 0; i < column_height; ++i) {
            std::fill_n(
                attention_mask.data<float>() + (i * row_width) + (row_width - padding_width_blocks),
    ...
    if (padding_height >= 14) {
        ...
        std::fill(
            attention_mask.data<float>() + (column_height - padding_height_blocks) * row_width,
...
EncodedImage VisionEncoderPhi4MM::encode(const ov::Tensor& image, const ov::AnyMap& config_map) {
    ...
    ov::Tensor img_features{ov::element::f32, {}};
    {
        ...
        std::transform(
            image_attention_mask.data<float>(),
...
NormalizedPrompt InputsEmbedderPhi4MM::normalize_prompt(
