...
public:
    InputsEmbedder(const std::filesystem::path& model_dir, const std::string& device, const ov::AnyMap device_config);
    ...
    get_position_ids(const size_t inputs_embeds_size, const size_t history_size);
    ...
    std::pair<ov::Tensor, std::optional<int64_t>>
    get_generation_phase_position_ids(const size_t inputs_embeds_size, const size_t history_size, int64_t rope_delta);
    ...
    void
    update_chat_history(const std::string& decoded_results, const ov::genai::GenerationStatus generation_finish_status);
    ...
    virtual NormalizedPrompt
    normalize_prompt(const std::string& prompt, size_t base_id, const std::vector<EncodedImage>& images) const;
...
private:
    ...
    public:
        ...
        virtual std::pair<ov::Tensor, std::optional<int64_t>>
        get_position_ids(const size_t inputs_embeds_size, const size_t history_size);
        ...
        virtual std::pair<ov::Tensor, std::optional<int64_t>> get_generation_phase_position_ids(
        ...
        virtual void update_chat_history(
        ...
        virtual NormalizedPrompt
        normalize_prompt(const std::string& prompt, size_t base_id, const std::vector<EncodedImage>& images) const = 0;
        virtual NormalizedPrompt normalize_prompt(
            const std::string& prompt,
            size_t base_image_id,
            size_t base_video_id,
...
universal_to_native(const std::string& prompt, const Func& write_native, VisionType vision_type = VisionType::IMAGE) {
    std::stringstream stream;
    std::vector<size_t> vision_sequence;
    std::smatch match;
    auto universal_pattern = vision_type == VisionType::IMAGE ? UNIVERSAL_IMAGE_PATTERN : UNIVERSAL_VIDEO_PATTERN;
