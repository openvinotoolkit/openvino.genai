...
VisionEncoder::VisionEncoder(const std::filesystem::path& model_dir,
    ...
    auto compiled_model =
        utils::singleton_core().compile_model(model_dir / "openvino_vision_embeddings_model.xml", device, properties);
    ...
    m_video_processor_config =
        utils::from_config_json_if_exists<VideoProcessorConfig>(model_dir, "video_preprocessor_config.json");
}

VisionEncoder::VisionEncoder(const ModelsMap& models_map,
    ...
    auto compiled_model =
        utils::singleton_core().compile_model(vision_encoder_model, vision_encoder_weights, device, device_config);
    ov::genai::utils::print_compiled_model_properties(compiled_model, "VLM vision embeddings model");
    m_ireq_queue_vision_encoder = std::make_unique<CircularBufferQueue<ov::InferRequest>>(
        compiled_model.get_property(ov::optimal_number_of_infer_requests),
        [&compiled_model]() -> ov::InferRequest {
    ...
    m_processor_config =
        utils::from_config_json_if_exists<ProcessorConfig>(config_dir_path, "preprocessor_config.json");
    m_video_processor_config =
        utils::from_config_json_if_exists<VideoProcessorConfig>(config_dir_path, "video_preprocessor_config.json");
}
...
VisionEncoder::Ptr VisionEncoder::create(const std::filesystem::path& model_dir,
    ...
    if (model_type == VLMModelType::MINICPM) {
        ...
        OPENVINO_THROW(
            "Unsupported model type in VLM VisionEncoder class. Please, create feature request on new model support");
    }
}

VisionEncoder::Ptr VisionEncoder::create(const ModelsMap& models_map,
                                         const std::filesystem::path& config_dir_path,
                                         const VLMModelType model_type,
                                         const std::string& device,
                                         const ov::AnyMap device_config) {
