...
apply_qwen3_postprocessing(std::shared_ptr<Model> model, const Qwen3PostprocessingParams& params) {
    PartialShape output_shape = model->get_output_partial_shape(0);

    ov::preprocess::PrePostProcessor processor(model);

    processor.output().postprocess().custom(
        [&output_shape, &params](const ov::Output<ov::Node>& node) -> std::shared_ptr<ov::Node> {
            ...
            auto indices = std::make_shared<op::v0::Constant>(
                ov::element::i64,
                ov::Shape{2},
...
TextRerankPipeline::TextRerankPipeline(
    ...
    : m_impl{std::make_unique<TextRerankPipelineImpl>(
          models_path,
          device,
          Config(properties),
...
TextRerankPipeline::rerank(const std::string& query, const std::vector<std::string>& texts) {
