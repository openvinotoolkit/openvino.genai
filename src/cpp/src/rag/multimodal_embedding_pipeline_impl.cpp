...
private:
    void init_multimodal(const std::filesystem::path& models_path,
        ...
        m_compiled_language_model =
            utils::singleton_core().compile_model(language_model, device, language_model_properties);
    ...
    EmbedResult multimodal_embed(const std::vector<std::string>& texts,
        ...
        for (const std::string& batch_text : texts) {
            ...
            const NormalizedPrompt normalized_prompt =
                m_inputs_embedder->normalize_prompt(format_prompt(formatted_text, prompt),
                                                    0,
                                                    0,
                                                    encoded_images,
                                                    encoded_videos);
        ...
        if (batch_items[0].position_ids.has_value() && has_lm_input("position_ids")) {
            ...
            OPENVINO_ASSERT(first_pos_shape.size() == 3,
                            "Expected position_ids to have rank 3, got rank ",
                            first_pos_shape.size());
            ov::Shape batched_pos_shape = first_pos_shape;
            batched_pos_shape[1] = batch_size;
        ...
        for (size_t batch_idx = 0; batch_idx < batch_size; ++batch_idx) {
            const auto& item = batch_items[batch_idx];
            const size_t seq_length = item.inputs_embeds.get_shape().at(1);
            ...
            float* dst_embeds =
                batched_inputs_embeds.data<float>() + batch_idx * max_seq_length * embed_dim;
            std::copy_n(item.inputs_embeds.data<const float>(), seq_length * embed_dim, dst_embeds);

            int64_t* dst_mask = batched_attention_mask.data<int64_t>() + batch_idx * max_seq_length;
            std::fill_n(dst_mask, seq_length, 1);

            if (item.token_type_ids.has_value()) {
                int64_t* dst_token_type_ids =
                    batched_token_type_ids->data<int64_t>() + batch_idx * max_seq_length;
                std::copy_n(item.token_type_ids->data<const int64_t>(), seq_length, dst_token_type_ids);
            }

            if (batched_position_ids.has_value() && item.position_ids.has_value()) {
                const size_t num_dims = batched_position_ids->get_shape()[0];
                const int64_t* src_position_ids = item.position_ids->data<const int64_t>();
                for (size_t dim_idx = 0; dim_idx < num_dims; ++dim_idx) {
                    int64_t* dst_position_ids = batched_position_ids->data<int64_t>() +
                                                dim_idx * batch_size * max_seq_length +
                                                batch_idx * max_seq_length;
                    std::copy_n(src_position_ids + dim_idx * seq_length, seq_length, dst_position_ids);
                }
            }
        }
        ...
        std::copy_n(batched_output.data<const float>(), batched_output.get_size(), output_copy.data<float>());
    ...
    std::string format_prompt(const std::string& text, const std::optional<std::string>& prompt) const {
        ...
        ChatHistory history({{{"role", "system"}, {"content", *prompt}},
    ...
    std::string append_added_special_tokens(Tokenizer& tokenizer, const std::string& text) const {
        ...
        for (size_t token_idx = 0; token_idx < without_shape[1]; ++token_idx) {
            OPENVINO_ASSERT(with_data[token_idx] == without_data[token_idx],
...
std::unique_ptr<EmbeddingPipelineImpl> make_multimodal_embedding_pipeline_impl(
    ...
    const std::string& device,
    const ov::AnyMap& properties) {
    return std::make_unique<MultimodalEmbeddingPipelineImpl>(models_path, device, properties);
}
