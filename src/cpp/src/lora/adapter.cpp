...
ConstantMap safetensor_to_constant_map(const ov::Tensor& safetensor) {
    ...
    OPENVINO_ASSERT(
        safetensors_file_init(safetensor_data, safetensor.get_byte_size(), &safe_tensors_file) == nullptr,
...
LoRATensors group_lora_tensors(const ConstantMap& tensors, const LoRAPartsParser& parts_parser) {
    ...
    for (const auto& named_tensor : tensors) {
        if (auto parsed = parts_parser.A(named_tensor.first)) {
            ...
            DEBUG_PRINT(
                "Ignored LoRA tensor \"" << named_tensor.first << "\" because couldn't recognize expected name pattern."
            );
        }
    }
    ...
    for (const auto& lora_tensor : result) {
        OPENVINO_ASSERT(
            lora_tensor.second.A && lora_tensor.second.B,
...
LoRAConstantTensors
group_lora_constant_tensors(const ConstantMap& tensors, const std::vector<RegexParser>& const_parsers) {
...
void deduce_input_output_dims(NodePtr node, ov::Dimension& input_dim, ov::Dimension& output_dim) {
    if (std::dynamic_pointer_cast<v1::Convolution>(node)) {
        ...
        OPENVINO_THROW(
            "deduce_input_output_dims expects MatMul or Convolution, but got ",
            node,
...
#endif
    model->add_sinks(
        {std::make_shared<v6::Assign>(read_value, variable)}
    );  // FIXME: Required? -- Yes, create ticket against CPU
    return read_value;
}
}  // namespace
...
struct LoRAStateGetterForConst {
    ...
    LoRAStateGetterForConst(
        const LoRAConstantGetter& getter,
        std::shared_ptr<ov::Model> model,
        std::map<std::string, ov::op::util::VariableInfo>& variable_ids
    )
        : model(model),
        ...
        register_matcher(
            ...
            ([lora_weight_getter, this](ov::pass::pattern::Matcher& m) {
                ...
                try {
                    ...
                    DEBUG_PRINT(
                        "Exception happens on layer: " << node << " with exception message: " << exception.what()
                    );
                    throw;
                } catch (...) {
...
public:
    LoRAReplaceConstantTransform(const LoRAConstantByNodeGetter& getter) {
        register_matcher(
            std::make_shared<ov::pass::pattern::Matcher>(
                ov::pass::pattern::wrap_type<v0::Constant>(),
                this->get_type_info().name
            ),
            ([getter, this](ov::pass::pattern::Matcher& m) {
                ...
                try {
                    ...
                    DEBUG_PRINT(
                        "Exception happens on layer: " << src_node << " with exception message: " << exception.what()
                    );
                    throw;
                } catch (...) {
...
protected:
    ...
    bool apply(NodePtr node, const NodePtr& lora_weight) override {
        ...
        auto then_param = std::make_shared<ov::op::v0::Parameter>(
            lora_output->get_element_type(),
...
NodePtr tensors_multiplication(
    NodePtr input,
    ...
    for (size_t i = 0; i < multipliers.size(); ++i) {
        ...
        if (input) {
            if (i == alpha_pos) {  // Multiply for alpha
                ...
                input = std::make_shared<v0::MatMul>(
                    input,
                    normalized,
                    /*transpose_a = */ false,
                    transpose_weights
                );  // FIXME: verify transpose_a == true
            }
...
NodePtr decompression_convert(NodePtr node) {
    ...
    OPENVINO_ASSERT(
        ...
        "LoRA adapter application: not supported decompression pattern at the weight input (presumably low-bit "
        "compression). Use f32/f16/bf16 weights only if MODE_FUSE is used."
    );
    return convert;
}
...
public:
    ...
    bool apply(NodePtr node, const LoRANode& lora_weight) override {
        ...
        if (!fusers.exist(signature)) {
            ...
            auto target_parameter = std::make_shared<v0::Parameter>(
                weights_constant.get_element_type(),
            ...
            for (auto multiplier : adapter) {
                parameters.push_back(
                    std::make_shared<v0::Parameter>(
                        multiplier->get_output_element_type(0),
                        multiplier->get_output_partial_shape(0)
                    )
                );
            }

            auto result = std::make_shared<v0::Result>(tensors_multiplication(
                nullptr,
...
public:
    ...
    bool apply(NodePtr node, const LoRANode& lora_weight) override {
        ...
        replacement = tensors_multiplication(
...
public:
    SafetensorsAdapterImpl(const std::filesystem::path& path) {
        ...
        OPENVINO_ASSERT(
            path.extension().string() == ".safetensors",
...
struct AdapterControllerImpl {
          ...
          lora_state_evaluators("CPU")  // FIXME: Try to run on the same device that is used for model inference
    {
        ...
        for (auto const& adapter : current_config.get_adapters()) {
            ...
            params_getter.weight_getter.push_back(
                LoRAWeightGetterDefault<LoRAWeight, LoRANode>(
                    &adapter_impl->get_tensors(),
                    config.get_tensor_name_prefix().value_or("")
                )
            );
        ...
        auto weight_as_constant = [&, this](NodePtr node) -> std::optional<LoRANode> {
            ...
            auto lora_weight = prepare_lora_tensors(
                name,
    ...
    void apply(ov::InferRequest& infer_request, std::optional<AdapterConfig> config) {
        ...
        if (config) {
            ...
            OPENVINO_ASSERT(
                config->get_mode() == AdapterConfig::MODE_AUTO || config->get_mode() == AdapterConfig::MODE_DYNAMIC ||
    ...
    std::vector<LoRAWeight>
    collect_applicable_tensors(const std::string& lora_name, const std::vector<LoRAWeightGetter>& weight_getters) {
        const auto& adapters = current_config.get_adapters();
    ...
    LoRAParts<ov::Tensor>
    concat_adapters(const std::vector<LoRAWeight>& inputs, LoRAParts<ov::Tensor>& outputs, bool alpha_only) {
            ...
            build_concat_model(
                ...
                [](const LoRAWeight& lora_weight) {
                    return std::make_shared<v0::Parameter>(
                        ...
                        lora_weight.alpha->get_output_partial_shape(0)
                    );  // TODO: Consider using dynamic LoRA rank dimension instead of static dimension
                },
                [](const LoRAWeight& lora_weight, NodePtr parameter) {
                    ...
                    auto lora_rank_constant = v0::Constant::create(
                        ov::element::u32,
                        Shape{2},
                ...
                build_concat_model(
                    ...
                    [](const LoRAWeight& lora_weight) {
                        return std::make_shared<v0::Parameter>(
                            ...
                            lora_weight.A->get_output_partial_shape(0)
                        );  // TODO: Consider using dynamic LoRA rank dimension instead of static dimension
                    }
                );
                ...
                build_concat_model(
                    ...
                    [](const LoRAWeight& lora_weight) {
                        return std::make_shared<v0::Parameter>(
                            ...
                            lora_weight.B->get_output_partial_shape(0)
                        );  // TODO: Consider using dynamic LoRA rank dimension instead of static dimension
                    }
                );
            }
    ...
    LoRAParts<ov::Tensor> prepare_lora_tensors(
        ...
        auto lora_tensors = collect_applicable_tensors(
            name,
            weight_getters
        );  // request A and B regardless of alpha_only, because it is a way to get lora_rank later when alpha is
            // broadcasted
...
AdapterConfig& AdapterConfig::add(const Adapter& adapter, float alpha) {
    ...
    OPENVINO_ASSERT(
        adapters.end() == std::find(adapters.begin(), adapters.end(), adapter),
...
float AdapterConfig::get_alpha(const Adapter& adapter) const {
    ...
    OPENVINO_ASSERT(
        adapters.end() != it,
        "Unknown adapter object passed to AdapterConfig::get_alpha, alpha can be retrieved for previously registered "
        "adapters only"
    );
