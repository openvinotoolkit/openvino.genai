...
ConstantMap safetensor_to_constant_map(const ov::Tensor& safetensor) {
    ...
    for (int i = 0; i < safe_tensors_file.num_tensors; i++) {
        ...
        constant->get_rt_info()["__safetensors_buffer_holder"] =
            safetensor;  // to automatically deallocate underlying memory buffer when last constant that holds it is
                         // destroyed
...
LoRATensors group_lora_tensors(const ConstantMap& tensors, const LoRAPartsParser& parts_parser) {
    ...
    for (const auto& named_tensor : tensors) {
        if (auto parsed = parts_parser.A(named_tensor.first)) {
            ...
            DEBUG_PRINT("Ignored LoRA tensor \"" << named_tensor.first
                                                 << "\" because couldn't recognize expected name pattern.");
        }
    }

    // Check that A and B exist for each LoRA entry
    for (const auto& lora_tensor : result) {
        OPENVINO_ASSERT(lora_tensor.second.A && lora_tensor.second.B,
                        "Either A, B or both matrices are missing in LoRA tensors for layer: ",
                        lora_tensor.first);
    }
    return result;
}
...
LoRAConstantTensors group_lora_constant_tensors(const ConstantMap& tensors,
                                                const std::vector<RegexParser>& const_parsers) {
...
NodePtr squeeze_2d(const ov::Output<ov::Node>& input) {
    ...
    OPENVINO_ASSERT(dims.end() == std::find_if(dims.begin() + 2,
                                               dims.end(),
                                               [](const ov::Dimension& d) {
                                                   return d.get_max_length() > 1;
                                               }),
...
struct LoRAParameters {
    ...
    ov::element::Type
        type;  // element type of a tensor that will be applied to the model, negotiated based on multiple LoRA adapters
    bool fine_grained_alpha;  // use 1D tensor of the same rank for alpha instead of a scalar to blend multiple weighted
                              // LoRAs
...
struct LoRAWeightGetterDefault {
    ...
    mutable bool active = false;  // true if operator() was called at least once to filter out the case when this object
                                  // is temporary object that is not used for tensor queries

    LoRAWeightGetterDefault(const std::map<std::string, TENSOR_TYPE>* lora_tensors, const std::string& prefix)
    ...
    std::optional<NODE_TYPE> operator()(const std::string& name) const {
        ...
        // TODO: Investigate what is the root cause for this replacement in the name. Customize mapping or change PT FE
        // to produce correct weight names.
        ...
        auto it = std::find_if(
            lora_tensors->begin(),
            lora_tensors->end(),
            [this, variants](const std::pair<std::string, TENSOR_TYPE>& pair) {
                ...
                // TODO: Should it be an exact match instead of substring taking into account that we should provide
                // custom mapper for names?
                return variants.end() !=
                       std::find_if(variants.begin(), variants.end(), [lora_name](const std::string& name) {
                           return name.find(lora_name) != std::string::npos;
                       });
            });
    ...
    ~LoRAWeightGetterDefault() {
        ...
        std::cerr << "[ WARNING ] There unused LoRA tensors. The result of generation can be not accurate. Check if a "
                     "given adapter file is compatible with the base model.\n";
...
// Maps a node in the base model to LoRA parameters object that describes how the LoRA tensors should be injected for
// that node. Works with multiple LoRAs accumulating their properties into a single LoRAParameter instance. Returns
// std::nullopt, if there is no LoRA adapter for a given node.
    ...
    std::optional<LoRAParameters> operator()(NodePtr node) const {
        ...
        if (dynamic_lora_rank) {
            ...
            auto size = std::accumulate(
                ...
                weight_getter.end(),
                0u,
                [node](unsigned int acc, const LoRAWeightGetter& getter) {
...
void deduce_input_output_dims(NodePtr node, ov::Dimension& input_dim, ov::Dimension& output_dim) {
    if (std::dynamic_pointer_cast<v1::Convolution>(node)) {
        ...
        OPENVINO_THROW("deduce_input_output_dims expects MatMul or Convolution, but got ",
                       node,
...
#endif
    model->add_sinks(
        {std::make_shared<v6::Assign>(read_value, variable)});  // FIXME: Required? -- Yes, create ticket against CPU
    return read_value;
}
}  // namespace
...
struct LoRAWeightStateGetter {
    ...
    std::optional<LoRANode> operator()(NodePtr node) const {
        if (auto params = params_getter(node)) {
            ...
            // FIXME: Potential name conflict if LoRA is applied multiple times by using this infrastructure
            // independently each time (not a recommended approach).
            ...
            // FIXME: No guarantees on ordering of state in InferRequest makes impossible using indices of variables
            // later, forced to use variable_id instead
...
struct LoRAStateGetterForConst {
    ...
    std::optional<NodePtr> operator()(NodePtr node) const {
        ...
        if (auto params = getter(name)) {
            ...
            // No guarantees on ordering of state in InferRequest makes impossible using indices of variables later,
            // forced to use variable_id instead
            ov::op::util::VariableInfo variable_info =
                ov::op::util::VariableInfo{ov::Output<ov::Node>(*params, 0).get_shape(),
    ...
    NodePtr create_if_input() {
        ...
        auto variable_info = ov::op::util::VariableInfo{ov::Shape{1}, ov::element::Type_t::boolean, if_variable_id};
...
public:
    ...
    LoRATransformBase(const LoRAWeightByNodeGetter& lora_weight_getter) {
        register_matcher(
            std::make_shared<ov::pass::pattern::Matcher>(ov::pass::pattern::wrap_type<v0::MatMul, v1::Convolution>(),
                                                         this->get_type_info().name),
            ([lora_weight_getter, this](ov::pass::pattern::Matcher& m) {
                ...
                try {
                    ...
                    DEBUG_PRINT("Exception happens on layer: " << node
                                                               << " with exception message: " << exception.what());
                    throw;
                } catch (...) {
...
public:
    LoRAReplaceConstantTransform(const LoRAConstantByNodeGetter& getter) {
        register_matcher(std::make_shared<ov::pass::pattern::Matcher>(ov::pass::pattern::wrap_type<v0::Constant>(),
                                                                      this->get_type_info().name),
                         ([getter, this](ov::pass::pattern::Matcher& m) {
                             ...
                             try {
                                 ...
                                 DEBUG_PRINT("Exception happens on layer: " << src_node << " with exception message: "
                                                                            << exception.what());
                                 throw;
                             } catch (...) {
...
public:
    LoRAReplaceConstantTransformStatic(const LoRAConstantByNodeGetter& getter) : LoRAReplaceConstantTransform(getter) {}
...
public:
    LoRAReplaceConstantTransformDynamic(const LoRAConstantByNodeGetter& getter, const NodePtr if_input)
...
protected:
    ...
    bool apply(NodePtr node, const NodePtr& lora_weight) override {
        ...
        auto then_param = std::make_shared<ov::op::v0::Parameter>(lora_output->get_element_type(),
                                                                  lora_output->get_output_partial_shape(0));
        ...
        auto else_param =
            std::make_shared<ov::op::v0::Parameter>(node->get_element_type(), node->get_output_partial_shape(0));
...
// TODO [ticket 171466]: LoRA should support two separate parameters: alpha (defined in the config or .safetensors),
// weight (set by the user) Currently, the implementation merges them into a single effective scaling factor: alpha =
// (alpha_from_file / rank) * weight Check whether `weight` should be supported for text generation
...
NodePtr decompression_convert(NodePtr node) {
    ...
    OPENVINO_ASSERT(std::dynamic_pointer_cast<v0::Constant>(node),
                    "LoRA adapter application: not supported decompression pattern at the weight input (presumably "
                    "low-bit compression). Use f32/f16/bf16 weights only if MODE_FUSE is used.");
    return convert;
}
...
class InferRequestSignatureCache {
    // Infer request with additional input-output pairs that are bypassed from input to output to eliminate Parameter ->
    // Result pairs from the OV model
    struct RequestWithBypass {
        ov::InferRequest request;
        std::vector<std::pair<size_t, size_t>> bypass;  // a set of index pairs [j, k], where j is an index of input
                                                        // tensor to be forwarded to k-th output tensor
        std::vector<size_t> inputs;   // inputs[i] gives an index in the original input tensor vector to be set to i-th
                                      // input of the request
        std::vector<size_t> outputs;  // outputs[i] gives an index in the original output tensor vector to be set as an
                                      // i-th output of the request
    };
...
public:
    ...
    void insert(const Signature& signature, ov::ResultVector& results, ov::ParameterVector& parameters) {
        // Detect Parameter -> Result patterns and do not allow them to be included into compiled model to avoid
        // unnecessary overheads, and handle them via a bypass. Assume that each parameter from parameters vector do not
        // have other consumers outside model formed by parameters -> ... -> results. That allows filter out those
        // parameters that are consumed by Result operations only detecting them by count of consumers instead of
        ...
        for (size_t result_index = 0; result_index < results.size(); ++result_index) {
            ...
            if (parameter) {
                ...
                size_t parameter_index =
                    std::distance(parameters.begin(), std::find(parameters.begin(), parameters.end(), parameter));
    ...
    void evaluate(const Signature& signature, const ov::TensorVector& inputs, ov::TensorVector& outputs) {
        ...
        for (size_t i = 0; i < rwb.outputs.size(); ++i) {
            ...
            if (target_shape != output_tensor.get_shape() && target_shape.is_static()) {
                // do it for static case only, because if target shape is dynamic, the plugin is allowed to set shape on
                // its own
...
class LoRAFuseTransform : public LoRATransformBase {
    ...
    void signature_push_back(InferRequestSignatureCache::Signature& signature, ov::Output<ov::Node> input) const {
        ...
        signature += "(el: " + input.get_element_type().get_type_name() +
                     ", shape: " + input.get_partial_shape().to_string() + ")";
    }
...
public:
    ...
    bool apply(NodePtr node, const LoRANode& lora_weight) override {
        ...
        ConstantVector adapter = {std::dynamic_pointer_cast<v0::Constant>(lora_weight.alpha),
        ...
        if (!fusers.exist(signature)) {
            ...
            auto target_parameter = std::make_shared<v0::Parameter>(weights_constant.get_element_type(),
                                                                    weights_constant.get_partial_shape());
            parameters.push_back(target_parameter);  // original weights input is one of the parameters
            ov::Output<ov::Node> target =
                weights_convert ? weights_convert->clone_with_new_inputs({target_parameter}) : target_parameter;
            for (auto multiplier : adapter) {
                parameters.push_back(std::make_shared<v0::Parameter>(multiplier->get_output_element_type(0),
                                                                     multiplier->get_output_partial_shape(0)));
            }
        ...
        // Newly created constants in the next line are not mapped unlike original weights, so it will inflate required
        // memory eventually allocating up to 2x of the base model size. 2X is due to usually applied compression in the
        // base model that is not retained in the current version of this code. But even if the compression is used,
        // then still a copy of all weights that affected by the LoRA adapters are allocated in memory.
        // FIXME: Provide a way for postponed weight repacking that will be triggered by the plugin in compile_model
        // call for the base model. Constant sub-expression can be a solution, but it requires improvements inside
        // plugins, because currently it works extremely slow.
        auto replacement_const =
            std::make_shared<v0::Constant>(weights_input.get_element_type(), weights_input.get_shape());
        ...
        inputs.push_back(
            std::dynamic_pointer_cast<v0::Constant>(weights_constant.get_node_shared_ptr())->get_tensor_view());
...
// Transformation that modifies the base model inserting new nodes that do LoRA matrix multiplications alongside with
// the original MatMul/Convolution.
...
public:
    SafetensorsAdapterImpl(const std::filesystem::path& path) {
        ...
        OPENVINO_ASSERT(path.extension().string() == ".safetensors",
...
std::string convert_gguf_name_to_hf(const std::string& name) {
    ...
    while ((pos = new_name.find("blk.", pos)) != std::string::npos) {
        ...
        if (num_end != std::string::npos) {
            ...
            bool is_number = !layer_num.empty() && std::all_of(layer_num.begin(), layer_num.end(), [](unsigned char c) {
...
public:
    GGUFAdapterImpl(const std::filesystem::path& path) {
        ...
        for (auto& [name, tensor] : raw_tensors) {
            ...
            auto constant =
                std::make_shared<v0::Constant>(tensor.get_element_type(), tensor.get_shape(), tensor.data());
...
/// Two objects instantiated from the same Derivation type are equal when both origins and derivations are equal (while
/// comparing with operator==). The derivation is postponed to the first call of get_tensors(), giving a way to compare
/// Adapters without applying the derivation. It is supposed that Derivation works always in the same way and don't have
/// a side effect.
template <typename Derivation>
class DerivedAdapterImpl : public AdapterImpl {
public:
    DerivedAdapterImpl(const std::shared_ptr<AdapterImpl>& origin, const Derivation& derivation)
        : origin(origin),
...
Adapter::Adapter(const ov::Tensor& safetensor) : m_pimpl(std::make_shared<SafetensorsAdapterImpl>(safetensor)) {}
...
struct AdapterControllerImpl {
          ...
          lora_state_evaluators("CPU")  // FIXME: Try to run on the same device that is used for model inference
    {
        ...
        for (auto const& adapter : current_config.get_adapters()) {
            ...
            params_getter.weight_getter.push_back(
            ...
            if (params_getter.type != ov::element::f32) {
                for (auto const& tensor : adapter_impl->get_tensors()) {
                    ...
                    if (params_getter.type == ov::element::dynamic) {
                        ...
                        // TODO: Provide a more smart negotiation between multiple LoRAs: check ranges, try to pack to
                        // f16
                        //       Make decision on precision per node in LoRAWeightStateGetter instead of setting this
                        //       global precision
        ...
        auto weight_as_constant = [&, this](NodePtr node) -> std::optional<LoRANode> {
            ...
            LoRAParts<ov::Tensor> lora_placeholder{ov::Tensor(ov::element::f32, Shape{0}),
                                                   ov::Tensor(params_getter.type, ov::Shape{0}),
                                                   ov::Tensor(params_getter.type, ov::Shape{0})};
            auto name = node->get_friendly_name();
            auto lora_weight = prepare_lora_tensors(name,
                                                    params_getter.weight_getter,
                                                    lora_placeholder,
                                                    /*set_empty_tensors=*/false,
                                                    /*alpha_only=*/false);
        ...
        if (mode == AdapterConfig::MODE_DYNAMIC || mode == AdapterConfig::MODE_STATIC_RANK ||
            mode == AdapterConfig::MODE_AUTO) {
            // State mode
    ...
    ConfigChanged compare_configs(const AdapterConfig& config1, const AdapterConfig& config2) {
        ...
        // TODO: Use `set` from this commented block when the config change tracking is implemented at adapter
        // granularity and will track order of adapters correctly std::set<Adapter>
    ...
    void apply(ov::InferRequest& infer_request, std::optional<AdapterConfig> config) {
        // FIXME: If a part of LoRA state tensors are not set here, then need to carefully reset state in LLMPipeline
        // where global reset is called after the generation
        ConfigChanged diff;
        if (config) {
            diff = compare_configs(current_config, *config);
            OPENVINO_ASSERT(
                !diff.mode || config->get_mode() ==
                                  AdapterConfig::MODE_AUTO,  // MODE_AUTO in this call means that mode is not changed
                "AdapterConfig::mode cannot be changed and should be configured once for a model at the "
                "initialization");
            OPENVINO_ASSERT(config->get_mode() == AdapterConfig::MODE_AUTO ||
                                config->get_mode() == AdapterConfig::MODE_DYNAMIC ||
                                config->get_mode() == AdapterConfig::MODE_STATIC_RANK || (!diff.alpha && !diff.adapter),
    ...
    void set_new_adapter_tensors(ov::InferRequest& infer_request, bool alpha_only = false) {
        ...
        for (const auto& adapter : adapters) {
            ...
            if (!adapter_impl->get_constant_tensors().empty()) {
                OPENVINO_ASSERT(!const_getter, "OpenVINO.GenAI does not support several LoRA adapters with constants!");
                const_getter =
                    LoRAWeightGetterDefault<NodePtr, NodePtr>(&adapter_impl->get_constant_tensors(),
        ...
        // TODO: Forced to use variable_id instead of index to address the state tensors, require the same order for
        // state as for variables from plugins

        // Convert LoRAVarIDs to LoRAIndices to speedup search for state with a given name
        // TODO: If state order is stable, then the mapping should be done once for a given infer request, TODO: cache
        // it based on the infer request
    ...
    std::vector<LoRAWeight> collect_applicable_tensors(const std::string& lora_name,
        ...
        for (size_t i = 0; i < adapters.size(); ++i) {
            if (auto lora_tensors = weight_getters[i](lora_name)) {
                // TODO: Is it practical to use alpha from the adapter file itself. In the current code it is ignored
                // and only alpha from config is used.
                ...
                result.push_back(LoRAWeight(std::dynamic_pointer_cast<v0::Constant>(lora_tensors->alpha),
    ...
    Signature get_lora_signature(const std::vector<LoRAWeight>& inputs, const LoRAParts<ov::Tensor>& outputs) {
        ...
        for (const auto& input : inputs) {
            signature +=
                std::string("(") + get_tensor_signature(input.alpha) +
                get_tensor_signature(input.A) +  // TODO: Adjust shape to have a dynamic low-rank LoRA dimension in case
                                                 // of fully static shape doesn't have significant speedup
                get_tensor_signature(input.B) +  // TODO: Adjust shape to have a dynamic low-rank LoRA dimension in case
                                                 // of fully static shape doesn't have significant speedup
                ")";
        }
        signature += std::string("(") +
                     // Shape is set to be dynamic because it doesn't matter for signature as it is completely
                     // determined by the corresponding model The corresponding model hasn't been created at the moment
                     // when this function is got called, so to avoid duplicated shape propagation logic for outputs we
                     // just use the target rank of output tensors that is enough to distinguish output signatures.
    ...
    void build_concat_model(
        ...
        std::function<NodePtr(const LoRAWeight&, NodePtr)> parameter_postprocessing = [](const LoRAWeight&,
                                                                                         NodePtr node) {
            return node;
        }) {
    ...
    LoRAParts<ov::Tensor> concat_adapters(const std::vector<LoRAWeight>& inputs,
                                          LoRAParts<ov::Tensor>& outputs,
                                          bool alpha_only) {
            ...
            build_concat_model(
                ...
                [](const LoRAWeight& lora_weight) {
                    return std::make_shared<v0::Parameter>(
                        ...
                        lora_weight.alpha->get_output_partial_shape(
                            0));  // TODO: Consider using dynamic LoRA rank dimension instead of static dimension
                },
                [](const LoRAWeight& lora_weight, NodePtr parameter) {
                    ...
                    auto lora_rank_constant = v0::Constant::create(ov::element::u32,
                                                                   Shape{2},
                                                                   std::vector<decltype(lora_rank)>{1, lora_rank});
                ...
                build_concat_model(
                    ...
                    [](const LoRAWeight& lora_weight) {
                        return std::make_shared<v0::Parameter>(
                            ...
                            lora_weight.A->get_output_partial_shape(
                                0));  // TODO: Consider using dynamic LoRA rank dimension instead of static dimension
                    });
                ...
                build_concat_model(
                    ...
                    [](const LoRAWeight& lora_weight) {
                        return std::make_shared<v0::Parameter>(
                            ...
                            lora_weight.B->get_output_partial_shape(
                                0));  // TODO: Consider using dynamic LoRA rank dimension instead of static dimension
                    });
            }
    ...
    void set_lora_tensors(std::vector<VariableState>& state,
        ...
        auto new_tensors =
            prepare_lora_tensors(name, weight_getters, lora_state_tensors, /*set_empty_adapters=*/true, alpha_only);
    ...
    LoRAParts<ov::Tensor> prepare_lora_tensors(const std::string& name,
                                               const std::vector<LoRAWeightGetter>& weight_getters,
                                               LoRAParts<ov::Tensor>& output,
                                               bool set_empty_adapters,
                                               bool alpha_only) {
        auto lora_tensors =
            collect_applicable_tensors(name,
                                       weight_getters);  // request A and B regardless of alpha_only, because it is a
                                                         // way to get lora_rank later when alpha is broadcasted
...
AdapterController::AdapterController(std::shared_ptr<ov::Model> model,
    ...
    if (config.get_mode() == AdapterConfig::MODE_AUTO) {
        ...
        if (device.find("GPU") !=
            std::string::npos) {  // to handle GPU device variants which doesn't matter for adapter mode
            device = "GPU";
        }
        ...
        if (default_mode != default_modes.end()) {
            ...
            std::cout << "[ WARNING ] " << device_msg << " to deduce default device-dependent LoRA application mode.\n"
                      << "This warning appears because no specific LoRA mode was set in AdapterConfig or MODE_AUTO was "
                         "used explicitly.\n"
...
// Call it every time when adapter config is changed; if adapter was configured as a static one, this call is not
// required
...
AdapterConfig& AdapterConfig::add(const Adapter& adapter, float alpha) {
    ...
    OPENVINO_ASSERT(adapters.end() == std::find(adapters.begin(), adapters.end(), adapter),
                    "Adapter object passed to AdapterConfig::add was already registered");
...
AdapterConfig& AdapterConfig::set_alpha(const Adapter& adapter, float alpha) {
    ...
    OPENVINO_ASSERT(adapters.end() != it,
                    "Unknown adapter object passed to AdapterConfig::set_alpha, register adapter object first with "
                    "AdapterConfig::add");
...
float AdapterConfig::get_alpha(const Adapter& adapter) const {
    ...
    auto it = std::find(adapters.begin(), adapters.end(), adapter);
    OPENVINO_ASSERT(adapters.end() != it,
                    "Unknown adapter object passed to AdapterConfig::get_alpha, alpha can be retrieved for previously "
...
AdapterConfig& AdapterConfig::remove(const Adapter& adapter) {
    ...
    OPENVINO_ASSERT(
        adapters.end() != it,
        "Unknown adapter object passed to AdapterConfig::remove, you can remove previously registered adapters only");
    auto index = it - adapters.begin();
