...
utils::SharedOptional<const AnyMap> extract_adapters_from_properties(
    const AnyMap& properties,
    std::optional<AdapterConfig>* adapter_config = nullptr);

// Search for `adapters` property in `properties` map. If it is found, set `adapter_config` with found value and return
// true. If `adapters` property is not found, do nothing and return false.
...
// Update `properties` map with new `adapters` property value. If `properties` map contains `adapters` property,
// call `action` with the value of `adapters` property and update `adapters` property with the result of `action`.
utils::SharedOptional<const AnyMap> update_adapters_in_properties(const AnyMap& properties,
                                                                  const AdapterConfigAction& action);
