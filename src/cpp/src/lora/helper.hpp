...
utils::SharedOptional<const AnyMap>
extract_adapters_from_properties(const AnyMap& properties, std::optional<AdapterConfig>* adapter_config = nullptr);
...
// call `action` with the value of `adapters` property and update `adapters` property with the result of `action`.
utils::SharedOptional<const AnyMap>
update_adapters_in_properties(const AnyMap& properties, const AdapterConfigAction& action);
