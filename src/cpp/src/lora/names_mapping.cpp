...
NameMap maybe_map_sgm_blocks_to_diffusers(
    ...
    if (!state_dict.empty()) {
        ...
        throw std::runtime_error(
            "At this point all state dict entries have to be converted. Remaining keys: " + remaining_keys
        );
    }
...
public:
    SplitRule(
        const std::string& _src_pattern,
        const std::vector<FormattedString>& _dst_patterns,
    ...
    std::optional<LoRATensors> operator()(const LoRATensors::value_type& weight) const {
        if (auto match = src_pattern(weight.first)) {
            ...
            for (size_t i = 0; i < dst_patterns.size(); ++i) {
                ov::Coordinate begin = {cur_index, 0}, end = total_size;
                cur_index += cur_dims[i];
                end[0] = cur_index;
                auto B =
                    std::make_shared<ov::op::v0::Constant>(ov::Tensor(weight.second.B->get_tensor_view(), begin, end));

                // ov::Tensor ROI constructor doesn't keep the origin reference so we need to keep a pointer to the
                // original constant to avoid its early disposal
                B->get_rt_info()["__lora_parent_constant_holder"] = weight.second.B;

                result.emplace(dst_patterns[i].format(*match), LoRAWeight(nullptr, weight.second.A, B));
            }
...
LoRATensors flux_kohya_lora_preprocessing(const LoRATensors& tensors) {
    std::vector<ReplaceRule> replace_rules = {
        {"lora_unet_double_blocks_(\\d+)_img_attn_proj", {"transformer.transformer_blocks.", ".attn.to_out.0"}},
        {"lora_unet_double_blocks_(\\d+)_img_mlp_0", {"transformer.transformer_blocks.", ".ff.net.0.proj"}},
        {"lora_unet_double_blocks_(\\d+)_img_mlp_2", {"transformer.transformer_blocks.", ".ff.net.2"}},
        {"lora_unet_double_blocks_(\\d+)_img_mod_lin", {"transformer.transformer_blocks.", ".norm1.linear"}},
        {"lora_unet_double_blocks_(\\d+)_txt_attn_proj", {"transformer.transformer_blocks.", ".attn.to_add_out"}},
        {"lora_unet_double_blocks_(\\d+)_txt_mlp_0", {"transformer.transformer_blocks.", ".ff_context.net.0.proj"}},
        {"lora_unet_double_blocks_(\\d+)_txt_mlp_2", {"transformer.transformer_blocks.", ".ff_context.net.2"}},
        {"lora_unet_double_blocks_(\\d+)_txt_mod_lin", {"transformer.transformer_blocks.", ".norm1_context.linear"}},
        {"lora_unet_single_blocks_(\\d+)_linear2", {"transformer.single_transformer_blocks.", ".proj_out"}},
        {"lora_unet_single_blocks_(\\d+)_modulation_lin", {"transformer.single_transformer_blocks.", ".norm.linear"}},
...
LoRATensors flux_xlabs_lora_preprocessing(const LoRATensors& tensors) {
    std::vector<ReplaceRule> replace_rules = {
        ...
        {"(diffusion_model\\.)?single_blocks\\.(\\d+).*proj_lora",
         {"transformer.single_transformer_blocks.", ".attn.to_out.0"},
         2},
    };
