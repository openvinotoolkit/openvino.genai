    ...
    if (sequence_group->get_sequence_group_type() == SequenceGroupType::TOKENS) {
        ...
        if (block_start_idx < prompt_ids.size()) {
            content.insert(content.end(),
                           prompt_ids.begin() + block_start_idx,
                           prompt_ids.begin() + std::min(prompt_ids.size(), content_length));
        }
        if (content_length > prompt_ids.size()) {
            size_t start = block_start_idx < prompt_ids.size() ? 0 : block_start_idx - prompt_ids.size();
            // Use parentheses around (content_length - prompt_ids.size()) to suppress MSVC debug assert: "cannot seek
            // vector iterator after end"
            content.insert(content.end(),
                           m_generated_ids.begin() + start,
                           m_generated_ids.begin() + (content_length - prompt_ids.size()));
        }
...
std::vector<int64_t> Sequence::_reduce_embedding(const std::vector<float>& embedding) {
    size_t res_size = std::min((size_t)ceil(float(embedding.size()) / m_embeddings_hash_calculation_stride),
                               m_embeddings_hash_max_num_values);
