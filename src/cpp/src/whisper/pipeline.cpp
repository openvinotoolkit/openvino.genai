...
public:
    ...
    std::vector<WhisperDecodedResults> generate(const std::vector<RawSpeechInput>& raw_speech_inputs,
        ...
        for (auto& generate_result : generate_results) {
            ...
            // Convert internal segments to public chunks (present only when the user requested public
            // return_timestamps), mirroring the single-audio path. Each audio owns its own chunk list.
