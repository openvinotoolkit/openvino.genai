...
private:
  ...
  static bool should_skip_punctuation_output(const std::vector<Token> &tokens, std::size_t index) {
    ...
    if (raw_prev.empty() || raw_prev.size() > 4) {
      return false;
    }
    ...
    for (std::size_t i = 0; i < raw_prev.size(); ++i) {
      const auto uc = static_cast<unsigned char>(raw_prev[i]);
      if (!std::isalpha(uc)) {
        all_alpha = false;
        break;
      }
  ...
  static bool is_case_stress_exempt_function_word(const std::string &key) {
    static const std::unordered_set<std::string> exempt_words = {
      "a", "an", "the", "that",
      "to", "in", "at", "of", "on",
    };
    return exempt_words.find(key) != exempt_words.end();
  }
  ...
  std::optional<std::string> resolve_word_token(const std::vector<Token> &tokens, std::size_t index) const {
    ...
    if (tokens[index].linked_stress.has_value()) {
      ...
      const auto lower_word = ascii_lower(tokens[index].text);
      if (!is_case_stress_exempt_function_word(lower_word)) {
  ...
  static bool starts_with_vowel_letter(const std::string &text) {
    ...
    static const std::array<std::string, 5> silent_h_prefixes = {
        "honest", "honor", "hour", "heir", "herb"};
    static const std::array<std::string, 8> consonant_onset_prefixes = {
        "one", "once", "uni", "use", "user", "euro", "ewe", "ubiq"};
    for (const auto &prefix : silent_h_prefixes) {
      if (lower.rfind(prefix, 0) == 0) {
        return true;
      }
    }
    for (const auto &prefix : consonant_onset_prefixes) {
      if (lower.rfind(prefix, 0) == 0) {
        return false;
      }
    }

    const bool all_upper_letters = !text.empty() && std::all_of(text.begin(), text.end(), [](char c) {
      return std::isupper(static_cast<unsigned char>(c));
    });
    if (all_upper_letters) {
      static const std::unordered_set<char> vowel_onset_letter_names = {
          'A', 'E', 'F', 'H', 'I', 'L', 'M', 'N', 'O', 'R', 'S', 'X',
      };
      return vowel_onset_letter_names.find(text.front()) != vowel_onset_letter_names.end();
    }
  ...
  static std::optional<std::string> guess_pos_hint(const std::vector<Token> &tokens, std::size_t index) {
    ...
    std::optional<TokenKind> next_non_ws_kind;
    for (std::size_t i = index + 1; i < tokens.size(); ++i) {
      if (tokens[i].kind == TokenKind::Whitespace) {
        continue;
      }
      next_non_ws_kind = tokens[i].kind;
      break;
    }

    if (ends_with(key, "s")) {
      if (next_non_ws_kind.has_value() && *next_non_ws_kind == TokenKind::Punctuation) {
        return std::string{"NOUN"};
      }
      if (next_word) {
        const auto next = ascii_lower(tokens[*next_word].text);
        if (next == "a" || next == "an" || next == "the" || next == "this" || next == "that" ||
            next == "these" || next == "those" || next == "my" || next == "your" || next == "his" ||
            next == "her" || next == "our" || next == "their" || next == "some" || next == "any" ||
            next == "several" || next == "many" || next == "few" || next == "more" || next == "less") {
    ...
    if (key == "there") {
      ...
      if (!existential_there && prev_word.has_value()) {
  ...
  std::optional<std::string> lookup_direct_pronunciation(
    ...
    static const std::unordered_map<std::string, std::string> hardcoded_us = {
      {"inspirited", "ɪnspˈɪɹəTᵻd"},
      {"entreated", "ɪntɹˈiTᵻd"},
      {"temperament", "tˈɛmpɹəmənt"},
      {"lived", "lˈɪvd"},
      {"participated", "pɑɹtˈɪsəpˌATᵻd"},
      {"whale-fishers", "wˌAlfˈɪʃəɹz"},
    };
    ...
    if (token_text.size() > 2 && token_text.back() == 's') {
      const auto stem = token_text.substr(0, token_text.size() - 1);
      if (is_all_upper_alpha(stem)) {
        if (const auto stem_pronunciation = lookup_direct_pronunciation(stem, pos_hint)) {
          return apply_plural_s(normalize_pronunciation(*stem_pronunciation));
        }
      }
    }
    ...
    while (i < text.size()) {
      ...
      if (std::isalnum(c)) {
        ...
        while (j < text.size()) {
          ...
          if (std::isalnum(cj) || text[j] == '&' || text[j] == '-') {
            ++j;
            continue;
          }
          break;
        ...
        const auto candidate = text.substr(i, j - i);
        const bool has_lower = std::any_of(candidate.begin(), candidate.end(), [](char ch) {
          return std::islower(static_cast<unsigned char>(ch));
        });

        auto emit_component = [&](const std::string& component) {
          if (component.empty()) {
            return false;
          }

          const bool has_digit = std::any_of(component.begin(), component.end(), [](char ch) {
            return std::isdigit(static_cast<unsigned char>(ch));
          });
          const bool has_alpha = std::any_of(component.begin(), component.end(), [](char ch) {
            return std::isalpha(static_cast<unsigned char>(ch));
          });

          if (has_digit && has_alpha) {
            std::size_t digit_prefix = 0;
            while (digit_prefix < component.size() && std::isdigit(static_cast<unsigned char>(component[digit_prefix]))) {
              ++digit_prefix;
            }
            if (digit_prefix > 0 && digit_prefix < component.size()) {
              const auto suffix = component.substr(digit_prefix);
              const bool suffix_upper = std::all_of(suffix.begin(), suffix.end(), [](char ch) {
                return std::isupper(static_cast<unsigned char>(ch));
              });
              if (suffix_upper) {
                out.push_back({TokenKind::Word, component.substr(0, digit_prefix), ""});
                out.push_back({TokenKind::Whitespace, " ", ""});
                out.push_back({TokenKind::Word, suffix, ""});
                return true;
              }
            }

            std::size_t upper_prefix = 0;
            while (upper_prefix < component.size() && std::isupper(static_cast<unsigned char>(component[upper_prefix]))) {
              ++upper_prefix;
            }
            if (upper_prefix > 0 && upper_prefix < component.size()) {
              const auto suffix = component.substr(upper_prefix);
              const bool suffix_digits = std::all_of(suffix.begin(), suffix.end(), [](char ch) {
                return std::isdigit(static_cast<unsigned char>(ch));
              });
              if (suffix_digits) {
                out.push_back({TokenKind::Word, component.substr(0, upper_prefix), ""});
                out.push_back({TokenKind::Whitespace, " ", ""});
                out.push_back({TokenKind::Word, suffix, ""});
                return true;
              }
          ...
          out.push_back({TokenKind::Word, component, ""});
          return true;
        };

        const bool ampersand_code_like = !has_lower && candidate.find('&') != std::string::npos;
        if (ampersand_code_like) {
          ...
          for (char ch : candidate) {
            ...
            if (ch == '&' && ampersand_code_like) {
              valid = emit_component(component);
              component.clear();
              if (!valid) {
                break;
              }
              out.push_back({TokenKind::Whitespace, " ", ""});
              out.push_back({TokenKind::Word, "&", ""});
              out.push_back({TokenKind::Whitespace, " ", ""});
              continue;
            }
  ...
  std::optional<std::string> lookup_contextual_pronunciation(const Token &token, const TokenContext &ctx) const {
    ...
    if (key == "through") {
      return has_future_word ? "θɹu" : "θɹˈu";
    }
  ...
  std::optional<std::string> lookup_numeric_word_pronunciation(const std::string &word) const {
    static const std::unordered_map<std::string, std::string> numeric_us = {
      {"zero", "zˈiəɹO"},
      {"eleven", "ɪlˈɛvən"},
    };

    static const std::unordered_map<std::string, std::string> numeric_gb = {
      {"zero", "zˈiəɹO"},
      {"eleven", "ɪlˈɛvən"},
    };

    const auto key = ascii_lower(word);
    const auto &numeric = (variant_ == "en-gb") ? numeric_gb : numeric_us;
    const auto it = numeric.find(key);
    if (it != numeric.end()) {
      return it->second;
    }

    return lookup_word_pronunciation(word);
  }
  ...
  static void apply_compound_stress_balance(const std::vector<std::string>& parts,
    ...
    if (parts.size() == 2 && pronunciations.size() == 2 &&
        has_primary_stress(pronunciations[0]) && has_primary_stress(pronunciations[1])) {
      const bool first_has_internal_upper = std::any_of(parts[0].begin() + (parts[0].empty() ? 0 : 1), parts[0].end(), [](char c) {
        return std::isupper(static_cast<unsigned char>(c));
      });
      auto count_marker = [](const std::string& text, const std::string& marker) {
        std::size_t count = 0;
      ...
      const std::size_t first_stress_count = count_marker(pronunciations[0], "ˈ") + count_marker(pronunciations[0], "ˌ");
      if (first_has_internal_upper || first_stress_count > 1) {
        return;
      }
    }
  ...
  std::optional<std::string> pronounce_integer_token(const std::string &digits) const {
    ...
    return words_to_phonemes(words, true);
  }
  ...
  std::optional<std::string> pronounce_code_segment(const std::string &segment) const {
    ...
    if (is_all_digits(segment) && segment.size() == 3 && segment[0] != '0') {
      const auto head = pronounce_integer_token(segment.substr(0, 1));
      const auto tail = pronounce_integer_token(segment.substr(1));
      if (head && tail) {
        return *head + " " + *tail;
      }
    }
  ...
  std::string words_to_phonemes(const std::vector<std::string> &words, bool numeric_context = false) const {
    std::string out;
    bool needs_space = false;
    for (const auto &word : words) {
      ...
      const auto pronunciation = numeric_context ? lookup_numeric_word_pronunciation(word)
                                                 : lookup_word_pronunciation(word);
      if (!pronunciation) {
  ...
  std::optional<std::string> expand_symbol_or_number(const std::vector<Token> &tokens, std::size_t index) const {
    ...
    if (symbol_it != symbol_words.end()) {
      if (token == "%" && index > 0 && tokens[index - 1].kind == TokenKind::Word &&
          is_numeric_token(tokens[index - 1].text)) {
        return std::string{};
      }
    ...
    const bool attach_adjacent_percent = (index + 1 < tokens.size() && tokens[index + 1].kind == TokenKind::Word &&
                                          tokens[index + 1].text == "%");

    const auto append_adjacent_percent = [&](std::string phonemes) {
      if (!attach_adjacent_percent) {
        return phonemes;
      }
      const auto percent_pron = lookup_word_pronunciation("percent");
      if (!percent_pron || percent_pron->empty()) {
        return phonemes;
      }
      if (!phonemes.empty()) {
        phonemes.push_back(' ');
      }
      phonemes += *percent_pron;
      return phonemes;
    };
    ...
    if (parsed.fractional.empty()) {
      ...
      if (has_leading_zero && clock_like) {
        ...
        auto phonemes = words_to_phonemes(words, true);
        if (morph_suffix) {
        ...
        return append_adjacent_percent(phonemes);
      }
  ...
  std::string lookup_pronunciation(const std::vector<Token> &tokens, std::size_t index) const {
    ...
    if (const auto hyphen_code = lookup_hyphen_code_pronunciation(token.text)) {
      return *hyphen_code;
    }
