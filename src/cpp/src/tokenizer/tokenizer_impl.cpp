...
template <typename T>
void Tokenizer::TokenizerImpl::set_state_value(
...
void Tokenizer::TokenizerImpl::setup_tokenizer(
    ...
    OPENVINO_ASSERT(
        !two_input_requested || is_paired_input || !ov_tokenizer,
...
TokenizedInputs Tokenizer::TokenizerImpl::encode(
...
TokenizedInputs Tokenizer::TokenizerImpl::encode(
    ...
    OPENVINO_ASSERT(
        prompts_1.size() == prompts_2.size() || prompts_1.size() == 1 || prompts_2.size() == 1,
...
TokenizedInputs
Tokenizer::TokenizerImpl::encode(const std::vector<std::string>& prompts, const ov::AnyMap& tokenization_params) {
    OPENVINO_ASSERT(
        m_ireq_queue_tokenizer,
        "Either openvino_tokenizer.xml was not provided or it was not loaded correctly. "
        "Tokenizer::encode is not available"
...
std::string
Tokenizer::TokenizerImpl::decode(const std::vector<int64_t>& tokens, const ov::AnyMap& detokenization_params) {
    OPENVINO_ASSERT(
        m_ireq_queue_detokenizer,
...
Tokenizer::TokenizerImpl::decode(const ov::Tensor& tokens, const ov::AnyMap& detokenization_params) {
    OPENVINO_ASSERT(
        m_ireq_queue_detokenizer,
        "Detokenizer model has not been provided. Tokenizer::decode is not available"
    );
...
std::vector<std::string> Tokenizer::TokenizerImpl::decode(
    const std::vector<std::vector<int64_t>>& lines,
...
std::string Tokenizer::TokenizerImpl::apply_chat_template(
    ...
    try {
        ...
        OPENVINO_THROW(
            "Minja failed to apply chat template. Possible solutions are\n"
