...
bool ov::genai::MakeAddSpecialTokensSatateful::run_on_model(const std::shared_ptr<ov::Model>& model) {
    ...
    for (size_t i = 0; i < num_segments; i++) {
        ...
        if (!ov::as_type_ptr<v1::Add>(end_input) && !ov::as_type_ptr<v1::Subtract>(begin_input) &&
            strcmp(end_input->get_type_name(), "Truncate") != 0) {
    ...
    auto variable = std::make_shared<op::util::Variable>(
        op::util::VariableInfo{Shape{}, element::boolean, ADD_SPECIAL_TOKENS_VAR_ID});
    ...
    for (size_t i = 0; i < add_spec_inputs.size(); i++) {
        auto select_node =
            std::make_shared<v1::Select>(read_value, add_spec_inputs[i].get_source_output(), zero_constant);
...
bool ov::genai::MakeVocabDecoderSatateful::run_on_model(const std::shared_ptr<ov::Model>& model) {
    ...
    std::shared_ptr<v0::Constant> skip_tokens_const =
        std::dynamic_pointer_cast<v0::Constant>(vocab_decoder_node->get_input_node_shared_ptr(4));
    std::shared_ptr<v8::Slice> skip_tokens_slice =
        std::dynamic_pointer_cast<v8::Slice>(vocab_decoder_node->get_input_node_shared_ptr(4));
    ...
    auto int_max_const =
        std::make_shared<v0::Constant>(ov::element::i32, ov::Shape{1}, std::vector{std::numeric_limits<int>::max()});
    ...
    if (skip_tokens_slice) {
        ...
        std::shared_ptr<v8::Slice> slice_node =
            std::make_shared<v8::Slice>(skip_tokens_const, start_const, stop, one_const);
...
bool ov::genai::MakePaddingSatateful::run_on_model(const std::shared_ptr<ov::Model>& model) {
    ...
    if (number_of_main_tokens_inputs != 1 && number_of_main_tokens_inputs != 2) {
        return false;
    }
    ...
    if (add_or_sub_node) {
        ...
        const_node =
            ov::as_type_ptr<v0::Constant>(trunc_node->get_input_node_shared_ptr(number_of_main_tokens_inputs * 3));
    } else {
        return false;
    }

    op::util::VariableInfo var_info{const_node->get_output_shape(0),
                                    const_node->get_output_element_type(0),
                                    ov::genai::MAX_LENGTH_VAR_ID};
    ...
    auto num_added_tokens_const = std::make_shared<v0::Constant>(const_node->get_output_element_type(0),
                                                                 const_node->get_output_shape(0),
                                                                 std::vector{num_added_tokens});
    ...
    auto default_max_length_const = std::make_shared<v0::Constant>(const_node->get_output_element_type(0),
                                                                   const_node->get_output_shape(0),
                                                                   std::vector{default_max_length});
    ...
    // TODO: int32_max 2147483647 becomes -2147483648 when accessed from Truncate be inputs[inputs.size() -
    // 3].data<const int32_t>()[0] int32_max - 1 (-2, -3, etc.) also strangely becomes still -2147483648. Only starting
    // from int32_max - 64 it is casted to adequate positive value.
    auto int32_max_constant =
    ...
    auto pad_to_max_length_var = std::make_shared<op::util::Variable>(
        op::util::VariableInfo{ov::Shape{1}, ov::element::boolean, ov::genai::PAD_TO_MAX_LENGTH_VAR_ID});
    ...
    auto pad_right_var = std::make_shared<op::util::Variable>(
        op::util::VariableInfo{ov::Shape{}, ov::element::boolean, ov::genai::PAD_RIGHT_VAR_ID});
    auto pad_right_const =
        std::make_shared<v0::Constant>(ov::element::boolean, ov::Shape{}, std::vector{default_pad_right});
    auto pad_right_rv = std::make_shared<v6::ReadValue>(pad_right_const, pad_right_var);

    // This cycle cannot be united with the cycle above since first we need to ensure that all RaggedToDense nodes have
    // the same padding side and only after that start to modify. Therefore we need to iterate over RaggedToDense nodes
    // twice. In 99% of cases there is only one RaggedToDense node and in the rest of cases it would be two
    // RaggedToDense nodes with the same padding side if they are created by the openvino_tokenizers.
    for (auto ragged_to_dense_node : ragged_to_dense_nodes) {
        if (!ragged_to_dense_node) {
            return true;  // true since at this point we already have modified the graph.
        }
