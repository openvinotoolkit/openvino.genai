...
TokenizedInputs Tokenizer::encode(
...
TokenizedInputs
Tokenizer::encode(const std::initializer_list<std::string>& text, const ov::AnyMap& tokenization_params) {
    check_arguments(
        tokenization_params,
        {ov::genai::add_special_tokens.name(),
         ov::genai::max_length.name(),
...
Tokenizer::decode(const std::vector<std::vector<int64_t>>& lines, const ov::AnyMap& detokenization_params) {
    check_arguments(detokenization_params, {ov::genai::skip_special_tokens.name()});
