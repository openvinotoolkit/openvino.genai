...
std::shared_ptr<v1::Transpose> split_heads(
    const Output<Node>& x,
    int num_h,
    int head_dim,
    float rms_norm_eps,
...
ov::Output<ov::Node> make_int4_weights(
...
) {  // Assuming GGML_QUANTIZATION_GROUP_SIZE = 32
    ...
    for (size_t i = 0; i < zero_point_tensor.get_byte_size(); ++i) {
        ...
        uint8_t bias2 = (uint8_t)std::round(
            -1.f * static_cast<float>(bias_data[i * 2 + 1]) / static_cast<float>(scale_data[i * 2 + 1])
        );
