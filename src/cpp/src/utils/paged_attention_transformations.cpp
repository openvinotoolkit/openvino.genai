...
void set_kv_cache_type_and_shape(std::shared_ptr<ov::Model> model, DeviceConfig& device_config) {
    ...
    int64_t head_size = k_shape[2].get_length();
    ...
    for (size_t idx = 0; idx < num_layers; idx++) {
        ...
        // TODO: CVS-145270
        k->set_partial_shape(to_partial_with_dyn_0_dim(device_config.get_key_cache_shape(idx)));
        v->set_partial_shape(to_partial_with_dyn_0_dim(device_config.get_value_cache_shape(idx)));
    }
