...
ContinuousBatchingPipeline::ContinuousBatchingPipeline(const std::filesystem::path& models_path,
    ...
    auto [properties_without_draft_model_without_gguf, enable_save_ov_model] =
        utils::extract_gguf_properties(properties_without_draft_model);
    ...
    if (is_prompt_lookup_enabled) {
        OPENVINO_ASSERT(draft_model_desr.model == nullptr,
                        "Speculative decoding and prompt lookup decoding are mutually exclusive");
        if (embedder) {
            m_impl = std::make_shared<PromptLookupImpl>(model,
                                                        embedder,
                                                        tokenizer,
                                                        scheduler_config,
                                                        device,
        ...
        auto main_model_descr = ov::genai::ModelDesc(model,
                                                     tokenizer,
                                                     device,
                                                     properties_without_draft_model_without_gguf,
                                                     scheduler_config,
                                                     generation_config);
        m_impl =
            std::make_shared<Eagle3DecodingImpl>(main_model_descr, draft_model_desr, eagle_rt_info.hidden_layers_list);
        ...
        m_impl = std::make_shared<ContinuousBatchingImpl>(model,
                                                          embedder,
                                                          tokenizer,
                                                          scheduler_config,
                                                          device,
...
ContinuousBatchingPipeline::ContinuousBatchingPipeline(const std::string& model_str,
    ...
    auto is_prompt_lookup_enabled = extract_prompt_lookup_from_config(properties_without_draft_model);
    auto eagle_rt_info =
        utils::eagle3::extract_eagle3_info_from_config(draft_model_desr.properties, std::filesystem::path(model_str));
    ...
    if (is_prompt_lookup_enabled) {
        ...
        m_impl = std::make_shared<PromptLookupImpl>(model,
                                                    ...
                                                    scheduler_config,
                                                    device,
                                                    properties_without_draft_model,
                                                    generation_config);
    } else if (draft_model_desr.model != nullptr && eagle_rt_info.eagle3_mode) {
        OPENVINO_ASSERT(embedder == nullptr, "Eagle speculative decoding is not supported for models with embeddings");
        auto main_model_descr = ov::genai::ModelDesc(model,
...
ContinuousBatchingPipeline::ContinuousBatchingPipeline(const ModelsMap& models_map,
