...
private:
    ...
    void _initialize_score_with_skips(std::vector<double>& dst,
                                      const std::vector<double>& src,
                                      const std::set<size_t> skipped_logical_block_ids);
    void _accumulate_initial_scores(const std::vector<double>& max_pooled_hh_scores,
                                    size_t decoder_layer_idx,
                                    size_t num_snapkv_scores,
                                    const std::set<size_t>& skipped_logical_block_ids);

    void _accumulate_layer_scores_to(size_t decoder_layer_idx,
                                     const std::vector<double>& src,
                                     const std::set<size_t>& skipped_logical_block_ids,
                                     std::vector<double>& dst);
    void _accumulate_with_existing_scores(const std::vector<double>& max_pooled_hh_scores,
                                          size_t decoder_layer_idx,
                                          size_t num_snapkv_scores,
                                          const std::set<size_t>& skipped_logical_block_ids);
...
public:
    ...
    size_t get_num_token_scores_to_aggregate(size_t prompt_len,
...
public:
     ...
     */
    std::pair<std::set<size_t>, size_t> get_diversity_block_set(size_t max_num_blocks_kept,
                                                                const std::vector<double>& evictable_area_token_scores);
     ...
     * the eviction area.
     */
    std::vector<double> get_filtered_block_diversity(const std::vector<double>& unfiltered_diversity,
     ...
     */
    std::set<size_t> get_most_diverse_blocks(size_t num_blocks_left_to_fill,
