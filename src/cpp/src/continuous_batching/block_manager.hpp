...
public:
    ...
    void add(const BlocksPerLayer& blocks_for_all_layers) {
        ...
        bool is_all_free = std::all_of(
            blocks_for_all_layers.begin(),
...
public:
    ...
    ~BlockAllocator() {
        ...
        for (auto& free_block : m_free_blocks_num) {
            ...
            OPENVINO_ASSERT(
                m_total_num_blocks == free_and_overwritable_block_cnt,
    ...
    void increase_kv_blocks_number(size_t new_kv_blocks_count) {
        OPENVINO_ASSERT(
            new_kv_blocks_count > m_total_num_blocks,
...
public:
    ...
    size_t num_free_blocks() const {
        return m_allocator.num_free_blocks(
            0
        );  // relying on the invariant that all layers have identical number of blocks
    }
    ...
    void free_sequence(size_t seq_id) {
        std::lock_guard<std::mutex> lock(m_cached_blocks_map_mutex);
        OPENVINO_ASSERT(
            m_block_table.find(seq_id) != m_block_table.end(),
            "sequence with id ",
            seq_id,
     ...
     */
    void
    free_blocks_from_sequence(size_t seq_id, const std::vector<std::set<size_t>>& logical_block_index_sets_to_free) {
        std::lock_guard<std::mutex> lock(m_cached_blocks_map_mutex);
        ...
        for (size_t i = 0; i < presumed_num_layers; i++) {
            OPENVINO_ASSERT(
                logical_block_indices_to_free[i].size() == logical_block_indices_to_free[0].size(),
        ...
        for (size_t block_idx = 0; block_idx < num_blocks_to_free; block_idx++) {
            ...
            for (size_t layer_idx = 0; layer_idx < presumed_num_layers; layer_idx++) {
                ...
                OPENVINO_ASSERT(
                    logical_block_idx <= block_table_size,
            ...
            if (!blocks.empty()) {
                ...
                for (size_t i = 1; i < m_block_size; i++) {
                    ...
                    if (!blocks.empty()) {
                        ...
                        group->update_processed_tokens_num(
                            prev_iteration_content_len + i == prompt_len ? prev_iteration_content_len + i - 1
