...
public:
    ...
    std::vector<size_t> create_indicators_kvcrush(
        size_t num_tokens_in_evictable_blocks,
     ...
     * mode */
    std::vector<size_t>
    create_anchor_point_kvcrush(size_t num_tokens_in_evictable_blocks, std::vector<size_t>& indicators);
    ...
    std::vector<std::pair<size_t, size_t>> calculate_hamming_distance_kvcrush(
        size_t num_tokens_in_evictable_blocks,
    ...
    /** @return A vector of size kvcrush_budget, where each element contains the index of a
     * representative block selected using KVCrush algorithm */
    std::vector<std::size_t> get_representative_blocks_kvcrush(
        size_t num_tokens_in_evictable_blocks,
