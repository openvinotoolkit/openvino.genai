    ...
    void _schedule_prompt_phase_dynamic_split_fuse(std::vector<SequenceGroup::Ptr>& sequence_groups, Output& scheduler_output) {
        ...
        for (size_t sequence_group_id = 0; sequence_group_id < sequence_groups.size(); ++sequence_group_id) {
            ...
            if (!sequence_group->can_generate_tokens() && !sequence_group->is_waiting() && !sequence_group->handle_stopped() && !sequence_group->handle_cancelled()) {
                ...
                // In the last prompt chunk, draft may schedule one extra token which is generated by the main model,
                // so total scheduled tokens can temporarily exceed max_num_batched_tokens; clamp remaining budget to 0.
                size_t num_tokens_in_megabatch =
                    scheduler_output.m_total_num_scheduled_tokens < m_config.max_num_batched_tokens
                ...
                if (it_expected_scheduled_tokens != m_expected_num_scheduled_tokens.end()) {
                    ...
                    if (expected_num_scheduled_tokens != num_scheduled_tokens &&
                ...
                // For speculative decoding alignment, expected scheduling may intentionally exceed
                // max_num_batched_tokens by a small amount (e.g. final prompt chunk + generated token).
                // Do not stop early while there are still requests with externally expected scheduling.
                if (scheduler_output.m_total_num_scheduled_tokens == m_config.max_num_batched_tokens &&
                    m_expected_num_scheduled_tokens.empty())
                    break;
            }
        }
    }
