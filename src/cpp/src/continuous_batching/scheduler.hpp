...
public:
    ...
    Scheduler(
        ...
        m_block_manager = std::make_shared<BlockManager>(
            m_config.num_kv_blocks,
    ...
    void free_blocks_from_sequence(
        size_t seq_id,
    ...
    void clear_kv_cache() {
        OPENVINO_ASSERT(
            m_config.enable_prefix_caching == false,
...
private:
    ...
    void _schedule_generate_phase_dynamic_split_fuse(
    ...
    void _schedule_prompt_phase_vllm(std::vector<SequenceGroup::Ptr>& sequence_groups, Output& scheduler_output) {
        ...
        OPENVINO_ASSERT(
            m_config.max_num_seqs <= m_config.max_num_batched_tokens,
        ...
        OPENVINO_ASSERT(
            scheduler_output.m_scheduled_sequence_groups_ids.empty(),
        ...
        for (size_t sequence_group_id = 0; sequence_group_id < sequence_groups.size(); ++sequence_group_id) {
            ...
            if ((!sequence_group->can_generate_tokens() || recompute_evicted_sequences) &&
                ...
                OPENVINO_ASSERT(
                    m_config.max_num_batched_tokens >= sequence_len,
                    "Sequence length (",
                    sequence_len,
