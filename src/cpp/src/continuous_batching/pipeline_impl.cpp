...
ContinuousBatchingPipeline::ContinuousBatchingImpl::ContinuousBatchingImpl(
    ...
    bool allow_xattention = scheduler_config.use_sparse_attention &&
                            scheduler_config.sparse_attention_config.mode == SparseAttentionMode::XATTENTION;
    bool allow_score_aggregation = true;
    bool allow_adaptive_rkv = scheduler_config.use_cache_eviction &&
...
void ContinuousBatchingPipeline::ContinuousBatchingImpl::initialize_pipeline(
    ...
    if (sampler_num_threads_it != filtered_properties->end()) {
        ...
        filtered_properties.fork().erase(
            "sampler_num_threads"
        );  // do not use iterator sampler_num_threads_it because a forked container may not be the same container
    }
    ...
    OPENVINO_ASSERT(
        all_gpu_device || execution_devices.size() == 1,
    ...
    if (is_use_cache_eviction) {
        ...
        m_model_runner = std::make_shared<ModelRunner>(
            infer_request,
            m_block_size,
            m_num_decoder_layers,
...
void ContinuousBatchingPipeline::ContinuousBatchingImpl::_prepare_rotation_data_storage(
    ...
    ov::Shape rotation_deltas_store_shape{
        normalized_config.num_kv_blocks,
        1
    };  // last dim can be later changed to BLOCK_SIZE for per-token granularity
    ...
    auto rotation_trig_lut =
        ov::Tensor(ov::element::f32, ov::Shape{max_sequence_cache_occupation_length_in_blocks, embedding_size});
...
GenerationHandle ContinuousBatchingPipeline::ContinuousBatchingImpl::add_request(
    ...
    OPENVINO_ASSERT(
        sampling_params_copy.max_length > prompt_len,
    ...
    if (m_model_input_type == ModelInputType::EMBEDDINGS) {
        ...
        sequence_group =
            std::make_shared<SequenceGroup>(request_id, input_ids, sampling_params_copy, m_block_size, token_type_ids);
    }
...

    {
        ...
        if (sched_config.use_cache_eviction) {
            if (sched_config.cache_eviction_config.apply_rotation) {
                ...
                m_model_runner->set_cache_rotation_data(
...
void ContinuousBatchingPipeline::ContinuousBatchingImpl::_compute_cache_rotation_data(
    ...
    for (const auto& seq_id_and_evicted_blocks : m_previous_evicted_block_logical_indices_per_sequence) {
        ...
        for (size_t layer_idx = 0; layer_idx < logical_blocks_to_evict.size(); layer_idx++) {
            ...
            auto rotation_multipliers = m_cache_rotation_calculator->get_rotation_data(
                logical_blocks_to_evict[layer_idx],
            ...
            for (size_t i = 0; i < rotation_multipliers.size(); i++) {
                ...
                auto rotation_deltas_tensor_data = m_rotation_deltas_stores[layer_idx].data<int32_t>() + block_offset;
...
void ContinuousBatchingPipeline::ContinuousBatchingImpl::_maybe_evict_cache_blocks(
    ...
    for (auto& seq_id_and_attention_scores : sequence_attention_scores) {
        ...
        if (m_seq_group_id_to_cache_eviction_algo_map.find(seq_id) == m_seq_group_id_to_cache_eviction_algo_map.end()) {
            ...
            m_seq_group_id_to_cache_eviction_algo_map[seq_id] = CacheEvictionAlgorithm(
        ...
        if (skip_set.empty()) {
            ...
            cache_eviction_algo.register_new_token_scores(
                attention_scores_for_all_decoder_layers,
                skip_set,
                scheduler_output.m_score_aggregation_windows.at(seq_id)
            );
        }
