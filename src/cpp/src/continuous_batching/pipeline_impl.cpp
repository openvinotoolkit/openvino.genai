...
void ContinuousBatchingPipeline::ContinuousBatchingImpl::initialize_pipeline(std::shared_ptr<ov::Model> model,
    ...
    if (normalized_config.num_kv_blocks == 0 && normalized_config.cache_size > 0) {
        ...
        OPENVINO_ASSERT(size_in_bytes <= total_mem_size,
    ...
    if (is_use_cache_eviction) {
        ...
        m_scheduler = std::make_shared<Scheduler>(m_block_size,
                                                  cache_manager,
                                                  normalized_config,
...
void ContinuousBatchingPipeline::ContinuousBatchingImpl::_prepare_rotation_data_storage(
    ...
    m_cache_rotation_calculator =
        std::make_shared<CacheRotationCalculator>(m_block_size,
...
void ContinuousBatchingPipeline::ContinuousBatchingImpl::_maybe_evict_cache_blocks(
    ...
    for (auto& seq_id_and_attention_scores : sequence_attention_scores) {
        ...
        if (m_seq_group_id_to_cache_eviction_algo_map.find(seq_id) == m_seq_group_id_to_cache_eviction_algo_map.end()) {
            ...
            m_seq_group_id_to_cache_eviction_algo_map[seq_id] =
                CacheEvictionAlgorithm(sched_config.cache_eviction_config,
                                       m_block_size,
                                       num_decoder_layers,
