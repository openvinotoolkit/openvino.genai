...
class CacheManager {
    ...
    void update_request_tensor(size_t decoder_layer_id) {
        m_request.set_tensor(
            std::string("key_cache.") + std::to_string(decoder_layer_id),
            m_key_cache[decoder_layer_id]
        );
        m_request.set_tensor(
            std::string("value_cache.") + std::to_string(decoder_layer_id),
...
public:
    explicit CacheManager(ov::InferRequest request) : m_request(request) {
        ...
        OPENVINO_ASSERT(
            m_num_decoder_layers == m_key_precisions.size(),
                ...
                for (size_t decoder_layer_id = 0; decoder_layer_id < m_num_decoder_layers; ++decoder_layer_id) {
                    ...
                    if (m_key_cache.size() > decoder_layer_id && m_key_cache[decoder_layer_id]) {
                        ...
                        if (key_precision == ov::element::u4) {
                            ...
                            std::memcpy(
                                reinterpret_cast<uint8_t*>(key_cache.data()),
                        ...
                        if (value_precision == ov::element::u4) {
                            ...
                            std::memcpy(
                                reinterpret_cast<uint8_t*>(value_cache.data()),
    ...
    ov::Tensor get_key_cache(size_t decoder_layer_id) const {
        OPENVINO_ASSERT(
            decoder_layer_id < m_key_cache.size(),
            "decoder_layer_id = ",
            decoder_layer_id,
            ", num_layers = ",
            m_key_cache.size()
    ...
    ov::Tensor get_value_cache(size_t decoder_layer_id) const {
        OPENVINO_ASSERT(
            decoder_layer_id < m_value_cache.size(),
            "decoder_layer_id = ",
            decoder_layer_id,
            ", num_layers = ",
            m_value_cache.size()
            ...
            for (size_t dst_block_id : dst_block_ids) {
                for (size_t decoder_layer_id = 0; decoder_layer_id < m_num_decoder_layers; ++decoder_layer_id) {
                    ...
                    if (value_cache_prec == ov::element::u4 || value_cache_prec == ov::element::i4) {
                        ...
                        ov::Tensor value_src_cache_roi(
                            m_value_cache[decoder_layer_id],
                            value_src_start_roi,
                            value_src_end_roi
                        );
                        ov::Tensor value_dst_cache_roi(
                            m_value_cache[decoder_layer_id],
                            value_dst_start_roi,
                            value_dst_end_roi
                        );
