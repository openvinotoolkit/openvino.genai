...
class CacheManager {
    ...
    void update_request_tensor(size_t decoder_layer_id) {
        m_request.set_tensor(std::string("key_cache.") + std::to_string(decoder_layer_id),
                             m_key_cache[decoder_layer_id]);
        m_request.set_tensor(std::string("value_cache.") + std::to_string(decoder_layer_id),
                             m_value_cache[decoder_layer_id]);
    }

public:
    explicit CacheManager(ov::InferRequest request) : m_request(request) {
        ...
        for (const auto& input : compiled_model.inputs()) {
            for (auto& name : input.get_names()) {
                ...
                if (name.find("key_cache.") == 0) {
                    ...
                    m_block_size_in_bytes += pshape[1].get_length() * pshape[2].get_length() * pshape[3].get_length() *
                                             cache_precision.size();
        ...
        try {
            ...
            if (m_context) {  // Allocate KV caches
                for (size_t decoder_layer_id = 0; decoder_layer_id < m_num_decoder_layers; ++decoder_layer_id) {
                    ...
                    ov::Tensor key_cache =
                        m_context.create_tensor(get_key_cache_precision(decoder_layer_id), key_cache_shape);
                ...
                for (size_t decoder_layer_id = 0; decoder_layer_id < m_num_decoder_layers; ++decoder_layer_id) {
                    ...
                    if (m_key_cache.size() > decoder_layer_id && m_key_cache[decoder_layer_id]) {
                        ...
                        if (key_precision == ov::element::u4) {
                            size_t key_stride =
                                std::accumulate(end_key.begin(), end_key.end(), 1, std::multiplies<size_t>());
                            size_t key_roi_size_byte =
                                key_stride + (key_stride & 1) / sub_byte_data_type_multiplier(key_precision);
                            std::memcpy(reinterpret_cast<uint8_t*>(key_cache.data()),
                                        reinterpret_cast<uint8_t*>(m_key_cache[decoder_layer_id].data()),
                                        key_roi_size_byte);
                        } else {
                        ...
                        if (value_precision == ov::element::u4) {
                            size_t value_stride =
                                std::accumulate(end_value.begin(), end_value.end(), 1, std::multiplies<size_t>());
                            size_t value_roi_size_byte =
                                value_stride + (value_stride & 1) / sub_byte_data_type_multiplier(value_precision);
                            std::memcpy(reinterpret_cast<uint8_t*>(value_cache.data()),
                                        reinterpret_cast<uint8_t*>(m_value_cache[decoder_layer_id].data()),
                                        value_roi_size_byte);
                        } else {
            ...
            for (size_t dst_block_id : dst_block_ids) {
                for (size_t decoder_layer_id = 0; decoder_layer_id < m_num_decoder_layers; ++decoder_layer_id) {
                    ...
                    if (key_cache_prec == ov::element::u4 || key_cache_prec == ov::element::i4) {
                        size_t stride = std::accumulate(std::next(key_shape.begin()),
                                                        key_shape.end(),
                                                        1,
                    ...
                    if (value_cache_prec == ov::element::u4 || value_cache_prec == ov::element::i4) {
                        size_t stride = std::accumulate(std::next(value_shape.begin()),
                                                        value_shape.end(),
                                                        1,
                                                        std::multiplies<size_t>()) /
                                        2;
                        copy_one_block(m_value_cache[decoder_layer_id],
                                       m_value_cache[decoder_layer_id],
                                       value_src_start_roi[0],
                                       value_dst_start_roi[0],
                                       stride);
                    } else {
                        ov::Tensor value_src_cache_roi(m_value_cache[decoder_layer_id],
                                                       value_src_start_roi,
                                                       value_src_end_roi);
                        ov::Tensor value_dst_cache_roi(m_value_cache[decoder_layer_id],
                                                       value_dst_start_roi,
                                                       value_dst_end_roi);
