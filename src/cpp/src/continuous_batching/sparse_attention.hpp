...
public:
     ...
     * @param num_last_dense_tokens_in_prefill The number of tokens in the end of the prompt phase for which sparse
     * attention should not be applied.
    ...
    explicit TriShapeSparseAttentionTokenSkipper(
        ...
        OPENVINO_ASSERT(
            !(num_retained_start_tokens_in_cache % block_size),
            "num_last_dense_tokens_in_prefill in tokens must be a multiple of block size ",
            block_size
        );
        OPENVINO_ASSERT(
            !(num_retained_recent_tokens_in_cache % block_size),
            "num_retained_dense_tokens_in_prefill in tokens must be a multiple of block size ",
            block_size
        );
    }
