...
void EvictionScoreManager::remove_scores(const std::vector<size_t>& evicted_block_indices, size_t decoder_layer_idx) {
    ...
    if (m_aggregation_mode == AggregationMode::NORM_SUM) {
        OPENVINO_ASSERT(accumulated_scores_for_current_decoder_layer.size() ==
                        counter_for_current_decoder_layer.size());
    }
...
void EvictionScoreManager::register_new_token_scores(
    ...
    if (m_num_registered_snapkv_aggregated_scores < m_snapkv_window_size) {
        OPENVINO_ASSERT(num_snapkv_scores + m_num_registered_snapkv_aggregated_scores <= m_snapkv_window_size,
                        "Total number of aggregated SnapKV scores during prefill phase may not be larger than the "
                        "configured SnapKV window size");
    ...
    for (size_t decoder_layer_idx = 0; decoder_layer_idx < m_num_decoder_layers; decoder_layer_idx++) {
        ...
        if (accumulated_scores_for_current_decoder_layer.empty()) {
            _accumulate_initial_scores(processed_hh_scores,
                                       decoder_layer_idx,
                                       num_snapkv_scores,
                                       skipped_logical_block_ids);
        } else {
            _accumulate_with_existing_scores(processed_hh_scores,
                                             decoder_layer_idx,
                                             num_snapkv_scores,
                                             skipped_logical_block_ids);
        }
...
void EvictionScoreManager::_accumulate_initial_scores(const std::vector<double>& max_pooled_hh_scores,
...
void EvictionScoreManager::_accumulate_with_existing_scores(const std::vector<double>& max_pooled_hh_scores,
                                                            size_t decoder_layer_idx,
                                                            size_t num_snapkv_scores,
                                                            const std::set<size_t>& skipped_logical_block_ids) {
    if (m_aggregation_mode == AggregationMode::ADAPTIVE_RKV) {
        ...
        _accumulate_layer_scores_to(decoder_layer_idx,
                                    max_pooled_hh_scores,
                                    skipped_logical_block_ids,
...
void EvictionScoreManager::_accumulate_layer_scores_to(size_t decoder_layer_idx,
...
void EvictionScoreManager::_adjust_norm_sum_counters(size_t decoder_layer_idx,
...
std::vector<std::set<std::size_t>> CacheEvictionAlgorithm::evict_logical_blocks() {
    ...
    for (size_t decoder_layer_idx = 0; decoder_layer_idx < scores.size(); decoder_layer_idx++) {
        ...
        if (m_eviction_config.aggregation_mode == AggregationMode::ADAPTIVE_RKV) {
            ...
            OPENVINO_ASSERT(!m_last_block_diversity.empty(),
            ...
            OPENVINO_ASSERT(num_diversity_values_registered == num_expected_diversity_values,
            ...
            auto diversity_blocks_and_num_blocks_kept =
                arkv_calc.get_diversity_block_set(max_num_evictable_blocks_to_keep_after_eviction,
            ...
            if (num_blocks_left_to_fill != 0) {
                auto filtered_block_diversity =
                    arkv_calc.get_filtered_block_diversity(m_last_block_diversity[decoder_layer_idx],
                                                           num_evictable_blocks * m_block_size,
                                                           diversity_blocks);
                auto most_diverse_set = arkv_calc.get_most_diverse_blocks(num_blocks_left_to_fill,
                                                                          diversity_blocks,
...
void CacheEvictionAlgorithm::register_new_token_scores(
    ...
    const std::set<size_t>& skipped_logical_block_ids,
    size_t num_snapkv_scores_aggregated) {
    m_score_manager.register_new_token_scores(attention_scores_for_all_decoder_layers,
                                              skipped_logical_block_ids,
...
void CacheEvictionAlgorithm::register_block_diversity(
    ...
    for (size_t layer_idx = 0; layer_idx < m_num_decoder_layers; layer_idx++) {
        ...
        OPENVINO_ASSERT(num_diversity_values % m_block_size ==
                        0);  // expecting shape of [eviction_size / block_size, eviction_size]
    ...
    for (size_t logical_block_idx : logical_block_space) {
        if (evicted_block_logical_indices.find(logical_block_idx) != evicted_block_logical_indices.end()) {
            ...
            if (current_rotation_delta_in_blocks != 0) {
                ...
                OPENVINO_ASSERT(block_rotation_data.rotation_delta / m_block_size <= m_rope_cos_lut.size(),
...
size_t SnapKVScoreAggregationCalculator::get_num_token_scores_to_aggregate(size_t prompt_len,
