...
void EvictionScoreManager::remove_scores(const std::vector<size_t>& evicted_block_indices, size_t decoder_layer_idx) {
    ...
    auto new_size = accumulated_scores_for_current_decoder_layer.size() - evicted_block_indices.size() * m_block_size;
...
void EvictionScoreManager::register_new_token_scores(
    ...
    if (m_num_registered_snapkv_aggregated_scores < m_snapkv_window_size) {
        OPENVINO_ASSERT(
            num_snapkv_scores + m_num_registered_snapkv_aggregated_scores <= m_snapkv_window_size,
            "Total number of aggregated SnapKV scores during prefill phase may not be larger than the configured "
            "SnapKV window size"
        );
        m_num_registered_snapkv_aggregated_scores += num_snapkv_scores;
    }

    // FIXME (vshampor): currently in terms of counters we do not discern between the cases when the last chunk has been
    // prefill-only or last-prefill-chunk-plus-one-generation_token
    for (size_t decoder_layer_idx = 0; decoder_layer_idx < m_num_decoder_layers; decoder_layer_idx++) {
        ...
        size_t start_token_offset_in_scores =
            (m_ignore_first_n_blocks - num_skipped_blocks_in_ignore_area) * m_block_size;
        ...
        if (accumulated_scores_for_current_decoder_layer.empty()) {
            _accumulate_initial_scores(
                processed_hh_scores,
                decoder_layer_idx,
                num_snapkv_scores,
                skipped_logical_block_ids
            );
        } else {
            _accumulate_with_existing_scores(
                processed_hh_scores,
                decoder_layer_idx,
                num_snapkv_scores,
                skipped_logical_block_ids
...
void EvictionScoreManager::_accumulate_initial_scores(
    ...
    auto& accumulated_scores_for_current_decoder_layer = m_scores[decoder_layer_idx];
    _initialize_score_with_skips(
        accumulated_scores_for_current_decoder_layer,
        max_pooled_hh_scores,
        skipped_logical_block_ids
    );
    ...
    if (m_aggregation_mode == AggregationMode::NORM_SUM) {
        ...
        if (m_snapkv_window_size == 0) {
            ...
            std::generate(counter.begin(), counter.begin() + new_scores_size, [&new_scores_size] {
                return new_scores_size--;
            });
        } else {
...
void EvictionScoreManager::_accumulate_with_existing_scores(
    const std::vector<double>& max_pooled_hh_scores,
    size_t decoder_layer_idx,
    size_t num_snapkv_scores,
    const std::set<size_t>& skipped_logical_block_ids
...
void EvictionScoreManager::_accumulate_layer_scores_to(
    size_t decoder_layer_idx,
...
void EvictionScoreManager::_adjust_norm_sum_counters(
    size_t decoder_layer_idx,
    size_t old_size_in_tokens,
    size_t new_size_in_tokens
) {
    OPENVINO_ASSERT(new_size_in_tokens >= old_size_in_tokens);
    ...
    if (m_snapkv_window_size == 0 || m_num_registered_snapkv_aggregated_scores == m_snapkv_window_size) {
        ...
        std::fill(
            counter_for_current_decoder_layer.begin(),
            counter_for_current_decoder_layer.end() - m_num_registered_snapkv_aggregated_scores,
            m_num_registered_snapkv_aggregated_scores
        );
        std::iota(
            counter_for_current_decoder_layer.rbegin(),
            counter_for_current_decoder_layer.rbegin() + m_num_registered_snapkv_aggregated_scores,
            1
        );
    }
}

void EvictionScoreManager::_initialize_score_with_skips(
    std::vector<double>& dst,
    const std::vector<double>& src,
    const std::set<size_t> skipped_logical_block_ids
) {
...
void EvictionScoreManager::add_with_skips(
    std::vector<double>& dst,
    const std::vector<double>& src,
    const std::set<size_t>& skipped_logical_block_ids
) const {
...
CacheEvictionAlgorithm::CacheEvictionAlgorithm(
    ...
    : m_eviction_config(eviction_config),
      ...
      m_score_manager(
          ...
          eviction_config.snapkv_window_size,
          eviction_config.adaptive_rkv_config.window_size
      ),
      m_kvcrush_algo(eviction_config.kvcrush_config, block_size) {
    OPENVINO_ASSERT(
        !(m_eviction_config.get_start_size() % m_block_size),
        "CacheEvictionConfig.start_size in tokens must be a multiple of block size ",
        m_block_size
    );
    OPENVINO_ASSERT(
        !(m_eviction_config.get_recent_size() % m_block_size),
        "CacheEvictionConfig.recent_size in tokens must be a multiple of block size ",
        m_block_size
    );
    OPENVINO_ASSERT(
        !(m_eviction_config.get_max_cache_size() % m_block_size),
...
std::vector<std::set<std::size_t>> CacheEvictionAlgorithm::evict_logical_blocks() {
    // Returns the indices of logical KV cache blocks to evict (the rest is to be discarded) for each decoder layer in
    // order. The kept indices are determined using `attention_scores`, which is expected to be the attention head
    // scores that are already reduced by the batch and head dimensions, i.e. the shape of `attention_scores` must be
    // [num_new_tokens, current_seq_len], where `num_new_tokens` is the dimension corresponding to the number of freshly
    // generated tokens since the last cache eviction has taken place, and the `current_seq_len` is the dimension
    // corresponding to the current sequence length at this stage in the generation process, i.e. the dimension over
    // which the attention scores over individual previous tokens was being computed.
    ...
    for (size_t decoder_layer_idx = 0; decoder_layer_idx < scores.size(); decoder_layer_idx++) {
        ...
        if (m_eviction_config.aggregation_mode == AggregationMode::ADAPTIVE_RKV) {
            auto arkv_calc =
                AdaptiveRKVBlockCalculator(m_eviction_config.adaptive_rkv_config.attention_mass, m_block_size);
            OPENVINO_ASSERT(
                !m_last_block_diversity.empty(),
                "Token diversity must be registered before each eviction in the Adaptive R-KV scenario"
            );
            ...
            OPENVINO_ASSERT(
                num_diversity_values_registered == num_expected_diversity_values,
                "Diversity score size mismatch - registered ",
                num_diversity_values_registered,
                " diversity scores, but expected ",
                ...
                m_block_size,
                "=",
                num_expected_diversity_values,
                " scores"
            );
            size_t max_num_evictable_blocks_to_keep_after_eviction =
                m_eviction_config.get_evictable_size() / m_block_size;

            auto diversity_blocks_and_num_blocks_kept = arkv_calc.get_diversity_block_set(
                max_num_evictable_blocks_to_keep_after_eviction,
            ...
            if (num_blocks_left_to_fill != 0) {
                auto filtered_block_diversity = arkv_calc.get_filtered_block_diversity(
                    m_last_block_diversity[decoder_layer_idx],
                    num_evictable_blocks * m_block_size,
                    diversity_blocks
                );
                auto most_diverse_set = arkv_calc.get_most_diverse_blocks(
                    num_blocks_left_to_fill,
                    diversity_blocks,
                    filtered_block_diversity
                );
            ...
            evicted_block_indices =
                get_indices_of_blocks_to_evict(scores_for_all_evictable_blocks, num_blocks_to_evict);
            // KVCrush: start
        ...
        for (auto& idx : evicted_block_indices)
            idx += get_num_blocks(m_eviction_config.get_start_size());
        for (auto& idx : evicted_block_indices)
            retval[decoder_layer_idx].insert(idx);
    }

    m_last_block_diversity.clear();
    return retval;
}
...
CacheEvictionAlgorithm::CacheEvictionRange CacheEvictionAlgorithm::get_evictable_block_range(size_t layer_idx) const {
    std::size_t current_sequence_length =
        m_eviction_config.get_start_size() + m_score_manager.get_current_scores_length_in_tokens(layer_idx);
...
void CacheEvictionAlgorithm::register_new_token_scores(
    ...
    m_score_manager.register_new_token_scores(
        attention_scores_for_all_decoder_layers,
        skipped_logical_block_ids,
        num_snapkv_scores_aggregated
    );
}
...
std::vector<double> CacheEvictionAlgorithm::get_scores_for_all_evictable_blocks(size_t decoder_layer_idx) const {
    ...
    OPENVINO_ASSERT(
        (num_tracked_tokens + m_eviction_config.get_start_size()) > get_max_cache_size_after_eviction(),
...
std::vector<std::size_t> CacheEvictionAlgorithm::get_indices_of_blocks_to_evict(
    ...
    std::nth_element(
        evictable_block_score_and_index_pairs.begin(),
...
void CacheEvictionAlgorithm::remove_scores_of_evicted_blocks(
    const std::vector<std::size_t>& evicted_block_indices,
    size_t decoder_layer_idx
) {
    m_score_manager.remove_scores(evicted_block_indices, decoder_layer_idx);
}

void CacheEvictionAlgorithm::register_block_diversity(
    const BlockDiversityForEachDecoderLayer& block_diversities_for_all_decoder_layers
) {
    OPENVINO_ASSERT(
        m_last_block_diversity.empty(),
        "CacheEvictionAlgorithm already has block_diversity, must evict before new diversity is registered"
    );
    OPENVINO_ASSERT(block_diversities_for_all_decoder_layers.size() == m_num_decoder_layers);
    m_last_block_diversity.resize(m_num_decoder_layers);
    for (size_t layer_idx = 0; layer_idx < m_num_decoder_layers; layer_idx++) {
        const auto& layer_diversity = block_diversities_for_all_decoder_layers[layer_idx];
        const float* data = layer_diversity.data<float>();
        size_t num_diversity_values = layer_diversity.get_size();
        OPENVINO_ASSERT(
            num_diversity_values % m_block_size == 0
        );  // expecting shape of [eviction_size / block_size, eviction_size]
        m_last_block_diversity[layer_idx].resize(num_diversity_values);
        for (size_t tok_idx = 0; tok_idx < layer_diversity.get_size(); tok_idx++) {
            m_last_block_diversity[layer_idx][tok_idx] = data[tok_idx];
        }
    }
}

CacheRotationCalculator::CacheRotationCalculator(
    size_t block_size,
    ...
    for (size_t i = 0; i < max_context_length_in_blocks; i++) {
        ...
        for (size_t j = 0; j < num_freqs; j++) {
            ...
            m_rope_sin_lut[i].push_back(
                -std::sin(i * block_size * base_angle)
            );  // minus since we will be rotating by an inverse angle
    ...
    for (size_t logical_block_idx : logical_block_space) {
        if (evicted_block_logical_indices.find(logical_block_idx) != evicted_block_logical_indices.end()) {
            ...
            if (current_rotation_delta_in_blocks != 0) {
                ...
                OPENVINO_ASSERT(
                    block_rotation_data.rotation_delta / m_block_size <= m_rope_cos_lut.size(),
                    "rotation delta larger than LUT size"
                );
                ...
                if (!deltas_only) {
                    ...
                    for (size_t i = 0; i < m_block_size; i++) {
                        block_rotation_data.cosines.push_back(m_rope_cos_lut[current_rotation_delta_in_blocks]);
                        block_rotation_data.sines.push_back(m_rope_sin_lut[current_rotation_delta_in_blocks]);
                    }
                }
...
size_t SnapKVScoreAggregationCalculator::get_num_token_scores_to_aggregate(
    size_t prompt_len,
    ...
    if (num_processed_tokens_after_this_chunk > first_scored_token_position) {
        if (num_processed_tokens_before_this_chunk > first_scored_token_position) {
            ...
            num_scored_token_positions_in_this_chunk =
                num_processed_tokens_after_this_chunk - first_scored_token_position;
        }
    }
    return num_scored_token_positions_in_this_chunk;
}

std::pair<std::set<size_t>, size_t> AdaptiveRKVBlockCalculator::get_diversity_block_set(
    size_t max_num_blocks_kept,
    const std::vector<double>& evictable_area_block_scores
) {
    struct ScoreAndBlockIdx {
        double score;
        size_t block_idx;
        bool operator<(const ScoreAndBlockIdx& rhs) const {
...
std::vector<double> AdaptiveRKVBlockCalculator::get_filtered_block_diversity(
    const std::vector<double>& unfiltered_diversity,
    size_t eviction_size,
    const std::set<size_t>& diversity_blocks
) {
    std::vector<double> retval(eviction_size / m_block_size);
...
std::set<size_t> AdaptiveRKVBlockCalculator::get_most_diverse_blocks(
    size_t num_blocks_left_to_fill,
    const std::set<size_t>& diversity_blocks,
    const std::vector<double>& filtered_block_diversity
) {
    OPENVINO_ASSERT(num_blocks_left_to_fill <= diversity_blocks.size());
    struct ScoreAndBlockIdx {
        double score;
        size_t block_idx;
        bool operator<(const ScoreAndBlockIdx& rhs) const {
    ...
    std::priority_queue<ScoreAndBlockIdx> score_block_queue;
    for (size_t block_idx : diversity_blocks) {
