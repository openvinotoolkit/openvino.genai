...
ov::genai::utils::GenerationFinishInfo get_lm_encoded_results(
    ...
    for (SequenceGroup::Ptr sequence_group : sequence_groups) {
        generations.push_back(
            std::make_shared<GenerationHandleImpl>(
    ...
    if (m_embedding) {
        ...
        cache_state.add_inputs(input_ids);
    ...
    while (!active_sequence_groups.empty()) {
        ...
        if (m_embedding) {
            ...
            for (const auto& [name, tensor] : lm_extra_inputs) {
                ...
                if (name == "deepstack_visual_embeds") {
                    ...
                    std::fill_n(
                        new_deepstack_visual_embeds.data<float>(),
        ...
        if (position_ids.has_value()) {
            if (position_ids->get_shape().size() == 3 && rope_delta.has_value()) {
                update_3d_position_ids(
                    m_llm.get_tensor("position_ids"),
...
TokenizedInputs get_chat_encoded_input(const ov::Tensor& new_chat_tokens, utils::CacheState& cache_state) {
    ...
    if (
        ...
        ov::Tensor new_tensor = ov::Tensor(
            new_chat_tokens.get_element_type(),
