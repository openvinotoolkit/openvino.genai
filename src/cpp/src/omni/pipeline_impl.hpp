...
public:
    OmniPipelineImpl(const std::shared_ptr<VLMPipeline::VLMBackend>& vlm,
    ...
    void set_talker_speech_config(const OmniTalkerSpeechConfig& new_config) {
        validate_omni_talker_speech_config(new_config);
...
private:
    ...
    public:
        PendingAudiosGuard(VLMPipeline::VLMBackend& vlm, const std::vector<ov::Tensor>& audios) : m_vlm(vlm) {
    ...
    private:
        VLMPipeline::VLMBackend& m_vlm;
    };

    std::shared_ptr<VLMPipeline::VLMBackend> m_vlm;
