...
void apply_paged_attention_transformations(std::shared_ptr<ov::Model> model, DeviceConfig& device_config) {
    ...
    const size_t num_layers = std::count_if(parameters.begin(), parameters.end(), [](std::shared_ptr<ov::op::v0::Parameter> parameter) {
        return parameter->get_friendly_name().find("key_cache.") == 0;
    });

    // extract num_kv_heads and head_size
    const size_t kv_caches_inputs_offset = 2;
    const ov::PartialShape k_shape = parameters[kv_caches_inputs_offset]->get_partial_shape();
    OPENVINO_ASSERT(k_shape.rank().get_length() == 3, "KV cache shape is expected to have rank 3, while shape is ", k_shape);
    const size_t num_kv_heads = k_shape[1].get_length(), head_size = k_shape[2].get_length();
