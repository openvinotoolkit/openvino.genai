...
public:
    ...
    static Sequence::Ptr
    create(const uint64_t id, const SequenceGroupType type = SequenceGroupType::TOKENS, const size_t hidden_size = 0) {
    ...
    GenerationOutput get_last_generation_output(size_t token_cnt = 1, size_t num_token_to_ignore = 0) {
        ...
        if (token_cnt > 0) {
            ...
            if (get_generated_len() > num_token_to_ignore) {
                ...
                std::vector<int64_t> token_id(
                    generated_token_id.begin() + offset,
                    generated_token_id.begin() + offset_back
                );
                std::vector<float> log_probs(
                    generated_log_probs.begin() + offset,
    ...
    void append_generated_ids_embeds(ov::Tensor generated_ids_embeds) {
        ...
        for (size_t i = current_embeds_size, idx = 0; i < current_embeds_size + embeds_count; i++, idx++) {
            ...
            std::copy_n(
                generated_ids_embeds.data<float>() + idx * m_hidden_size,
                m_hidden_size,
    ...
    get_position_ids_elem_coordinates(const ov::Shape& position_ids_elem_shape, size_t idx, bool need_batch_dimention) {
        ...
        if (position_ids_elem_shape.size() == 3) {
            ...
            OPENVINO_THROW(
                "Unexpected rank of position ids element. Expected rank 2 or 3, got: " +
...
public:
    ...
    void push_outputs() {
        ...
        for (auto& sequence : m_sequences) {
            ...
            if (m_sampling_params.echo) {
                ...
                output.generated_log_probs
                    .insert(output.generated_log_probs.begin(), m_prompt_log_probs.begin(), m_prompt_log_probs.end());
            }
    ...
    void notify_handle_echo_only() {
        ...
        GenerationOutput output;
        output.generated_ids = std::vector<int64_t>(
            m_prompt_ids.begin() + first_token_position,
            m_prompt_ids.begin() + last_token_position
        );
        output.generated_log_probs = std::vector<float>(
            m_prompt_log_probs.begin() + first_token_position,
