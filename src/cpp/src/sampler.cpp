...
bool
is_continue_generation(Sequence::Ptr running_sequence,
                       size_t token_id,
                       size_t max_gen_len,
                       size_t& decrease_context_len) {
    if (max_gen_len == 0) {
        running_sequence->remove_last_tokens(token_id);
        decrease_context_len = std::max(decrease_context_len, token_id);
        return false;
    }
    return true;
}
...
SamplerOutput Sampler::sample(std::vector<SequenceGroup::Ptr> & sequence_groups,
    ...
    for (size_t sequence_group_id = 0, currently_processed_tokens = 0; sequence_group_id < sequence_groups.size(); ++sequence_group_id) {
            ...
            if (sampling_params.is_greedy_decoding() || sampling_params.is_multinomial()) {
                ...
                for (size_t running_sequence_id = 0; running_sequence_id < num_running_sequences; ++running_sequence_id) {
                    ...
                    for (size_t i = 0; i <= num_tokens_to_process; ++i) {
                        ...
                        size_t max_gen_len = sampling_params.max_new_tokens + token_offset - running_sequence->get_generated_len();
                        if (!is_continue_generation(running_sequence, token_offset, max_gen_len, decrease_context_len)) {
                            break;
                        }
                        
                        // do sampling only for token validation/generation. Extending KV cache will be skipped
                        ...
                        if (is_validation_mode_enabled) {
                            is_continue_generation = validate_candidate(running_sequences[running_sequence_id], token_offset, sampled_token_id, is_extend_sequence, decrease_context_len);
                        }
        ...
        if (decrease_context_len) {
            const auto num_processed_tokens = sequence_group->get_num_processed_tokens();
            OPENVINO_ASSERT(num_processed_tokens >= decrease_context_len);
            OPENVINO_ASSERT(sequence_group->get_context_len() >= decrease_context_len);
            sequence_group->update_processed_tokens_num(num_processed_tokens - decrease_context_len);
        }

        // accumulate a number of processed tokens
        currently_processed_tokens += (padded_amount_of_processed_tokens - decrease_context_len) * num_running_sequences;
    }
