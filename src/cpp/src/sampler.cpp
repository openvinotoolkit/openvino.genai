...
SamplerOutput Sampler::sample(std::vector<SequenceGroup::Ptr> & sequence_groups,
    ...
    for (size_t sequence_group_id = 0, currently_processed_tokens = 0; sequence_group_id < sequence_groups.size(); ++sequence_group_id) {
            ...
            if (sampling_params.is_greedy_decoding() || sampling_params.is_multinomial()) {
                ...
                auto register_new_token = [&](const Token& sampled_token_id, size_t running_sequence_id, bool is_extend_sequence = true) {
                    ...
                    if (is_extend_sequence) {
                        ...
                        OPENVINO_ASSERT(generated_len < running_sequences[running_sequence_id]->get_generated_len());
                        running_sequences[running_sequence_id]->update_generated_log_prob(generated_len, sampled_token_id.m_log_prob);
                    }
                ...
                for (size_t running_sequence_id = 0; running_sequence_id < num_running_sequences; ++running_sequence_id) {
                    ...
                    while (--token_id_per_seq >= 0 && --max_new_tokens_cnt >= 0) {
                        if (max_new_tokens_cnt == 0) {
                            running_sequences[running_sequence_id]->remove_last_tokens(token_id_per_seq);
        ...
        if (decrease_len) {
            ...
            OPENVINO_ASSERT(num_processed_tokens >= decrease_len);
            OPENVINO_ASSERT(sequence_group->get_context_len() >= decrease_len);
            sequence_group->update_processed_tokens_num(num_processed_tokens - decrease_len);
        }
