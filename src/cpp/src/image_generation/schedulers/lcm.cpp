...
LCMScheduler::LCMScheduler(const Config& scheduler_config)
    ...
    if (!m_config.trained_betas.empty()) {
        ...
        for (size_t i = 0; i < m_config.num_train_timesteps; i++) {
            betas.push_back(m_config.beta_start +
                            (m_config.beta_end - m_config.beta_start) * i / (m_config.num_train_timesteps - 1));
        }
...
std::map<std::string, ov::Tensor> LCMScheduler::step(ov::Tensor noise_pred,
                                                     ov::Tensor latents,
