...
void PNDMScheduler::set_timesteps(size_t num_inference_steps, float strength) {
    ...
    default:
        OPENVINO_THROW("Unsupported value for 'timestep_spacing'. Please make sure to choose one of 'linspace', "
                       "'leading' or 'trailing'.");
    }
...
std::map<std::string, ov::Tensor> PNDMScheduler::step(ov::Tensor noise_pred,
                                                      ov::Tensor latents,
...
std::map<std::string, ov::Tensor> PNDMScheduler::step_plms(ov::Tensor model_output,
                                                           ov::Tensor sample,
                                                           size_t timestep) {
    OPENVINO_ASSERT(
        m_num_inference_steps != -1,
...
ov::Tensor PNDMScheduler::get_prev_sample(ov::Tensor sample,
                                          size_t timestep,
                                          int prev_timestep,
