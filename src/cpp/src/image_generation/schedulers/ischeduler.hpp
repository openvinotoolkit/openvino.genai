...
public:
    ...
    virtual std::map<std::string, ov::Tensor> step(ov::Tensor noise_pred,
                                                   ov::Tensor latents,
                                                   size_t inference_step,
                                                   std::shared_ptr<Generator> generator) = 0;
    ...
    virtual void set_timesteps(size_t image_seq_len, size_t num_inference_steps, float strength) {
        OPENVINO_THROW("Scheduler doesn't support `set_timesteps(size_t image_seq_len, size_t num_inference_steps, "
                       "float strength)` method");
    }
