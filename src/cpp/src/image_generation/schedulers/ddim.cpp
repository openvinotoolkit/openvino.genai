...
std::map<std::string, ov::Tensor> DDIMScheduler::step(ov::Tensor noise_pred,
                                                      ov::Tensor latents,
    ...
    for (size_t j = 0; j < noise_pred.get_size(); j++) {
        ...
        case PredictionType::EPSILON:
            pos_val = (latents.data<float>()[j] - std::sqrt(beta_prod_t) * noise_pred.data<float>()[j]) /
        ...
        case PredictionType::V_PREDICTION:
            pos_val = std::sqrt(alpha_prod_t) * latents.data<float>()[j] -
                      std::sqrt(beta_prod_t) * noise_pred.data<float>()[j];
            pe_val = std::sqrt(alpha_prod_t) * noise_pred.data<float>()[j] +
                     std::sqrt(beta_prod_t) * latents.data<float>()[j];
            pred_original_sample.push_back(pos_val);
            pred_epsilon.push_back(pe_val);
            break;
        default:
    ...
    OPENVINO_ASSERT(!m_config.thresholding, "Parameter 'thresholding' is not supported. Please, add support.");
    // TODO: support m_config.clip_sample
    OPENVINO_ASSERT(!m_config.clip_sample, "Parameter 'clip_sample' is not supported. Please, add support.");
    ...
    std::transform(pred_epsilon.begin(),
                   pred_epsilon.end(),
                   pred_sample_direction.begin(),
                   [alpha_prod_t_prev](auto x) {
