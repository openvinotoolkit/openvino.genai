...
void stretch_shift_to_terminal(std::vector<float>& sigmas, float shift_terminal) {
    ...
    OPENVINO_ASSERT(
        std::abs(1.0 - static_cast<double>(shift_terminal)) > 1e-6,
...
std::map<std::string, ov::Tensor> FlowMatchEulerDiscreteScheduler::step(
...
void FlowMatchEulerDiscreteScheduler::add_noise(
    ov::Tensor init_latent,
    ov::Tensor noise,
    int64_t latent_timestep
) const {
    // use
