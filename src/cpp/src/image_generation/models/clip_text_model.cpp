...
std::shared_ptr<CLIPTextModel> CLIPTextModel::clone() {
    OPENVINO_ASSERT(
        ...
        "CLIPTextModel must have exactly one of m_model or m_request initialized"
    );
...
ov::Tensor
CLIPTextModel::infer(const std::string& pos_prompt, const std::string& neg_prompt, bool do_classifier_free_guidance) {
    ...
    auto perform_tokenization = [&](const std::string& prompt, ov::Tensor input_ids) {
        ...
        if (input_ids.get_element_type() == ov::element::i32) {
            ...
            std::copy_n(
                input_ids_token.data<int32_t>(),
                std::min(input_ids_token.get_size(), input_ids.get_size()),
                input_ids.data<int32_t>()
            );
        } else {
            std::fill_n(input_ids.data<int64_t>(), input_ids.get_size(), pad_token_id);
            std::copy_n(
                input_ids_token.data<int64_t>(),
                std::min(input_ids_token.get_size(), input_ids.get_size()),
    ...
    if (compiled_input_partial_shape.is_dynamic()) {
        ...
        OPENVINO_ASSERT(
            text_embedding_batch_size <= compiled_input_shape[0],
            "text_embedding_batch_size (",
            text_embedding_batch_size,
            ") > CLIP text encoder model batch size (",
            compiled_input_shape[0],
            ")."
        );
        OPENVINO_ASSERT(
            m_config.max_position_embeddings == compiled_input_shape[1],
            "max_position_embeddings (",
            m_config.max_position_embeddings,
            ") != what CLIP text encoder model was compiled for (",
            compiled_input_shape[1],
            ")."
        );
    }
...
void CLIPTextModel::import_model(
