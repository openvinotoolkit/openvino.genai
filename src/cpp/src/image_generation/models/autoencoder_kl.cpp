...
ov::AnyMap handle_scale_factor(std::shared_ptr<ov::Model> model, const std::string& device, ov::AnyMap properties) {
    ...
    ov::element::Type wa_inference_precision =
        it != properties.end() ? it->second.as<ov::element::Type>() : ov::element::dynamic;
    if (it != properties.end()) {
        properties.erase(it);
    }

    const std::vector<std::string> activation_scale_factor_path = {
        "runtime_options",
    ...
    if (device.find("GPU") != std::string::npos && !activation_scale_factor_defined &&
        wa_inference_precision != ov::element::dynamic) {
...
AutoencoderKL AutoencoderKL::clone() {
    OPENVINO_ASSERT(
        (m_decoder_model != nullptr) ^ static_cast<bool>(m_decoder_request),
        "AutoencoderKL must have exactly one of m_decoder_model or m_decoder_request initialized"
    );  // encoder is optional
...
AutoencoderKL& AutoencoderKL::compile(const std::string& device, const ov::AnyMap& properties) {
    ...
    if (m_encoder_model) {
        ov::CompiledModel encoder_compiled_model = core.compile_model(
            m_encoder_model,
            device,
            handle_scale_factor(m_encoder_model, device, *filtered_properties)
        );
    ...
    ov::CompiledModel decoder_compiled_model =
        core.compile_model(m_decoder_model, device, handle_scale_factor(m_decoder_model, device, *filtered_properties));
...
ov::Tensor AutoencoderKL::encode(ov::Tensor image, std::shared_ptr<Generator> generator) {
    OPENVINO_ASSERT(
        m_encoder_request || m_encoder_model,
        "AutoencoderKL is created without 'VAE encoder' capability. Please, pass extra argument to constructor to "
        "create 'VAE encoder'"
    );
...
void AutoencoderKL::import_model(
    const std::filesystem::path& blob_path,
    const std::string& device,
    const ov::AnyMap& properties
) {
    auto decoder_compiled_model =
        utils::import_model(blob_path / "vae_decoder" / "openvino_model.blob", device, properties);
    ov::genai::utils::print_compiled_model_properties(decoder_compiled_model, "Auto encoder KL decoder model");
    m_decoder_request = decoder_compiled_model.create_infer_request();

    if (std::filesystem::exists(blob_path / "vae_encoder" / "openvino_model.blob")) {
        auto encoder_compiled_model =
            utils::import_model(blob_path / "vae_encoder" / "openvino_model.blob", device, properties);
        ov::genai::utils::print_compiled_model_properties(encoder_compiled_model, "Auto encoder KL encoder model");
        m_encoder_request = encoder_compiled_model.create_infer_request();
    }
}
