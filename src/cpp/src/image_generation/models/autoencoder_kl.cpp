...
ov::AnyMap handle_scale_factor(std::shared_ptr<ov::Model> model, const std::string& device, ov::AnyMap properties) {
    ...
    const std::vector<std::string> activation_scale_factor_path = {"runtime_options",
...
AutoencoderKL AutoencoderKL::clone() {
    OPENVINO_ASSERT(
        ...
        "AutoencoderKL must have exactly one of m_decoder_model or m_decoder_request initialized");  // encoder is
                                                                                                     // optional
...
AutoencoderKL& AutoencoderKL::compile(const std::string& device, const ov::AnyMap& properties) {
    ...
    if (m_encoder_model) {
        ov::CompiledModel encoder_compiled_model =
            core.compile_model(m_encoder_model,
                               device,
...
ov::Tensor AutoencoderKL::encode(ov::Tensor image, std::shared_ptr<Generator> generator) {
    OPENVINO_ASSERT(m_encoder_request || m_encoder_model,
                    "AutoencoderKL is created without 'VAE encoder' capability. Please, pass extra argument to "
                    "constructor to create 'VAE encoder'");
...
void AutoencoderKL::import_model(const std::filesystem::path& blob_path,
                                 const std::string& device,
                                 const ov::AnyMap& properties) {
