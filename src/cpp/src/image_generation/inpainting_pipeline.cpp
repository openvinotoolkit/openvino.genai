...
InpaintingPipeline::InpaintingPipeline(const Image2ImagePipeline& pipe) {
    auto start_time = std::chrono::steady_clock::now();
    if (auto stable_diffusion_xl = std::dynamic_pointer_cast<StableDiffusionXLPipeline>(pipe.m_impl);
        stable_diffusion_xl != nullptr) {
        m_impl = std::make_shared<StableDiffusionXLPipeline>(PipelineType::INPAINTING, *stable_diffusion_xl);
    } else if (auto stable_diffusion = std::dynamic_pointer_cast<StableDiffusionPipeline>(pipe.m_impl);
               stable_diffusion != nullptr) {
        m_impl = std::make_shared<StableDiffusionPipeline>(PipelineType::INPAINTING, *stable_diffusion);
    } else if (auto stable_diffusion_3 = std::dynamic_pointer_cast<StableDiffusion3Pipeline>(pipe.m_impl);
               stable_diffusion_3 != nullptr) {
...
InpaintingPipeline::InpaintingPipeline(const std::shared_ptr<DiffusionPipeline>& impl) : m_impl(impl) {
    assert(m_impl != nullptr);
}

InpaintingPipeline InpaintingPipeline::stable_diffusion(const std::shared_ptr<Scheduler>& scheduler,
...
InpaintingPipeline InpaintingPipeline::latent_consistency_model(const std::shared_ptr<Scheduler>& scheduler,
                                                                const CLIPTextModel& clip_text_model,
                                                                const UNet2DConditionModel& unet,
                                                                const AutoencoderKL& vae) {
    auto impl = std::make_shared<StableDiffusionPipeline>(PipelineType::INPAINTING, clip_text_model, unet, vae);
...
InpaintingPipeline InpaintingPipeline::stable_diffusion_xl(
    ...
    const UNet2DConditionModel& unet,
    const AutoencoderKL& vae) {
    auto impl = std::make_shared<StableDiffusionXLPipeline>(PipelineType::INPAINTING,
                                                            clip_text_model,
                                                            clip_text_model_with_projection,
                                                            unet,
                                                            vae);
...
InpaintingPipeline InpaintingPipeline::flux(const std::shared_ptr<Scheduler>& scheduler,
                                            const CLIPTextModel& clip_text_model,
                                            const T5EncoderModel& t5_text_encoder,
                                            const FluxTransformer2DModel& transformer,
                                            const AutoencoderKL& vae) {
    auto impl =
        std::make_shared<FluxPipeline>(PipelineType::INPAINTING, clip_text_model, t5_text_encoder, transformer, vae);
...
InpaintingPipeline InpaintingPipeline::flux_fill(const std::shared_ptr<Scheduler>& scheduler,
                                                 const CLIPTextModel& clip_text_model,
                                                 const T5EncoderModel& t5_text_encoder,
                                                 const FluxTransformer2DModel& transformer,
                                                 const AutoencoderKL& vae) {
    auto impl = std::make_shared<FluxFillPipeline>(PipelineType::INPAINTING,
                                                   clip_text_model,
                                                   t5_text_encoder,
                                                   transformer,
                                                   vae);
...
InpaintingPipeline InpaintingPipeline::stable_diffusion_3(const std::shared_ptr<Scheduler>& scheduler,
                                                          ...
                                                          const AutoencoderKL& vae) {
    auto impl = std::make_shared<StableDiffusion3Pipeline>(PipelineType::INPAINTING,
                                                           clip_text_model_1,
                                                           clip_text_model_2,
                                                           t5_encoder_model,
                                                           transformer,
...
void InpaintingPipeline::reshape(const int num_images_per_prompt,
                                 const int height,
                                 const int width,
