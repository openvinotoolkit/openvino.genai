...
InpaintingPipeline::InpaintingPipeline(const Image2ImagePipeline& pipe) {
    ...
    if (auto stable_diffusion_xl = std::dynamic_pointer_cast<StableDiffusionXLPipeline>(pipe.m_impl);
        ...
        auto stable_diffusion = std::dynamic_pointer_cast<StableDiffusionPipeline>(pipe.m_impl);
        stable_diffusion != nullptr
    ) {
        m_impl = std::make_shared<StableDiffusionPipeline>(PipelineType::INPAINTING, *stable_diffusion);
    } else if (
        auto stable_diffusion_3 = std::dynamic_pointer_cast<StableDiffusion3Pipeline>(pipe.m_impl);
...
InpaintingPipeline InpaintingPipeline::stable_diffusion_xl(
    ...
    auto impl = std::make_shared<StableDiffusionXLPipeline>(
        PipelineType::INPAINTING,
        clip_text_model,
...
InpaintingPipeline InpaintingPipeline::flux_fill(
    ...
    auto impl = std::make_shared<FluxFillPipeline>(
        PipelineType::INPAINTING,
        clip_text_model,
        t5_text_encoder,
        transformer,
...
InpaintingPipeline InpaintingPipeline::stable_diffusion_3(
    ...
    auto impl = std::make_shared<StableDiffusion3Pipeline>(
        PipelineType::INPAINTING,
        clip_text_model_1,
        clip_text_model_2,
        t5_encoder_model,
...
void InpaintingPipeline::reshape(
    const int num_images_per_prompt,
    const int height,
    const int width,
...
void InpaintingPipeline::compile(
...
ov::Tensor InpaintingPipeline::generate(
