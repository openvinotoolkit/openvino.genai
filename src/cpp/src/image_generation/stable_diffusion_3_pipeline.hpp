...
void padding_right(ov::Tensor src, ov::Tensor res) {
    ...
    OPENVINO_ASSERT(
        src_shape.size() == 3 && src_shape.size() == res_shape.size(),
        "Rank of tensors within 'padding_right' must be 3"
    );
    OPENVINO_ASSERT(
        src_shape[0] == res_shape[0] && src_shape[1] == res_shape[1],
...
public:
    ...
    reshape(const int num_images_per_prompt, const int height, const int width, const float guidance_scale) override {
        ...
        m_transformer
            ->reshape(num_images_per_prompt * batch_size_multiplier, height, width, transformer_tokenizer_max_length);
    ...
    std::shared_ptr<DiffusionPipeline> clone() override {
        ...
        if (m_t5_text_encoder) {
            ...
            pipeline = std::make_shared<StableDiffusion3Pipeline>(
                m_pipeline_type,
                *clip_text_encoder_1,
                *clip_text_encoder_2,
                *t5_text_encoder,
    ...
    compute_hidden_states(const std::string& positive_prompt, const ImageGenerationConfig& generation_config) override {
        ...
        ov::Tensor text_encoder_1_output = m_clip_text_encoder_1->infer(
            positive_prompt,
            negative_prompt_1_str,
        ...
        ov::Tensor text_encoder_2_output = m_clip_text_encoder_2->infer(
            prompt_2_str,
        ...
        if (m_t5_text_encoder) {
            infer_start = std::chrono::steady_clock::now();
            text_encoder_3_output = m_t5_text_encoder->infer(
                prompt_3_str,
            ...
            ov::Shape t5_prompt_embed_shape = {
        ...
        ov::Shape pad_embeds_shape = {
            clip_prompt_embeds_shape[0],
...
private:
    ...
    void check_image_size(const int height, const int width) const override {
        ...
        OPENVINO_ASSERT(
            (height % (vae_scale_factor * patch_size) == 0 || height < 0) &&
    ...
    void check_inputs(const ImageGenerationConfig& generation_config, ov::Tensor initial_image) const override {
        ...
        OPENVINO_ASSERT(
            is_classifier_free_guidance || generation_config.negative_prompt_3 == std::nullopt,
