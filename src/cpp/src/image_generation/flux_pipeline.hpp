...
inline ov::Tensor pack_latents(const ov::Tensor latents,
                               size_t batch_size,
    ...
    OPENVINO_ASSERT(latents.get_size() == permuted_latents.get_size(),
...
public:
    FluxPipeline(PipelineType pipeline_type, const std::filesystem::path& root_dir) : FluxPipeline(pipeline_type) {
        ...
        OPENVINO_ASSERT(!is_inpainting_model() || class_name == "FluxFillPipeline",
                        "inpainting model is not currently supported by Flux InpaintingPipeline. Please, contact "
                        "OpenVINO GenAI developers.");
    ...
    FluxPipeline(PipelineType pipeline_type,
        ...
        OPENVINO_ASSERT(!is_inpainting_model(),
                        "inpainting model is not currently supported by Flux InpaintingPipeline. Please, contact "
                        "OpenVINO GenAI developers.");
    }
    ...
    void compute_hidden_states(const std::string& positive_prompt,
        ...
        if (m_transformer->get_config().guidance_embeds) {
            ...
            std::fill_n(guidance.data<float>(),
                        guidance.get_size(),
    ...
    std::tuple<ov::Tensor, ov::Tensor> prepare_mask_latents(ov::Tensor mask_image,
        ...
        auto repeat_mask = [](const ov::Tensor& mask, size_t num_channels_latents) -> ov::Tensor {
            ...
            OPENVINO_ASSERT(mask_shape.size() == 4 && mask_shape[1] == 1,
