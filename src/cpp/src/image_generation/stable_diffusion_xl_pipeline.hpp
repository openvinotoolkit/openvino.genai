...
public:
    ...
    StableDiffusionXLPipeline(
        ...
        auto updated_properties =
            update_adapters_in_properties(properties_without_blob, &DiffusionPipeline::derived_adapters);
    ...
    StableDiffusionXLPipeline(PipelineType pipeline_type, const StableDiffusionXLPipeline& pipe)
        ...
        m_clip_text_encoder_with_projection =
            std::make_shared<CLIPTextModelWithProjection>(*pipe.m_clip_text_encoder_with_projection);
    ...
    std::shared_ptr<DiffusionPipeline> clone() override {
        ...
        std::shared_ptr<CLIPTextModelWithProjection> clip_text_encoder_with_projection =
            std::static_pointer_cast<CLIPTextModelWithProjection>(m_clip_text_encoder_with_projection->clone());
    ...
    compute_hidden_states(const std::string& positive_prompt, const ImageGenerationConfig& generation_config) override {
        ...
        // see
        // https://github.com/huggingface/diffusers/blob/v0.31.0/src/diffusers/pipelines/stable_diffusion_xl/pipeline_stable_diffusion_xl.py#L423-L427
        bool force_zeros_for_empty_prompt =
            generation_config.negative_prompt == std::nullopt && m_force_zeros_for_empty_prompt;
        ...
        if (compute_negative_prompt) {
            ...
            add_text_embeds = m_clip_text_encoder_with_projection
                                  ->infer(positive_prompt, negative_prompt_1_str, batch_size_multiplier > 1);
            auto infer_duration =
            ...
            ov::Tensor encoder_hidden_states_2 =
            ...
            ov::Tensor encoder_hidden_states_2_positive =
                m_clip_text_encoder_with_projection->get_output_tensor(idx_hidden_state_2);
            ...
            OPENVINO_ASSERT(
                ehs_1_shape[0] == ehs_2_shape[0] && ehs_1_shape[1] == ehs_2_shape[1],
            ...
            ov::Shape encoder_hidden_states_shape = {
                ehs_1_shape[0] * batch_size_multiplier,
                ehs_1_shape[1],
                ehs_1_shape[2] + ehs_2_shape[2]
            };
            ...
            std::copy_n(
                add_text_embeds_positive.data<float>(),
                add_text_embeds_positive.get_size(),
                add_text_embeds_data
            );
            ...
            for (size_t i = 0; i < ehs_1_shape[0] * ehs_1_shape[1]; ++i,
                        ...
                        ehs_1_data += ehs_1_shape[2],
                        ehs_2_data += ehs_2_shape[2]) {
...
private:
    void initialize_generation_config(const std::string& class_name) override {
        ...
        if (class_name == "StableDiffusionXLPipeline" || class_name == "StableDiffusionXLImg2ImgPipeline" ||
            class_name == "StableDiffusionXLInpaintPipeline") {
            if (m_pipeline_type == PipelineType::TEXT_2_IMAGE) {
    ...
    void check_inputs(const ImageGenerationConfig& generation_config, ov::Tensor initial_image) const override {
        ...
        OPENVINO_ASSERT(
            is_classifier_free_guidance || generation_config.negative_prompt == std::nullopt,
            "Negative prompt is not used when guidance scale <= 1.0"
        );
        OPENVINO_ASSERT(
            is_classifier_free_guidance || generation_config.negative_prompt_2 == std::nullopt,
    ...
    utils::SharedOptional<const ov::AnyMap>
    properties_for_text_encoder(const ov::AnyMap& properties, const std::string& tensor_name_prefix) {
        return update_adapters_in_properties(
            properties,
