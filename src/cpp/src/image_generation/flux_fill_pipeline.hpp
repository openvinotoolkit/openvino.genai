...
public:
    ...
    std::tuple<ov::Tensor, ov::Tensor> prepare_mask_latents(ov::Tensor mask_image,
        ...
        ov::Tensor repeated_masked_image_latent =
            repeat_mask(masked_image_latent,
                        generation_config.num_images_per_prompt / masked_image_latent.get_shape()[0]);
        masked_image_latent = pack_latents(repeated_masked_image_latent,
        ...
        transform_mask(repeated_mask_condition,
                       generation_config.num_images_per_prompt,
                       height,
                       width,
                       vae_scale_factor);
        ov::Tensor mask = pack_latents(repeated_mask_condition,
                                       generation_config.num_images_per_prompt,
...
private:
    ...
    ov::Tensor repeat_mask(const ov::Tensor& masked_image_latents, size_t batch_size) {
        ...
        OPENVINO_ASSERT(batch_size % input_batch_size == 0,
