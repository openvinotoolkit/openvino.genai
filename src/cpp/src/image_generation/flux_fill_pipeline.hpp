...
public:
    ...
    std::tuple<ov::Tensor, ov::Tensor> prepare_mask_latents(
        ...
        OPENVINO_ASSERT(
            processed_image_shape[2] == mask_condition_shape[2] && processed_image_shape[3] == mask_condition_shape[3],
        ...
        ov::Tensor repeated_masked_image_latent = repeat_mask(
            masked_image_latent,
            generation_config.num_images_per_prompt / masked_image_latent.get_shape()[0]
        );
        masked_image_latent = pack_latents(
            repeated_masked_image_latent,
        ...
        ov::Tensor repeated_mask_condition =
            repeat_mask(mask_condition, generation_config.num_images_per_prompt / mask_condition.get_shape()[0]);
        transform_mask(
            repeated_mask_condition,
            generation_config.num_images_per_prompt,
            height,
            width,
            vae_scale_factor
        );
        ov::Tensor mask = pack_latents(
            repeated_mask_condition,
            generation_config.num_images_per_prompt,
...
private:
    ...
    void check_inputs(const ImageGenerationConfig& generation_config, ov::Tensor initial_image) const override {
        ...
        OPENVINO_ASSERT(
            ...
            "Negative prompt is not used by FluxFillPipeline"
        );
        OPENVINO_ASSERT(
            generation_config.negative_prompt_2 == std::nullopt,
            "Negative prompt 2 is not used by FluxFillPipeline"
        );
        OPENVINO_ASSERT(
            generation_config.negative_prompt_3 == std::nullopt,
            "Negative prompt 3 is not used by FluxFillPipeline"
        );
    ...
    ov::Tensor repeat_mask(const ov::Tensor& masked_image_latents, size_t batch_size) {
        ...
        OPENVINO_ASSERT(
            batch_size % input_batch_size == 0,
            "'batch_size' must be a multiple of the 'input_batch_size'"
        );
