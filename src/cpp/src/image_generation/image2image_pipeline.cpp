...
Image2ImagePipeline::Image2ImagePipeline(const std::shared_ptr<DiffusionPipeline>& impl) : m_impl(impl) {
    assert(m_impl != nullptr);
}

Image2ImagePipeline Image2ImagePipeline::stable_diffusion(const std::shared_ptr<Scheduler>& scheduler,
...
Image2ImagePipeline Image2ImagePipeline::latent_consistency_model(const std::shared_ptr<Scheduler>& scheduler,
                                                                  const CLIPTextModel& clip_text_model,
                                                                  const UNet2DConditionModel& unet,
                                                                  const AutoencoderKL& vae) {
    auto impl = std::make_shared<StableDiffusionPipeline>(PipelineType::IMAGE_2_IMAGE, clip_text_model, unet, vae);
...
Image2ImagePipeline Image2ImagePipeline::stable_diffusion_xl(
    ...
    const UNet2DConditionModel& unet,
    const AutoencoderKL& vae) {
    auto impl = std::make_shared<StableDiffusionXLPipeline>(PipelineType::IMAGE_2_IMAGE,
                                                            clip_text_model,
...
Image2ImagePipeline Image2ImagePipeline::flux(const std::shared_ptr<Scheduler>& scheduler,
                                              const CLIPTextModel& clip_text_model,
                                              const T5EncoderModel t5_encoder_model,
                                              const FluxTransformer2DModel& transformer,
                                              const AutoencoderKL& vae) {
    auto impl = std::make_shared<FluxPipeline>(PipelineType::IMAGE_2_IMAGE,
                                               clip_text_model,
                                               t5_encoder_model,
                                               transformer,
                                               vae);
...
Image2ImagePipeline Image2ImagePipeline::stable_diffusion_3(const std::shared_ptr<Scheduler>& scheduler,
                                                            ...
                                                            const AutoencoderKL& vae) {
    auto impl = std::make_shared<StableDiffusion3Pipeline>(PipelineType::IMAGE_2_IMAGE,
                                                           clip_text_model_1,
                                                           clip_text_model_2,
                                                           t5_encoder_model,
                                                           transformer,
...
void Image2ImagePipeline::reshape(const int num_images_per_prompt,
                                  const int height,
                                  const int width,
...
ov::Tensor Image2ImagePipeline::generate(const std::string& positive_prompt,
