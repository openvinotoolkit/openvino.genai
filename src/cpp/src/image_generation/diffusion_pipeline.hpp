...
public:
    explicit DiffusionPipeline(PipelineType pipeline_type) : m_pipeline_type(pipeline_type) {
        ...
        if (m_pipeline_type == PipelineType::IMAGE_2_IMAGE || m_pipeline_type == PipelineType::INPAINTING) {
            ...
            m_image_resizer = std::make_shared<ImageResizer>(device,
                                                             ov::element::u8,
                                                             "NHWC",
        ...
        if (m_pipeline_type == PipelineType::INPAINTING) {
            ...
            m_mask_resizer = std::make_shared<ImageResizer>(device,
                                                            ov::element::f32,
                                                            "NCHW",
    ...
    virtual void reshape(const int num_images_per_prompt,
                         const int height,
                         const int width,
                         const float guidance_scale) = 0;
    ...
    virtual std::tuple<ov::Tensor, ov::Tensor, ov::Tensor, ov::Tensor> prepare_latents(
        ov::Tensor initial_image,
        const ImageGenerationConfig& generation_config) = 0;

    virtual void compute_hidden_states(const std::string& positive_prompt,
                                       const ImageGenerationConfig& generation_config) = 0;

    virtual void set_lora_adapters(std::optional<AdapterConfig> adapters) = 0;

    virtual ov::Tensor generate(const std::string& positive_prompt,
                                ov::Tensor initial_image,
...
protected:
    ...
    virtual void blend_latents(ov::Tensor image_latent,
                               ov::Tensor noise,
                               ov::Tensor mask,
                               ov::Tensor latent,
        ...
        if (is_inpainting_model()) {
            ...
            m_perf_metrics.vae_encoder_inference_duration +=
                std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - encode_start)
                    .count();
            masked_image_latent = numpy_utils::repeat(masked_image_latent,
