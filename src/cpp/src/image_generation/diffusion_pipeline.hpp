...
public:
    explicit DiffusionPipeline(PipelineType pipeline_type) : m_pipeline_type(pipeline_type) {
        ...
        if (m_pipeline_type == PipelineType::INPAINTING) {
            ...
            m_mask_resizer = std::make_shared<ImageResizer>(
                device,
                ov::element::f32,
                "NCHW",
                ov::op::v11::Interpolate::InterpolateMode::NEAREST
            );
        }
    }
    ...
    virtual void
    reshape(const int num_images_per_prompt, const int height, const int width, const float guidance_scale) = 0;
    ...
    virtual std::tuple<ov::Tensor, ov::Tensor, ov::Tensor, ov::Tensor>
    prepare_latents(ov::Tensor initial_image, const ImageGenerationConfig& generation_config) = 0;

    virtual void
    compute_hidden_states(const std::string& positive_prompt, const ImageGenerationConfig& generation_config) = 0;

    virtual void set_lora_adapters(std::optional<AdapterConfig> adapters) = 0;

    virtual ov::Tensor generate(
...
protected:
    ...
    virtual void blend_latents(
        ov::Tensor image_latent,
        ov::Tensor noise,
        ov::Tensor mask,
        ov::Tensor latent,
        ...
        OPENVINO_ASSERT(
            image_latent.get_shape() == latent.get_shape(),
            "Shapes for current",
            latent.get_shape(),
    ...
    virtual std::tuple<ov::Tensor, ov::Tensor> prepare_mask_latents(
        ov::Tensor mask_image,
        ...
        ov::Tensor mask = m_mask_resizer->execute(
            mask_condition,
            target_shape[2] / vae_scale_factor,
            target_shape[3] / vae_scale_factor
        );
        ...
        if (is_inpainting_model()) {
            ...
            m_perf_metrics.vae_encoder_inference_duration +=
                std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - encode_start)
                    .count();
            masked_image_latent = numpy_utils::repeat(
                masked_image_latent,
                generation_config.num_images_per_prompt * batch_size_multiplier
            );
        }
    ...
    std::shared_ptr<IImageProcessor> m_image_processor = nullptr, m_mask_processor_rgb = nullptr,
                                     m_mask_processor_gray = nullptr;
