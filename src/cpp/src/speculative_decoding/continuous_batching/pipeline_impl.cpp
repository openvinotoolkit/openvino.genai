...
ContinuousBatchingPipeline::ContinuousBatchingForSpeculativeDecodingImpl::update_request(uint64_t request_id,
    ...
    for (auto& request : m_requests) {
        ...
        if (!m_is_validation_mode_enabled && num_processed_tokens < prompt_len) {
            auto expected_num_scheduled_tokens = candidates.begin()->second.num_processed_tokens -
                                                 num_processed_tokens + request->get_num_tokens_to_validate();
    ...
    while (to_generate) {
        ...
        for (auto& request : m_requests) {
            ...
            if (!sampling_params.is_assisting_generation()) {
                ...
                request->pause_generation(true);
            } else if (m_scheduler->get_expected_num_scheduled_tokens(request->get_request_id()) > 0 &&
                       request->get_num_processed_tokens() < request->get_prompt_len() &&
                       request->get_num_processed_tokens() >=
                           m_scheduler->get_expected_num_scheduled_tokens(request->get_request_id())) {
