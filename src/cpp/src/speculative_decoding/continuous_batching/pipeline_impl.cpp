...
ContinuousBatchingPipeline::ContinuousBatchingForSpeculativeDecodingImpl::update_request(uint64_t request_id,
    ...
    for (auto& request : m_requests) {
        ...
        // During the prefill phase, the draft model should process the same number of tokens as the main model to
        // ensure they are aligned. For the final prompt chunk, the draft model additionally needs to process the token
        // that generated by the main model.
        if (!m_is_validation_mode_enabled && m_scheduler && m_scheduler->get_config().dynamic_split_fuse &&
            num_processed_tokens < prompt_len) {
            const size_t candidate_num_processed_tokens = candidates.begin()->second.num_processed_tokens;
            const size_t scheduled_delta = candidate_num_processed_tokens > num_processed_tokens
                                               ? candidate_num_processed_tokens - num_processed_tokens
                                               : 0;
        ...
        // Pause `draft_model` generation in two cases:
        // 1. `generated_len >= max_new_tokens - 1` to ensure the last token is generated by `main_model`.
        // 2. `main model` has not started processing this request yet when there are multiple request.
        ...
        if (!m_is_validation_mode_enabled) {
            ...
            if (generated_len >= max_new_tokens - 1 || (generated_len != 0 && result.inserted_tokens_cnt == 0) ||
                candidates.begin()->second.num_processed_tokens == 0) {
    ...
    while (to_generate) {
        ...
        for (auto& request : m_requests) {
            ...
            if (!sampling_params.is_assisting_generation()) {
                ...
                request->pause_generation(true);
            } else if (m_scheduler && m_scheduler->get_config().dynamic_split_fuse &&
