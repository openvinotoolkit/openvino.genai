...
ContinuousBatchingPipeline::ContinuousBatchingForSpeculativeDecodingImpl::update_request(uint64_t request_id,
    ...
    for (auto& request : m_requests) {
        ...
        if (!m_is_validation_mode_enabled && m_scheduler && m_scheduler->has_linear_attention_cache() &&
            request->get_num_processed_tokens() < num_processed_tokens) {
            const size_t accepted_endpoint = request->get_num_processed_tokens();
            for (const auto& sequence : running_sequences) {
                m_scheduler->free_sequence(sequence->get_id());
            }
            request->update_processed_tokens_num(0);
            if (m_scheduler->get_config().enable_prefix_caching) {
                m_scheduler->restore_cached_blocks(request);
            }
            const size_t restored_endpoint = request->get_num_processed_tokens();
            OPENVINO_ASSERT(restored_endpoint <= accepted_endpoint,
                            "Hybrid draft restore exceeds the accepted endpoint");
            request->update_processed_tokens_num(accepted_endpoint);
            request->preempt_tokens(accepted_endpoint - restored_endpoint);
        }
