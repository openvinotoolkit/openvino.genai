...
ContinuousBatchingPipeline::ContinuousBatchingForSpeculativeDecodingImpl::update_request(uint64_t request_id,
    ...
    for (auto& request : m_requests) {
        ...
        bool is_prompt_phase = num_processed_tokens < prompt_len;
        if (eagle_mode_enabled && !m_is_validation_mode_enabled && m_scheduler &&
            ...
            const size_t scheduled_delta = main_num_processed_tokens > num_processed_tokens
                                               ? main_num_processed_tokens - num_processed_tokens
                                               : 0;
