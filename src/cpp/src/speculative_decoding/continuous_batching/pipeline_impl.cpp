...
ContinuousBatchingPipeline::ContinuousBatchingForSpeculativeDecodingImpl::update_request(uint64_t request_id,
    ...
    for (auto& request : m_requests) {
        ...
        if (!m_is_validation_mode_enabled && m_scheduler->get_config().dynamic_split_fuse && num_processed_tokens < prompt_len) {
            const size_t draft_num_processed_tokens = candidates.begin()->second.num_processed_tokens;
            const size_t scheduled_delta = draft_num_processed_tokens > num_processed_tokens ?
                                              draft_num_processed_tokens - num_processed_tokens :
                                              0;
    ...
    while (to_generate) {
        ...
        for (auto& request : m_requests) {
            ...
            if (!sampling_params.is_assisting_generation()) {
                ...
                request->pause_generation(true);
            } else if (m_scheduler->get_config().dynamic_split_fuse &&
