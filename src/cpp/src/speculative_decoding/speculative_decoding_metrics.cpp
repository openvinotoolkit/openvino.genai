...
size_t SpeculativeDecodingMetrics::get_main_iteration_number(int64_t request_id) {
    OPENVINO_ASSERT(m_acceptance_rate.count(request_id));
    return m_acceptance_rate[request_id].size() + 1;
}

size_t SpeculativeDecodingMetrics::get_draft_iteration_number(int64_t request_id) {
    OPENVINO_ASSERT(m_acceptance_rate.count(request_id));
