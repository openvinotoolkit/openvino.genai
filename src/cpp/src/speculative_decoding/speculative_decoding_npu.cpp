...
SpeculativeLLMPipelineNPU::SpeculativeLLMPipelineNPU(
    ...
    m_draft_device = draft_model_desc_copy.device;
    ...
    m_main_device = main_model_desc_copy.device;
    ...
    */
    ...
    while (m_main_request->can_infer() && (streaming_status == ov::genai::StreamingStatus::RUNNING)) {
        ...
        if (draft_prefix_token != -1) {
            if (m_draft_device == "NPU") {
                // NB: Need to add last token generated by draft model to the KV-cache:
