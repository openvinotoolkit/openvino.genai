...
namespace ov::genai {
    constexpr size_t KV_CACHES_INPUTS_OFFSET = 2;

    inline size_t get_key_cache_tensor_id_for_decoder_layer(size_t decoder_layer_id) {
        return KV_CACHES_INPUTS_OFFSET + 2 * decoder_layer_id;
    }


    inline size_t get_value_cache_tensor_id_for_decoder_layer(size_t decoder_layer_id) {
        return KV_CACHES_INPUTS_OFFSET + 2 * decoder_layer_id + 1;
    }
        ...
        void copy_blocks(const std::map<size_t, std::list<size_t>> &block_copy_map) {
            ...
            for (const auto &blocks_pair: block_copy_map) {
                ...
                for (size_t dst_block_id: dst_block_ids) {
                    ...
                    for (size_t decoder_layer_id = 0;
                         decoder_layer_id < m_device_config.get_num_layers(); ++decoder_layer_id) {
                        ...
                        ov::Tensor value_src_cache_roi(m_value_cache[decoder_layer_id], value_src_start_roi,
                                                       value_src_end_roi);
                        ov::Tensor value_dst_cache_roi(m_value_cache[decoder_layer_id], value_dst_start_roi,
                                                       value_dst_end_roi);
