...
AutoencoderKLLTXVideo::AutoencoderKLLTXVideo(const std::filesystem::path& vae_decoder_path)
    ...
    std::tie(m_transformer_patch_size, m_transformer_patch_size_t) =
        get_transformer_patch_size(vae_decoder_path.parent_path() / "transformer" / "config.json");
...
AutoencoderKLLTXVideo& AutoencoderKLLTXVideo::compile(const std::string& device, const ov::AnyMap& properties) {
    ...
    ov::genai::utils::print_compiled_model_properties(decoder_compiled_model,
                                                      "Auto encoder KL LTX video decoder model");
...
AutoencoderKLLTXVideo& AutoencoderKLLTXVideo::reshape(int64_t batch_size,
    ...
    int64_t spatial_compression_ratio =
        get_config().patch_size * std::pow(2,
                                           std::accumulate(get_config().spatio_temporal_scaling.begin(),
                                                           get_config().spatio_temporal_scaling.end(),
                                                           0));
    int64_t temporal_compression_ratio =
        get_config().patch_size_t * std::pow(2,
                                             std::accumulate(get_config().spatio_temporal_scaling.begin(),
                                                             get_config().spatio_temporal_scaling.end(),
                                                             0));
