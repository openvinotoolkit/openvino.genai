...
public:
    explicit XGrammarLogitsTransformer(const xgrammar::CompiledGrammar& compiled_grammar,
...
public:
     ...
     *
     * This constructor initializes m_grammar_compiler with the provided tokenizer, which is used to compile the
     * grammar. Instantiating m_grammar_compiler can be expensive, so it is done only once per instance of
     ...
     */
    XGrammarStructuredOutput(const ov::genai::Tokenizer::TokenizerImpl& tokenizer_impl,
     ...
     * @param sampling_parameters The generation configuration parameters that may include JSON schema, regex, or EBNF
     * grammar.
     * @return A shared pointer to the logit transformer that applies XGrammar grammar matching.
     */
    std::shared_ptr<LogitTransformers::ILogitTransformer> get_logits_transformer(
        const ov::genai::GenerationConfig& sampling_parameters) override;
...
static bool registerXGrammarBackend() {
    StructuredOutputController::register_backend(
        "xgrammar",
