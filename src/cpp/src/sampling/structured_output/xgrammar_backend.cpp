...
XGrammarStructuredOutput::XGrammarStructuredOutput(const ov::genai::Tokenizer::TokenizerImpl& tokenizer_impl,
    ...
    auto tokenizer_info =
        xgrammar::TokenizerInfo(std::move(vocab_vector),
                        ...
                        std::visit(
                            [&error_message](const auto& err) {
                                if constexpr (std::is_member_function_pointer<
                                                  decltype(&std::decay_t<decltype(err)>::what)>::value) {
                                    error_message = err.what();
                                } else {
                                    error_message = "Unknown error type";
                                }
...
std::shared_ptr<LogitTransformers::ILogitTransformer> XGrammarStructuredOutput::get_logits_transformer(
    ...
    std::vector<int> override_stop_tokens(sampling_parameters.stop_token_ids.begin(),
...
XGrammarLogitsTransformer::XGrammarLogitsTransformer(const xgrammar::CompiledGrammar& compiled_grammar,
