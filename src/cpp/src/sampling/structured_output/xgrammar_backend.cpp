...
XGrammarStructuredOutput::XGrammarStructuredOutput(
    const ov::genai::Tokenizer::TokenizerImpl& tokenizer_impl,
...
xgrammar::Grammar XGrammarStructuredOutput::parse_structural_tag(
    const StructuredOutputConfig::CompoundGrammar& compound_grammar
) {
...
xgrammar::Grammar XGrammarStructuredOutput::create_grammar(
    ...
    if (structured_output_config.value().json_schema.has_value()) {
        ...
        return std::visit(
            [](const auto& config) -> xgrammar::Grammar {
                ...
                if constexpr (std::is_same_v<ConfigType, StructuralTagsConfig>) {
                    ...
                    GENAI_WARN(
                        "The use of \"structural_tags_config\" with StructuralTagsConfig instance is "
                    ...
                    if (std::holds_alternative<xgrammar::Grammar>(result)) {
                        ...
                        std::visit(
                            [&error_message](const auto& err) {
                                if constexpr (
                                    std::is_member_function_pointer<decltype(&std::decay_t<decltype(err)>::what)>::value
                                ) {
                                    error_message = err.what();
                                } else {
                                    error_message = "Unknown error type";
...
std::shared_ptr<LogitTransformers::ILogitTransformer> XGrammarStructuredOutput::get_logits_transformer(
    ...
    std::vector<int> override_stop_tokens(
        sampling_parameters.stop_token_ids.begin(),
        sampling_parameters.stop_token_ids.end()
    );
    return std::make_shared<LogitTransformers::XGrammarLogitsTransformer>(
...
XGrammarLogitsTransformer::XGrammarLogitsTransformer(
    ...
    : m_grammar_matcher(compiled_grammar, override_stop_tokens, terminate_without_stop_token, max_rollback_tokens) {
