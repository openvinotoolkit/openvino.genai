...
void Sampler::TreeSearcher::tree_reset() {
    const size_t num_speculative_tokens = m_parameters.eagle_tree_params.num_speculative_tokens;
    OPENVINO_ASSERT(num_speculative_tokens > 0,
                    "eagle_tree_params.num_speculative_tokens must be greater than 0, got ", num_speculative_tokens);
    m_candidate_graph.emplace(-1, 0.0f, static_cast<int>(num_speculative_tokens), static_cast<int>(m_parameters.eagle_tree_params.tree_depth));
...
SequenceGroupSamplingInfo Sampler::sample_from_sequence_group(SequenceGroup::Ptr sequence_group, ov::Tensor sequence_group_logits, 
    ...
    if (sampling_params.is_greedy_decoding() || sampling_params.is_multinomial()) {
        ...
        for (size_t running_sequence_id = 0; running_sequence_id < num_running_sequences; ++running_sequence_id) {
            ...
            for (size_t i = 0; i <= num_tokens_to_process; ++i) {
                ...
                if (sampling_params.is_greedy_decoding()) {
                    ...
                    if (!is_validation_passed) {
                        ...
                        if (p_prime > 0.f) {
                            float prob = std::exp(sampled_token.m_log_prob);
                            prob /= p_prime;
                            sampled_token.m_log_prob = std::log(prob);
                        }
                    }
                }
                if (!is_validation_mode_enabled && m_d2t_mapping) {
                ...
                if (!is_validation_passed) {
                    ...
                    const auto& current_sampling_params = sequence_group->get_sampling_parameters();
                    if (is_stop_token_id_hit(sampled_token.m_index, current_sampling_params.stop_token_ids) &&
                        !current_sampling_params.ignore_eos) {
                        running_sequence->set_status(SequenceStatus::FINISHED);
                        running_sequence->set_finish_reason(GenerationFinishReason::STOP);
            ...
            assisting_pipeline_info.min_generated_len = std::min(assisting_pipeline_info.min_generated_len,
