...
std::vector<int64_t> wrap_tokens(const std::vector<int64_t>& tokens,
...
int match_stop_string2(Tokenizer& tokenizer,
                       const TokenIds& generated_tokens,
...
void Sampler::GroupBeamSearcher::select_next_tokens(const ov::Tensor& logits,
    ...
    for (size_t group_id = 0; group_id < m_groups.size(); ++group_id) {
        ...
        for (const Beam& beam : group.ongoing) {
            ...
            full_text.insert(full_text.end(),
        ...
        for (size_t cand_idx = 0; cand_idx < candidates.size(); ++cand_idx) {
            ...
            if (is_stop_token_id_hit(candidate.m_token_id,
...
std::vector<Token> Sampler::_multinomial_sample(const Logits& logits, size_t num_tokens_per_sequence) {
    ...
    auto dist = std::discrete_distribution<size_t>(
        ...
        multinomial_weights.end());  // equivalent to multinomial with number of trials == 1
...
std::list<uint64_t> create_n_forked_sequences(SequenceGroup::Ptr sequence_group,
                                              LogitProcessor& logit_processor,
...
void stop_sample_tokens(Sequence::Ptr running_sequence,
                        size_t token_idx,
                        size_t max_gen_len,
...
void align_all_sequence_len(SequenceGroup::Ptr& sequence_group,
...
bool Sampler::validate_candidate(Sequence::Ptr running_sequence,
                                 size_t& token_idx,
                                 Token& sampled_token,
                                 bool& is_extend_sequence,
...
std::pair<size_t, std::set<std::string>> process_stop_strings(const std::set<std::string>& stop_strings,
                                                              Tokenizer& tokenizer) {
        ...
        for (size_t running_sequence_id = 0; running_sequence_id < num_running_sequences; ++running_sequence_id) {
            ...
            for (size_t i = 0; i <= num_tokens_to_process; ++i) {
                ...
                if (is_validation_mode_enabled && !is_extend_sequence) {
                    is_validation_passed = validate_candidate(running_sequences[running_sequence_id],
                                                              generated_seq_token_offset,
                                                              sampled_token,
                                                              is_extend_sequence,
                                                              assisting_pipeline_info.max_removed_tokens_per_request,
...
SamplerOutput Sampler::sample(const std::vector<SequenceGroup::Ptr>& sequence_groups,
    ...
    for (size_t sequence_group_id = 0, currently_processed_tokens = 0; sequence_group_id < sequence_groups.size();
        ...
        ov::Tensor sequence_group_logits(ov::element::f32,
        ...
        if (sequence_group->requires_sampling()) {
            ...
            sg_sampling_future_map[request_id] = m_thread_pool.submit(&Sampler::sample_from_sequence_group,
                                                                      this,
                                                                      sequence_group,
    ...
    for (auto& sequence_group : sequence_groups) {
        ...
        if (sg_sampling_future_map.find(request_id) != sg_sampling_future_map.end()) {
            ...
            sampler_output.m_dropped_sequences.insert(sampler_output.m_dropped_sequences.end(),
...
void Sampler::create_logit_processor(uint64_t request_id,
