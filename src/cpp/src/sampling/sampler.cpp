...
void Sampler::TreeSearcher::finalize_eagle2_candidates(SamplerOutput& sampler_output) {
    ...
    for (auto& indice : retrieve_indices) {
        ...
        auto sequence = find_candidate_sequence(m_sequence_group, indice, m_parameters.eagle_tree_params.tree_depth,
    m_past_generate_len); parent_2_num_childs_map[sequence->get_id()] += 1;
    ...
    for (size_t i = 0; i < retrieve_indices.size(); ++i) {
        ...
        // locate the candidate sequence
        auto sequence = find_candidate_sequence(m_sequence_group, path, m_parameters.eagle_tree_params.tree_depth,
    m_past_generate_len);
...
void Sampler::TreeSearcher::select_top_k(const ov::Tensor& logits,
    ...
    if (m_tree_layer_counter < m_parameters.eagle_tree_params.tree_depth + 1) {
        std::cout << "[SELECT_TOP_K DEBUG] Layer " << m_tree_layer_counter << " < depth+1 ("
                  << (m_parameters.eagle_tree_params.tree_depth + 1) << ") - continuing tree expansion" << std::endl;
        ...
        for (const Beam& beam : m_beams) {
            size_t num_childs = parent_2_num_childs_map[beam.m_sequence->get_id()];
            // reserve the sequence which has the group id as m_org_group_id
            if (num_childs == 0) {
        ...
        // child become parents
        m_beams = child_beams;
        std::cout << "[SELECT_TOP_K DEBUG] Updated m_beams with " << m_beams.size() << " child beams for next iteration"
                  << std::endl;
    } else {  // at this point, we already have the full candidate tree
        std::cout << "\n[SELECT_TOP_K DEBUG] ========== Tree Complete - Finalization ==========" << std::endl;
        std::cout << "[SELECT_TOP_K DEBUG] Tree layer counter: " << m_tree_layer_counter << " >= depth+1 ("
                  << (m_parameters.eagle_tree_params.tree_depth + 1) << ")" << std::endl;
        ...
        for (const auto& cand : final_candidates) {
            ...
            for (const auto& other_cand : final_candidates) {
                if (m_eagle2_candidate_graph->is_ancestor(other_cand.m_node_id, cand.m_node_id)) {
                    mask_row.push_back(1);
                } else {
                    mask_row.push_back(0);
                }
            }
        ...
        for (size_t t = 1; t < final_candidates.size(); ++t) {
            ...
            if (t <= 5 || t >= final_candidates.size() - 2) {  // Print first few and last few tokens
...
size_t Sampler::validate_tree_candidates(Sequence::Ptr& sequence,
    ...
    for (const auto& path : retrieve_indices) {
        ...
        for (const auto& index : path) {
            candidate_tokens.push_back(generated_ids[generated_len - num_tokens_to_validate - 1 + index]);
        }
        ...
        if (is_validation_mode_enabled && num_generated_tokens_to_validate > 0) {
            ...
            for (size_t running_sequence_id = 0; running_sequence_id < num_running_sequences; ++running_sequence_id) {
                ...
                for (size_t i = 0; i <= num_tokens_to_process; ++i) {
                    ...
                    auto logit_vector =
                        _get_logit_vector(sequence_group_logits, running_sequence_id, logit_token_offset);
                    logit_processor.apply(logit_vector);
                    ...
                    if (sampling_params.is_greedy_decoding()) {
                        ...
                        if (!is_validation_passed) {
                            float p_prime =
                                get_p_prime(running_sequence, sampled_token, generated_seq_token_offset + 1);
                            assisting_pipeline_info.max_removed_tokens_per_request =
                                std::max(assisting_pipeline_info.max_removed_tokens_per_request,
                                         generated_seq_token_offset);
