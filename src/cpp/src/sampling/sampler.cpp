...
void Sampler::_calibrate_logit_vector(Logits& logit_vector, Token& rejected_token) {
    if (logit_vector.is_vector_initialized()) {
        bool need_update = false;
        for (auto& logit_token : logit_vector.m_vector) {
            if (logit_token.m_index == rejected_token.m_index) {
                logit_token.m_log_prob = 0.0f;
                need_update = true;
                break;
            }
        }
        if (need_update) {
            float norm_sum = 0.0f;
            for (auto& logit_token : logit_vector.m_vector) {
                norm_sum += logit_token.m_log_prob;
            }
            for (auto& logit_token : logit_vector.m_vector) {
                logit_token.m_log_prob /= norm_sum;
            }
        }
    } else {
        // in case of non-initialized vector we need to normalize data buffer
        logit_vector.m_data[rejected_token.m_index] = 0.0f;
        float norm_sum = 0.0f;
        for (size_t i = 0; i < logit_vector.m_size; i++) {
            norm_sum += logit_vector.m_data[i];
        }
        for (size_t i = 0; i < logit_vector.m_size; i++) {
            logit_vector.m_data[i] /= norm_sum;
        }
    }
}
...
bool Sampler::validate_candidate(
    ...
    if (do_sample && has_real_probolities &&
        ...
        if (!is_eagle3_model) {
            ...
            float p_i = 0.0f; // save probability for target model
            if (logit.is_vector_initialized()) {
                for (auto& logit_token : logit.m_vector) {
                    if (logit_token.m_index == *it_token_id) {
                        p_i = logit_token.m_log_prob;
                        break;
                    }
                }
            } else {
                // should not arrive here??
                p_i = logit.m_data[*it_token_id];
            }
            float q_i = 1.0,
                    probability_ratio = p_i / q_i;
            ...
            float r_i = dist(rng_engine);
            r_i /= 100;
            is_candidate_accepted = 0.9f <= probability_ratio;
            if (is_candidate_accepted) {
                sampled_token.m_index = *it_token_id;
            } else {
                rejected_token.m_index = *it_token_id; // for p prime calibration
            }
        }
    } else {
        ...
        for (size_t running_sequence_id = 0; running_sequence_id < num_running_sequences; ++running_sequence_id) {
            ...
            for (size_t i = 0; i <= num_tokens_to_process; ++i) {
                ...
                if (sampling_params.is_greedy_decoding()) {
                    ...
                    if (!sampling_params.eagle_sample_mode || num_tokens_to_process == 0 || i == num_generated_tokens_to_validate) {
                    ...
                    if (!is_validation_passed) {
                        if (!sampling_params.eagle_sample_mode) {
                            ...
                            // get updated p prime and sample the bonus
                            auto logit_vector = _get_logit_vector(sequence_group_logits, running_sequence_id, logit_token_offset + 1);
                            // nomial sampling have issues if log already applied, e.g, temp = 1.0, but not vectorlized
                            // logit_processor.apply(logit_vector);
                            _calibrate_logit_vector(logit_vector, rejected_token);
                            // sample from the calibrated distribution
                            auto sampled_token_ids = _multinomial_sample(logit_vector, num_tokens_per_sequence);
                            sampled_token = sampled_token_ids.front();
                        }
                    }
                }
                if (!is_validation_mode_enabled && m_d2t) { // compute token offset for draft model in speculative sampling
                    ov::Tensor d2t_tensor = m_d2t->get_tensor_view();
                ...
                if (is_validation_mode_enabled && !is_extend_sequence) {
                    is_validation_passed = validate_candidate(running_sequences[running_sequence_id], generated_seq_token_offset, logit_vector,
                                                              sampled_token, rejected_token, is_extend_sequence, assisting_pipeline_info.max_removed_tokens_per_request,
                                                              sampling_params.do_sample, !sampling_params.is_prompt_lookup(), sampling_params.eagle_sample_mode);
