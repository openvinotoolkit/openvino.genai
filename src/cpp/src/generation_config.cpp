...
void GenerationConfig::validate() const {
    ...
    OPENVINO_ASSERT(
        stop_token_ids_it == stop_token_ids.end(),
        "'stop_token_ids' must be non-negative, but it contains a value ",
        *stop_token_ids_it
    );

    OPENVINO_ASSERT(
        !ignore_eos || max_new_tokens != SIZE_MAX || max_length != SIZE_MAX,
        "ignore_eos is true, in this case either 'max_new_tokens', or 'max_length' should be defined."
    );

    OPENVINO_ASSERT(
        eos_token_id != -1 || !stop_token_ids.empty() || !stop_strings.empty() || max_new_tokens != SIZE_MAX ||
            max_length != SIZE_MAX,
        "Either 'eos_token_id', or 'stop_token_ids', or 'stop_strings', or 'max_new_tokens', or 'max_length' should be "
        "defined."
    );

    OPENVINO_ASSERT(
        max_new_tokens > 0 || (max_new_tokens == 0 && echo),
    ...
    if (!is_beam_search()) {
        OPENVINO_ASSERT(
            frequency_penalty >= -2.0f && frequency_penalty <= 2.0f,
    ...
    if (is_beam_search()) {
        ...
        if (num_beam_groups > 1) {
            OPENVINO_ASSERT(
                diversity_penalty != 0.0f,
                "For grouped beam search 'diversity_penalty' should not be zero, otherwise it fallbacks to non-grouped "
                "beam search"
            );
        } else {
            OPENVINO_ASSERT(
                diversity_penalty == 0.0f,
                "For beam search 'diversity_penalty' is applicable only when grouped beam search is used, but got "
                "'num_beam_groups' == 1"
            );
        }
    } else {
    ...
    if (is_assisting_generation()) {
        OPENVINO_ASSERT(
            !is_beam_search() && num_return_sequences == 1,
            "Beam search and parallel sampling are not compatible with assistant generation"
        );
        OPENVINO_ASSERT(
            assistant_confidence_threshold == 0.0f || num_assistant_tokens == 0,
            "Parameters `assistant_confidence_threshold` and `num_assistant_tokens` are mutually exclusive in "
            "`GenerationConfig`"
        );
    }
...
void StructuredOutputConfig::validate() const {
    ...
    OPENVINO_ASSERT(
        registry.find(backend_name) != registry.end(),
