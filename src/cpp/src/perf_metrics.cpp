...
ov::genai::MeanStdPair calc_mean_and_std(const std::vector<ov::genai::MicroSeconds>& durations) {
    ...
    double sum_square_durations = std::accumulate(
        durations.begin(),
        durations.end(),
        0.0,
        [](const double& acc, const ov::genai::MicroSeconds& duration) -> double {
...
PerfMetrics PerfMetrics::operator+(const PerfMetrics& right) const {
    ...
    new_inference_durations
        .insert(new_inference_durations.end(), right_inference_durations.begin(), right_inference_durations.end());
    new_token_infer_durations.insert(
        new_token_infer_durations.end(),
        right_token_infer_durations.begin(),
        right_token_infer_durations.end()
    );
    new_times_to_first_token
        .insert(new_times_to_first_token.end(), right_times_to_first_token.begin(), right_times_to_first_token.end());
    ...
    new_grammar_compile_times.insert(
        new_grammar_compile_times.end(),
