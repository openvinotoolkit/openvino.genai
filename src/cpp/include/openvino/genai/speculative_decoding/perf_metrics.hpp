 ...
 * @param num_draft_tokens the number of draft candidate tokens offered for validation.
 *
 */
struct OPENVINO_GENAI_EXPORTS SDPerModelsPerfMetrics : public ov::genai::SDPerfMetrics {
    ...
    size_t num_draft_tokens; // num draft candidate tokens offered for validation.

    SDPerModelsPerfMetrics();

    /**
    * @return The number of draft-model tokens accepted by the main model.
    *         This value is not equal to the number of generated tokens, since the main model also generates its own token on each inference.
    *         Thus, the total number of generated tokens is the sum of accepted draft-model tokens and the main model's own token.
    */
    size_t get_num_accepted_tokens();

    /**
    * @return The number of draft candidate tokens offered for validation by the main model.
    *         This differs from draft_model_metrics.get_num_generated_tokens(), which reports draft-model
    *         execution work and may include non-candidate processing such as KV/cache alignment.
    */
    size_t get_num_draft_tokens();

    /**
    * @return Draft-model execution work in generated/processed tokens.
    ...
    */
    ...
    /**
    * @return The number of draft candidate tokens rejected by the main model.
    */
    size_t get_num_rejected_tokens();

    /**
    * @return Accepted draft candidate tokens divided by draft candidate tokens.
    *         The value is in the [0, 1] range. Returns NaN when no draft candidate tokens were recorded.
    */
    float get_draft_acceptance_rate();

    /**
    * @return Draft-model execution tokens divided by draft candidate tokens.
    *         Returns NaN when no draft candidate tokens were recorded.
    */
    float get_draft_processed_to_candidate_ratio();

    /**
    * @return Draft-model inference duration divided by main-model inference duration.
