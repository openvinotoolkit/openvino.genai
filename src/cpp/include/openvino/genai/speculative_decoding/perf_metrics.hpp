 ...
 * @param avg_latency Mean and standard deviation of the latency from the third token in milliseconds per inference,
 * which includes also prev and post processing. The latency of first and second tokens can be obtained with methods
 * get_ttft() and get_ttst(). First and second tokens take longer than next tokens, since the first token includes
 * prompt analysis, and they both can include additional memory and compilation manipulations, so let's exclude it from
 * total statistic.
 *
 * Additional points:
...
struct OPENVINO_GENAI_EXPORTS SDPerfMetrics : public ov::genai::ExtendedPerfMetrics {
    ...
    ov::genai::MeanStdPair avg_latency;  // Latency from the third token (in ms), first and second token time is
                                         // presented separately as ttft and ttst.
...
struct OPENVINO_GENAI_EXPORTS SDPerModelsPerfMetrics : public ov::genai::SDPerfMetrics {
     ...
     *        This value is not equal to the number of generated tokens, since the main model generates one own tokens
     * on each inference. Thus, number of generated tokens = SUM(number of accepted tokens(generated by draft model and
     * accepted by main model) + 1 token(generated by main model))
     */
