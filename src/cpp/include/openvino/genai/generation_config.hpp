 ...
 *        NOTE: ContinuousBatching backend uses `num_assistant_tokens` as is. Stateful backend uses `num_assistant_tokens`'s copy as initial value and adjusts it
 *        based on recent number of accepted tokens.
