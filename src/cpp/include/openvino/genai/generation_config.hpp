...
/**
 * @brief controls the type of assisted generation. The following values are possible:
 *        "SPECULATIVE_DECODING" matches candidates based on https://arxiv.org/pdf/2211.17192.
          "ASSISTANT_GENERATION" matches candidate ans sampled token exactly.
          "NEVER" Disable speculative decoding case.
 */
enum class CandidatesMathingType { SPECULATIVE_DECODING, ASSISTANT_GENERATION, NONE };
 ...
 * @param candidates_matching_type defines the matching type of candidates by draft model with sampled token by main model.
...
public:
    ...
    CandidatesMathingType candidates_matching_type = CandidatesMathingType::NONE;
