...
// ov::genai::StreamingStatus flag, RUNNING means continue generation, STOP means stop generation, CANCEL means stop
// generation and remove last prompt and answer from history
using StreamerVariant =
    std::variant<std::function<StreamingStatus(std::string)>, std::shared_ptr<StreamerBase>, std::monostate>;
...
public:
     ...
     */
    LLMPipeline(const std::filesystem::path& models_path, const std::string& device, const ov::AnyMap& properties = {});

    LLMPipeline(const std::string& model_str,
    ...
    LLMPipeline(const std::filesystem::path& models_path, const std::string& device, Properties&&... properties)
     ...
     */
    LLMPipeline(const ov::InferRequest& request,
                const ov::genai::Tokenizer& tokenizer,
     ...
     */
    LLMPipeline(const std::filesystem::path& models_path,
                const ov::genai::Tokenizer& tokenizer,
                const std::string& device,
                const ov::AnyMap& properties = {});
     ...
     * chat_template will be applied to the prompt, run pipe.get_tokenizer().set_chat_template(custom_chat_template) to
     * update it. To disable it for non-chat mode, please, use custom_chat_template eq "" or set
     ...
     */
    template <typename... Properties>
    util::EnableIfAllStringAny<DecodedResults, Properties...> generate(StringInputs inputs,
    ...
    DecodedResults operator()(StringInputs inputs,
                              OptionalGenerationConfig generation_config = std::nullopt,
                              StreamerVariant streamer = std::monostate()) {
    ...
    template <typename... Properties>
    util::EnableIfAllStringAny<DecodedResults, Properties...> operator()(StringInputs inputs,
                                                                         Properties&&... properties) {
        return generate(inputs, AnyMap{std::forward<Properties>(properties)...});
    }
     ...
     * @return DecodedResults decoded resulting text
     *
     * Chat template will be applied to the prompt, run `pipe.get_tokenizer().set_chat_template(custom_chat_template)`
     * to update it. To disable chat template set `generation_config.apply_chat_template` to `false`.
     */
     ...
     */
    template <typename... Properties>
    util::EnableIfAllStringAny<DecodedResults, Properties...> generate(const ChatHistory& history,
                                                                       Properties&&... properties) {
    ...
    DecodedResults operator()(const ChatHistory& history,
                              OptionalGenerationConfig generation_config = std::nullopt,
                              StreamerVariant streamer = std::monostate()) {
    ...
    template <typename... Properties>
    util::EnableIfAllStringAny<DecodedResults, Properties...> operator()(const ChatHistory& history,
                                                                         Properties&&... properties) {
        return generate(history, AnyMap{std::forward<Properties>(properties)...});
    }
     ...
     */
    template <typename... Properties>
    util::EnableIfAllStringAny<EncodedResults, Properties...> generate(const EncodedInputs& inputs,
                                                                       Properties&&... properties) {
        return generate(inputs, AnyMap{std::forward<Properties>(properties)...});
    }
     ...
     */
    OPENVINO_DEPRECATED("start_chat() / finish_chat() API is deprecated and will be removed in the next major release. "
...
OPENVINO_GENAI_EXPORTS std::pair<std::string, Any> draft_model(const std::filesystem::path& models_path,
                                                               const std::string& device = {},
                                                               const ov::AnyMap& properties = {});

template <typename... Properties, typename std::enable_if<ov::util::StringAny<Properties...>::value, bool>::type = true>
inline std::pair<std::string, Any> draft_model(const std::filesystem::path& models_path,
                                               const std::string& device,
                                               Properties&&... properties) {
...
template <typename... Properties, typename std::enable_if<ov::util::StringAny<Properties...>::value, bool>::type = true>
inline std::pair<std::string, Any> draft_model(const std::filesystem::path& models_path, Properties&&... properties) {
...
/**
 * @brief enable enable_save_ov_model property serves to serialize ov model (xml/bin) generated from gguf model on disk
 * for re-use. Set `true` to activate this mode. And create LLMPipeline instance with this config.
 */
