...
public:
     ...
     *
     * @param expect_open_tag If true then open_tag is expected to be generated, if false then it's already part of the
     * model input string
 ...
 *
 * DeepSeekR1ReasoningParser is configured for the DeepSeek R1 model's reasoning format, which doesn't expect an opening
 * tag.
 */
 ...
 *
 * Phi4ReasoningParser is configured specifically for the Phi-4 model's reasoning format, which expects an opening tag
 * by default.
 */
 ...
 * in Llama 3's JSON style, e.g. {"type": "function", "function": {"name": "get_weather", "parameters": {"location":
 * "New York, NY", ...}}}. It does not modify the original content, only extracts and adds tool call information to the
 * message.
 */
...
public:
     ...
     * @param delta_tokens Optional vector of new token IDs to be processed in case if more fast token-based processing
     * is needed.
     * @return std::string filtered text that should be added to the 'content'
     */
    virtual std::string parse(JsonContainer& delta_message,
                              std::string& delta_text,
                              const std::optional<std::vector<int64_t>>& delta_tokens = std::nullopt) = 0;
...
public:
     ...
     * @param expect_open_tag If true then open_tag is expected to be generated, if false then it's already part of the
     * model input string
     * @param keep_original_content If true then original 'content' is preserved, otherwise reasoning text is removed
     * from 'content'
     * @param open_tag The opening tag (default: "<think>")
     * @param close_tag The closing tag (default: "</think>")
     */
     ...
     * @return std::string filtered text that should be added to the 'content'
     */
    std::string parse(JsonContainer& delta_message,
                      std::string& delta_text,
                      const std::optional<std::vector<int64_t>>& delta_tokens = std::nullopt) override;
 ...
 *
 * DeepSeekR1ReasoningIncrementalParser is configured for the DeepSeek R1 model's reasoning format, which doesn't expect
 * an opening tag.
 */
 ...
 *
 * Phi4ReasoningIncrementalParser is configured specifically for the Phi-4 model's reasoning format, which expects an
 * opening tag by default.
 */
