...
#include "openvino/core/any.hpp"  // util::EnableIfAllStringAny
#include "openvino/genai/chat_history.hpp"
#include "openvino/genai/generation_config.hpp"
#include "openvino/genai/llm_pipeline.hpp"  // StreamerVariant
...
public:
    ...
    /// @param vlm Backing VLM pipeline base. Shared ownership allows independent lifetime management.
    /// @param talker Backing speech generator. The default impl is `Talker`.
    OmniPipeline(const std::shared_ptr<VLMPipeline::VLMBackend>& vlm, const std::shared_ptr<TalkerBase>& talker);
    ...
    /// @brief Return the underlying VLM base. Useful for inspecting model metadata.
    /// @note The returned type is an internal implementation detail. Use the public
    /// VLMPipelineBase interface methods only.
    std::shared_ptr<VLMPipeline::VLMBackend> get_vlm() const;
