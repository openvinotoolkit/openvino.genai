...
struct SchedulerConfig {
    ...
    // (in contrast to max_batch_size which combines independent sequences, we consider total amount of tokens in a
    // batch)
    // TODO: benchmark this value and understand a required value to ensure inference is not memory bound
    // When ContinuousBatching is invoked from LLMPipeline (client scenario) by default max_num_batched_tokens is not
    // limited.
    ...
    // You can think of it as "max batch size" on generate phase, on prompt phase number of scheduled tokens usually
    // more than number of sequences.
    ...
    bool operator==(const SchedulerConfig& other) const {
        return max_num_batched_tokens == other.max_num_batched_tokens && num_kv_blocks == other.num_kv_blocks &&
               cache_size == other.cache_size && dynamic_split_fuse == other.dynamic_split_fuse &&
               use_cache_eviction == other.use_cache_eviction && max_num_seqs == other.max_num_seqs &&
               enable_prefix_caching == other.enable_prefix_caching;
    }
