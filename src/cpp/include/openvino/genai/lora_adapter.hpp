...
public:
    ...
    inline std::pair<std::string, ov::Any>
    operator()(const Adapter& adapter, float alpha, AdapterConfig::Mode mode) const {
    ...
    template <typename AT, typename std::enable_if<std::is_constructible<Adapter, AT>::value, bool>::type = true>
    inline std::pair<std::string, ov::Any>
    operator()(const std::initializer_list<AT>& adapters, AdapterConfig::Mode mode) const {
        return operator()(AdapterConfig(adapters, mode));
    }

    inline std::pair<std::string, ov::Any> operator()(
        const std::initializer_list<std::pair<Adapter, float>>& adapters
    ) const {
        return operator()(AdapterConfig(adapters));
    }

    inline std::pair<std::string, ov::Any>
    operator()(const std::initializer_list<std::pair<Adapter, float>>& adapters, AdapterConfig::Mode mode) const {
        return operator()(AdapterConfig(adapters, mode));
    }
    ...
    inline std::pair<std::string, ov::Any>
    operator()(const std::vector<Adapter>& adapters, AdapterConfig::Mode mode) const {
        return operator()(AdapterConfig(adapters, mode));
    }
    ...
    inline std::pair<std::string, ov::Any>
    operator()(const std::vector<std::pair<Adapter, float>>& adapters, AdapterConfig::Mode mode) const {
        return operator()(AdapterConfig(adapters, mode));
    }
};
