...
struct OPENVINO_GENAI_EXPORTS AdapterConfig {
    enum Mode {
        ...
        MODE_STATIC_RANK,  // A and B have static shape, alpha is variable // FIXME: WA to unlock experiments, gives a
                           // unique perf level
        MODE_STATIC,       // A, B and alpha are constants. Use instead of MODE_FUSE if preserving weights precision is
                           // required at the cost of inference time
    ...
    // Methods to get and set optional name prefix to filter tensor names in LoRA adapter file applicable to a
    // particular model. The prefix can be set at the user level or at a particular GenAI pipeline level. Usually GenAI
    // pipelines should set the prefix appropriately, and no need to be worried from user side. But if the user has
    // non-standard adapter file where the default prefix doesn't work, in this case user should set the prefix. If the
    // prefix is set at the user side, it is not overridden by the pipeline logic. Use nullopt to indicate that the
    // prefix is not set from the user side and let a particular GenAI pipeline set the default value. The default value
    // is nullopt.
    const std::optional<std::string>& get_tensor_name_prefix() const {
        return tensor_name_prefix;
    }
    void set_tensor_name_prefix(const std::optional<std::string>& _tensor_name_prefix) {
        tensor_name_prefix = _tensor_name_prefix;
    }

    AdapterConfig(Mode mode = MODE_AUTO);

    AdapterConfig(const Adapter& adapter, float alpha, Mode mode = MODE_AUTO)
        : AdapterConfig(std::vector<std::pair<Adapter, float>>{{adapter, alpha}}, mode) {}
    ...
    AdapterConfig(const std::initializer_list<AT>& adapters, Mode mode = MODE_AUTO)
        : AdapterConfig(std::vector<Adapter>(adapters), mode) {}

    AdapterConfig(const std::initializer_list<std::pair<Adapter, float>>& adapters, Mode mode = MODE_AUTO)
        : AdapterConfig(std::vector<std::pair<Adapter, float>>(adapters), mode) {}
    ...
    // Update adapters and alphas from other config. Mode and tensor_name_prefix are updated if they are set not to
    // default values in other config. It means that if other.get_mode() == MODE_AUTO, it will not override value in
    // this config. If tensor_name_prefix is not set (== nullopt) then it won't be updated either.
...
public:
    ...
    inline std::pair<std::string, ov::Any> operator()(const Adapter& adapter,
                                                      float alpha,
                                                      AdapterConfig::Mode mode) const {
    ...
    template <typename AT, typename std::enable_if<std::is_constructible<Adapter, AT>::value, bool>::type = true>
    inline std::pair<std::string, ov::Any> operator()(const std::initializer_list<AT>& adapters,
                                                      AdapterConfig::Mode mode) const {
        return operator()(AdapterConfig(adapters, mode));
    }

    inline std::pair<std::string, ov::Any> operator()(
        const std::initializer_list<std::pair<Adapter, float>>& adapters) const {
        return operator()(AdapterConfig(adapters));
    }

    inline std::pair<std::string, ov::Any> operator()(const std::initializer_list<std::pair<Adapter, float>>& adapters,
                                                      AdapterConfig::Mode mode) const {
        return operator()(AdapterConfig(adapters, mode));
    }
    ...
    inline std::pair<std::string, ov::Any> operator()(const std::vector<Adapter>& adapters,
                                                      AdapterConfig::Mode mode) const {
        return operator()(AdapterConfig(adapters, mode));
    }
    ...
    inline std::pair<std::string, ov::Any> operator()(const std::vector<std::pair<Adapter, float>>& adapters,
                                                      AdapterConfig::Mode mode) const {
        return operator()(AdapterConfig(adapters, mode));
    }
};
...
public:
    ...
    // Apply adapters configured in the current config set last time, or set and use new config given as optional
    // `config` argument
