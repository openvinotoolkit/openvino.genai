...
public:
    ...
    /// @brief Get the attention backend used by the pipeline.
    /// @return "PA" for Paged Attention (continuous batching), "SDPA" for scaled dot-product attention.
    ...
    /// @brief Get the backing internal VLMBackend. Used by OmniPipeline's shared-VLM ctor
    /// to reuse an already-loaded VLM without reloading multi-GB weights.
    std::shared_ptr<VLMBackend> get_base() const {
        return m_pimpl;
    }
