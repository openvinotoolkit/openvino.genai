...
public:
    ...
    class VLMPipelineBase;  // internal Omni-aware base; defined in src/cpp/src/visual_language/pipeline_base.hpp.

    /// @brief Construct a backing internal VLMPipelineBase from a model directory.
    ...
    /// `VLMPipeline::VLMPipelineBase` (CB adapter or VLMPipelineImpl), which extends the
    ...
    /// @brief Get the backing internal VLMPipelineBase. Used by OmniPipeline's shared-VLM ctor
    /// to reuse an already-loaded VLM without reloading multi-GB weights.
