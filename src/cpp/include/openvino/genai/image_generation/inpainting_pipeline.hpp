...
public:
    ...
    static InpaintingPipeline stable_diffusion(const std::shared_ptr<Scheduler>& scheduler_type,
                                               const CLIPTextModel& clip_text_model,
                                               const UNet2DConditionModel& unet,
                                               const AutoencoderKL& vae);

    // creates either LCM or SD pipeline from building blocks
    static InpaintingPipeline latent_consistency_model(const std::shared_ptr<Scheduler>& scheduler_type,
                                                       const CLIPTextModel& clip_text_model,
                                                       const UNet2DConditionModel& unet,
                                                       const AutoencoderKL& vae);

    // creates SDXL pipeline from building blocks
    static InpaintingPipeline stable_diffusion_xl(const std::shared_ptr<Scheduler>& scheduler_type,
                                                  const CLIPTextModel& clip_text_model,
    ...
    static InpaintingPipeline flux(const std::shared_ptr<Scheduler>& scheduler,
                                   const CLIPTextModel& clip_text_model,
    ...
    // creates Flux pipeline from building blocks
    static InpaintingPipeline flux_fill(const std::shared_ptr<Scheduler>& scheduler,
                                        const CLIPTextModel& clip_text_model,
                                        const T5EncoderModel& t5_text_encoder,
                                        const FluxTransformer2DModel& transformer,
                                        const AutoencoderKL& vae);
    ...
    static InpaintingPipeline stable_diffusion_3(const std::shared_ptr<Scheduler>& scheduler,
