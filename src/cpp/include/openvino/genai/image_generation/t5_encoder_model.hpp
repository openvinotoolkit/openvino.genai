...
public:
    ...
    T5EncoderModel(const std::filesystem::path& root_dir, const std::string& device, const ov::AnyMap& properties = {});

    T5EncoderModel(const std::string& model, const Tensor& weights, const Tokenizer& tokenizer);
    ...
    T5EncoderModel(const std::filesystem::path& root_dir, const std::string& device, Properties&&... properties)
    ...
    template <typename... Properties>
    ov::util::EnableIfAllStringAny<T5EncoderModel&, Properties...> compile(const std::string& device,
                                                                           Properties&&... properties) {
