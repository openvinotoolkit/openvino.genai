...
OPENVINO_GENAI_EXPORTS std::pair<std::vector<std::vector<ov::Tensor>>, std::pair<size_t, size_t>> llava_image_embed_make_with_bytes_slice(struct clip_ctx * ctx_clip, const ov::Tensor& img, int max_slice_nums, int scale_resolution, int patch_size, bool never_split);
