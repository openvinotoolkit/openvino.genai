 ...
 *
 * Preverable way to access values is via get functions. Getters calculate mean and std values from raw_metrics are
 * return pairs. If mean and std were already calculated getters return cached values.
 ...
 * @param get_grammar_compiler_init_times Returns a map with the time to initialize the grammar compiler for each
 ...
 * @param inference_duration Mean and standard deviation of the time spent on model inference during generate call in
 * milliseconds.
