...
ov_status_e ov_genai_whisper_generation_config_get_word_timestamps(const ov_genai_whisper_generation_config* config,
                                                                   bool* word_timestamps) {
    if (!config || !(config->object) || !word_timestamps) {
        return ov_status_e::INVALID_C_PARAM;
    }
    try {
        *word_timestamps = config->object->word_timestamps;
    } catch (...) {
        return ov_status_e::UNKNOW_EXCEPTION;
    }
    return ov_status_e::OK;
