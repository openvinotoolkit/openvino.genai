...
function createBatchedTestImageTensor(height, width) {
  const tensor = createTestImageTensor(height, width);
  // The inpainting pipeline expects a batch dimension, even for single images.
  ...
  tensor.setShape([1, height, width, 3]);
  return tensor;
}
...
describe("InpaintingPipeline methods", { skip: os.platform() === "darwin" }, () => {
  let pipeline;
  let testImage;
  let testMask;
  ...
  it("generate(prompt, image, mask) returns image tensor", async () => {
    const result = await pipeline.generate("a tiny robot", testImage, testMask);
  ...
  it("generate(prompt, image, mask, generationConfig) honors width/height overrides", async () => {
    const result = await pipeline.generate("a tiny robot", testImage, testMask, {
      width: 128,
      height: 128,
    });
  ...
  it("generate(prompt, image, mask) rejects unbatched rank-3 image tensor", async () => {
    ...
    await assert.rejects(
      pipeline.generate("a tiny robot", unbatchedImage, testMask),
      /batched NHWC shape \[1, H, W, 3\]/,
    );
  });

  it("generate(prompt, image, mask) rejects unbatched rank-3 mask tensor", async () => {
    const unbatchedMask = createTestImageTensor(64, 64);
    assert.strictEqual(unbatchedMask.getShape().length, 3);
    await assert.rejects(
      pipeline.generate("a tiny robot", testImage, unbatchedMask),
      /batched NHWC shape \[1, H, W, 3\]/,
    );
  });

  it("generate(prompt, image, mask) rejects non-u8 image tensor", async () => {
    // The pipeline expects u8 tensors. This f32 tensor is used to verify rejection.
    const floatImage = new ov.Tensor("f32", [1, 64, 64, 3], new Float32Array(64 * 64 * 3));
    await assert.rejects(
      pipeline.generate("a tiny robot", floatImage, testMask),
      /u8 element type/,
    );
  });

  it("generate(prompt, image, mask) rejects non-u8 mask tensor", async () => {
    const floatMask = new ov.Tensor("f32", [1, 64, 64, 3], new Float32Array(64 * 64 * 3));
    await assert.rejects(
