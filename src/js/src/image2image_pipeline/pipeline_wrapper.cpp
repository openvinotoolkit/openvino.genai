...
struct Image2ImageTsfnContext {
    Image2ImageTsfnContext(std::string prompt,
                           ov::Tensor image,
    ...
    std::shared_ptr<std::atomic<bool>> is_busy;
    std::shared_ptr<std::atomic<bool>> is_generating;
    std::shared_ptr<ov::genai::Image2ImagePipeline> pipe = nullptr;
};

void image2image_perform_inference_thread(Image2ImageTsfnContext* context) {
    auto report_error = [context](const std::string& message) {
        auto status = context->callback_tsfn.BlockingCall([message](Napi::Env env, Napi::Function js_callback) {
            try {
                js_callback.Call({Napi::Error::New(env, "image2image_perform_inference_thread error. " + message).Value(),
                                  env.Null()});
            } catch (const std::exception& err) {
                std::cerr << "The callback failed when returning an error from image2image_perform_inference_thread. Details:\n"
                          << err.what() << std::endl;
        ...
        if (status != napi_ok) {
            std::cerr << "The BlockingCall failed with status " << status
                      << " when trying to return an error from image2image_perform_inference_thread." << std::endl;
            std::cerr << "Original error message:\n" << message << std::endl;
        }
    };
    ...
    try {
        ...
        ov::Tensor result = context->pipe->generate(context->prompt, context->image, context->generation_properties);
        context->is_busy->store(false);
...
Image2ImagePipelineWrapper::Image2ImagePipelineWrapper(const Napi::CallbackInfo& info)
    : Napi::ObjectWrap<Image2ImagePipelineWrapper>(info) {}

Napi::Function Image2ImagePipelineWrapper::get_class(Napi::Env env) {
    return DefineClass(env,
                       "Image2ImagePipeline",
                       {
                           InstanceMethod("init", &Image2ImagePipelineWrapper::init),
                           InstanceMethod("generate", &Image2ImagePipelineWrapper::generate),
                           InstanceMethod("decode", &Image2ImagePipelineWrapper::decode),
                           InstanceMethod("getPerformanceMetrics",
                                          &Image2ImagePipelineWrapper::get_performance_metrics),
                           InstanceMethod("getGenerationConfig", &Image2ImagePipelineWrapper::get_generation_config),
                           InstanceMethod("setGenerationConfig", &Image2ImagePipelineWrapper::set_generation_config),
                       });
}

Napi::Value Image2ImagePipelineWrapper::init(const Napi::CallbackInfo& info) {
    auto env = info.Env();
    try {
        ...
        auto async_worker = new Image2ImageInitWorker(callback,
                                                      this->pipe,
...
Napi::Value Image2ImagePipelineWrapper::generate(const Napi::CallbackInfo& info) {
    auto env = info.Env();
    try {
        OPENVINO_ASSERT(this->pipe, "Image2ImagePipeline is not initialized");
        ...
        OPENVINO_ASSERT(image_shape.size() == 4 && image_shape[0] == 1 && image_shape[3] == 3,
                        ...
                        image_shape,
                        ".");
        auto generation_properties = js_to_cpp<ov::AnyMap>(env, info[2]);
        Napi::Function callback = info[4].As<Napi::Function>();

        auto* context = new Image2ImageTsfnContext(std::move(prompt),
                                                   std::move(image),
        ...
        context->callback_tsfn =
            Napi::ThreadSafeFunction::New(env, callback, "Image2Image_generate_callback", 0, 1, [context](Napi::Env) {
        ...
        context->native_thread = std::thread(image2image_perform_inference_thread, context);
    } catch (const std::exception& ex) {
...
Napi::Value Image2ImagePipelineWrapper::decode(const Napi::CallbackInfo& info) {
    auto env = info.Env();
    try {
        OPENVINO_ASSERT(this->pipe, "Image2ImagePipeline is not initialized");
        ...
        this->is_busy->store(true);
        auto* async_worker =
            new ImageDecodeWorker<ov::genai::Image2ImagePipeline>(callback, this->pipe, std::move(latent), this->is_busy);
        async_worker->Queue();
    } catch (const std::exception& ex) {
        Napi::Error::New(env, ex.what()).ThrowAsJavaScriptException();
    }
...
Napi::Value Image2ImagePipelineWrapper::get_generation_config(const Napi::CallbackInfo& info) {
    auto env = info.Env();
    try {
        OPENVINO_ASSERT(this->pipe, "Image2ImagePipeline is not initialized");
...
Napi::Value Image2ImagePipelineWrapper::set_generation_config(const Napi::CallbackInfo& info) {
    auto env = info.Env();
    try {
        OPENVINO_ASSERT(this->pipe, "Image2ImagePipeline is not initialized");
...
Napi::Value Image2ImagePipelineWrapper::get_performance_metrics(const Napi::CallbackInfo& info) {
    auto env = info.Env();
    try {
        OPENVINO_ASSERT(this->pipe, "Image2ImagePipeline is not initialized");
