...
ov::Any js_set_to_any(const Napi::Env& env, const Napi::Value& value) {
    ...
    OPENVINO_THROW(
        "Cannot convert Set to ov::Any. " + std::string(first_element_type.name()) + " is not a supported element type."
    );
}
...
ov::Any js_array_to_any(const Napi::Env& env, const Napi::Array& array) {
    ...
    OPENVINO_THROW(
        "Cannot convert Array to ov::Any. " + std::string(first_element_type.name()) +
...
ov::genai::StopCriteria js_to_cpp<ov::genai::StopCriteria>(const Napi::Env& env, const Napi::Value& value) {
    ...
    default:
        OPENVINO_THROW(
            "Invalid stop criteria: " + std::to_string(num) + ". Expected 0 (EARLY), 1 (HEURISTIC), or 2 (NEVER)."
        );
    }
}
...
ov::Tensor js_to_cpp<ov::Tensor>(const Napi::Env& env, const Napi::Value& value) {
    ...
    OPENVINO_ASSERT(
        get_external_tensor_val.IsFunction(),
...
ov::genai::PerfMetrics& unwrap<ov::genai::PerfMetrics>(const Napi::Env& env, const Napi::Value& value) {
    ...
    OPENVINO_ASSERT(
        obj.InstanceOf(prototype.Value().As<Napi::Function>()),
...
template <>
Napi::Value
cpp_to_js<ov::genai::StopCriteria, Napi::Value>(const Napi::Env& env, const ov::genai::StopCriteria& value) {
...
Napi::Value cpp_to_js<ov::Tensor, Napi::Value>(const Napi::Env& env, const ov::Tensor& tensor) {
    try {
        ...
        auto external = Napi::External<ov::Tensor>::New(
            env,
...
Napi::Value cpp_to_js<ov::genai::StructuredOutputConfig::StructuralTag, Napi::Value>(
    ...
    return std::visit(
        overloaded{
            [&env](const std::string& s) -> Napi::Value {
...
Napi::Function get_prototype_from_ov_addon(const Napi::Env& env, const std::string& ctor_name) {
    ...
    OPENVINO_ASSERT(
        !ov_addon.IsUndefined() && !ov_addon.IsNull() && ov_addon.IsObject(),
