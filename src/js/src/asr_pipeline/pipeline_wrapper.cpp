...
struct ASRTsfnContext {
    ASRTsfnContext(std::vector<float> raw_speech,
    ...
    ov::AnyMap generation_config;
    std::shared_ptr<std::atomic<bool>> is_generating;
    std::shared_ptr<ov::genai::ASRPipeline> pipe = nullptr;
};

void asrPerformInferenceThread(ASRTsfnContext* context) {
    auto report_error = [context](const std::string& message) {
        auto status = context->callback_tsfn.BlockingCall([message](Napi::Env env, Napi::Function js_callback) {
            try {
                js_callback.Call(
                    {Napi::Error::New(env, "asrPerformInferenceThread error. " + message).Value(), env.Null()});
            } catch (const std::exception& err) {
                std::cerr << "The callback failed when attempting to return an error from asrPerformInferenceThread. "
                             "Details:\n"
                          << err.what() << std::endl;
        ...
        if (status != napi_ok) {
            std::cerr << "The BlockingCall failed with status " << status
                      << " when trying to return an error from asrPerformInferenceThread." << std::endl;
            std::cerr << "Original error message:\n" << message << std::endl;
        }
    };
    ...
    try {
        ...
        // Move the raw speech into AudioInputs once. Passing the std::vector<float> directly would
        // trigger an implicit std::vector<float> -> AudioInputs (std::variant) conversion that copies
        // the whole audio buffer on every generate call.
        ov::genai::AudioInputs audio_inputs{std::move(context->raw_speech)};
        if (context->streamer_tsfn.has_value()) {
            auto config = context->pipe->get_generation_config();
            config.update_generation_config(context->generation_config);
            result = context->pipe->generate(audio_inputs, config, streamer_var);
        } else {
            result = context->pipe->generate(audio_inputs, context->generation_config);
        }
    ...
    *context->is_generating = false;
    ...
    try {
        if (!streamer_exceptions.empty()) {
            ...
            std::shared_ptr<ov::genai::ASRDecodedResults> final_result =
                std::make_shared<ov::genai::ASRDecodedResults>(std::move(result));
            ...
            napi_status status = context->callback_tsfn.BlockingCall(
                [final_result, final_callback_error](Napi::Env env, Napi::Function js_callback) {
                    try {
                        js_callback.Call({env.Null(), to_asr_decoded_result(env, *final_result)});
                    } catch (const std::exception& err) {
...
ASRPipelineWrapper::ASRPipelineWrapper(const Napi::CallbackInfo& info) : Napi::ObjectWrap<ASRPipelineWrapper>(info) {}

Napi::Function ASRPipelineWrapper::get_class(Napi::Env env) {
    return DefineClass(env,
                       "ASRPipeline",
                       {
                           InstanceMethod("init", &ASRPipelineWrapper::init),
                           InstanceMethod("generate", &ASRPipelineWrapper::generate),
                           InstanceMethod("getTokenizer", &ASRPipelineWrapper::get_tokenizer),
                           InstanceMethod("getGenerationConfig", &ASRPipelineWrapper::get_generation_config),
                           InstanceMethod("setGenerationConfig", &ASRPipelineWrapper::set_generation_config),
                       });
}

Napi::Value ASRPipelineWrapper::init(const Napi::CallbackInfo& info) {
    auto env = info.Env();
    try {
        ...
        *this->is_initializing = true;
        ...
        auto async_worker = new ASRInitWorker(callback,
                                              this->pipe,
...
Napi::Value ASRPipelineWrapper::generate(const Napi::CallbackInfo& info) {
    auto env = info.Env();
    try {
        OPENVINO_ASSERT(this->pipe, "ASRPipeline is not initialized");
        ...
        *this->is_generating = true;
        ...
        auto* context = new ASRTsfnContext(std::move(raw_speech), std::move(generation_config), this->is_generating);
        ...
        if (info[2].IsFunction()) {
            context->streamer_tsfn =
                Napi::ThreadSafeFunction::New(env, info[2].As<Napi::Function>(), "ASR_generate_streamer", 0, 1);
        }

        context->native_thread = std::thread(asrPerformInferenceThread, context);
    } catch (const std::exception& ex) {
...
Napi::Value ASRPipelineWrapper::get_tokenizer(const Napi::CallbackInfo& info) {
    auto env = info.Env();
    try {
        OPENVINO_ASSERT(this->pipe, "ASRPipeline is not initialized");
...
Napi::Value ASRPipelineWrapper::get_generation_config(const Napi::CallbackInfo& info) {
    auto env = info.Env();
    try {
        OPENVINO_ASSERT(this->pipe, "ASRPipeline is not initialized");
        auto config = this->pipe->get_generation_config();
        return cpp_to_js<ov::genai::ASRGenerationConfig, Napi::Value>(env, config);
    } catch (const std::exception& ex) {
        Napi::Error::New(env, ex.what()).ThrowAsJavaScriptException();
        return env.Undefined();
    }
...
Napi::Value ASRPipelineWrapper::set_generation_config(const Napi::CallbackInfo& info) {
    auto env = info.Env();
    try {
        OPENVINO_ASSERT(this->pipe, "ASRPipeline is not initialized");
        ...
        // Merge the provided fields onto the current config so partial updates keep required fields
        // (e.g. stopping criteria) that ASRGenerationConfig::validate() expects.
        auto config = this->pipe->get_generation_config();
        config.update_generation_config(js_to_cpp<ov::AnyMap>(env, info[0]));
