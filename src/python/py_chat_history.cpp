...
void init_chat_history(py::module_& m) {
    py::class_<ChatHistory>(m, "ChatHistory", class_docstring)
        ...
        .def(
            py::init([](const py::list& messages) {
                JsonContainer history = pyutils::py_object_to_json_container(messages);
                return ChatHistory(history);
            }),
            py::arg("messages"),
            R"(Create chat history from a list of message dicts.)"
        )

        .def(
            "get_messages",
            [](const ChatHistory& self) -> py::list {
        ...
        .def(
            "set_messages",
            [](ChatHistory& self, const py::list& messages) {
                self.get_messages() = pyutils::py_object_to_json_container(messages);
            },
            py::arg("messages"),
            R"(Replace all messages with a new list.)"
        )

        .def(
            "append",
            [](ChatHistory& self, const py::dict& message) {
                JsonContainer message_jc = pyutils::py_object_to_json_container(message);
                self.push_back(message_jc);
            },
            py::arg("message"),
            R"(Add a message to the end of chat history.)"
        )

        .def(
            "pop",
            [](ChatHistory& self) -> py::dict {
                if (self.empty()) {
        ...
        .def(
            "set_tools",
            [](ChatHistory& self, const py::list& tools) {
                self.set_tools(pyutils::py_object_to_json_container(tools));
            },
            py::arg("tools"),
            R"(Set the tools definitions array.)"
        )
        ...
        .def(
            "set_extra_context",
            [](ChatHistory& self, const py::dict& extra_context) {
                self.set_extra_context(pyutils::py_object_to_json_container(extra_context));
            },
            py::arg("extra_context"),
            R"(Set the extra context object.)"
        )

        .def(
            "get_extra_context",
            [](const ChatHistory& self) -> py::dict {
