...
PYBIND11_MODULE(py_generate_pipeline, m) {
    ...
    py::class_<LLMPipeline>(m, "LLMPipeline", "This class is used for generation with LLMs")
        .def(py::init([](
            ...
            properies.insert(kwargs_properies.begin(), kwargs_properies.end());
            return std::make_unique<LLMPipeline>(model_path, device, properies);
        }),
        ...
        .def(py::init([](
            ...
            auto kwargs_properies = utils::kwargs_to_any_map(kwargs);
            properies.insert(kwargs_properies.begin(), kwargs_properies.end());
            return std::make_unique<LLMPipeline>(model_path, tokenizer, device, properies);
        }),
        py::arg("model_path"),
        py::arg("tokenizer"),
        py::arg("device") = "CPU",
