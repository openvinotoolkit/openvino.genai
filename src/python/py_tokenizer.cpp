...
void init_tokenizer(py::module_& m) {
    ...
    py::class_<ov::genai::Tokenizer>(m, "Tokenizer", class_docstring)
        ...
        .def(py::init([](const std::filesystem::path& tokenizer_path,
        ...
        .def(py::init([](const std::string& tokenizer_model,
                         const ov::Tensor& tokenizer_weights,
                         const std::string& detokenizer_model,
                         const ov::Tensor& detokenizer_weights,
                         const py::kwargs& kwargs) {
                 ScopedVar env_manager(pyutils::ov_tokenizers_module_path());
                 auto kwargs_properties = pyutils::kwargs_to_any_map(kwargs);

                 return std::make_unique<ov::genai::Tokenizer>(tokenizer_model,
                                                               tokenizer_weights,
                                                               detokenizer_model,
                                                               detokenizer_weights,
                                                               kwargs_properties);
             }),
             py::arg("tokenizer_model"),
             py::arg("tokenizer_weights"),
             py::arg("detokenizer_model"),
             py::arg("detokenizer_weights"))
               ...
               std::optional<std::string> padding_side) {
                ...
                for (auto item : prompts) {
                    if (!py::isinstance<py::list>(item) || py::len(item) != 2) {
                        throw std::runtime_error("Expected a list of lists with sizes 2. E.g. [[\"What is the capital "
                                                 "of GB?\", \"London in the capital of GB\"], ...]");
                    }
        ...
        .def(
            ...
            [](Tokenizer& tok,
               ...
               bool skip_special_tokens) -> py::typing::List<py::str> {
                ov::AnyMap detokenization_params;
                detokenization_params[ov::genai::skip_special_tokens.name()] = skip_special_tokens;
                return pyutils::handle_utf8(tok.decode(tokens, detokenization_params));
            },
        ...
        .def(
            ...
            [](Tokenizer& tok,
               ...
               const std::optional<py::dict>& extra_context) {
                ...
                std::visit(pyutils::overloaded{[&](ChatHistory chat_history_obj) {
                                                   chat_history = chat_history_obj;
                                               },
                                               [&](const std::vector<py::dict>& list_of_dicts) {
                                                   chat_history = ChatHistory(
                                                       pyutils::py_object_to_json_container(py::cast(list_of_dicts)));
                                               }},
                           history);
                ...
                return tok.apply_chat_template(chat_history,
        ...
        .def("set_chat_template",
             ...
             py::arg("chat_template"),
             "The new template to override with.",
             "Override a chat_template read from tokenizer_config.json.")

        .def_property("chat_template", &Tokenizer::get_chat_template, &Tokenizer::set_chat_template)
