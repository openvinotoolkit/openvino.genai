...
void init_tokenizer(py::module_& m) {
    ...
    py::class_<ov::genai::Tokenizer>(m, "Tokenizer", class_docstring)
        ...
        .def(
            py::init([](const std::filesystem::path& tokenizer_path,
        ...
        .def(
            py::init([](const std::string& tokenizer_model,
                ...
                return std::make_unique<ov::genai::Tokenizer>(
                    tokenizer_model,
                    tokenizer_weights,
                    detokenizer_model,
                    detokenizer_weights,
                ...
                for (auto item : prompts) {
                    if (!py::isinstance<py::list>(item) || py::len(item) != 2) {
                        throw std::runtime_error(
                            "Expected a list of lists with sizes 2. E.g. [[\"What is the capital of GB?\", \"London in "
                            "the capital of GB\"], ...]"
                        );
                    }
