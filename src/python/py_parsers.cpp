...
// and we need to convert JsonContainer to py::dict and then update back JsonContainer from the py::dict which was
// modified in Python.
...
public:
    ...
    void reset() override {
        PYBIND11_OVERLOAD_PURE(void, IncrementalParser, reset, );
    }
};
...
public:
    ...
    VLLMParserWrapper(py::object py_parser) {
        ...
        if (py::hasattr(py_parser, "extract_tool_calls")) {
            m_parsers.push_back([py_parser](const std::string& content) -> JsonContainer {
                ...
                if (py::hasattr(parsed, "model_dump_json")) {
                    return JsonContainer::from_json_string(parsed.attr("model_dump_json")().cast<std::string>());
        ...
        OPENVINO_ASSERT(!m_parsers.empty(),
                        "Provided vLLM parser does not have supported parsing methods: 'extract_tool_calls' or "
                        "'extract_reasoning'");
    }
...
void init_parsers(py::module_& m) {
    ...
    py::class_<DeepSeekR1ReasoningParser, std::shared_ptr<DeepSeekR1ReasoningParser>, ReasoningParser>(
        m,
    ...
    py::class_<Llama3PythonicToolParser, std::shared_ptr<Llama3PythonicToolParser>, Parser>(m,
                                                                                            "Llama3PythonicToolParser")
        .def(py::init<>());

    py::class_<IncrementalParser, ConstructableIncrementalParser, std::shared_ptr<IncrementalParser>>(
        m,
        "IncrementalParser")
        .def(py::init<>())
        .def(
            "parse",
            [](IncrementalParser& self,
            ...
            py::arg("delta_message"),
            py::arg("delta_text"),
            py::arg("delta_tokens") = std::nullopt,
            "Parse is called every time new text delta is decoded. Returns a string with any additional text to append "
            "to the current output.")
        .def("reset", &IncrementalParser::reset, "Reset the internal state of the parser.");

    py::class_<ReasoningIncrementalParser, std::shared_ptr<ReasoningIncrementalParser>, IncrementalParser>(
        m,
    ...
    py::class_<Phi4ReasoningIncrementalParser, std::shared_ptr<Phi4ReasoningIncrementalParser>, IncrementalParser>(
        m,
        "Phi4ReasoningIncrementalParser")
        .def(py::init<>());

    py::class_<DeepSeekR1ReasoningIncrementalParser,
               std::shared_ptr<DeepSeekR1ReasoningIncrementalParser>,
               IncrementalParser>(m, "DeepSeekR1ReasoningIncrementalParser")
        .def(py::init<>());
