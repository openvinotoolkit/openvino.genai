...
class ConstructableStreamer : public StreamerBase {
    StreamingStatus write(int64_t token) override {
        PYBIND11_OVERRIDE_PURE(StreamingStatus,  // Return type
...
void init_streamers(py::module_& m) {
    ...
    py::class_<TextStreamer, std::shared_ptr<TextStreamer>, StreamerBase>(m, "TextStreamer", text_streamer_docstring)
        .def(py::init([](const Tokenizer& tokenizer,
                 ...
                 return std::make_shared<TextStreamer>(tokenizer,
                                                       callback,
    ...
    py::class_<TextParserStreamer, ConstructableTextParserStreamer, std::shared_ptr<TextParserStreamer>, TextStreamer>(
        ...
        .def(py::init([](const Tokenizer& tokenizer, std::vector<std::shared_ptr<IncrementalParser>> parsers) {
