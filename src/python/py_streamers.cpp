...
void init_streamers(py::module_& m) {
    ...
    auto streamer =
        py::class_<StreamerBase, ConstructableStreamer, std::shared_ptr<StreamerBase>>(
            m,
            "StreamerBase",
            streamer_base_docstring
        )  // Change the holder form unique_ptr to shared_ptr
            .def(py::init<>())
            .def(
                "write",
                ...
                "Write is called every time new token or vector of tokens is decoded. Returns a StreamingStatus flag "
                "to indicate whether generation should be stopped or cancelled",
                py::arg("token")
            )
            .def(
                "end",
    ...
    py::class_<TextStreamer, std::shared_ptr<TextStreamer>, StreamerBase>(m, "TextStreamer", text_streamer_docstring)
        .def(
            py::init([](const Tokenizer& tokenizer,
                        std::function<CallbackTypeVariant(std::string)> callback,
                        const std::map<std::string, py::object>& detokenization_params) {
                return std::make_shared<TextStreamer>(
                    tokenizer,
                    callback,
                    pyutils::properties_to_any_map(detokenization_params)
                );
            }),
            py::arg("tokenizer"),
            py::arg("callback"),
    ...
    py::class_<TextParserStreamer, ConstructableTextParserStreamer, std::shared_ptr<TextParserStreamer>, TextStreamer>(
        m,
        "TextParserStreamer",
        text_parser_streamer_docstring
    )
        .def(
            py::init([](const Tokenizer& tokenizer, std::vector<std::shared_ptr<IncrementalParser>> parsers) {
        ...
        // But for texts we need to check that when we call write with strings/integer tokens they are accumulated and
        // stored correctly in py::dict. Therefore we provide a private method '_write' which is used to call 'write'
        // with correct parameters from C++ side.
        .def(
            "_write",
            ...
            py::arg("chunk"),
            "This is a private method is used to call write with integer tokens or text chunks. Is used for text "
            "purposes only."
        )
