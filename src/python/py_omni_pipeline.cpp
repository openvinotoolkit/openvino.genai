...
void init_omni_pipeline(py::module_& m) {
    ...
    py::class_<ov::genai::TalkerResult>(m, "TalkerResult",
        ...
        .def(py::init<>())
        .def_readonly("speech_outputs", &ov::genai::TalkerResult::speech_outputs)
        .def_readonly("perf_metrics", &ov::genai::TalkerResult::perf_metrics);
    ...
    py::class_<OmniPipeline>(m, "OmniPipeline", omni_pipeline_docstring)
        ...
        // Note: DI constructor (taking shared_ptr<VLMPipelineBase>) is not exposed to Python
        // because VLMPipelineBase is an incomplete type in the public headers. Python users
        // should use the path-based constructor or instantiate VLMPipeline/Talker separately
        // if they need device-level control.
        ...
        .def(
            ...
            [](OmniPipeline& pipe,
               ...
               const pyutils::PyBindOmniSpeechStreamerVariant& speech_streamer)
                ...
                // When configs are None, pass default-constructed instances.
                // The pipeline's impl uses its internal defaults for unset fields.
        ...
        // Note: get_vlm() returns shared_ptr<VLMPipeline::VLMBackend>, which is not currently
