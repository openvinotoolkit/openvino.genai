...
void init_ltx_video_transformer_3d_model(py::module_& m) {
    auto ltx_transformer = py::class_<ov::genai::LTXVideoTransformer3DModel>(
                               m,
            ...
            kwargs: Device properties.
                               ...
                               .def(
                                   py::init([](const ov::genai::LTXVideoTransformer3DModel& model) {
    ...
    py::class_<ov::genai::LTXVideoTransformer3DModel::Config>(
        ltx_transformer,
        "Config",
...
void init_autoencoder_kl_ltx_video(py::module_& m) {
    auto vae =
        py::class_<ov::genai::AutoencoderKLLTXVideo>(
            m,
            ...
            .def(
                py::init([](const std::filesystem::path& vae_decoder_path,
                            const std::string& device,
                            const py::kwargs& kwargs) {
                    return std::make_unique<ov::genai::AutoencoderKLLTXVideo>(
                        vae_decoder_path,
                        device,
                        pyutils::kwargs_to_any_map(kwargs)
                    );
