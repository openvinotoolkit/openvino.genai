...
void init_perf_metrics(py::module_& m) {
    ...
    py::class_<MeanStdPair>(m, "MeanStdPair")
        ...
        .def(
            ...
            py::keep_alive<0, 1>()
        );  // Keep object alive while the iterator is used;
    ...
    py::class_<ExtendedPerfMetrics, std::shared_ptr<ExtendedPerfMetrics>>(
        m,
        "ExtendedPerfMetrics",
    ...
    py::class_<SDPerfMetrics, ExtendedPerfMetrics, std::shared_ptr<SDPerfMetrics>>(
        m,
        "SDPerfMetrics",
