...
public:
        ...
        try {
            ...
            if (e.matches(PyExc_ModuleNotFoundError)) {
                throw std::runtime_error(
                    "The 'torch' package is not installed. Please, call 'pip install torch' or use 'rng_seed' "
                    "parameter."
                );
            } else {
                throw;
    ...
    ov::Tensor randn_tensor(const ov::Shape& shape) override {
        ...
        return ov::Tensor(
            ...
            shape,
            TorchTensorAllocator(
                ov::shape_size(shape) * ov::element::f32.size(),
...
void init_image_generation_pipelines(py::module_& m) {
    ...
    py::class_<RawImageGenerationPerfMetrics>(
        m,
    ...
    py::class_<ImageGenerationPerfMetrics>(m, "ImageGenerationPerfMetrics", image_generation_perf_metrics_docstring)
        ...
        .def(
            "get_first_and_other_iter_duration",
        ...
        .def(
            "get_first_and_other_trans_infer_duration",
            [](ImageGenerationPerfMetrics& self) -> py::tuple {
        ...
        .def(
            "get_first_and_other_unet_infer_duration",
            [](ImageGenerationPerfMetrics& self) -> py::tuple {
                float first_infer_time, other_infer_avg_time;
    ...
    auto text2image_pipeline =
        py::class_<ov::genai::Text2ImagePipeline>(
            m,
            "Text2ImagePipeline",
            ...
            .def(
                py::init(
                    [](const std::filesystem::path& models_path, const std::string& device, const py::kwargs& kwargs) {
                        ScopedVar env_manager(pyutils::ov_tokenizers_module_path());
                        return std::make_unique<ov::genai::Text2ImagePipeline>(
                            models_path,
                            device,
            ...
            .def_static(
                "stable_diffusion_3",
                py::overload_cast<
                    const std::shared_ptr<ov::genai::Scheduler>&,
    ...
    auto image2image_pipeline =
        py::class_<ov::genai::Image2ImagePipeline>(
            m,
            "Image2ImagePipeline",
            ...
            .def(
                py::init(
                    [](const std::filesystem::path& models_path, const std::string& device, const py::kwargs& kwargs) {
                        ScopedVar env_manager(pyutils::ov_tokenizers_module_path());
                        return std::make_unique<ov::genai::Image2ImagePipeline>(
                            models_path,
                            device,
                            pyutils::kwargs_to_any_map(kwargs)
                        );
    ...
    auto inpainting_pipeline =
        py::class_<ov::genai::InpaintingPipeline>(
            m,
            "InpaintingPipeline",
            ...
            .def(
                py::init(
                    [](const std::filesystem::path& models_path, const std::string& device, const py::kwargs& kwargs) {
                        ScopedVar env_manager(pyutils::ov_tokenizers_module_path());
                        return std::make_unique<ov::genai::InpaintingPipeline>(
                            models_path,
                            device,
                            pyutils::kwargs_to_any_map(kwargs)
                        );
    ...
    text2image_pipeline
        .def(
            py::init([](const ov::genai::Image2ImagePipeline& pipe) {
                return std::make_unique<ov::genai::Text2ImagePipeline>(pipe);
            }),
            py::arg("pipe")
        )
        .def(
            py::init([](const ov::genai::InpaintingPipeline& pipe) {
                return std::make_unique<ov::genai::Text2ImagePipeline>(pipe);
            }),
            py::arg("pipe")
        );

    image2image_pipeline.def(
        py::init([](const ov::genai::InpaintingPipeline& pipe) {
