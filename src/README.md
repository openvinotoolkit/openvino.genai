...
>**Note**: TPOT (Time Per Output Token) represents the average time required to generate each output token in the final result. For beam search scenario, TPOT is calculated based on the effective output tokens delivered to users, not the tokens generated by individual beams during internal processing.
...
For using debug log, refer to [DEBUG Log](./docs/DEBUG_LOG.md).
