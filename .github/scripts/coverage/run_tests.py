...
def _normalize_python_xml_filename(raw_path: str, *, repo_source_dir: Path, installed_dirs: tuple[Path, ...]) -> str:
    ...
    if not path.is_absolute():
        ...
        if stripped.startswith("openvino_genai/"):
            stripped = stripped[len("openvino_genai/") :]
    ...
    for marker in ("site-packages", "dist-packages"):
        ...
        tail = parts[idx + 1 :]
        if tail and tail[0] == "openvino_genai":
            return Path(*tail[1:]).as_posix()

    if "openvino_genai" in parts:
        openvino_genai_indexes = [idx for idx, part in enumerate(parts) if part == "openvino_genai"]
        for idx in reversed(openvino_genai_indexes):
            tail = parts[idx + 1 :]
            if tail:
...
def run_python(ctx: CoverageContext) -> None:
    ...
    py_source_root = ctx.workspace / "src" / "python"
    py_source_dir = py_source_root / "openvino_genai"
    ...
    installed_openvino_genai_dir = _detect_installed_python_package_dir("openvino_genai")
    if installed_openvino_genai_dir is None and pip_install_path:
        fallback_dir = Path(pip_install_path) / "openvino_genai"
        if fallback_dir.is_dir():
            installed_openvino_genai_dir = fallback_dir

    py_cov_source_dir = installed_openvino_genai_dir if installed_openvino_genai_dir is not None else py_source_dir
    py_cov_source = str(py_cov_source_dir)
    openvino_wheel_lib_dir = Path(pip_install_path) / "openvino" / "libs" if pip_install_path else None
    runtime_extra_paths = [tests_dir]
    if installed_openvino_genai_dir is not None:
        runtime_extra_paths.append(installed_openvino_genai_dir)
    if openvino_wheel_lib_dir is not None:
        runtime_extra_paths.append(openvino_wheel_lib_dir)
    ...
    py_cov_config.write_text(
        _pycov_config(
            ...
            installed_dirs=tuple(path for path in (installed_openvino_genai_dir,) if path is not None),
        ),
        encoding="utf-8",
    )
