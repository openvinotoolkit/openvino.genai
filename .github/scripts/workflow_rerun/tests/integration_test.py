...
class IntegrationTest(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.errors_to_look_for_file = cls._cwd.parent.joinpath("errors_to_look_for.json")
    ...
    def test_log_collection_and_analysis(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            collect_logs_for_run(run=self.wf_run, logs_dir=logs_dir, session=self.session)

            analyzer = LogAnalyzer(
                path_to_logs=logs_dir,
                path_to_errors_file=self.errors_to_look_for_file,
            )
            self.assertTrue(len(analyzer._log_files) > 0, "Failed run log files should be collected for failed jobs")
            analyzer.analyze()
