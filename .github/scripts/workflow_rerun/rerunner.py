...
def record_rerun_to_db(
    ...
    conn = psycopg2.connect(host=db_host, port=db_port, user=db_username, password=db_password, database=db_database)
...
def analyze_and_rerun(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        collect_logs_for_run(run=run, logs_dir=logs_dir, session=session)

        log_analyzer = LogAnalyzer(path_to_logs=logs_dir, path_to_errors_file=errors_file)
