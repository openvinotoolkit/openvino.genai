...
1. C++ Proficiency:
    * Strong C++ skills, including modern C++ standards (C++17 and beyond)
...
1. Avoid copies: Ensure that large data structures (like tensors) are passed by reference or moved, not copied.
1. Python Bindings: If C++ APIs are changed, check if the corresponding Python pybind11 wrappers in src/python need updates.
1. Exceptions: Use OPENVINO_ASSERT(condition, ...) for checks instead of if + throw.
1. Documentation: Ensure that any new public APIs have docstrings in C++ headers and Python bindings. Ensure that new public APIs have documentation updated in /site.
1. Test Coverage: Ensure that new features or changes have corresponding tests.
1. Formatting & Safety:
