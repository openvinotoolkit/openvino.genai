...
RUN git clone https://github.com/openvinotoolkit/openvino.genai.git && \
        ...
        mkdir /workspace/openvino.genai/build && \
        cmake -DENABLE_CONTINUOUS_BATCHING=ON -DENABLE_APPS=ON -DENABLE_PYTHON=ON -DCMAKE_BUILD_TYPE=Release -B /workspace/openvino.genai/build /workspace/openvino.genai/ && \
        make -j${JOBS}
